-- IUSENTRA - Registro uffici giudiziari e indirizzi telematici PEC
...
CREATE TABLE IF NOT EXISTS indirizzi_telematici (
    ...
    metadata_json TEXT NOT NULL DEFAULT '{}',
    FOREIGN KEY (ufficio_id) REFERENCES uffici_giudiziari(id) ON DELETE CASCADE,
    CHECK (uso IN (
        'deposito_pct',
        'deposito_penale',
...
CREATE TABLE IF NOT EXISTS uffici_giudiziari_verifiche (
    id TEXT PRIMARY KEY,
    tenant_id TEXT NOT NULL DEFAULT '',
    verificato_il TEXT NOT NULL DEFAULT (datetime('now')),
...
CREATE TABLE IF NOT EXISTS uffici_giudiziari_variazioni (
    ...
    FOREIGN KEY (verifica_id) REFERENCES uffici_giudiziari_verifiche(id) ON DELETE CASCADE
);
