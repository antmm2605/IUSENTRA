...
const LegalSkillsProfilePage = lazy(() => import('./features/legal-skills/pages/LegalSkillsProfilePage').then((module) => ({ default: module.LegalSkillsProfilePage })))
const LegalSkillsRunPage = lazy(() => import('./features/legal-skills/pages/LegalSkillsRunPage').then((module) => ({ default: module.LegalSkillsRunPage })))
const LegalSkillsReviewPage = lazy(() => import('./features/legal-skills/pages/LegalSkillsReviewPage').then((module) => ({ default: module.LegalSkillsReviewPage })))
...
export default function App() {
  ...
  const isLegalSkillsReviewPage = routeKey === '/legal-skills/review' || /^\/legal-skills\/runs\/[^/]+$/.test(routeKey)
  const isLegalSkillsPage = isLegalSkillsCatalogPage || isLegalSkillsProfilePage || isLegalSkillsRunPage || isLegalSkillsReviewPage
  ...
  const isDashboardPage = !isSearchPage && !isAgendaImportPage && !isAgendaPage && !isNewAppointmentPage && !isAppointmentEditPage && !isRegiaPage && !isDocumentEditorPage && !isFascicoliPage && !isClientiPage && !isClientFolderPage && !isClientEditPage && !isNewClientPage && !isSoggettiPage && !isNewSubjectPage && !isSubjectEditPage && !isEmailComposePage && !isEmailOrdinariaComposePage && !isNotificheLegaliPage && !isEmailPage && !isEmailOrdinariaPage && !isMessagesPage && !isNewMessagePage && !isScadenziarioPage && !isNewDeadlinePage && !isDeadlineEditPage && !isTimesheetPage && !isCartelleCondivisePage && !isWizardProPage && !isTelematicoPage && !isTelematicoSurfacePage && !isPrivacyRegistroPage && !isAdminDatabasePage && !isStatistichePage && !isImpostazioniPage && !isAuditPage && !isRegistroAttivitaPage && !isUtentiPage && !isProfiliPage && !isProfiloPage && !isBackupPage && !isSitoStudioBuilderPage && !isSitoStudioRedazioneAiPage && !isSitoStudioPage && !isStudioPage && !isAmministrazionePage && !isFatturazionePage && !isIncassiPagamentiPage && !isPreventivoWizardPage && !isPreventiviPage && !isCompensiForensiPage && !isTariffarioPage && !isTemplateAttiPage && !isRedazioneAttiPage && !isGiurisprudenzaPage && !isLegalIntelligencePage && !isLegalSkillsPage && !isStudioModulePage
  const isStandalonePage = isSearchPage || isAgendaImportPage || isAgendaPage || isNewAppointmentPage || isAppointmentEditPage || isDocumentEditorPage || isFascicoliPage || isClientiPage || isClientFolderPage || isClientEditPage || isNewClientPage || isSoggettiPage || isNewSubjectPage || isSubjectEditPage || isEmailComposePage || isEmailOrdinariaComposePage || isNotificheLegaliPage || isEmailPage || isEmailOrdinariaPage || isMessagesPage || isNewMessagePage || isScadenziarioPage || isNewDeadlinePage || isDeadlineEditPage || isTimesheetPage || isCartelleCondivisePage || isWizardProPage || isTelematicoPage || isTelematicoSurfacePage || isPrivacyRegistroPage || isAdminDatabasePage || isStatistichePage || isImpostazioniPage || isAuditPage || isRegistroAttivitaPage || isUtentiPage || isProfiliPage || isProfiloPage || isBackupPage || isSitoStudioBuilderPage || isSitoStudioRedazioneAiPage || isSitoStudioPage || isStudioPage || isAmministrazionePage || isFatturazionePage || isIncassiPagamentiPage || isPreventivoWizardPage || isPreventiviPage || isCompensiForensiPage || isTariffarioPage || isTemplateAttiPage || isRedazioneAttiPage || isGiurisprudenzaPage || isLegalIntelligencePage || isLegalSkillsPage || isStudioModulePage
