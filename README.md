...
- `/preventivi/wizard` e' una console React operativa in 4 step: cliente/contesto, tipologia e motore preventivo, compenso base/fasi e bozza editabile, con sidebar riepilogo, sticky footer, calcolo backend reale e creazione preventivo tramite API Flask.
- Nel preventivo guidato le fasi tabellari restano visibili anche quando la regola usa `Compenso unico`: il flag unico e' aggiuntivo, attivo valorizza la voce unica e disattivo calcola le fasi selezionate; la complessita stimata bassa/media/alta usa la stessa colonna minimo/base/massimo del tariffario reale della pratica, con bonus telematico allineato e spese generali esposte come voce separata `Spese forfettarie` con percentuale applicata; la clausola controversie resta una sola sezione modificabile e trasferibile al conferimento.
