...
function isSitoStudioReactPath(path) {
  return ['/sito-studio/contatti', '/sito-studio/builder', '/sito-studio/redazione-ai'].includes(path) ||
    /^\/sito-studio\/articoli\/[^/]+\/modifica$/.test(path)
}
...
function isBlockedBySpecialRule(path) {
  ...
  return (
    ...
    (path.startsWith('/sito-studio/') && !isSitoStudioReactPath(path)) ||
...
for (const route of ['/app/portale-clienti', '/portale-cliente']) {
  reactExact.add(normaliseRoute(route))
}
