...
- Nel wizard PST, React e classico, arrivando da una pratica locale lo Step 4 mostra e preseleziona `Aggiorna pratica esistente`; sono accettati anche `fascicolo_id`, `target_fascicolo_id` e `mode=update_existing`. Per Giudice di Pace/SIGP il catalogo atti viene raccolto nel batch di visualizzazione e il download resta un secondo batch, evitando prompt PIN ripetuti fuori dalle due operazioni reali.
...
- Pipeline PEC audit-grade end-to-end: MIME originale immutabile, JSON estratto versionato, SHA-256 per versione, audit append-only, worker asincroni, validazione non bloccante, lettura semantica dei contesti processuali PEC/PCT/L.53/PAT/PTT/SNT/PDP, UI React con confidence/firme/semaforo e Lex informato tramite `pec_audit`. Il percorso compatto `pct.pec_ocr_pipeline` aggiunge WORM locale, antivirus, verifica firma, ZIP sicuro, raw blob deduplicati, `ocr.task`, `ocr.result` e `lex.ingest.doc`, documentati in [docs/PEC_INGEST_OCR_PIPELINE.md](docs/PEC_INGEST_OCR_PIPELINE.md).
...
- `/tariffario` e' una console React operativa: parametri, fasi, accessori, ADR, mediazione D.M. 150/2023, spese vive, voci manuali, risultato tabellare, riepilogo economico, profilo attivo, audit e CTA verso preventivo/parcella restano collegati al motore backend reale e al fallback tecnico `?_legacy=1`.
- `/preventivi/wizard` e' una console React operativa in 4 step: cliente/contesto, tipologia e motore preventivo, compenso base/fasi e bozza editabile, con sidebar riepilogo a sinistra, sticky footer compatto, calcolo backend reale e creazione preventivo tramite API Flask. I metadati tecnici di classificazione e tassonomia restano silenziosi, mentre `Area pratica` gestisce selezione, ricerca, reset dei filtri visibili e aggiunta/rimozione di ulteriori voci area pratica nel payload del preventivo.
- Nel preventivo guidato le fasi tabellari restano visibili anche quando la regola usa `Compenso unico`: il flag unico valorizza la voce unica, mentre a flag spento l'avvocato decide quali fasi includere e il motore ripartisce in modo tracciato l'importo unico quando la tabella ministeriale non espone valori autonomi per fase; `Aggiungi voce area pratica` porta in bozza tutte le tipologie selezionate con il relativo importo. La complessita stimata bassa/media/alta usa la stessa colonna minimo/base/massimo del tariffario reale della pratica, con bonus telematico allineato e spese generali esposte come voce separata `Spese forfettarie` con percentuale applicata; la clausola controversie resta una sola sezione modificabile e trasferibile al conferimento.
- Il conferimento incarico nel wizard viene predisposto solo dopo l'accettazione cliente del preventivo: l'opzione finale registra prima l'accettazione e solo dopo abilita conferimento e apertura fascicolo guidato.
...
- `Documenti AI Fascicolo` aggiunge una base nativa per upload PDF/DOCX/DOC, hash SHA-256, versioni, estrazione testo, ricerca nel documento e tool Lex governati, senza usare codice AGPL di Mike e senza esporre path filesystem nei payload API.
- `Generazione atti con Lex nell'editor` crea veri documenti dell'editor professionale collegati al fascicolo, con template reali, rilettura post-generazione, versioni, fonti, proposte modifica accetta/rifiuta ed export DOCX/PDF.
...
- Local Signer come ponte operativo locale per firma digitale, AI locale, test/invio PEC dal PC dello studio e controllo remoto assistito: dalla versione `1.6.72` espone anche `/support/status`, `/support/arm`, `/support/disarm`, `/support/screenshot` e `/support/execute` su `127.0.0.1:27272/support`, armati solo dopo consenso cliente e token valido. Se il servizio è vecchio o non espone il modulo assistenza, la UI prova l'aggiornamento automatico via `/update` prima di proporre il pacchetto manuale. Nelle firme documento distingue anche il caso Windows in cui il token compare nel `token_probe_fresh`, riavvia l'app locale tramite protocollo registrato, non chiede il PIN finché `token[]` non è pronto e conserva la posizione della firma visibile configurata con coccarda trasparente.
...
- Lex include il `Legal Skills Engine`: pack read-only per contratti, privacy, contenzioso e regolatorio, con profilo studio obbligatorio, fonti governate, note di revisione, approvazione e UI React dedicata attiva di default; trust layer, skill custom e agenti schedulati restano dietro feature flag espliciti.
- Lex include anche la [Regia Agentica Studio](docs/LEX_WORKFLOW_AGENTS.md): workflow agentici governati per triage, redazione, consuntivo, nuovo incarico, pre-deposito e ricerca preliminare, con preview read-only, coda approvazioni, RBAC/tenant isolation e metriche di riduzione tempo all'80%.
- [Lex Oggi](docs/LEX_DAILY_PLAN.md) è la regia giornaliera dello studio: collettori deterministici su PEC, presidi fascicolo, agenda, scadenze ed economia producono ogni mattina un piano per avvocato (priorità P0–P3 spiegabili, evidenze, assegnazione, fasce proposte) materializzato in uno snapshot tenant-aware; la pagina `/oggi` legge solo lo snapshot (zero analisi, zero LLM nel percorso di lettura) e le scritture restano proposte approvabili.
- Lex distingue ora in modo rigido frammenti locali di sentenza, fonti pubbliche ufficiali e dati interni cliente: le sentenze specifiche forzano exact-match governato quando il web e' consentito, mentre le richieste cliente usano solo l'anagrafica interna senza fonti web o cabina studio.
...
- Lex opera ora in modalita' governed-only di default (`LEX_GOVERNED_ONLY=1`): la raw chat e' bloccata salvo doppio consenso tecnico, gli allegati diventano evidenze governate e le risposte senza fonti sufficienti espongono `needs_review`, warning, confidence bassa/media e prossime azioni concrete.
...
- Assistenza remota cliente sempre da `SUPERADMIN`, con schermo WebRTC via browser (`getDisplayMedia`), microfono opzionale (`getUserMedia`), chat tecnica, audit, consensi espliciti e controllo completo del PC tramite Local Signer `1.6.72` già presente nello studio o agente dedicato `27273` come fallback.
...
- Migrazione React progressiva: la shell `/app-v2` convive con la UI Jinja attuale e usa API Flask reali; `Clienti e Anagrafiche`, `Soggetti e Parti`, `Comunicazioni`, `Servizi Telematici`, `Gestione Database` (`/admin/database`), `Statistiche`, `Audit`, `Registro attivita`, `Utenti`, `Profili e permessi`, `Studio`, `Amministrazione`, `Fatturazione`, `Nuova parcella`, `Incassi e pagamenti`, `Preventivi e Incarichi`, `Nuovo preventivo`, `Preventivo guidato`, `Nuovo conferimento incarico`, `Compensi forensi` e `Tariffario` sono ora servite in React sui GET ufficiali, con salvataggi JSON operativi dove la parita' e' stata dimostrata e fallback tecnico `_legacy=1` solo se governato. `/fatturazione/nuova` usa `POST /api/v1/ui/fatturazione/nuova` con CSRF/sessione, permessi e calcolo canonico backend; `/tariffario` usa anche `POST /api/v1/ui/tariffario/calcola`; `/preventivi/wizard` usa `/api/v1/ui/preventivi/wizard`, `/calculate` e `/create`, riusando il motore Python senza duplicare logiche economiche in frontend. Le superfici telematiche `PolisWeb / PST`, `PDP`, `PAT`, `PTT`, `Tribunali / PEC`, `Checklist deposito` e `Guida firma digitale` restano governate dai vincoli specifici di portale e non vengono promosse da questa tranche. `Regia Operativa`, `Ricerca Studio`, `Agenda`, `Nuovo Appuntamento` e la suite `Fascicoli` vivono rispettivamente in `/app-v2/regia-operativa`, `/app-v2/ricerca-studio`, `/app-v2/agenda`, `/app-v2/agenda/nuovo` e `/app-v2/fascicoli`. La suite fascicoli include lista, nuovo/modifica, archivio, dettaglio cabina fascicolo, quadro intelligente AI, editor professionale anche sulla route profonda `/fascicoli/<id>/documenti/<id_doc>/editor` con font/dimensioni/interlinea in stile studio, anteprima PDF nativa fedele all'originale per documenti impaginati con stemmi/timbri/firme e blocco professionale delle conversioni non affidabili, compilatore atti, anteprima documento interna anche per `.pdf.p7m`, upload senza reload ed export, con scritture ancora sulle route storiche per rollback immediato. Ogni pagina migrata viene collegata alla nav React, usa solo dati reali da repository/sessione/API/configurazione e nessuna pagina React viene annidata dentro un'altra durante la migrazione pagina per pagina.
- La migrazione React residua e' governata da `tools/react-migration/route-manifest.json` e dagli script `scripts/react-migration/*`: audit route, cattura contratti legacy, controllo gate, consistency UI e runner sicuro. Dalla Parte 12A il manifest non usa piu' `react_full` per pagine mascherate: distingue `react_shell`, `react_bridge`, `react_operational_partial`, `react_operational_full` e `legacy_operational`; `/statistiche` e' ora `react_operational_full` read-only senza fallback legacy nel payload React. `/preventivi/wizard` e `/tariffario` restano console operative con fallback tecnico `?_legacy=1`, mantenendo sottopercorsi compensi/tariffario, dettagli `/preventivi/*`, formule, log economici, generazione documenti, `/fatturazione/*`, impostazioni, documentale e telematico sui percorsi legacy fino a parita' dimostrata.
- La Parte 13A promuove `/profili` a React operativo pieno: ruoli, permessi, matrice e override arrivano da `GET /api/v1/ui/profili`, mentre il salvataggio override usa `POST /api/v1/ui/profili` con CSRF/sessione, permesso `utenti.scrivi`, audit e rollback legacy solo tecnico.
- La Parte 14A promuove `/utenti` a React operativo pieno: lista, filtri locali, ruolo, stato account, profilo minimo e reimpostazione credenziale usano API JSON sotto `/api/v1/ui/utenti`, con permessi backend, CSRF, audit e rollback legacy solo tecnico.
- La Parte 16A promuove `/backup` a React operativo pieno: stato backup, lista copie, creazione e verifica integrita usano API JSON sotto `/api/v1/ui/backup`, con permesso `backup.esegui`, CSRF/sessione, audit e payload senza path sensibili; download resta link backend sicuro, mentre restore e delete restano legacy/protetti.
- La Parte 17A promuove `/fatturazione/nuova` a React operativo pieno: clienti/fascicoli/default arrivano da `GET /api/v1/ui/fatturazione/nuova`, la creazione usa `POST /api/v1/ui/fatturazione/nuova` con permesso `fatturazione.scrivi`, validazione backend, audit e calcolo fiscale canonico nel manager fatturazione; PDF, XML, export e `/fatturazione/*` restano legacy/backend protetti.
- Le Parti 18A-21A promuovono `/preventivi/nuovo`, `/preventivi/conferimento/nuovo`, `/preventivi` e `/fatturazione` a React operativo pieno: letture e scritture supportate usano API JSON con permessi e audit backend, mentre calcoli forensi/fiscali, PDF/DOCX/XML/export, wizard e sottoroute non autorizzate restano nei percorsi backend/legacy.
- Le Parti 22A-25A promuovono `/incassi-pagamenti`, `/compensi-forensi`, `/tariffario`, `/audit` e `/registro-attivita` a React operativo pieno: dashboard incassi, calcolo compensi, tariffario e dettaglio audit usano API JSON con payload sanificati, mentre provider/webhook, formule DM55, scaglioni canonici, export e documenti restano backend/legacy.
- La Tranche 9A abilita `/template-atti`, `/template-atti/catalogo` e `/redazione-atti` in React full come superfici documentali di ingresso read-only: catalogo reale, metadati template, variabili come soli nomi e link operativi sicuri. Restano legacy `/template-atti/nuovo`, sottopercorsi template/redazione, checklist, deposito, editor, produzione file e AI redazionale.
- La Tranche 10A abilita `/giurisprudenza`, `/legal-intelligence`, `/legal-intelligence/news`, `/legal-intelligence/mediazione` e `/ricerca-legale` come superfici React di consultazione read-only: fonti, metadati, stato verifica, news e registro mediazione arrivano da bridge backend sicuri; import, classificazione, dettagli, testo integrale, approvazione contenuti, AI, scraping/crawling, `/giurisprudenza/nuova`, sottopercorsi, `/checklist` e `/deposito/checklist` restano legacy.
- La pagina `Gestione Database` non si limita a segnalare anomalie referenziali: `Verifica e ripara` chiama una route Flask auditata, crea backup prima di scrivere e corregge automaticamente i riferimenti orfani risolvibili senza inventare dati.
...
Per la sincronizzazione bidirezionale server-side di Agenda e Scadenziario con Google Calendar, Outlook/Microsoft 365, Apple iCloud/CalDAV, WebCal/ICS e provider locale persistente vedi [docs/CALENDAR_SYNC_ENGINE.md](docs/CALENDAR_SYNC_ENGINE.md).
Per il piano di migrazione React vedi [docs/REACT_MIGRATION_MASTER_PLAN.md](docs/REACT_MIGRATION_MASTER_PLAN.md). La tranche `2.198.120` aggiunge audit Full React, inventario Jinja, shell/app/API/features governate, primitive UI legali e runner anti-mascheramento senza promuovere route non ancora verificate. La tranche `2.198.121` introduce il design system IUSENTRA basato su shadcn/ui, Lucide Icons e Tailwind, documentato in [docs/UI_DESIGN_SYSTEM.md](docs/UI_DESIGN_SYSTEM.md), mantenendo invariati route, API e fallback governati. Le tranche `2.198.122`-`2.198.126` aggiungono il runner pytest a fasi documentato in [docs/PYTEST_PHASES.md](docs/PYTEST_PHASES.md), dividono `Pytest core`, coverage critica, Local Signer, overlay qualita', E2E e frontend React in shard CI con aggregatori e mantengono i guardrail governance sui target storici, per evitare timeout monolitici senza ridurre la copertura finale richiesta. La tranche `2.198.127` integra in `AGENTS.md` le regole professionali Codex per UI React, shadcn/ui, Open Design/Open Designer, Impeccable, anti-monolite, performance, sicurezza, accessibilita', quality gate e report finale. Le fasi `fasereact` 1-10 governano App V2 con feature flag default-off, registro route, flag per pagina, routing sicuro, security backend, OpenAPI, guard frontend, requisiti area, UI regression e piano test completo; i documenti operativi sono `docs/app-v2-page-registry.md`, `docs/frontend-app-v2-pages.md`, `docs/app-v2-area-requirements.md`, `docs/test-plan-app-v2.md`, `docs/test-inventory.md` e `docs/test-matrix-app-v2.md`.
