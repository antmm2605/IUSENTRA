  ...
  return (
    <main className="iu-content iu-fascicoli-page">
      ...
      <section className="iu-fas-stats" aria-label="Indicatori fascicoli">
        ...
        <StatCard icon={<CalendarDays size={19}/>} label="Scadenze 7g" value={data.summary.deadlines7} note="priorit? immediata" tone="danger"/>
      ...
      <section className="iu-fas-status-line">
        ...
        <small><ShieldCheck size={14}/> React gestisce la vista; salvataggi e audit restano sui servizi backend gi? governati.</small>
      ...
      <section className="iu-fas-lower-grid">
        <Panel title="Controllo qualit? fascicoli" subtitle="Cose da non lasciare implicite" icon={<BriefcaseBusiness size={17}/>}>
  ...
  return (
    <main className="iu-content iu-fascicoli-page">
      <section className="iu-fas-hero iu-fas-hero--archive">
        <div><span className="iu-fas-eyebrow"><Archive size={16}/> Archivio</span><h1>Archivio Fascicoli</h1><p>Procedimenti definiti, archiviati, ZIP e possibilit? di ripristino.</p></div>
    ...
    <main className="iu-content iu-fascicoli-page iu-fascicolo-form-page">
      ...
      <section className="iu-fas-form-layout">
        ...
        <aside className="iu-fas-form-side">
          ...
          <Panel title="Prossimi passi" icon={<ListChecks size={17}/>}><div className="iu-fas-help"><p>Dopo il salvataggio potrai aggiungere documenti, scadenze processuali, attivit?, depositi telematici e note.</p></div></Panel>
        </aside>
      </section>
  ...
  return (
    <article className="iu-fas-activity-row">
      ...
      <div className="iu-fas-actions iu-fas-actions--wrap">
        ...
        {activity.deleteAction ? <PostAction action={activity.deleteAction} tone="danger" confirm="Eliminare questa attivit??"><Trash2 size={14}/></PostAction> : null}
      </div>
    </article>
  )
}
...
function DetailPage({ id }:{id:string}) {
  ...
  if (!loading && data.notFound) return <main className="iu-content iu-fascicoli-page"><EmptyState icon={<FolderOpen size={34}/>} title="Fascicolo non trovato" action={<Button href="/fascicoli">Torna ai fascicoli</Button>}>Il fascicolo non ? disponibile o non hai i permessi per aprirlo.</EmptyState></main>
  return (
    <main id="fascicolo-top" className="iu-content iu-fascicoli-page iu-fascicolo-detail-page">
      ...
      <nav className="iu-fas-section-nav" aria-label="Sezioni fascicolo"><a href="#profilo">Profilo <b>{data.quickCounts.profilo || 0}</b></a><a href="#documenti">Documenti <b>{data.documents.length}</b></a><a href="#attivita">Attivit? <b>{data.activities.length}</b></a><a href="#udienze">Udienze / scadenze <b>{data.deadlines.length + data.appointments.length}</b></a><a href="#cancelleria">Cancelleria <b>{data.deposits.length}</b></a><a href="#istanze">Istanze <b>{data.requests.length}</b></a><a href="#gestione">Gestione</a><a href="#economia">Economia</a><a href="#conformita">Conformit?</a><a href="#soggetti">Soggetti <b>{data.parties.length}</b></a></nav>
      <section className="iu-fas-detail-grid">
        <div className="iu-fas-detail-main">
          <section className="iu-fas-cockpit"><StatCard icon={<FileText size={19}/>} label="Documenti" value={data.documents.length} note="acquisiti o da portale" tone="primary"/><StatCard icon={<CalendarDays size={19}/>} label="Scadenze" value={data.deadlines.length} note="aperte e concluse" tone="warning"/><StatCard icon={<ListChecks size={19}/>} label="Attivit?" value={data.activities.length} note="timeline processuale" tone="success"/><StatCard icon={<WalletCards size={19}/>} label="Economia" value={data.economics.length} note="preventivi, parcelle, tempo" tone="purple"/></section>
        ...
        <aside className="iu-fas-detail-side">
          ...
          <DetailSection id="conformita" title="Conformit? e qualit?" icon={<ShieldCheck size={17}/>} count={data.quality.length}><div className="iu-fas-quality-list">{data.quality.map((item) => <span key={item.label}><Badge tone={item.tone}>{item.ok ? 'OK' : 'Verifica'}</Badge><strong>{item.label}</strong><small>{item.value}</small></span>)}</div><form className={`iu-fas-compliance-toggle ${f.complianceControlsEnabled ? 'is-on' : 'is-off'}`} method="post" action={f.complianceControlsEnabled ? data.actions.complianceOff : data.actions.complianceOn}><input type="hidden" name="enabled" value={f.complianceControlsEnabled ? '0' : '1'}/><input type="hidden" name="next" value={detailReturnHref}/><button type="submit" aria-pressed={f.complianceControlsEnabled}><span className="iu-fas-compliance-toggle__switch" aria-hidden="true"><i/></span><span><strong>{f.complianceControlsEnabled ? 'Controlli automatici attivi' : 'Controlli automatici disattivati'}</strong><small>{f.complianceControlsEnabled ? 'Disattiva i controlli qualit? sul fascicolo' : 'Riattiva i controlli qualit? sul fascicolo'}</small></span></button></form></DetailSection>
...
function QuadroPage({ id }:{id:string}) {
  ...
  if (!loading && data.notFound) return <main className="iu-content iu-fascicoli-page"><EmptyState icon={<Gauge size={34}/>} title="Quadro non disponibile" action={<Button href="/fascicoli">Torna ai fascicoli</Button>}>Il fascicolo non ? disponibile o non hai i permessi per aprire il quadro.</EmptyState></main>
  return (
    <main id="fascicolo-quadro-top" className="iu-content iu-fascicoli-page iu-fascicolo-quadro-page">
      ...
      <section className="iu-fas-quadro-kpis" aria-label="Indicatori quadro fascicolo">
        ...
        <StatCard icon={<ShieldCheck size={19}/>} label="Conformit?" value={qualityStatus} note={qualityIssues ? 'da verificare' : 'nessun blocco critico'} tone={qualityIssues ? 'warning' : 'success'}/>
      </section>
      ...
      <section className="iu-fas-quadro-grid">
        ...
        <QuadroAxis id="conformita" title="Conformit?" icon={<ShieldCheck size={18}/>} status={qualityStatus} tone={qualityIssues ? 'warning' : 'success'}><div className="iu-fas-quadro-quality">{data.quality.map((item) => <span key={item.label}><Badge tone={item.tone}>{item.ok ? 'OK' : 'Verifica'}</Badge><strong>{item.label}</strong><small>{item.value}</small></span>)}{!data.quality.length ? <p className="iu-empty">Nessuna verifica registrata.</p> : null}</div><a className="iu-fas-inline-link" href={`${detailHref}#conformita`}><ShieldCheck size={14}/> Apri controlli qualit?</a></QuadroAxis>
