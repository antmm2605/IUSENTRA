...
function entryFrom(value: unknown): TimesheetEntry | null {
  ...
  return {
    ...
    hourlyRateLabel: text(value.hourlyRateLabel ?? value.hourly_rate_label, 'EUR 0,00'),
    totalValue: number(value.totalValue ?? value.total_value),
    totalValueLabel: text(value.totalValueLabel ?? value.total_value_label, 'EUR 0,00'),
...
function billingFrom(value: unknown): TimesheetBilling {
  ...
  return {
    ...
    valueLabel: text(value.valueLabel ?? value.value_label, 'EUR 0,00'),
