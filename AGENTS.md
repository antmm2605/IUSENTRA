...
- **Completamento funzionale integrale obbligatorio:** ogni incarico che richiede
  una capacità del prodotto deve essere perseguito come una consegna unica e
  completa: analisi e fonti, modello dati SQL con parità SQLite/PostgreSQL,
  resolver/regole, pipeline e job, repository/API JSON, superficie React,
  sicurezza/RBAC/audit, test automatici, prova reale e visiva su
  `127.0.0.1:8080`, documentazione, commit/push e deploy Hetzner. Non è
  ammesso presentare analisi, dati, prototipi, singoli endpoint, fallback,
  simulazioni o una sola fase come risultato dell'incarico: finché uno step
  necessario al funzionamento reale manca, il lavoro resta formalmente aperto
  e il report deve indicarlo senza usare “completo”, “funzionante al 100%”,
  “verde” o equivalenti. La chiusura richiede che la funzione sia operativa
  dall'inizio alla fine sul dato reale o controllato dello studio, senza
  regressioni, con tutte le procedure previste per il suo dominio.
...
- **Verifiche locali sempre sulla copia reale dell'utente:** per IUSENTRA la copia locale valida è `http://127.0.0.1:8080` / `http://localhost:8080`. È vietato dichiarare funzionante o concluso un lavoro basandosi su server temporanei, porte inventate o copie parallele; eventuali server temporanei possono servire solo come diagnosi interna, ma la prova finale visibile e i test end-to-end reali devono essere eseguiti sulla copia Docker/locale dell'utente su porta `8080`, aggiornata e healthy.
- **Test di accettazione solo su macchina reale, per tutto il prodotto:** qualunque sviluppo, modifica, fix, integrazione o regressione che produca un comportamento percepibile dall'utente deve essere accettato solo sulla macchina reale dell'utente e sulla copia reale IUSENTRA `127.0.0.1:8080`/`localhost:8080`, aggiornata e healthy. La regola vale per tutto: navigazione, pagine, API collegate alla UI, salvataggi, permessi, microfono, fotocamera, Local Signer, upload, download, notifiche, pannelli, form, responsive, performance percepita, testi, pulsanti, card, menu e flussi operativi. Codex non e' autorizzato a dichiarare esito positivo usando come prova finale test simulati, mock, CDP sintetico, Playwright/Chromium isolato, screenshot statici, server temporanei o copie parallele. Unit test, typecheck, lint, test sintetici e audit automatizzati sono ammessi solo come guardrail tecnici per trovare errori prima della prova reale, ma non sono mai prova di funzionamento utente. L'accettazione richiede browser reale visibile, click reali, scroll completo dove presente, dati reali o controllati sulla copia reale, stati osservabili nella UI e report che dica esattamente cosa e' stato visto. Se la verifica reale non e' stata eseguita, il report deve dire "non verificato su macchina reale" e non puo' usare parole come completato, funzionante, verde o risolto.
- **Regola anti falso-verde:** quando una regola di test reale non viene rispettata, il lavoro deve restare formalmente aperto anche se build, unit test, typecheck, lint, CDP, Playwright o altri controlli automatici sono verdi. Codex deve scrivere chiaramente cosa manca, correggere il codice o il deploy locale se la macchina reale mostra un difetto, e ripetere la prova materiale prima di qualsiasi report positivo. Le regole non sono promemoria: sono blocchi operativi.
- **Divieto assoluto di nascondere problemi:** se un pannello, una regola o un presidio mostra un problema, Codex non deve mai rimuovere solo il messaggio, filtrarlo per far sparire l'avviso o presentare come risolto un dato non corretto. Deve accertare la causa sul dato reale, correggere la fonte primaria, registrare l'azione/audit quando serve, ripetere i test mirati e, per funzioni visibili, provare il flusso con navigazione reale e click reali nella copia reale o sul server richiesto. Un problema può sparire dalla UI solo dopo che il software lo ha realmente risolto o dopo che resta una motivazione esplicita, verificabile e corretta.
- **Regola anti-fallback come soluzione finale:** quando un problema reale blocca una funzione, Codex deve risolverlo con un comportamento primario governato, verificabile e coerente con sicurezza, norme e vincoli tecnici del dominio; non deve presentare scorciatoie, percorsi alternativi, fallback, simulazioni o pulsanti secondari come soluzione conclusiva. Se un vincolo esterno non puo' essere superato in modo legittimo e sicuro, il lavoro resta aperto con diagnosi, procedura conforme proposta, UI esplicita e piano di integrazione reale; non va dichiarato chiuso finche' l'utente non ha una procedura operativa completa e verificata.
- **Risoluzione obbligatoria di ogni problema:** qualunque problema emerso durante analisi, sviluppo, test, browser reale, deploy, CI, dati, UI o uso operativo non deve mai essere lasciato irrisolto, minimizzato o rinviato senza presidio. Codex deve sempre analizzarlo fino alla causa, usare tutti i mezzi leciti e sicuri disponibili per risolverlo, aggiornare il codice o la procedura primaria e ripetere le verifiche. La struttura dati va sempre controllata secondo il modello già stabilito: tenant proprietario, path tenant-aware, eventuale JSON storico o mirror, SQLite, PostgreSQL, repository dedicato, API JSON, route, payload, componente React full, permessi, audit, test e script collegati. Ogni modifica visibile deve prevedere prove reali e visive su `127.0.0.1:8080` per pulsanti, card, pannelli, testi italiani con accenti corretti, date italiane, stati loading/errore/successo, layout, scroll completo, responsive desktop/tablet/mobile e coerenza grafica con tutto IUSENTRA. Route, payload, React full, test mirati, audit, script e documentazione devono seguire le procedure corrette; lo sviluppo deve restare sempre intuitivo, professionale, veloce e coerente con il resto del software.
- **Esaurimento obbligatorio delle alternative prima di segnalare un blocco:** un primo errore di rete, di parsing, di test, di browser, di dati o di integrazione non è una conclusione. Prima di evidenziare un passaggio come bloccato o non raggiungibile, Codex deve provare e documentare tutte le strade lecite, sicure e pertinenti: endpoint ufficiale alternativo, copia ufficiale scaricabile, fonte istituzionale equivalente, browser reale, formato alternativo, controllo del contenuto e della provenienza, riuso di componenti o procedure native. Deve distinguere rigorosamente un guasto del canale di acquisizione dal difetto della fonte o della funzione. Solo dopo l'esaurimento verificabile delle alternative può riportare un vincolo esterno; anche in quel caso deve lasciare una procedura primaria completa, una revisione esplicita e nessuna classificazione, conferma o certezza fittizia.
- **Ragionamento tecnico obbligatorio prima del codice:** prima di scrivere codice, modificare UI, creare dati o lanciare procedure operative, Codex deve fermarsi ad analizzare con logica da programmatore senior: quale risultato concreto va raggiunto, cosa serve davvero, cosa non serve, quali parti esistenti del software o librerie già presenti possono essere riusate, quali fonti ufficiali o procedure native governano il caso, quali soluzioni sono lecite e sicure, cosa potrebbe non funzionare, perché potrebbe non funzionare e come ogni rischio verrà risolto. L'analisi deve coprire codice, grafica, struttura dati, route, payload, React full, API, test, audit, script, documentazione, performance, sicurezza, accessibilità, italiano visibile, date italiane e responsive. Se durante questo ragionamento emerge un problema, Codex deve trattarlo come parte del lavoro da risolvere al 100% con verifiche reali e visive, non come limite da lasciare indietro o da coprire con un fallback.
- **Riallineamento locale post-GitHub obbligatorio:** se dopo un commit/push vengono fatte modifiche per far passare GitHub Actions, CodeQL, lint, shard Pytest, coverage, Local Signer o qualunque gate remoto, quelle stesse modifiche devono essere riportate anche sulla macchina locale reale. Dopo il nuovo push verde, ricostruire o ricreare la copia Docker locale su `127.0.0.1:8080`, verificare `/api/pronto` sullo stesso commit e ripulire eventuali file runtime generati prima del report finale.
...
- **Container applicativo Hetzner unico obbligatorio:** dopo ogni deploy, commit/push operativo o hotfix server su Hetzner bisogna verificare che esista un solo container applicativo e che il suo nome sia esattamente `iusentra-app`, gestito dal profilo `deploy/hetzner`. Non devono restare container paralleli come `iusentra-app-1`, container del progetto Compose `repo`, app su porta 8080 fuori profilo Hetzner o copie applicative con lo stesso servizio. Se compare piu' di un container app o il nome non e' `iusentra-app`, il deploy e' non valido: fermare/rimuovere la copia parallela senza toccare volumi o dati applicativi, riallineare `deploy/hetzner/docker-compose.hetzner.yml`, ricreare il servizio corretto e solo dopo verificare `docker ps`, `docker compose -f deploy/hetzner/docker-compose.hetzner.yml ps`, container healthy e `https://app.iusentra.it/api/pronto`.
- **Pulizia immagini Docker obbligatoria dopo ogni deploy Hetzner:** a fine deploy eseguire sempre `bash deploy/hetzner/cleanup_docker_images.sh` dal repo sul server, verificando che rimuova le immagini IUSENTRA vecchie non usate dai container attivi, esegua `docker builder prune --all --force` e rimuova eventuale `/opt/iusentra/tmp-backup-snapshot` residuo. Immagini IUSENTRA obsolete, cache build e snapshot temporanei non operativi sono rigenerabili/non attivi e non contengono il dato corrente degli studi; non usare mai comandi che cancellano volumi, database, tenant, documenti, PEC o dati applicativi.
...
- **Importi visibili sempre in formato italiano:** ogni importo mostrato all'utente in UI, PDF, email, PEC, report, pannelli operativi, dashboard, preview e documenti generati deve usare formato italiano con simbolo euro, per esempio `€ 1.234,56`. Sono vietati nei testi visibili formati come `EUR 1.234,56`, `€ 1,234.56` o numeri economici senza simbolo quando il contesto è monetario. Eccezione: payload tecnici, XML FatturaPA/SdI, tracciati ministeriali, API interne numeriche e campi macchina devono mantenere il formato richiesto dallo schema tecnico.
- **Standard unico obbligatorio per lingua, date e orari visibili:** tutto IUSENTRA deve parlare italiano coerente e mostrare date/orari con un solo standard, senza eccezioni locali di pagina: date come `28/06/2026` o mese scritto in italiano quando il contesto lo richiede, data e ora come `28/06/2026 21:30`, fuso sempre `Europe/Rome` quando c'è un orario. Ogni data, orario o timestamp mostrato all'utente in UI, PDF, email, PEC, report, audit, dashboard, preview, pannelli amministrativi, documenti generati, ricevute, pagamenti, scadenze, topbar, assistente, export leggibili e modali deve rispettare questo formato. È vietato mostrare all'utente valori raw come `2026-04-09`, `2026-06-28T19:30:19Z`, etichette `Data UTC`, suffissi `UTC`, formati inglesi o formati internazionali in testi visibili. La regola vale espressamente anche per data/ora di arrivo PEC, email ordinaria, invio/consegna/ricevuta, esiti SdI/PCT, notifiche, log operativi, audit visibili, pagamenti e PDF di fatture, parcelle, preventivi, incarichi e atti. Eccezioni ammesse solo per valori macchina non rivolti all'utente: payload tecnici, XML/XSD/EML originali, header email originali, campi `input type="date"` come valore HTML, API interne numeriche/ISO, firme, timestamp RFC 3161 e tracciati ministeriali; appena quei valori vengono rappresentati a video o in un documento leggibile devono essere convertiti in italiano `Europe/Rome` con gli helper condivisi (`format_date_it`, `format_datetime_it`, `formatDateIt`, `formatDateTimeIt`).
- **Controllo professionale dei dettagli visibili obbligatorio:** prima di dichiarare consegnato un lavoro, Codex deve ampliare la verifica a tutti i dettagli percepiti dall'utente: orari, stati, pulsanti abilitati/disabilitati, testi, microcopy, accenti, punteggiatura, layout, spazi, responsive e coerenza del flusso. I dettagli e le "virgole" sono parte della qualità professionale, non rifiniture opzionali.
- **Verifica visiva materiale obbligatoria per ogni funzione UI:** quando una modifica introduce o cambia pulsanti, card, pannelli, form, input, permessi browser, microfoni, upload, menu, navigazione o qualunque interazione visibile, Codex deve provarla cliccando materialmente nel browser reale su `127.0.0.1:8080` dopo build/rebuild della copia Docker locale. I test automatici, CDP sintetico, unit test o screenshot non bastano da soli: bisogna verificare davanti all'interfaccia reale che ogni pulsante reagisca, che i testi siano leggibili e in italiano, che gli stati di errore/successo/loading siano comprensibili, che card e layout non taglino contenuti e che mobile/tablet/desktop restino coerenti. La verifica deve includere sempre anche passaggio del mouse e focus da tastiera sui controlli interattivi principali: hover, focus, selected, disabled e loading devono mantenere testo leggibile, contrasto professionale, icone visibili, nessun cambio colore che faccia sparire le etichette e nessun salto di layout. La verifica deve scorrere sempre tutta la pagina o il pannello, dall'alto al centro e fino al fondo, senza fermarsi alla prima schermata visibile. Se durante il controllo emerge qualunque difetto grafico, testo tagliato, card sproporzionata, pulsante poco chiaro, overflow, stato ambiguo, hover/focus illeggibile o comportamento non funzionante, Codex deve correggerlo subito e ripetere la verifica visiva completa. Se l'utente chiede di vedere, il browser va reso visibile e la prova va eseguita davanti a lui prima di dichiarare risolto.
- **Sviluppo grafico rapido obbligatorio prima dei gate completi:** quando l'utente segnala un difetto UI visibile, Codex deve prima usare un ciclo breve e materiale: guardare la pagina reale gia' aperta o aprirla su `127.0.0.1:8080`, identificare il difetto visivo, fare una modifica minima mirata, ricostruire/ricaricare solo quanto necessario e visualizzare subito il risultato davanti all'interfaccia. Solo dopo che il comportamento e la grafica risultano corretti nella prova reale si passa a procedure complete, test estesi, documentazione, commit, push e deploy. Non e' ammesso consumare ore in gate generali prima di verificare se la modifica grafica risolve davvero il problema percepito.
- **Regola sviluppo reale-prima per difetti visibili:** d'ora in poi, quando l'utente segnala un problema percepibile nella UI o in un flusso operativo reale, Codex deve sviluppare subito la correzione minima, provarla immediatamente nella vista reale indicata dall'utente e solo dopo che funziona creare o aggiornare i test automatici e lanciare i gate necessari. I test servono a blindare una soluzione gia' verificata materialmente, non a sostituire la prova reale iniziale. Se la prova reale fallisce, si corregge subito e si riprova prima di passare a test lunghi, commit, push o deploy finale.
- **Workflow server-first per difetti di produzione:** quando il difetto e' visibile su `https://app.iusentra.it` o l'utente chiede prova sul server reale, Codex deve applicare prima il fix sul server Hetzner reale usando solo codice/sorgenti e senza toccare volumi o dati applicativi, verificare la pagina reale in produzione, poi riportare lo stesso codice nella copia locale, eseguire gate mirati, commit, push dei branch gemelli, deploy ordinato e igiene. Il lavoro resta aperto finche' server, locale e GitHub non coincidono sullo stesso commit verificato.
- **Sequenza unica obbligatoria per richieste composte sul server:** quando l'utente affida piu' modifiche collegate e chiede di lavorare o provare direttamente in produzione, Codex deve prima completare l'intero perimetro funzionale sul server reale, mantenendo un registro puntuale delle voci fatte e ancora aperte; solo dopo deve eseguire un'unica campagna end-to-end con click reali, scroll, hover/focus, salvataggi e risultati osservabili su `https://app.iusentra.it`. Soltanto dopo il superamento di questa campagna deve riallineare una volta la copia locale, eseguire i gate mirati e completi richiesti, aggiornare la documentazione, creare commit, pushare entrambi i branch gemelli e svolgere il deploy finale ordinato. Sono vietati cicli alternati server-locale-test parziali che interrompono l'implementazione complessiva o fanno ripetere all'utente le stesse prove. Un hotfix urgente puo' essere provato immediatamente sul server per ripristinare una funzione bloccante, ma non autorizza un riallineamento locale intermedio: confluisce comunque nella stessa chiusura unica del perimetro composto.
- **Dove si trova il browser reale da usare:** quando l'utente ha il browser integrato Codex aperto su `http://localhost:8080` o `http://127.0.0.1:8080`, quella scheda e' la fonte visiva primaria. Se il controllo automatico non riesce ad agganciare la scheda autenticata del browser integrato, Codex non deve dichiarare che "non c'e'": deve usare lo screenshot reale dell'utente o la schermata visibile come prova, correggere il codice e chiedere/ottenere nuova visione materiale sulla stessa pagina. Non usare come prova finale Chrome temporanei non autenticati o sessioni parallele.
- **Percorso UI deposito React:** il flusso `Prepara deposito`, compresa la lista `Documenti da inviare`, i pulsanti icona `Visualizza`/`Scarica`, il selettore ruolo e il pannello laterale `Dati fascicolo / Slot documentali / Audit`, vive in `frontend/src/components/FascicoliPage.tsx` con stili in `frontend/src/components/FascicoliPage.css`. Prima di dire che un elemento non esiste, cercare questi due file.
- **Sotto-modulo note vocali obbligatorio nell'assistente Studio:** le note vocali sono parte dell'assistente vocale, non una funzione separata. Devono attivarsi con `Studio, note`, ma il sotto-comando deve essere personalizzabile dallo studio senza cambiare per forza la frase di attivazione generale. Devono accettare dettatura libera, chiudersi con `fine nota` o `fine note`, estrarre data e ora quando presenti, salvare la trascrizione testuale e predisporre un promemoria browser 10 minuti prima dell'evento quando il permesso notifiche è concesso. Ogni orario estratto o mostrato deve essere in ora italiana con fuso `Europe/Rome`, mai in UTC o in formato ambiguo. Per tutela dei dati legali non va salvato audio grezzo. Ogni consegna su questo sotto-modulo deve documentare il limite operativo: notifiche e timer frontend funzionano mentre IUSENTRA resta aperto; per promemoria affidabili a browser chiuso serve integrazione successiva con backend, Agenda, service worker e Web Push. I test devono includere flusso visivo completo da configurazione, attivazione, comando `Studio, note`, dettatura, `fine nota`, salvataggio, promemoria, responsive desktop/tablet/mobile e disattivazione ascolto.
- **Orari visibili sempre in ora italiana:** ogni timestamp salvato o ricevuto in UTC deve essere convertito e mostrato in `Europe/Rome` su UI, report, pannelli amministrativi, audit, PEC/email in arrivo, ricevute, notifiche, esiti telematici e verifiche browser. Non dichiarare corretto un flusso se un orario visibile resta in UTC, espone una stringa ISO raw o non coincide con l'ora italiana attesa.
- **Parità struttura dati SQLite/PostgreSQL obbligatoria:** ogni modifica a dati, repository, tabella, indice, vincolo, audit o nuova funzionalità persistente deve aggiornare e verificare sia la struttura SQLite locale sia la struttura PostgreSQL di produzione. È vietato aggiungere campi o flussi dati solo su uno dei due backend.
- **SQL/PostgreSQL fonte di verità, JSON solo mirror:** per ogni studio in modalità `SQLITE` o `POSTGRESQL`, la fonte operativa dei dati strutturati è `studio.db` o PostgreSQL. I file JSON tenant-aware possono esistere solo come bootstrap controllato, import/export storico, cache o mirror rigenerabile (`moduli_dati`/`moduli_json_records`), ma non devono essere usati per conteggi reali, riparazioni massive, audit conclusivi o decisioni operative quando esiste SQL. Se `studio.db` manca o non è popolato, il runtime deve provare a crearlo/riallinearlo; se non riesce, deve bloccare con messaggio chiaro invece di ricadere su JSON vecchi. Gli script di audit devono indicare esplicitamente `source_of_truth=sqlite/postgresql` e trattare JSON mancanti o non allineati come avvisi sui mirror, non come verità dei dati.
- **Mirror SQL governati, mai scansione ricorsiva runtime:** il bootstrap e il caricamento ordinario del tenant devono sincronizzare in `moduli_dati`/`moduli_json_records` soltanto i moduli JSON core dichiarati esplicitamente. È vietato includere automaticamente tramite `rglob` o scansione ricorsiva alberi pesanti o rigenerabili come `documenti_ai`, OCR, testi estratti, staging di importazione, dataset Lex, PDF/ZIP o indici di ricerca: questi dati restano nei repository verticali dedicati. Una scansione ricorsiva è ammessa solo in script di audit/riparazione espliciti, con opzione nominata, stima preventiva di file/byte, backup e report `source_of_truth`; non deve mai partire da Agenda, Scadenziario, topbar, worker PEC o apertura pagina.
- **UI lato studio/prodotto sempre full React:** tutte le pagine lato studio e lato prodotto devono essere full React. Se Codex incontra una pagina, sezione, form o flusso lato studio non full React, deve promuoverlo automaticamente a React end-to-end con API reali, senza aspettare che l'utente lo segnali. Al momento l'unica area esclusa da questa regola automatica è il lato SUPERADMIN, che resta fuori finché non viene pianificata la relativa tranche.
- **Produzione React, non legacy:** da ora la produzione IUSENTRA lato studio/prodotto deve essere considerata React come superficie primaria. Diagnosi, fix, test visivi, Local Signer, deposito, PEC, fascicoli, impostazioni, topbar e nuovi flussi operativi devono puntare alle route/componenti React e alle relative API JSON. Il legacy può restare solo come fallback tecnico governato, documentato e non primario; non va usato come prova utente, non va indicato come percorso operativo e non va scelto per comodità quando esiste o deve esistere la superficie React.
- **Lettore documenti unico interno obbligatorio:** ogni documento o fonte aperta da Agenda, Scadenziario, PEC/email, Fascicoli, Notifiche legali, topbar o web push deve restare dentro IUSENTRA usando il nostro lettore/preview, senza uscire dal software come comportamento primario. La logica deve essere unica e tenant-aware per tutti i casi equivalenti: `Apri fonte`, `Apri origine`, `Visualizza fonte`, `Visualizza` allegato e preview da PEC devono risolvere la fonte reale collegata e aprire subito il contenuto utile quando disponibile. I formati da supportare senza regressioni sono almeno: PDF; ZIP PEC con individuazione/estrazione del documento utile interno, in particolare il PDF del provvedimento; XML; EML/MIME; TXT; DOC; DOCX; immagini JPG/JPEG, PNG, TIFF/TIF; P7M/SMIME come buste/firme con metadati e contenuto leggibile quando estraibile. Se un formato non può essere reso in modo sicuro, la UI deve mostrarne chiaramente il motivo e offrire download/azione coerente, ma non deve restare muta, vuota o aprire una pagina esterna come soluzione finale. Ogni modifica al lettore va provata con click reale su `127.0.0.1:8080` per corpo PEC, allegati, ZIP e almeno un formato non-PDF coinvolto.
- **Nessun ritorno operativo al fallback legacy nelle aree già dichiarate full React:** se una route/sezione è stata promossa o dichiarata full React, Codex deve trattare eventuali fallback legacy solo come paracadute interno da rimuovere o neutralizzare progressivamente, non come alternativa di lavoro. Prima di citare, usare o testare un fallback legacy bisogna dimostrare che non esiste ancora una superficie React governata; in caso contrario il fix deve avvenire nel componente React, nell'API JSON o nel gate React collegato.
- **Perimetro full React confermato dall'utente:** sono da considerare produzione full React `Panoramica`, `Regia Operativa`, `Ricerca Studio`, `Agenda`, `Fascicoli`, `Clienti e Anagrafiche`, `Soggetti e Parti`, `Comunicazioni`, `Scadenze e Termini`, `Servizi Telematici`, `Studio` con `Parcelle e Fatture`, `Preventivi e Incarichi`, `Compensi Forensi`, `Documenti`, `Editor professionale`, `Redazione Atti`, `Statistiche`, `Ricerca Legale`, `Legal Skills`, `Regia Agentica`, `Archivio Giurisprudenza`, `Strumenti Forensi`, `Strumenti Operativi`, `Sito Studio`, `Impostazioni` e `Amministrazione` lato studio. L'unica parte rimasta fuori da questa regola è `SUPERADMIN`, finché non viene pianificata e promossa anche quella.
- **Deposito telematico: risoluzione immediata, non rinvio guidato:** nel flusso `Prepara deposito` Codex non deve trattare classificazione, indice documenti o firma multipla come "fase finale guidata" se il software può risolverli subito. Il software deve leggere l'intero fascicolo, classificare automaticamente atto principale/allegati/prove, generare l'indice in tempo reale e, quando Local Signer o backend firma lo supportano, firmare più documenti con un solo comando/PIN verificando e salvando ogni esito nel fascicolo prima dell'invio. Solo i requisiti obbligatori realmente mancanti o non producibili dal software possono bloccare l'invio; gli elementi non obbligatori vanno mostrati come avvisi non bloccanti. Se manca l'adapter ministeriale reale per `Atto.msg` cifrato in `Atto.enc` AES256, il software deve preparare tutto il pacchetto possibile, spiegare esattamente cosa manca per la conformità e impedire solo la registrazione dell'invio come deposito valido, senza blocchi muti o rinvii generici.
- **Deposito telematico: bottone `Invia deposito reale`:** il bottone `Invia deposito reale` non deve restare disabilitato per prudenza generica o per vecchi blocchi non più attuali. Dopo prova senza invio, firme salvate, indice verificabile, destinatario PEC verificato, testo PEC controllato, busta/trasporto ministeriale conforme e assenza di requisiti obbligatori mancanti, il bottone deve attivarsi. Se resta disabilitato, la UI deve mostrare esattamente quale requisito obbligatorio manca; se tutti i requisiti previsti risultano rispettati, Codex deve correggere subito la logica di abilitazione prima di commit, push o deploy.
- **Deposito/notifiche/PEC operative: invio sempre dal PC locale:** il server IUSENTRA non deve mai essere il canale SMTP reale per depositi, notifiche legali o invii PEC operativi. La regola da preservare è quella visibile in `/impostazioni?tab=pec`, sezione `Verifiche PEC`: `Il controllo dell'invio parte dal PC in uso: la password resta sul dispositivo locale.` Anche quando l'app gira su `https://app.iusentra.it`, il server prepara e verifica busta, destinatario, oggetto, corpo PEC, allegato `Atto.enc` e ricevute, ma l'invio effettivo parte dal PC dell'avvocato tramite Local Signer/servizio locale. Qualunque variabile, rotta legacy o fallback che abiliti invio SMTP server-side per un invio legale è regressione bloccante.
- **Configurazione rapida nello stesso flusso:** quando un requisito configurabile blocca una funzione operativa, per esempio PEC SdI, PEC mittente, email commercialista, firma o canale Local Signer, la UI deve offrire nello stesso pannello un'azione rapida per inserire o correggere i dati essenziali, salvare tramite le API reali di Impostazioni, ricaricare lo stato e permettere di proseguire senza far perdere contesto all'avvocato. Il collegamento alle impostazioni complete resta ammesso, ma non deve essere l'unica via quando bastano pochi campi governati.
- **Deposito telematico: chiusura al 100% o lavoro aperto:** il deposito non può essere dichiarato completo se restano mancanti certificato pubblico PST `.cer`, `Atto.enc` AES256, invio PEC reale abilitabile, presidio ricevute o prova visiva reale. Se un requisito dipende da fonte ministeriale esterna, Codex deve provare il recupero sul caso concreto, salvare la prova tecnica/documentale, spiegare in UI e nei report se il PST non pubblica quel dato, e cercare il canale/ufficio ufficiale corretto prima di fermarsi. Un fallback, una simulazione, una prova guidata o un blocco corretto non sono consegna finale quando l'incarico dell'utente richiede invio reale funzionante.
- **Deposito telematico e firma multipla: prova reale e blocco anti-regressione:** ogni nuova funzione del deposito, in particolare la firma multipla Local Signer/PKCS#11, deve essere provata subito sulla macchina reale quando è pronta. Non dichiarare funzionante la firma multipla finché, con browser reale su `127.0.0.1:8080`, l'utente non ha inserito il PIN e il software non ha firmato più documenti selezionati nella stessa operazione, salvato ogni `.p7m`, aggiornato il fascicolo e riabilitato il passo successivo del deposito senza errori. Dopo un test reale positivo, la parte verificata va blindata con test mirati, contratti o guardrail statici e documentazione di stato: qualunque modifica futura su quel codice deve preservare tali guardrail, rilanciare le verifiche specifiche e ripetere la prova reale quando il comportamento utente cambia. Sono vietati refactor non necessari o modifiche non presidiate su codice già verificato realmente.
- **Relata di notifica e prova notifica: chiusura solo con prova reale:** la relata/notifica può essere dichiarata conforme solo dopo confronto con fonti normative ufficiali, generazione o visualizzazione del testo reale, verifica dei dati obbligatori, controllo firma digitale della relata quando richiesta, prova senza invio reale e verifica documentale nel fascicolo. Se tutto è conforme, l'esito va scritto in `artifacts/react-migration/procedura-deposito-telematico.md` e nei report di test; se manca un requisito, la UI deve indicare precisamente cosa manca e non deve registrare la notifica come effettiva.
- **Documentazione obbligatoria per funzioni operative sensibili:** ogni intervento, diagnosi o test che tocchi deposito telematico, fascicoli, classificazione documenti, connessioni ai portali ministeriali, PEC, notifiche legali, firma digitale, Local Signer, PKCS#11, buste, ricevute o agenda/scadenziario derivati da PEC deve essere trascritto in file prima del report finale. La documentazione deve indicare cosa è stato cambiato, fonti/norme usate, dati o allegati reali consultati, test automatici eseguiti, prova reale su `127.0.0.1:8080` eseguita o mancante, limiti residui e stato anti-regressione. Per il deposito usare sempre anche `artifacts/react-migration/procedura-deposito-telematico.md`; non lasciare decisioni operative solo nella chat.
- **Memoria obbligatoria documenti del fascicolo d'ufficio:** prima di modificare consultazione PST/PolisWeb, catalogo documenti, struttura principale/allegati, download copia/originale, deduplicazione o acquisizione nel fascicolo, rileggere `artifacts/react-migration/confronto-documenti-fascicolo-ufficio.md`. Quel dossier conserva il confronto con il flusso decompilato, la mappa dei campi, le regole di riconciliazione, la copertura già presente in IUSENTRA e la matrice di collaudo. Ogni nuova scoperta o prova reale deve aggiornare lo stesso file; è vietato lasciare l'evoluzione soltanto nella chat.
- **Il fascicolo è il cuore operativo del prodotto:** ogni capacità che legge, crea, classifica, analizza, firma, deposita, notifica, archivia o collega un documento deve nascere dal fascicolo e ritornarvi con collegamento verificabile. Non sono ammessi archivi documentali isolati, decisioni prive di `tenant_id` e `fascicolo_id`, né automazioni che perdano provenienza, versione, evidenze, audit, stato di revisione, scadenze o azioni conseguenti. SQL, repository, resolver, pipeline, API e React devono quindi mantenere il fascicolo come contesto unico e immediatamente consultabile dall'avvocato, senza duplicazioni concorrenti o percorsi esterni al prodotto.
- **Incarico dati/tenant/React/topbar permanente:** dopo ogni compattazione e prima di proseguire su flussi dati, route React, topbar, deposito, fascicoli, PEC, firma, portali o amministrazione database, rileggere anche `artifacts/data-flow/incarico-operativo-permanente.md`. Ogni nuova funzionalità deve passare il doppio controllo operativo: dato inserito, tenant proprietario, path tenant-aware, eventuale JSON storico, tabella SQLite, tabella PostgreSQL o repository dedicato, API JSON, route/componente React, test e prova reale. La topbar non deve essere solo grafica: `Voce Studio`, `Assistenza remota`, data italiana, `Nuovo`, notifiche, ultimi elementi aperti, scadenze rapide e timer devono restare collegati a dati/API reali e coperti da test.
...
- **Ogni gate tecnico deve essere seguito da accettazione reale:** i test automatici, inclusi quelli con browser headless, CDP, Playwright, dati simulati, permessi mockati, riconoscimento vocale simulato o servizi finti, servono solo a diagnosticare e prevenire regressioni tecniche. Dopo qualunque modifica che l'utente puo' vedere o usare, Codex deve ricostruire/aggiornare la copia reale locale, aprire la funzione sulla macchina reale e verificare materialmente il flusso completo. Esempi inclusi ma non limitativi: microfono/fotocamera, registrazione voce, PIN, Local Signer, creazione cliente, salvataggi, apertura pannelli, ricerca, email/PEC, agenda, notifiche, upload/download, impostazioni, responsive e performance percepita. Se la macchina reale mostra un problema anche dopo test verdi, il problema prevale sui test e va corretto nel codice o nel deploy locale prima di qualunque report positivo.
- **Test permesso microfono assistente vocale:** il controllo del permesso reale deve essere eseguito su Google Chrome installato (`C:\Program Files\Google\Chrome\Application\chrome.exe`) e non su Chromium/Playwright. Il report deve indicare `chromePath`, stato restituito da Chrome (`granted`, `prompt`, `denied`), screenshot visibile e comportamento dell'app: se Chrome blocca il microfono, la registrazione non deve partire e nessun profilo vocale deve essere salvato.
...
- **Salvataggio permanente direttive normative obbligatorio:** ogni direttiva normativa, fonte ufficiale, specifica ministeriale, schema XSD/DTD/XML, tracciato PEC/EML, regola tecnica, matrice casi/destinatari, regola su firma digitale, allegati, ricevute, deposito prova o prassi operativa usata dal software deve essere salvata e versionata nella repository prima o insieme al codice che la applica. Le regole non devono restare nella memoria della chat o in assunzioni implicite: vanno rappresentate in documenti sotto `docs/` o `docs/specs/ministero/`, in costanti/fixture strutturate quando servono al runtime, e in test di regressione che impediscano modifiche arbitrarie. Se una fonte cambia o non è certa, registrare fonte, data di consultazione, ambito, limite e comportamento prudente adottato.
- **Ricerca fonte mancante obbligatoria:** se manca una normativa, direttiva, schema ministeriale, XSD, DTD, tracciato PEC/EML o regola tecnica necessaria a implementare una funzione telematica, Codex deve cercarla in fonti ufficiali o nei pacchetti ministeriali già presenti, salvarla nel repository con data/ambito di consultazione e solo dopo implementare la regola. È vietato colmare lacune normative con logica inventata; se la fonte resta incerta, il software deve produrre un warning professionale configurabile e non un automatismo arbitrario.
- evitare scraping HTML non autorizzato dei portali;
- non salvare PIN, credenziali CNS/CIE/SPID o sessioni portale nel cloud;
- **Packaging Windows Local Signer blindato:** ogni nuovo pacchetto Windows deve usare lo stesso profilo certificato del `SetupLocalSigner-1.6.35.exe`: builder nativo IExpress da `tools/build_local_signer_windows_exe.ps1`, file pubblico `SetupLocalSigner-<versione>.exe` con alias `SetupLocalSigner.exe`, SED `Class=IEXPRESS` / `InsideCompressed=0`, avvio tramite `powershell.exe -NoProfile -ExecutionPolicy Bypass -File installa_local_signer_locale.ps1` e file principali copiati dal CAB locale. Sono vietati PyInstaller, NSIS, zip autoestraenti, link `.ps1` come download Windows pubblico, cambio di formato o download dinamici dei file principali durante l'installazione Windows senza nuova certificazione reale, aggiornamento dei test e autorizzazione esplicita dell'utente.
...
## OCR — REGOLA OBBLIGATORIA (motore unico)

- Il riconoscimento del testo passa da **un solo motore**: `legal_ocr/motore/` (runtime Tesseract con un thread per processo, preparazione immagine, strategia di lettura, secondo lettore PDF Inspector a consenso, pagina con impaginazione e formato, solo testo, provisioning verificato del dizionario) e `legal_ocr/formulario/` (correzioni legali deterministiche e dichiarate: abbreviazioni, punteggiatura, cifre, numeri romani, euro, accenti, marcatori degli elenchi).
- `web/services/document_ocr*.py`, `pct/ocr.py`, `pct/editor.py`, `pct/document_intelligence/extraction.py`, `legal_ocr/engines.py`, `pct/pec_ocr_pipeline.py`, `lex/tools/_doc_extractor.py` e il lettore delle notifiche sono adattatori del motore: **è vietato chiamare `pytesseract` o il binario `tesseract` altrove**, e ogni nuova lettura di immagini o PDF scansionati usa `legal_ocr.motore` (`riconosci_immagine`, `testo_da_immagine`, `testo_da_pdf`).
- Ogni regola nuova del formulario dichiara `id`, etichetta e motivo, ha un test in `tests/test_formulario_legale.py` e non interpreta il contenuto: normalizza solo la scrittura alle forme forensi d'uso (Normattiva, Cassazione). Documentazione: `docs/OCR_LEGAL.md`.
...
- **Archivio di alimentazione (2.317.0)**: i dati letti dai documenti e dalle PEC (date con il loro campo, numeri di ruolo, prove di notifica, eventi) entrano nell'archivio `letture_fatti` solo attraverso i due motori di `pct/archivio_letture/` (motore documenti e motore PEC) e solo dopo il collaudo automatico (`collaudo.py`: calendario, forma, ancoraggio, orizzonte, doppia lettura, concordanza). I presìdi (documentale, notifiche, economico, Lettura del fascicolo) **consultano l'archivio** con `web/services/archivio_letture_runtime.fatti_fascicolo` e le viste di `pct/archivio_letture/presidi.py`: vietato rileggere o riparsare i documenti in un presidio o in una richiesta. Una data senza ancora nel testo non è un fatto; un fatto `respinto` non si propone; solo udienze e termini `plausibili` si chiedono in conferma. Nessuna lettura nella richiesta dell'avvocato: alimentano l'archivio la lettura automatica dello scheduler, gli eventi documento/PEC in sfondo e il worker OCR. Ogni nuova regola di estrazione va coperta dal corpus di collaudo (`legal_ocr/collaudo/corpus.py`), che gira con l'OCR reale nei test e ogni notte.
- **Riferimenti normativi, lessico e riconvalida (2.318.0)**: un riferimento normativo non è mai una data — le abbreviazioni forensi si dichiarano solo in `legal_ocr/formulario/riferimenti_normativi.py` e chi cerca date chiama `e_riferimento_normativo` prima di accettare un token. Le parole si correggono solo con `legal_ocr/lessico/` e solo quando la parola letta non è nel lessico e **una sola** parola del lessico si ottiene con le confusioni dichiarate: mai scegliere fra due parole esistenti, mai toccare numeri, date, codici o riferimenti normativi. Ogni regola che si stringe deve prevedere la **riconvalida** (`pct/registro_letture/riconvalida.py`): le anomalie aperte e i fatti già registrati che oggi non nascerebbero si chiudono o si respingono da soli, con il motivo e senza toccare le decisioni dell'avvocato. Si chiede conferma solo per le date che cambiano agenda o scadenziario (udienze, termini e costituzioni **future**, `da_confermare_ora`), e nessun oggetto dell'inventario può restare «da leggere» per sempre: se il documento o la PEC non ci sono più, la lettura si chiude come non leggibile con il motivo.
- Base normativa: art. 3 D.M. 44/2011 e art. 20 CAD (impronta e integrità dei documenti informatici).

## Presidio del fascicolo e fase di deposito — REGOLA OBBLIGATORIA

- **I controlli della busta valgono quando si deposita.** Il presidio del fascicolo e il percorso di deposito devono dire la stessa cosa: PDF/A (art. 12 D.M. 44/2011), firma CAdES, limiti dimensionali DGSIA, codice oggetto PST e generazione della busta si eseguono solo quando `pct/practice_engine/fase_deposito.fase_deposito()` dichiara la fase `preparazione` o `in_corso`. Fuori da quella fase i controlli restano dichiarati come rinviati (`NOT_APPLICABLE`, `status: NON_IN_DEPOSITO`) con il motivo in italiano e **non bloccano**: un fascicolo importato dal portale contiene copie di atti già depositati da altri, che non devono essere PDF/A né firmati da noi.
- **Ogni voce della checklist dichiara i propri controlli** in `pct/practice_engine/voci_checklist.py`: una voce senza controlli non può diventare «completata» e falsa il completamento. Aggiungendo una voce al catalogo delle procedure si aggiunge la sua riga in `CONTROLLI_PER_VOCE`; un controllo della busta si aggiunge anche a `CONTROLLI_DI_DEPOSITO`.
- **La percentuale di completamento è spiegabile**: conta solo le verifiche che il software può misurare ora (`pct/practice_engine/completamento.py`) ed espone `header.completionDetail`. Vietato contare nel denominatore requisiti rinviati o verifiche che nessun controllo automatico può decidere.
- **Lo stato del deposito è quello reale del fascicolo**: si legge da `depositi_pct` con `deposito_reale()` e si mostra con l'etichetta del flusso ufficiale PCT. Un deposito inviato e accettato non può risultare «non inviato» perché manca la sessione della regia.
- Base normativa: D.M. 44/2011 artt. 12 e 14 e Specifiche tecniche DGSIA per la busta; art. 196-quater disp. att. c.p.c. per l'obbligo di deposito telematico.
