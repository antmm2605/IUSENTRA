...
| `python -m py_compile lex\legal_sources\adapters\brocardi.py scripts\import_normative_brocardi.py`; `python -m flake8 lex\legal_sources\adapters\brocardi.py scripts\import_normative_brocardi.py`; `python -m pytest -q tests/test_brocardi_adapter.py --tb=short` | OK | Adapter Brocardi registrato come fonte secondaria sicura, disabilitata di default e senza accesso rete nel campione. |

## PEC, Agenda, Scadenziario e calendario diretto 2.253.12 - 2026-06-14

| Comando / verifica | Esito | Nota |
| --- | --- | --- |
| `python -m py_compile pct\calendar_sync_engine.py pct\calendar_providers\google.py pct\ical.py pct\pec_pipeline.py web\blueprints\pec_pipeline_api.py` | OK | Sintassi confermata dopo motore calendario diretto, push selettivo scadenze PEC e fallback pulito quando il database calendario non e' configurato. |
| `python -m pytest tests/test_pec_audit_pipeline.py::test_remote_hearing_report_uses_pdf_ocr_link_and_persists_lawyer_actions tests/test_pec_audit_pipeline.py::test_remote_hearing_extracts_full_teams_launcher_url_without_truncation tests/test_pec_audit_pipeline.py::test_remote_hearing_rebuilds_teams_url_split_by_pdf_ocr_spaces tests/test_pec_audit_pipeline.py::test_remote_hearing_profile_reads_pdf_zip_even_if_misclassified_as_daticert tests/test_pec_audit_pipeline.py::test_pec_remote_hearing_link_arrives_in_scadenziario_and_agenda tests/test_pec_audit_pipeline.py::test_pec_remote_hearing_clickable_pdf_link_arrives_in_scadenziario_and_agenda tests/test_pec_audit_pipeline.py::test_pct_deposit_lifecycle_explains_expected_pec_sequence tests/test_pec_audit_pipeline.py::test_pec_api_acquisisce_mime_locale_da_casella_storica tests/test_pec_audit_pipeline.py::test_pec_api_acquisisci_locali_prosegue_a_blocchi_e_azzera_presidio tests/test_pec_audit_pipeline.py::test_pec_api_schedula_duplicato_audit_senza_report_da_mime_locale tests/test_pec_audit_pipeline.py::test_pec_deadline_without_time_pushes_to_calendar_engine_as_all_day tests/test_react_shell.py::test_react_agenda_bridge_usa_agenda_e_scadenziario_reali tests/test_react_scadenziario_additions.py::test_react_scadenziario_page_collegata_nav_api_e_lex tests/test_react_scadenziario_additions.py::test_react_scadenziario_bridge_espone_link_udienza_remota tests/test_react_scadenziario_additions.py::test_react_scadenziario_bridge_nasconde_testi_tecnici_da_pec tests/test_calendar_sync_engine.py tests/test_calendar_sync.py -q --tb=short` | OK | 26/26: PEC deposito fuori da Agenda/Scadenziario, presidio locale incrementale, link udienza remota preservati, scadenza PEC senza orario non crea Agenda e viene pushata come evento a giornata intera verso il motore calendario. |
| `pnpm --filter @iusentra/studio typecheck` | OK | TypeScript confermato dopo UI Impostazioni calendario e tooltip Agenda. |
| `pnpm --filter @iusentra/studio build` | OK | Build Vite riuscita; resta il warning storico sul chunk principale sopra 500 kB. Non sostituisce la verifica reale Docker/browser su `127.0.0.1:8080`. |
| `node frontend/scripts/check-react-contracts.mjs` | OK | Contratti React ancora coerenti dopo route deposito, Agenda/Scadenziario e Impostazioni calendario. |
| `node scripts/react-migration/check-route-gate.mjs` | OK | Route gate coerente: nessuna regressione nota a legacy sulle route governate. |
| `python -m pytest tests/test_utf8_integrity.py -q --tb=short` | OK | 4/4: integrita' UTF-8 confermata dopo aggiornamento dei report e dei testi calendario. |
| Ricerca mirata dei pattern mojibake sui file toccati | OK | Nessuna sequenza mojibake trovata nei report aggiornati e nei sorgenti PEC/calendario/Impostazioni toccati. |
| `docker compose build --no-cache app scheduler-worker ocr-worker`; `docker compose up -d --force-recreate app scheduler-worker ocr-worker`; `GET http://127.0.0.1:8080/api/pronto` | OK | Copia Docker locale reale ricostruita su `127.0.0.1:8080`; app, scheduler e OCR healthy; `/api/pronto` risponde `ok=true`, `versione=2.253.12`. |

## Fascicoli deposito React 2.253.12 - 2026-06-14

| Comando / verifica | Esito | Nota |
| --- | --- | --- |
| `python -m py_compile web\blueprints\react_shell.py web\bootstrap\deposito_routes.py` | OK | Sintassi confermata dopo apertura React della route `/fascicoli/<id>/deposito/prepara`. |
| `pnpm --filter @iusentra/studio typecheck` | OK | TypeScript standard verde dopo rigenerazione pulita di `node_modules` locale. |
| `python -m pytest tests/test_deposito_guidato.py::test_pagina_deposito_prepara_renderizza_anche_senza_correction_query tests/test_react_shell.py::test_react_route_gate_copre_rotte_profonde_e_preserva_contratti_operativi -q` | OK | 2/2: la GET ufficiale serve la shell React; il fallback classico resta accessibile solo con `?_legacy=1`. |
| `node frontend/scripts/check-react-contracts.mjs` | OK | Contratti React aggiornati con la route deposito profonda. |
| `node scripts/react-migration/check-route-gate.mjs` | OK | Route gate coerente; riconosce anche le shell React dedicate di Portale Cliente già presenti. |
...
| Comando / verifica | Esito | Nota |
| --- | --- | --- |
| `python -m py_compile pct/client_portal.py web/services/react_client_portal_bridge.py web/blueprints/api_v1_client_portal.py web/blueprints/client_portal.py` | OK | Sintassi confermata su repository, bridge, API e shell Portale Cliente. |
| `python -m pytest tests/test_client_portal_repository.py tests/test_client_portal_api.py -q --tb=short` | OK | 9/9: parità schema SQLite/PostgreSQL, token non in chiaro, isolamento tenant, API studio protetta, invito, accettazione, dashboard cliente, privacy, chat, upload e filtro dei fascicoli di prova/campione dal Portale Cliente. |
| `python -m pytest tests/test_backend_security_phase5.py tests/test_tenant_isolation_runtime.py tests/test_feature_flags.py tests/test_app_v2_feature_flags.py tests/test_openapi_contracts_phase6.py -q --tb=short` | OK | 33/33: guardrail parametri server-controlled, tenant isolation, feature flag e contratti OpenAPI fase 6 dopo l'apertura del perimetro pubblico limitato. |
| `pnpm --filter @iusentra/studio test` | OK | Contratti React, preset UI, governance design system, App V2 frontend, legal skills e UI coverage verdi dopo rimozione inline style nel Portale Cliente. |
| `pnpm --filter @iusentra/studio typecheck`; `pnpm --filter @iusentra/studio build` | OK | TypeScript e build Vite verdi; chunk `ClientPortalPage` circa 26 kB minificati, senza nuovi warning specifici oltre al chunk principale storico. |
| Ottimizzazione topbar notifiche; `pnpm --filter @iusentra/studio test`; `pnpm --filter @iusentra/studio typecheck`; `pnpm --filter @iusentra/studio build` | OK | La topbar non avvia più la chiamata notifiche quando il pannello è chiuso: le notifiche si caricano all'apertura o su evento esplicito, evitando che una risposta lenta ritardi la console Portale Clienti. Gate frontend e build Vite verdi dopo la modifica. |
| `python scripts/react-migration/generate_api_contracts.py --check`; `python scripts/validate_openapi.py docs/openapi.yaml`; `python scripts/verify_openapi_provider.py`; `python -m pytest tests/test_openapi_contracts_phase6.py -q --tb=short` | OK | OpenAPI e provider verification allineati al blueprint Portale Cliente: `auth-error=242`, `public-safe=14`, `success=29`, `backend-security=1`; contratti fase 6 `5/5`. |
| `python scripts/react-migration/generate_app_v2_page_registry.py --check`; `python scripts/react-migration/generate_app_v2_area_requirements.py --check`; `python scripts/react-migration/generate_app_v2_test_docs.py --check`; `python -m pytest tests/test_app_v2_page_registry.py tests/test_app_v2_area_requirements_phase8.py tests/test_app_v2_test_plan_phase10.py -q --tb=short` | OK | Registry, area requirements, test inventory/matrix/plan App V2 aggiornati; test documentali `12/12`. |
| `python scripts/validate_docs_links.py`; `python scripts/validate_docs_commands.py` | OK | Documentazione coerente: 21 documenti e 164 link locali validati; 157 comandi/path documentali controllati. |
| `python tools/sync_packaging_files.py --check`; `python -m pytest tests/test_packaging_consistency.py tests/test_release_readiness.py -q --tb=short` | OK | Packaging sincronizzato e readiness release `10/10` su versione `2.249.33`. |
| `python -m pytest tests/test_utf8_integrity.py -q --tb=short`; `python -m pct.cli utf8-integrity --check-only ... --json` | OK | UTF-8 pulito: test dedicati `4/4`, scan mirato sui testi/report Portale Cliente `ok=true`, `no_backup_created=true`. |
| `python scripts/audit_tenant_data_structure.py --repair`; `git diff --check -- . ':!data/*' ':!.codex-remote-attachments/*'` | OK | Struttura dati tenant senza mancanze; whitespace senza errori, con soli warning CRLF informativi su file già toccati. |
| Pulizia asset Vite mirata; `python -m pytest tests/test_react_asset_retention.py -q --tb=short` | OK | Rimossi 1022 asset React untracked non referenziati nella prima build e 127 dopo la build finale; conservati gli asset correnti/referenziati dal manifest. Retention asset React `2/2`, nessun bundle punta a file mancanti. |
| `python -m pytest tests/test_security_headers.py tests/test_support_remote.py -q --tb=short`; `python -m pytest tests/test_scadenziario.py tests/test_react_scadenziario_additions.py -q --tb=short`; `python -m pytest tests/test_pec_audit_pipeline.py -q --tb=short` | OK | Verifiche mirate sulle modifiche sorgente già presenti nella worktree: header/supporto remoto `32/32`, Scadenziario `60/60`, PEC audit pipeline `45/45`. |
| `docker compose build app`; `docker compose up -d --no-deps --force-recreate app`; health Docker; `curl http://127.0.0.1:8080/api/pronto` | OK | Copia Docker reale allineata alla versione `2.249.33` e tornata `healthy`; `/api/pronto` risponde `ok=true`. I primi cicli dopo recreate possono richiedere warm-up, poi il controllo interno è tornato stabile. |
| Browser in-app su `http://127.0.0.1:8080/app/portale-clienti` mobile `390x844` | OK | Dashboard studio React completa, `Avvia chat` e `Scrivi al cliente` presenti, widget Lex nascosto, nessun overflow orizzontale, nessun errore console, nessun testo tecnico vietato e nessun fascicolo con dicitura di prova visibile. Screenshot: `artifacts/client-portal/browser-screenshots/studio-portale-clienti-mobile-after-demo-filter.png`. |
| Browser in-app su `http://127.0.0.1:8080/app/portale-clienti` dopo Docker no-cache, mobile `390x844` | OK | Dashboard studio visibile in 1,6s dopo l'ottimizzazione topbar; `Portale Clienti`, `Avvia chat` e `Scrivi al cliente` presenti, console senza errori, zero overflow orizzontale e nessun termine tecnico vietato. Nei log iniziali della route non parte `/api/notifications`; la dashboard arriva da `/api/v1/ui/client-portal/dashboard`. Screenshot: `artifacts/client-portal/browser-screenshots/studio-portale-clienti-mobile-final-no-cache-reactive.png`. |
| Browser in-app su `http://127.0.0.1:8080/portale-cliente` mobile/tablet/desktop | OK | Vista cliente completa con `Scrivi allo studio` e `Apri chat` in prima schermata; click su `Apri chat` porta a `#chat-cliente` con textarea e pulsante `Invia` visibili. Verificati mobile `390x844`, tablet `768x1024` e desktop `1440x900`, widget Lex nascosto, console senza errori e zero overflow orizzontale. Screenshot principali in `artifacts/client-portal/browser-screenshots/cliente-portale-*-final.png`. |
| Browser in-app su `http://127.0.0.1:8080/portale-cliente` dopo Docker no-cache, desktop `1440x900` e mobile `390x844` | OK | In alto sono visibili `Scrivi allo studio` e `Apri chat`; il click su `Apri chat` porta a `#chat-cliente` con compositore e `Invia` visibili. Tempi osservati: desktop 4,3s e mobile 4,1s, console senza errori, zero overflow orizzontale, widget Lex nascosto e nessun testo tecnico vietato. Screenshot: `cliente-portale-desktop-top-final-no-cache-reactive.png`, `cliente-portale-desktop-chat-final-no-cache-reactive.png`, `cliente-portale-mobile-top-final-no-cache-reactive.png`, `cliente-portale-mobile-chat-final-no-cache-reactive.png`. |
| Hotfix CodeQL post-push Portale Cliente; `python -m py_compile pct/client_portal.py web/services/react_client_portal_bridge.py web/blueprints/api_v1_client_portal.py tests/test_support_remote.py`; `python -m pytest tests/test_client_portal_repository.py tests/test_client_portal_api.py -q --tb=short`; `python -m pytest tests/test_support_remote.py::test_support_pc_agent_declares_chrome_edge_loopback_policies -q --tb=short` | OK | Corrette le annotazioni CodeQL dello SHA `acffecf08`: helper SQL del Portale Cliente con allowlist/quote identificatori, errori pubblici cliente senza messaggi da eccezione e test origine `https://app.iusentra.it` basato su `urlsplit`. Sintassi verde, Portale Cliente `9/9`, test URL assistenza `1/1`. |
| Secondo hotfix CodeQL post-push Portale Cliente; `python -m py_compile pct/client_portal.py`; `python -m pytest tests/test_client_portal_repository.py tests/test_client_portal_api.py -q --tb=short`; `python -m pytest tests/test_support_remote.py::test_support_pc_agent_declares_chrome_edge_loopback_policies -q --tb=short`; `python -m pytest tests/test_utf8_integrity.py -q --tb=short` | OK | Sullo SHA `327aaea16` CodeQL segnalava ancora le due query validate. Il repository ora costruisce SQL solo da mappe costanti interne per tabelle, colonne, filtri e ordinamenti; i dati cliente restano sempre parametri `?`. Portale Cliente `9/9`, test URL `1/1`, UTF-8 `4/4`. |
...
| `python scripts/react-migration/generate_app_v2_page_registry.py`; `python scripts/react-migration/generate_app_v2_test_docs.py`; `python scripts/smoke_app_v2_all.py --subset inventory`; `python -m pytest -q tests/test_app_v2_page_registry.py tests/test_app_v2_test_plan_phase10.py tests/test_ci_cd_gates_phase11.py --tb=short` | OK | Hotfix post-push per `Lint + syntax`: documenti App V2 rigenerati, inventario smoke PASS=3 e test App V2/CI gate 14/14. |
| `python -m py_compile web\blueprints\impostazioni.py tests\test_pec_audit_pipeline.py`; `python -m pytest tests\test_pec_audit_pipeline.py::test_pec_repository_persists_remote_hearing_pdf_zip_ocr_and_exact_link -q --tb=short`; `python -m pytest tests\test_local_ai.py::test_api_local_ai_bootstrap_aggiorna_cache_runtime_chat tests\test_utf8_integrity.py -q --tb=short`; `git diff --check` | OK | Hotfix CodeQL post-push sullo SHA `06a95bdfa`: link Teams da PDF verificato con parsing host esatto, stato PEC letto dal repository reale e bootstrap AI locale senza ritorno del risultato grezzo del servizio nel vecchio endpoint JSON. |
...
| `python scripts\react-migration\generate_api_contracts.py`; `python scripts\validate_openapi.py docs\openapi.yaml`; `python scripts\verify_openapi_provider.py`; `python scripts\smoke_app_v2_all.py --subset contracts`; `python -m pytest -q tests\test_openapi_contracts_phase6.py --tb=short` | OK | Hotfix post-push: `docs/openapi.yaml` riallineato ai contratti generati dopo il rosso remoto `Lint + syntax`; OpenAPI valido, provider verification `auth-error=230`, `success=27`, `backend-security=1`, smoke contratti PASS=2/SKIP live=1 e contratti fase 6 `5/5`. |
| `python -m py_compile web\blueprints\template_atti.py web\blueprints\impostazioni.py web\bootstrap\reference_lookup_routes.py pct\config_studio.py pct\uffici_giudiziari.py`; `python -m pytest tests\test_sync_uffici.py tests\test_uffici_giudiziari_security.py tests\test_impostazioni_ai_locale_react.py tests\test_local_ai.py::test_api_local_ai_bootstrap_aggiorna_cache_runtime_chat tests\test_template_atti_api_strict.py tests\test_template_atti_editor.py -q --tb=short` | OK | Hotfix CodeQL post-push: scanner placeholder lineare, payload WhatsApp/AI locale/report variazioni uffici sanificati; sintassi verde e test mirati `21/21`. |
...
| `python -m py_compile web\blueprints\api_v1_react.py`; `python -m pytest tests\test_backend_security_phase5.py tests\test_openapi_contracts_phase6.py tests\test_quickorganizer_import.py -q --tb=short`; `python scripts\react-migration\generate_backend_security_map.py`; `python scripts\react-migration\generate_api_contracts.py`; `python scripts\validate_openapi.py docs\openapi.yaml`; `python scripts\verify_openapi_provider.py` | OK | Hotfix post-push: gli endpoint automatici tokenizzati dichiarano il presidio `_richiedi_auth_studio_telematico_token`, mappa sicurezza e OpenAPI riallineati, gate backend security e import `27/27` verdi. |
| `python -m py_compile pct\soggetti.py pct\database.py pct\storage_postgres.py pct\storage_migration.py pct\storage_migration_full.py web\helpers.py web\services\core_runtime.py web\blueprints\api_v1_react.py scripts\audit_quickorganizer_import.py`; `python -m pytest tests\test_quickorganizer_import.py -q --tb=short`; `python -m pytest tests\test_storage_postgres_migration.py::test_migrate_full_storage_to_sqlite_copre_repository_estesi tests\test_storage_postgres_migration.py::test_audit_sqlite_migration_integrity_script_blocca_delta -q --tb=short`; `python -m pytest tests\test_storage_postgres_migration.py::test_migrate_core_storage_to_postgres_produce_report_consistente tests\test_quickorganizer_import.py::test_import_studio_telematico_sqlite_scrive_tabelle_core_senza_json -q --tb=short` | OK | Presidio SQL `2.249.2`: soggetti e parti processuali sono tabelle strutturate SQLite/PostgreSQL; l'import Studio Telematico in modalità SQL scrive clienti, fascicoli, soggetti e parti nel DB senza creare JSON. |
| `python scripts\react-migration\generate_api_contracts.py`; `python scripts\react-migration\generate_api_contracts.py --check`; `python scripts\react-migration\generate_backend_security_map.py --check`; `python scripts\validate_openapi.py docs\openapi.yaml`; `python scripts\verify_openapi_provider.py`; `python -m pytest tests\test_backend_security_phase5.py tests\test_openapi_contracts_phase6.py tests\test_quickorganizer_import.py -q --tb=short`; `python tools\sync_packaging_files.py --check` | OK | OpenAPI `2.249.2`, provider verification `auth-error=224`, `success=27`, `backend-security=1`, security map e packaging allineati dopo il blocco anti-fallback JSON sull'import. |
...
| `python -m pytest tests\test_storage_strategy.py::test_carica_tenant_bootstrappa_una_sola_volta_senza_riconciliazione_pesante -q --tb=short`; `python -m pytest tests\test_storage_strategy.py::test_bootstrap_legacy_runtime_data_non_riconcilia_storage_in_startup tests\test_storage_strategy.py::test_superadmin_database_ripara_accesso_studio_da_pannello -q --tb=short` | OK | Bootstrap tenant verificato senza riconciliazione storage pesante in login, mantenendo pronto lo stato runtime e la riparazione SUPERADMIN da `/admin/database`. |
| `python scripts\run_pytest_phases.py --suite coverage-critical --suite-shard 7 --suite-total-shards 12 --suite-subdivide-items --timeout-minutes 5 -- --cov=lex --cov=pct.auth --cov=pct.storage --cov=pct.storage_postgres --cov=pct.telematico_repository --cov=pct.telematico_workflow --cov-config=config/coverage-critical.ini --cov-report=`; `python scripts\run_pytest_phases.py --core-shard 7 --core-total-shards 10 --core-subshard 1 --core-total-subshards 3 --core-subdivide-items --timeout-minutes 5` | OK | Replicati i due gate GitHub rossi: coverage critical 7/12 `31/31` e Pytest core 7.01 `29/29` verdi dopo riallineamento del contratto sul bootstrap tenant. |
...
| `python -m pytest tests\test_installation_packs.py -q --tb=short` | OK | 6/6: i manifest Product Pack, Studio Local Pack e Update Pack usano `manifest_hash_sha256` pubblico, non serializzano più firme HMAC derivate da segreti e separano il bootstrap della chiave locale nel modulo `pct.installation_secrets`, fuori dal writer JSON pubblico. |
...
| `python scripts\react-migration\generate_api_contracts.py --check`; `python scripts\validate_openapi.py docs\openapi.yaml`; `python scripts\verify_openapi_provider.py`; `python -m pytest tests\test_openapi_contracts_phase6.py -q --tb=short` | OK | Contratti OpenAPI ricontrollati dopo hotfix CodeQL: generazione allineata, OpenAPI valido, provider verification `auth-error=217`, `success=27`, `backend-security=1`, test fase 6 `5/5`. |
...
| `python scripts\react-migration\generate_backend_security_map.py`; `python -m pytest -q tests/test_auth.py tests/test_backend_security_phase5.py tests/test_tenant_isolation_runtime.py tests/test_app_v2_feature_flags.py tests/test_app_v2_routing.py tests/test_openapi_contracts_phase6.py --tb=short` | OK | Mappa sicurezza backend riallineata dopo il rosso CI; blocco RBAC/tenant/App V2/OpenAPI verde. |
| `python scripts\react-migration\generate_app_v2_area_requirements.py --check`; `python scripts\smoke_app_v2_workflows.py --list`; `python scripts\validate_ui_coverage.py`; `python -m pytest -q tests/test_app_v2_area_requirements_phase8.py --tb=short`; `python -m pytest -q tests/test_ui_coverage_phase9.py --tb=short`; `npm --prefix frontend run test`; `npm --prefix frontend run typecheck`; `npm --prefix frontend run build` | OK | Requisiti area App V2 riallineati dopo il rosso CI; blocco Frontend App V2 verde, build Vite completata in 6.61s e asset locali ripuliti perché Docker ricompila. |
...
| `python scripts\react-migration\generate_api_contracts.py`; `python scripts\react-migration\generate_api_contracts.py --check`; `python scripts\validate_openapi.py docs\openapi.yaml`; `python scripts\verify_openapi_provider.py`; `python scripts\smoke_app_v2_all.py --subset contracts`; `python -m pytest -q tests\test_openapi_contracts_phase6.py --tb=short` | OK | Riallineato `docs/openapi.yaml` alla versione `2.248.78` dopo il primo push: il gate `Lint + syntax` aveva bloccato gli shard a valle perché OpenAPI era rimasto a `2.248.77`. Provider verification: `auth-error=211`, `success=27`, `backend-security=1`; smoke contratti offline PASS=2/SKIP live=1. |
| `python -m ruff check --output-format=github --select E9,F63,F7,F82 ...`; `python -m py_compile tools\local_signer.py tools\dist\local_signer.py`; `python -m pytest tests\test_local_signer.py -q -k "tabella_lavoro or qbuilder_sicid_family or varianti_ricerca_esatta" --tb=short`; `python tools\sync_packaging_files.py --check` | OK | Hotfix post-push `Lint + syntax`: corretto l'hint tabella del download documento singolo, usando il payload documento singolo invece della variabile batch `documenti`; ruff, sintassi, test Local Signer e sync packaging verdi. |
...
| `python scripts\react-migration\generate_api_contracts.py`; `python scripts\react-migration\generate_api_contracts.py --check`; `python scripts\validate_openapi.py docs\openapi.yaml`; `python scripts\verify_openapi_provider.py`; `python scripts\smoke_app_v2_all.py --subset contracts`; `python -m pytest -q tests/test_openapi_contracts_phase6.py --tb=short` | OK | OpenAPI e contratti API riallineati al bump `2.248.76`; smoke contratti offline PASS=2/SKIP live=1 e test contratti 5/5 verdi. |
| `python -m pytest -q tests/test_web_bootstrap.py::test_email_e_sync_fascicolo_usano_date_italiane_e_target_corrente --tb=short` | OK | Hotfix post-push: riallineato il contratto React della scheda fascicolo sulla nota di sincronizzazione PEC automatica richiesta dal gate `Pytest core fase 10/10`. |
| `pnpm --filter @iusentra/studio typecheck`; `git diff --cached --check` | OK | TypeScript e whitespace dello staged set confermati dopo il micro-hotfix React/PEC. |
...
| Comando | Esito | Note |
| --- | --- | --- |
| `pnpm --filter @iusentra/studio typecheck` | OK | TypeScript verde dopo il pannello collassabile uffici giudiziari in `Componi PEC` e la rimozione dell'invio PEC via Local Signer dalla pagina. |
| `python -m pytest -q tests/test_uffici_competenti.py tests/test_email_client.py::test_email_pec_scrivi_invia_dal_canale_pec_dedicato_senza_local_signer tests/test_email_client.py::test_email_pec_scrivi_server_fallito_non_viene_segnato_inviato tests/test_impostazioni_pec_local_signer_react.py --tb=short` | OK | 10/10 passati: filtri uffici/PEC, invio PEC dal canale backend dedicato, nessun falso positivo su invio fallito e impostazioni PEC ancora presidiate da Local Signer per i test locali. |
| `python -m pytest tests\test_email_client.py::test_bootstrap_carica_env_locale_prima_della_configurazione tests\test_email_client.py::test_email_pec_scrivi_invia_dal_canale_pec_dedicato_senza_local_signer tests\test_email_client.py::test_email_pec_scrivi_usa_smtp_backend_server_reale tests\test_email_client.py::test_email_pec_scrivi_invia_allegato_via_smtp_backend tests\test_email_client.py::test_email_pec_scrivi_server_fallito_non_viene_segnato_inviato -q --tb=short` | OK | 5/5 passati: bootstrap `.env` prima della decifratura, invio PEC backend, destinatari multipli, allegato multipart e fallimento senza falso `INVIATI`. |
| `python -m pytest tests\test_impostazioni_pec_local_signer_react.py -q --tb=short` | OK | 2/2 passati: il test PEC nelle impostazioni resta locale/Local Signer, mentre `/email/scrivi` usa il canale PEC dedicato. |
| Test reale `/email/scrivi` su Legalmail tenant `antonella-mammola` | OK | Inviate due PEC a sé stessa, una senza allegato e una con `verifica-iusentra.txt`; entrambe hanno risposta HTTP 200, `Message-ID` SMTP e record `INVIATO` nello storico. |
| `python -m py_compile pct/uffici_competenti.py web/blueprints/api_v1_react.py web/blueprints/email_client.py` | OK | Sintassi confermata su parser uffici, API React e route PEC. |
| `python -m pytest -q tests/test_react_shell.py::test_react_strumenti_legali_catalogo_form_e_calcolo_json tests/test_email_client.py::test_email_ordinaria_scrivi_restera_separata_da_email_pec --tb=short` | OK | 2/2 passati: endpoint uffici compatibile con Strumenti Forensi e email ordinaria ancora separata dalla PEC. |
| `pnpm --filter @iusentra/studio test`; `pnpm --filter @iusentra/studio build` | OK | Contratti React, governance design system e build Vite verdi; asset React rigenerati per `EmailPecPage`. |
| `python scripts\react-migration\generate_api_contracts.py`; `python scripts\react-migration\generate_api_contracts.py --check`; `python scripts\validate_openapi.py docs\openapi.yaml`; `python scripts\verify_openapi_provider.py`; `python scripts\smoke_app_v2_all.py --subset contracts`; `python -m pytest -q tests\test_openapi_contracts_phase6.py --tb=short` | OK | Contratti API riallineati dopo il bump `2.248.65`: OpenAPI valido, provider verification OK, smoke contratti offline PASS=2/SKIP live=1 e test contratti 5/5 verdi. |
| `python tools\check_repo_governance.py`; `python -m py_compile web\bootstrap\flask_app_factory.py web\services\local_env_runtime.py web\blueprints\email_client.py pct\messaggi.py pct\__init__.py`; `python tools\sync_packaging_files.py --check`; `python -m pytest tests\test_release_readiness.py -q --tb=short`; `python -m pytest tests\test_utf8_integrity.py -q --tb=short`; `git diff --check` | OK | Governance factory compatta, sintassi, packaging, readiness, presidio UTF-8 e whitespace confermati dopo lo spostamento del loader `.env` in servizio dedicato. |

Aggiornato: 2026-05-25, Local Signer 1.6.46 autenticazione PST 401 esplicita 2.248.53, EML fascicolo, cancellazione documenti e Local Signer EXE 2.248.52, Componi PEC via Local Signer 2.248.51, matrice SIECIC ministeriale Local Signer 1.6.44 2.248.50, batch snapshot PST non bloccante Local Signer 1.6.43 2.248.49, fallback PST SICID/SIECIC attivo di default Local Signer 1.6.42 2.248.48, diagnostica PST client e normalizzazione ufficio cached 2.248.47, hotfix PST codici ufficio React e Local Signer 1.6.40 2.248.46, hotfix PST Palmi fallback derivato da URL 2.248.45, hotfix PST Palmi SOAP Fault Client 2.248.44, hotfix PST Palmi fallback registro Local Signer 2.248.43, hotfix PST CF tenant non vincolante 2.248.42, hotfix PST ricerca esatta e baseline Local Signer 2.248.41, evoluzione grafica professionale e Lex 2.248.40, Design system unico governato 2.248.39, modifica articolo Sito Studio React full 2.248.37, React full senza pagine finte 2.248.36, sicurezza multi-studio API React 2.248.34, governance CI/deploy bloccante 2.248.33, OCR legal-grade e Lex fascicolo 2.248.30, Relata notifica PEC-first 2.248.28, PEC OCR pipeline compatta 2.248.26, notifiche legali guidate 2.248.21, hotfix fascicolo anagrafiche in nuova finestra 2.248.20, Guida Pratica TOP9 set6 2.248.19, Guida Pratica TOP9 set5 2.248.18, Sequenza preset globale IUSENTRA 2.248.13, Preset grafico globale IUSENTRA 2.248.12, Guida Pratica facoltativa e loader fascicoli 2.248.11, Guida Pratica Lex conversazionale e TOP9 set2 2.248.10, Guida Pratica fascicolo e codici PST/XSD 2.248.9, CodeQL context canonico 2.248.8, CodeQL follow-up definitivo 2.248.7, CodeQL PR diff cleanup 2.247.6, CodeQL PR annotations closure 2.247.5, CodeQL PR security sweep 2.247.4, CodeQL PR security sweep 2.247.3, Regia Agentica Studio hardening 2.247.2, hotfix CodeQL URL Cassazione 2.247.1, PEC notifiche automatiche 2.246.4, hotfix PEC UI/Lex 2.246.3, PEC audit-grade e notifiche giudiziarie 2.246.2, Procedure Lifecycle Knowledge Pipeline 2.246.1, Lex Studio Reasoner linguaggio/date 99%, conversazionale e web/RAG 99% 2.245.63, Lex Unified Chat operativa 2.245.61, Lex Studio Reasoner fase 3 2.245.60, Lex Studio Reasoner fase 2 2.245.59, Lex Studio Reasoner fase 1 2.245.58, Lex editor/dettatura/documenti caricati 2.245.57, hotfix Lex companion locale 2.245.56, Lex contesto studio/Web libero/documenti caricati 2.245.55, Lex Studio Database 2.245.54, Fase 11.5 chiusura buchi residui fonti legali 2.245.53, Fase 10 audit finale aggiornamenti legali 2.245.51, Fase 9 fonti verdi estese 2.245.50, Fase 8 scheduler progressivo 2.245.49, Fase 6 normativa e archivi base 2.245.47, Fase 5 fonti verdi Aggiornamenti legali 2.245.46, strumenti sicuri canary/backfill Aggiornamenti legali 2.245.42, capability registry/parser fixture Aggiornamenti legali 2.245.41, collegamento Lex/Ricerca Legale per Corte Costituzionale n. 50/2026, UI Intelligence Ricerca Legale/Giurisprudenza/AI, Auto-fetch governato fonti legali, Tool Registry e Model Routing Lex, Job queue fonti legali, TokenJuice Lex, Memory Tree Lex corpus, CI pnpm/route Impostazioni, Web libero manuale Lex, Cassazione QSP50194/OCR allegati legali, rientro backfill e ranking allegati, Lex dati studio, soggetti/parti, bozze, AI mobile produzione, Documenti AI p7m/txt/eml, backup Hetzner e Docker pnpm.

## Local Signer 1.6.46 autenticazione PST 401 esplicita 2.248.53 - 2026-05-25

| Comando | Esito | Note |
| --- | --- | --- |
| `python -m pytest -q tests/test_local_signer.py::test_pst_best_effort_batch_401_non_diventa_ricerca_vuota tests/test_local_signer.py::test_installer_local_signer_windows_ps1_legacy_restituisce_exe tests/test_local_signer.py::test_installer_local_signer_windows_setup_route_e_pubblica tests/test_local_signer.py::test_installer_local_signer_windows_exe_route_se_bundle_presente --tb=short` | OK | 4/4 passati: batch PST con solo `401 Unauthorized` non diventa più ricerca vuota e installer Windows resta EXE-only. |
| `python -m py_compile tools\local_signer.py tools\dist\local_signer.py`; `python tools\check_local_signer_boundaries.py`; `python tools\sync_packaging_files.py --check`; `python scripts\validate_openapi.py docs\openapi.yaml` | OK | Local Signer `1.6.46` compilato, boundary PST preservati, packaging sincronizzato e OpenAPI valido su `2.248.53`. |
| `python -m pytest -q tests/test_local_signer.py --tb=short`; `python -m pytest -q tests/test_build_dist.py tests/test_packaging_consistency.py tests/test_release_readiness.py tests/test_utf8_integrity.py tests/test_openapi_contracts_phase6.py --tb=short`; `python -m compileall -q pct web tools`; `git diff --check` | OK | Suite Local Signer completa verde, 23/23 packaging/readiness/UTF-8/OpenAPI passati, compileall verde e nessun errore whitespace. |

## EML fascicolo, cancellazione documenti e Local Signer EXE 2.248.52 - 2026-05-25

| Comando | Esito | Note |
| --- | --- | --- |
| `python -m pytest -q tests/test_fascicoli_document_resilience.py tests/test_react_document_editor.py tests/test_storage_strategy.py::test_studio_db_salva_tabella_ritenta_se_sqlite_bloccato tests/test_local_signer.py::test_installer_local_signer_windows_ps1_legacy_restituisce_exe tests/test_local_signer.py::test_installer_local_signer_windows_setup_route_e_pubblica tests/test_local_signer.py::test_installer_local_signer_windows_exe_route_se_bundle_presente --tb=short` | OK | 12/12 passati: cancellazione documento salva prima di rimuovere il file, recupero EML storico con suffisso, payload editor EML read-only e download Windows EXE-only coperti. |
| `python -m compileall -q pct web tools` | OK | Sintassi confermata su storage, fascicoli, preview EML, route documento e Local Signer. |
| `python tools\check_local_signer_boundaries.py` | OK | Confini runtime Local Signer preservati dopo il rilascio `1.6.45`. |
...
| Comando | Esito | Note |
| --- | --- | --- |
| `python -m pytest -q tests/test_messaggi.py tests/test_email_client.py -k "pec_scrivi or conferma_locale or registra_email_inviata or local_signer"` | OK | 4/4 passati: falso positivo PEC bloccato, conferma locale con `Message-ID` e storico messaggi verificati. |
| `python -m pytest -q tests/test_impostazioni_pec_local_signer_react.py tests/test_local_pec_bridge.py` | OK | 6/6 passati: React usa Local Signer per test/invio PEC e il bridge locale conserva payload e allegati base64. |
| `pnpm --filter @iusentra/studio typecheck` | OK | TypeScript verde dopo `JsonPostForm.customSubmit` e invio PEC locale. |
| `python -m pytest -q tests/test_messaggi.py tests/test_email_client.py tests/test_impostazioni_pec_local_signer_react.py tests/test_local_pec_bridge.py` | OK | 78/78 passati sui moduli PEC/email/Local Signer toccati. |
| `pnpm --filter @iusentra/studio build` | OK | Typecheck e build Vite produzione completati; asset generati locali ripuliti perché Docker/CI ricostruiscono il bundle dalla sorgente. |
| `python -m compileall -q pct web` | OK | Sintassi Python confermata su backend e servizi web. |
| `pnpm --filter @iusentra/studio test` | OK | Contratti React, audit preset, design system governance, App V2 frontend, Legal Skills e UI coverage verdi. |
| `python tools\sync_packaging_files.py --check`; `python scripts\validate_openapi.py docs\openapi.yaml`; `python -m pytest -q tests/test_packaging_consistency.py tests/test_release_readiness.py tests/test_utf8_integrity.py tests/test_openapi_contracts_phase6.py --tb=short`; `python tools\check_repo_governance.py`; `git diff --check` | OK | Packaging sincronizzato, OpenAPI valido, 18/18 test packaging/readiness/UTF-8/OpenAPI passati, governance verde e nessun errore whitespace. |

## Matrice SIECIC ministeriale Local Signer 1.6.44 2.248.50 - 2026-05-25

| Comando | Esito | Note |
| --- | --- | --- |
| `python -m py_compile tools/local_signer.py` | OK | Sintassi confermata dopo la matrice SIECIC `InfoFascicolo` / `ProfiloFascicolo` / `ElencoDocumenti`. |
| `python -m pytest -q tests/test_local_signer.py::test_qbuilder_siecic_usa_servizi_catalogo_ministeriale tests/test_local_signer.py::test_parse_qbuilder_siecic_supporta_nomi_catalogo_camel_case tests/test_local_signer.py::test_parse_qbuilder_siecic_documenti_elenco_documenti tests/test_local_signer.py::test_pst_ricerca_snapshot_prova_codice_ufficio_ufficiale_se_diverso --tb=short` | OK | 4/4 passati: nomi servizio SIECIC corretti, parser qbuilder camel case e fallback sul codice ufficio ufficiale `0910011` nello stesso batch. |
| `python -m pytest -q tests/test_local_signer.py::test_costruisce_body_qbuilder_ricerca_per_tipo tests/test_local_signer.py::test_qbuilder_documenti_e_profilo_usano_parametri_pst_live tests/test_local_signer.py::test_pst_ricerca_snapshot_batcha_ricerca_e_documenti_senza_preflight tests/test_local_signer.py::test_pst_ricerca_snapshot_fault_client_su_sicid_passa_a_siecic tests/test_local_signer.py::test_pst_ricerca_snapshot_sigp_include_ricerca_atti_nel_batch_visualizzazione --tb=short` | OK | 5/5 passati: baseline SICID, profilo/documenti, nessun preflight, fallback SIECIC e SIGP preservati. |
| `python -m pytest -q tests/test_local_signer.py --tb=short` | OK | Suite Local Signer completa verde dopo generazione installer `1.6.44`. |
| `python tools\check_local_signer_boundaries.py` | OK | Confini runtime Local Signer rispettati. |
| `python tools\sync_packaging_files.py --check` | OK | Packaging allineato. |
| `python scripts\validate_openapi.py docs\openapi.yaml` | OK | Contratto OpenAPI valido dopo bump `2.248.50`. |
| `python -m pytest -q tests/test_packaging_consistency.py tests/test_release_readiness.py --tb=short` | OK | 9/9 passati: pacchetti e release readiness allineati. |
| `python -m pytest -q tests/test_utf8_integrity.py --tb=short` | OK | 4/4 passati: presidio UTF-8 integro. |
| `python tools\check_repo_governance.py` | OK | Governance repository verde. |

## Batch snapshot PST non bloccante 2.248.49 - 2026-05-25

| Comando | Esito | Note |
| --- | --- | --- |
| `python -m py_compile tools/local_signer.py` | OK | Sintassi confermata sul fix Local Signer `1.6.43`. |
| `python -m pytest -q tests/test_local_signer.py::test_pst_ricerca_snapshot_batcha_ricerca_e_documenti_senza_preflight tests/test_local_signer.py::test_pst_ricerca_snapshot_fault_client_su_sicid_passa_a_siecic tests/test_local_signer.py::test_pst_ricerca_snapshot_sigp_include_ricerca_atti_nel_batch_visualizzazione tests/test_local_signer.py::test_pst_varianti_ricerca_esatta_palmi_prova_siecic_senza_cambiare_ufficio tests/test_local_signer.py::test_pst_varianti_ricerca_esatta_fallback_disattivabile_da_env --tb=short` | OK | 5/5 passati: la snapshot PST usa batch best-effort, non si ferma sulla SOAP Fault del primo registro e continua a valutare il registro parallelo mantenendo metadati fascicolo e documenti. |
| `python -m py_compile tools/local_signer.py tools/dist/local_signer.py`; `python tools/check_local_signer_boundaries.py`; `python tools\sync_packaging_files.py --check`; `python scripts\validate_openapi.py docs\openapi.yaml` | OK | Sorgente e dist `1.6.43` compilati, confini Local Signer e packaging sincronizzati, OpenAPI valido dopo il bump `2.248.49`. |
| `python -m pytest -q tests/test_packaging_consistency.py tests/test_release_readiness.py --tb=short` | OK | 9/9 passati su coerenza packaging e readiness release. |
| `powershell -ExecutionPolicy Bypass -File tools\build_local_signer_windows_exe.ps1` | OK | Generato installer Windows `SetupLocalSigner-1.6.43.exe` e script installazione Local Signer `1.6.43`. |
...
## Hotfix PST Local Signer 2.248.54 / 1.6.47 - 2026-05-25

| Comando / verifica | Esito | Nota |
| --- | --- | --- |
| `python -m pytest -q tests/test_local_signer.py::test_pst_download_batch_riusa_sessione_view_anche_se_client_chiede_import tests/test_local_signer.py::test_pst_best_effort_batch_401_non_diventa_ricerca_vuota tests/test_local_signer.py::test_pst_ricerca_snapshot_esegue_preflight_prima_del_batch tests/test_local_signer.py::test_pst_ricerca_snapshot_fault_client_su_sicid_passa_a_siecic tests/test_local_signer.py::test_pst_ricerca_snapshot_prova_codice_ufficio_ufficiale_se_diverso tests/test_local_signer.py::test_pst_ricerca_snapshot_sigp_include_ricerca_atti_nel_batch_visualizzazione tests/test_local_signer.py::test_pst_documenti_sigp_batcha_documenti_e_ricerca_atti_senza_chiamate_extra tests/test_local_signer.py::test_pst_ricerca_esatta_arricchisce_profilo_se_mancano_campi_identita --tb=short` | OK | 8/8 passati: ricerca esatta, snapshot, fallback Palmi/SIECIC/SIGP, 401 non mascherato e download batch passano dal gate certificato interno del Local Signer. |
| `python -m pytest -q tests/test_react_shell.py::test_react_wizard_pst_verifica_local_signer_dal_browser tests/test_react_shell.py::test_pst_acquisizione_usa_lookup_uffici_reali_importati tests/test_react_shell.py::test_local_signer_diagnostics_salva_server_studio --tb=short` | OK | 3/3 passati: wizard React usa il catalogo uffici prima della ricerca automatica, conserva `pst_session_id` e salva diagnosi tenant-aware senza PIN. |
| `python -m pytest -q tests/test_polisweb.py::test_acquisizione_wizard_pst_carica_documenti_local_signer_anche_in_modalita_assistita tests/test_polisweb.py::test_polisweb_classico_non_chiede_preflight_pin_prima_delle_operazioni_reali --tb=short` | OK | 2/2 passati: wizard classico normalizza `codiceMinistero` verso il codice ufficiale e non reintroduce chiamate `/pst/preflight-auth` dal browser. |
...
| Comando / verifica | Esito | Nota |
| --- | --- | --- |
| `python -m pytest -q tests/test_uffici_competenti.py tests/test_react_shell.py::test_react_strumenti_legali_catalogo_form_e_calcolo_json tests/test_react_shell.py::test_react_studio_module_frontend_supporta_rotte_profonde_e_form_reali --tb=short` | OK | 7/7 passati: parser Giustizia Map, normalizzazione uffici, endpoint React, form `uffici_competenti` e card frontend presidiate senza chiamate live nei test. |
| `python -m compileall -q pct/uffici_competenti.py pct/applicazioni_runtime.py pct/strumenti_legali.py web/blueprints/api_v1_react.py` | OK | Sintassi confermata sul servizio read-only, schema strumenti, catalogo e API React. |
...
## Lettore documento Nuovo Cliente 2.248.61 - 2026-05-26

| Comando / verifica | Esito | Nota |
| --- | --- | --- |
| `python -m py_compile web\services\client_document_reader.py web\blueprints\api_v1_react.py` | OK | Sintassi confermata per servizio OCR/MRZ in memoria ed endpoint upload cliente. |
| `python -m pytest -q tests\test_client_document_reader.py --tb=short` | OK | 3/3 passati: parser MRZ passaporto, testo OCR simulato con CF/documento/scadenze/indirizzo/recapiti e API upload in memoria. |
| `python -m pytest -q tests\test_react_shell.py::test_react_clienti_nuovo_e_soggetti_collegati_nav_api_lex_cf tests\test_react_shell.py::test_react_clienti_nuovo_e_soggetti_api_usa_repository_reali --tb=short` | OK | 2/2 passati: `/clienti/nuovo` espone il lettore reale, endpoint configurato e repository reali senza regressioni sul flusso anagrafica. |
| `python -m ruff check web\services\client_document_reader.py web\blueprints\api_v1_react.py tests\test_client_document_reader.py tests\test_react_shell.py`; `python -m compileall -q web\services\client_document_reader.py web\blueprints\api_v1_react.py tests\test_client_document_reader.py` | OK | Stile e bytecode confermati sul perimetro backend/test toccato. |
| `pnpm --filter @iusentra/studio typecheck`; `pnpm --filter @iusentra/studio test`; `pnpm --filter @iusentra/studio build` | OK | TypeScript, suite frontend e build Vite verdi dopo i nuovi controlli documento e pannello di conferma. |
| `python scripts\react-migration\generate_api_contracts.py --check`; `python scripts\validate_openapi.py docs\openapi.yaml`; `python scripts\verify_openapi_provider.py`; `python tools\sync_packaging_files.py --check` | OK | Contratti API, OpenAPI, provider verification e packaging allineati alla versione `2.248.61`; il nuovo endpoint `/api/v1/ui/clienti/nuovo/documento/leggi` è documentato come upload `multipart/form-data` con risposta `ClientDocumentReadResponse`. |
| `python -m pytest -q tests\test_openapi_contracts_phase6.py tests\test_packaging_consistency.py tests\test_release_readiness.py tests\test_utf8_integrity.py --tb=short` | OK | 18/18 passati: contratti OpenAPI, packaging, readiness e presidio UTF-8 confermati. |
| Browser in-app autenticato su `/clienti/nuovo` desktop e mobile 390x844 | OK | Lettore documento visibile con `Carica documento`, `Leggi documento / MRZ`, stati di errore senza file, zero errori console e mobile senza overflow orizzontale. |
...
| `python scripts\react-migration\generate_api_contracts.py --check`; `python scripts\validate_openapi.py docs\openapi.yaml`; `python scripts\verify_openapi_provider.py`; `python scripts\smoke_app_v2_all.py --subset contracts`; `python -m pytest -q tests/test_openapi_contracts_phase6.py --tb=short` | OK | Hotfix post-push: contratti API/OpenAPI riallineati al bump `2.248.62`, provider verification e smoke contratti offline verdi. |

## PEC server, Comuni e uffici giudiziari 2.248.64 - 2026-05-26

| Comando / verifica | Esito | Nota |
| --- | --- | --- |
| `python scripts\audit_territorio_italia_db.py` | OK | Audit Comuni italiani verde: 7.894/7.894 Comuni, CAP, province, regioni e codici Belfiore presenti; copertura Comuni e CAP 100%. |
| `python scripts\build_uffici_giudiziari_comuni_db.py --workers 6 --timeout 16`; `python scripts\audit_uffici_giudiziari_comuni_db.py` | OK | Completate le associazioni Comune -> uffici giudiziari: 7.894/7.894 Comuni OK, 78.816 associazioni, dieci tipologie richieste presenti e almeno una PEC per ogni Comune. |
| `python -m pytest tests/test_territorio_italia.py tests/test_uffici_competenti.py tests/test_uffici_giudiziari_comuni_db.py tests/test_email_client.py::test_email_pec_scrivi_usa_smtp_backend_server_reale tests/test_client_document_reader.py tests/test_react_shell.py::test_react_strumenti_legali_catalogo_form_e_calcolo_json -q` | OK | 23/23 passati: autocomplete Comuni, filtri uffici, associazioni DB, endpoint con `comune_istat`, invio PEC backend server e OCR/MRZ cliente/soggetto. |
| `python -m pytest tests/test_email_client.py -q` | OK | 55/55 passati: invio PEC/email e route collegate verdi dopo il canale SMTP backend e MIME con allegati. |
| `pnpm --filter @iusentra/studio typecheck`; `pnpm --filter @iusentra/studio build` | OK | TypeScript e build Vite verdi dopo pannello PEC, autocomplete Comuni e OCR/MRZ soggetti. |
| `python -m compileall pct web scripts tests -q` | OK | Sintassi Python confermata su servizi, script, API e test toccati. |
| `python scripts\react-migration\generate_api_contracts.py --check`; `python scripts\validate_openapi.py docs\openapi.yaml`; `python scripts\verify_openapi_provider.py` | OK | Contratti API, OpenAPI e provider verification riallineati al bump `2.248.64`. |
| `python -m pytest tests\test_openapi_contracts_phase6.py tests\test_app_v2_page_registry.py tests\test_app_v2_test_plan_phase10.py tests\test_ci_cd_gates_phase11.py tests\test_backend_security_phase5.py::test_mappa_sicurezza_backend_generata_e_allineata tests\test_ui_api_security_matrix.py -q --tb=short` | OK | 22/22 passati: OpenAPI, documenti App V2, governance CI e mappa sicurezza backend allineati. |
| `python tools\sync_packaging_files.py --check`; `python -m pytest tests\test_packaging_consistency.py tests\test_release_readiness.py tests\test_utf8_integrity.py -q --tb=short`; `git diff --check`; `python tools\check_repo_governance.py` | OK | Packaging/versione `2.248.64`, readiness, UTF-8, whitespace e governance repo confermati. |

## PEC fascicolo, Lex e notifiche legali 2.248.66 - 2026-05-26

| Comando / verifica | Esito | Nota |
| --- | --- | --- |
| `python -m pytest tests/test_pec_audit_pipeline.py::test_pec_api_demo_digest_mime_and_quick_action tests/test_document_intelligence_auto_indexing.py::test_processing_vecchio_viene_recuperato_e_reindicizzato tests/test_document_intelligence_extraction.py::test_document_ai_extraction_doc_legacy_testuale tests/test_fascicoli.py::test_rinomina_documento_aggiorna_nome_e_file tests/test_fascicoli.py::test_rinomina_documento_non_cambia_estensione tests/test_fascicoli.py::test_rinomina_documento_lungo_preserva_estensione tests/test_fascicolo_detail_ux.py::test_rinomina_documento_da_react_action tests/test_notifiche_legali.py::test_piano_orario_applica_scissione_e_blocco_notturno tests/test_notifiche_legali.py::test_api_react_notifiche_legali_espone_workflow_separati -q` | OK | 9/9 passati: matching PEC/fascicolo, recupero indexing bloccato, DOC legacy, rinomina documento e timestamp/invio PEC coperti. |
| `python -m pytest tests/test_pec_audit_pipeline.py tests/test_document_intelligence_auto_indexing.py tests/test_document_intelligence_extraction.py tests/test_fascicoli.py tests/test_fascicolo_detail_ux.py tests/test_notifiche_legali.py -q` | OK | 127 test passati sul perimetro email PEC, Document Intelligence, fascicoli e notifiche legali. |
| `pnpm --filter @iusentra/studio typecheck`; `pnpm --filter @iusentra/studio test`; `pnpm --filter @iusentra/studio build` | OK | TypeScript, contratti React/governance UI e build Vite verdi dopo i nuovi pulsanti, form di rinomina e orologio PEC automatico. |
| Browser in-app autenticato su `/notifiche-legali`, `/fascicoli/DC5BF1DB` e `/email` | OK | Campo `Data e ora verifica PEC` aggiornato da `2026-05-26T18:45:11` a `2026-05-26T18:45:12`, `step=1`, pulsanti `Controlla relata` e `Invia PEC` visibili; fascicolo caricato senza “non trovato”, 29 pulsanti rinomina visibili, nessun overflow orizzontale e nessun testo tecnico vietato. |
| `git diff --check`; ricerca mojibake sui file toccati | OK | Whitespace e testi UTF-8 dei file modificati confermati prima dei gate finali. |
| `python tools\check_repo_governance.py`; `python -m compileall web\services\fascicoli_document_rename.py web\bootstrap\fascicoli_document_routes.py`; `python -m pytest tests\test_react_asset_retention.py -q --tb=short` | OK | Governance repo rientrata dopo estrazione helper rinomina, bytecode route/servizio valido e asset React referenziati presenti dopo build. |
| `python scripts\react-migration\generate_api_contracts.py --check`; `python scripts\validate_openapi.py docs\openapi.yaml`; `python scripts\verify_openapi_provider.py` | OK | Contratti API, OpenAPI e provider verification allineati al bump `2.248.66` e ai nuovi payload notifiche legali. |
| `python tools\sync_packaging_files.py --check`; `python -m pytest tests\test_packaging_consistency.py tests\test_release_readiness.py tests\test_utf8_integrity.py tests\test_openapi_contracts_phase6.py -q --tb=short`; `python -m pct.cli utf8-integrity --check-only ... --json` | OK | Packaging sincronizzato, 18 test release/OpenAPI/UTF-8 verdi e report UTF-8 senza artefatti, senza backup creati. |

## Hotfix PST fascicolo/import 2.248.67 / Local Signer 1.6.50 - 2026-05-26

| Comando / verifica | Esito | Nota |
| --- | --- | --- |
| `python -m py_compile tools\local_signer.py web\bootstrap\portali_acquisizione_routes.py web\services\telematico_runtime.py web\services\react_telematico_bridge.py` | OK | Sintassi confermata dopo riuso sessione PST, messaggi import specifici e mapping fascicolo locale. |
| `python -m pytest -q tests/test_local_signer.py::test_pst_download_batch_recupera_sessione_view_se_client_non_la_invia tests/test_local_signer.py::test_pst_download_batch_riusa_sessione_view_anche_se_client_chiede_import tests/test_local_signer.py::test_download_documenti_batch_timeout_non_torna_al_download_singolo --tb=short` | OK | 3/3 passati: il batch recupera la sessione vista e non torna al download singolo su timeout/errore, evitando richieste PIN ripetute. |
| `python -m pytest -q tests/test_local_signer.py::test_local_signer_pst_curl_attiva_foreground_prompt_pin_windows tests/test_local_signer.py::test_local_signer_foreground_pin_riconosce_dialog_windows_senza_titolo tests/test_local_signer.py::test_cf_avvocato_pst_preferisce_certificato_a_cf_studio_diverso tests/test_local_signer.py::test_pst_download_batch_recupera_sessione_view_se_client_non_la_invia tests/test_local_signer.py::test_pst_download_batch_riusa_sessione_view_anche_se_client_chiede_import tests/test_local_signer.py::test_download_documenti_batch_timeout_non_torna_al_download_singolo --tb=short` | OK | 6/6 passati: helper foreground PIN rafforzato per dialog Windows senza titolo, CF del certificato prevalente sul CF dello studio e download batch PST ancora senza fallback singolo. |
| `python tools\check_local_signer_boundaries.py`; `python -m pytest -q tests/test_local_signer.py::test_pst_import_test_reale_note_blinda_passaggi_grafica_e_multi_studio tests/test_local_signer.py::test_local_signer_pst_curl_attiva_foreground_prompt_pin_windows tests/test_local_signer.py::test_cf_avvocato_pst_preferisce_certificato_a_cf_studio_diverso --tb=short` | OK | Blindatura finale verde: la nota del test reale è obbligatoria nel boundary check e il test blocca regressioni su passaggi, grafica, PIN foreground e multi-studio. |
| `python -m pytest -q tests/test_react_shell.py::test_react_wizard_pst_verifica_local_signer_dal_browser tests/test_react_shell.py::test_react_superfici_telematiche_collegate_nav_api_css --tb=short` | OK | 2/2 passati: wizard React mostra anteprima completa, avanzamento import, mapping con `practiceId` e asset superficie telematica coerenti. |
| `python -m pytest -q tests/test_polisweb.py::test_api_portale_acquisizione_import_pst_blocca_catalogo_senza_file --tb=short` | OK | Il backend continua a bloccare import solo-catalogo, ma ora espone un errore professionale sullo scaricamento PST senza file reali. |
| `npm --prefix frontend run typecheck`; `npm --prefix frontend run build` | OK | TypeScript e build Vite verdi dopo anteprima/documenti/avanzamento import e mapping fascicolo locale. |
| `python tools\build_dist.py`; ping `http://127.0.0.1:27273/ping?light=1` | OK | Generati `SetupLocalSigner-1.6.50.exe` e pacchetti `.ps1/.command/.run`; Local Signer 1.6.50 operativo in prova parallela su porta 27273. |
| Browser in-app autenticato su `/portali/pst/acquisizione` con RG 274/2026 Tribunale di Palmi | OK | Ricerca reale, anteprima con dati/parti/documenti, selezione, mappatura `B6A03AE6`, analisi e import conclusi con `POST /api/portali/pst/acquisizione/import` 200; secondo giro senza nuova selezione certificato. |
| `docs/specs/ministero/PST_FASCICOLO_IMPORT_TEST_REALE_2026-05-26.md` | OK | Nota operativa versionata con tutti i passaggi reali eseguiti, grafica attesa del wizard, regola multi-studio e regressioni vietate sullo scarico/import fascicolo. |
| `python tools\sync_packaging_files.py --check`; `python -m pytest -q tests/test_packaging_consistency.py tests/test_release_readiness.py tests/test_build_dist.py tests/test_local_signer.py::test_local_signer_dist_allineato_a_sorgente_e_installer_versionati --tb=short`; `python -m pytest -q tests/test_react_asset_retention.py --tb=short`; `python -m pytest -q tests/test_utf8_integrity.py --tb=short`; `git diff --check` | OK | Packaging/versione `2.248.67`, readiness/dist 16/16, asset React referenziati, presidio UTF-8 e whitespace confermati dopo changelog/report. |

## Hotfix PST import fascicolo reale 2.248.90 - 2026-05-30

| Comando / verifica | Esito | Nota |
| --- | --- | --- |
| Browser in-app su `http://127.0.0.1:8080/portali/pst/acquisizione?ufficio=Tribunale+di+Palmi&ufficio_codice=0910011&numero=1025&anno=2024` | OK | Prova reale completa con Local Signer `1.6.67`: ricerca `1` risultato, anteprima `51` documenti / `36` righe cronologia / `3` parti, download batch `51/51`, analisi `100` con `0` blocchi e import completato in pratica `2026/008`. |
| Verifica runtime su `data/tenants/tenant-8bf98719c459/fascicoli/fascicoli.json` e cartella documenti `487EE7F3` | OK | Fascicolo `RG 1025/2024 — Vendita di cose immobili`: `51` record documento, `51` file fisici, `0` collegamenti mancanti; duplicati reali `DatiAtto.xml.p7m` e `IndiceDocumentiDepositati.PDF` preservati come documenti distinti. |
| `python -m py_compile tools\local_signer.py tools\dist\local_signer.py web\services\fascicoli_runtime.py web\services\telematico_runtime.py` | OK | Sintassi confermata per Local Signer, copia dist e runtime import PST dopo job asincrono anteprima e deduplica per identificativi portale/hash. |
| `python tools\check_local_signer_boundaries.py`; `git diff --check` | OK | Boundary Local Signer preservati e controllo whitespace senza errori; presenti solo avvisi Git CRLF su file runtime/dati non destinati al commit. |
| `python -m pytest tests/test_polisweb.py::test_api_portale_acquisizione_import_pst_non_collassa_allegati_con_stesso_nome tests/test_polisweb.py::test_api_portale_acquisizione_import_pst_salva_lotto_sette_documenti_nel_fascicolo tests/test_polisweb.py::test_api_portale_acquisizione_import_pst_arricchisce_file_locali_con_metadati_preview -q` | OK | 3/3 passati: il backend non accorpa più documenti PST con stesso nome ma identificativi diversi, salva il lotto di file reali e mantiene metadati preview. |
| `python -m pytest tests/test_local_signer.py::test_pst_ricerca_snapshot_usa_batch_certificato_senza_preflight_separato tests/test_local_signer.py::test_pst_master_detail_arricchisce_anteprima_nella_sessione_di_visualizzazione tests/test_react_shell.py::test_react_wizard_pst_verifica_local_signer_dal_browser -q` | OK | 3/3 passati: nessun preflight PIN extra, master-detail in anteprima preservato e wizard aggiornato al job `/pst/fascicolo-snapshot-job` con polling `/pst/jobs/<id>`. |
| `pnpm --filter @iusentra/studio build` | OK | TypeScript e build Vite verdi; asset React aggiornati con conteggio cronologia aggregato e report import che non confonde `0` con fallback `Solo catalogo`. |
| `python -m pytest tests/test_react_asset_retention.py -q --tb=short`; `python -m pytest tests/test_utf8_integrity.py -q --tb=short`; `python tools\sync_packaging_files.py --check`; `python -m pytest tests/test_packaging_consistency.py tests/test_release_readiness.py tests/test_build_dist.py -q --tb=short`; `python -m pytest tests/test_local_signer.py::test_local_signer_dist_allineato_a_sorgente_e_installer_versionati -q --tb=short` | OK | Asset React pubblicati coerenti con manifest, testi UTF-8 integri, packaging sincronizzato, readiness/dist 16/16 e installer Local Signer `1.6.67` allineato alla sorgente dopo pulizia asset intermedi. |
| `python scripts\react-migration\generate_api_contracts.py`; `python scripts\validate_openapi.py docs\openapi.yaml`; `python scripts\verify_openapi_provider.py`; `python -m pytest -q tests\test_openapi_contracts_phase6.py --tb=short` | OK | OpenAPI riallineato alla versione `2.248.90`; validazione strutturale, provider verification e contratti API fase 6 verdi dopo il primo check CI. |

## Hotfix PST anagrafiche e classificazione documenti 2.248.68 - 2026-05-26

| Comando / verifica | Esito | Nota |
| --- | --- | --- |
| Controllo dati reali tenant `tenant-8bf98719c459`, fascicolo `B6A03AE6` | OK | Il fascicolo Palmi R.G. 274/2026 aveva cliente `Loprete Domenico` già presente in anagrafica clienti e documenti classificati (`Citazione`, `Decreto`, `AttoNonCodificato`), ma mancavano parti strutturate e un deposito aveva ID documento duplicati: difetti riprodotti e coperti da test. |
| `python -m py_compile pct\polisWeb.py pct\fascicoli.py` | OK | Sintassi confermata dopo fallback parti PST e deduplica collegamenti documento/deposito. |
| `python -m pytest -q tests/test_polisweb.py::test_importa_fascicolo_esistente_crea_parti_da_cliente_locale_se_pst_non_espone_parti tests/test_polisweb.py::test_importa_fascicolo_esistente_sincronizza_cliente_parti_e_attivita --tb=short` | OK | 2/2 passati: se il PST non consegna parti strutturate, assistito e controparte vengono comunque popolati in anagrafica parti con ruoli corretti. |
| `python -m pytest -q tests/test_fascicoli.py::test_collega_documenti_portale_deduplica_ids_e_mantiene_classificazione tests/test_fascicoli.py::test_sincronizza_deposito_portale_riusa_lotto_generico_e_compila_metadati --tb=short` | OK | 2/2 passati: i documenti importati restano collegati al deposito ufficiale, senza duplicati e con classificazione portale preservata. |

## Hotfix PST fonte/anteprima e Local Signer 1.6.51 - 2026-05-26

| Comando / verifica | Esito | Nota |
| --- | --- | --- |
| `python -m py_compile tools\local_signer.py` | OK | Sintassi confermata dopo l'avvio nascosto dei processi `curl` su Windows. |
| `python -m pytest -q tests/test_local_signer.py::test_local_signer_pst_curl_attiva_foreground_prompt_pin_windows tests/test_local_signer.py::test_run_curl_windows_silenzia_console_senza_perdere_foreground_pin tests/test_local_signer.py::test_preflight_auth_accetta_http_405_come_handshake_valido tests/test_local_signer.py::test_soap_call_curl_batch_raw_windows_preserva_cert_store_spec tests/test_local_signer.py::test_soap_call_curl_raw_windows_applica_ssl_no_revoke tests/test_local_signer.py::test_pst_preflight_windows_applica_ssl_no_revoke --tb=short` | OK | 6/6 passati: il PIN resta gestito dal watcher foreground e `curl` viene lanciato con finestra nascosta senza perdere timeout o flag esistenti. |
| `python tools\check_local_signer_boundaries.py`; `python -m py_compile tools\local_signer.py tools\dist\local_signer.py web\services\telematico_runtime.py pct\fascicoli.py pct\polisWeb.py web\services\react_fascicoli_bridge.py tools\build_dist.py`; `python -m pytest -q tests/test_local_signer.py::test_run_curl_windows_silenzia_console_senza_perdere_foreground_pin tests/test_local_signer.py::test_local_signer_pst_curl_attiva_foreground_prompt_pin_windows tests/test_local_signer.py::test_pst_ricerca_snapshot_usa_batch_certificato_senza_preflight_separato tests/test_local_signer.py::test_pst_ricerca_snapshot_fault_client_su_sicid_passa_a_siecic tests/test_local_signer.py::test_pst_ricerca_snapshot_prova_codice_ufficio_ufficiale_se_diverso tests/test_local_signer.py::test_pst_ricerca_snapshot_sigp_include_ricerca_atti_nel_batch_visualizzazione tests/test_local_signer.py::test_soap_call_pst_session_prova_cookie_anche_se_host_mtls_gia_noto tests/test_local_signer.py::test_soap_call_pst_session_batch_prova_cookie_anche_se_host_mtls_gia_noto tests/test_local_signer.py::test_pst_prepare_authenticated_session_non_disattiva_cookie_se_host_mtls_gia_noto --tb=short` | OK | Boundary Local Signer, sintassi e 9/9 regressioni su snapshot unico, cookie sessione PST e finestra `curl` nascosta confermati. |
| `python tools\build_dist.py`; `python tools\sync_packaging_files.py --check`; `python -m pytest -q tests/test_local_signer.py::test_run_curl_windows_silenzia_console_senza_perdere_foreground_pin tests/test_local_signer.py::test_local_signer_dist_allineato_a_sorgente_e_installer_versionati tests/test_local_signer.py::test_installer_local_signer_windows_setup_route_e_pubblica tests/test_local_signer.py::test_installer_local_signer_macos_e_pubblico tests/test_local_signer.py::test_installer_local_signer_linux_e_pubblico tests/test_build_dist.py --tb=short` | OK | Pacchetti `1.6.51` rigenerati, packaging sincronizzato e 11/11 test installer/dist verdi. |
| `powershell -NoProfile -ExecutionPolicy Bypass -File tools\dist\InstallaLocalSigner-1.6.51.ps1 -Quiet`; ping `http://127.0.0.1:27272/ping`; `GET /pst/status` | OK | Local Signer installato localmente con helper `_windows_hidden_subprocess_kwargs`, ping `1.6.51` e diagnostica PST senza PIN eseguita usando `curl` nascosto. |
| `npm --prefix frontend run typecheck`; `npm --prefix frontend run build`; `python -m pytest -q tests/test_polisweb.py::test_importa_fascicolo_esistente_crea_parti_da_cliente_locale_se_pst_non_espone_parti tests/test_polisweb.py::test_importa_fascicolo_esistente_sincronizza_cliente_parti_e_attivita tests/test_fascicoli.py::test_collega_documenti_portale_deduplica_ids_e_mantiene_classificazione tests/test_fascicoli.py::test_sincronizza_deposito_portale_riusa_lotto_generico_e_compila_metadati tests/test_react_shell.py::test_pst_acquisizione_usa_lookup_uffici_reali_importati --tb=short` | OK | TypeScript, build Vite e 5/5 regressioni su anagrafiche, classificazione, cronologia import, anteprima completa e auto-update Local Signer confermati. |
| `git diff --check`; `python tools\sync_packaging_files.py --check`; `python -m pytest -q tests/test_react_asset_retention.py tests/test_packaging_consistency.py tests/test_release_readiness.py tests/test_utf8_integrity.py --tb=short`; `python tools\check_repo_governance.py` | OK | Whitespace, packaging, asset React referenziati, readiness, UTF-8 e governance repo confermati dopo pulizia degli asset hashati non referenziati. |
| `python scripts\react-migration\generate_api_contracts.py`; `python scripts\react-migration\generate_api_contracts.py --check`; `python scripts\validate_openapi.py docs\openapi.yaml`; `python scripts\verify_openapi_provider.py`; `python scripts\smoke_app_v2_all.py --subset contracts`; `python -m pytest -q tests/test_openapi_contracts_phase6.py --tb=short` | OK | Hotfix post-push: `docs/openapi.yaml` riallineato alla versione `2.248.69`, contratti API/OpenAPI/provider e smoke contratti offline verdi. |
| `python scripts\react-migration\generate_app_v2_page_registry.py`; `python scripts\react-migration\generate_app_v2_test_docs.py`; `python scripts\react-migration\generate_app_v2_page_registry.py --check`; `python scripts\react-migration\generate_app_v2_test_docs.py --check`; `python scripts\smoke_app_v2_all.py --subset inventory`; `python -m pytest -q tests/test_app_v2_page_registry.py tests/test_app_v2_test_plan_phase10.py tests/test_ci_cd_gates_phase11.py --tb=short` | OK | Hotfix post-push: `docs/test-inventory.md` riallineato dopo i nuovi test Local Signer/PST, inventario App V2 PASS=3 e governance CI 14/14 verde. |
| Browser in-app su `/portali/pst/acquisizione?ufficio=Tribunale%20di%20Palmi&ufficio_codice=0910011&numero=274&anno=2026&fascicolo_id=B6A03AE6&mode=update_existing` | OK | Pagina PST caricata sullo step ricerca, Local Signer `1.6.51` raggiungibile e pulsante `Cerca fascicolo` pronto senza regressioni visibili nello stato corrente. |

## Hotfix PST download documenti nel fascicolo 2.248.77 - 2026-05-27

| Comando / verifica | Esito | Nota |
| --- | --- | --- |
| `npm --prefix frontend run typecheck`; `npm --prefix frontend run build` | OK | TypeScript e build Vite verdi dopo selezione documenti PST, pulsanti `Scarica selezionati` / `Scarica tutti` e testi destinazione pratica semplificati. |
| `python -m pytest -q tests/test_react_shell.py::test_react_wizard_pst_verifica_local_signer_dal_browser tests/test_react_shell.py::test_portale_acquisizione_accetta_alias_fascicolo_id_per_mapping tests/test_react_shell.py::test_portale_acquisizione_legacy_step4_preseleziona_aggiorna_pratica tests/test_polisweb.py::test_acquisizione_wizard_pst_carica_documenti_local_signer_anche_in_modalita_assistita tests/test_polisweb.py::test_api_portale_acquisizione_analyze_pst_pratica_esistente_non_blocca_parti_mancanti tests/test_polisweb.py::test_api_portale_acquisizione_import_pst_blocca_catalogo_senza_file tests/test_polisweb.py::test_api_portale_acquisizione_import_pst_prima_pratica_non_crea_vuoto_senza_file tests/test_polisweb.py::test_api_portale_acquisizione_import_pst_arricchisce_file_locali_con_metadati_preview tests/test_polisweb.py::test_api_portale_acquisizione_preview_pst_preserva_iscrizione_da_snapshot --tb=short` | OK | 9/9 passati: wizard, testi regressivi, alias payload Local Signer, prima importazione senza file reali e data iscrizione da snapshot governati. |
| `python -m pytest -q tests/test_polisweb.py::test_api_portale_acquisizione_import_pst_salva_lotto_sette_documenti_nel_fascicolo --tb=short` | OK | Riprodotto il caso `7/7` con `Atto_2767510.pdf`: il payload `downloaded_files` viene decodificato e i sette PDF finiscono nei documenti del fascicolo, collegati al deposito. |
| `python -m pytest -q tests/test_polisweb.py::test_api_portale_acquisizione_import_pst_salva_lotto_sette_documenti_nel_fascicolo tests/test_polisweb.py::test_api_portale_acquisizione_import_pst_arricchisce_file_locali_con_metadati_preview tests/test_polisweb.py::test_api_portale_acquisizione_import_pst_blocca_catalogo_senza_file tests/test_polisweb.py::test_api_portale_acquisizione_import_pst_prima_pratica_non_crea_vuoto_senza_file tests/test_polisweb.py::test_api_portale_acquisizione_analyze_pst_pratica_esistente_non_blocca_parti_mancanti --tb=short`; `python -m py_compile web/services/fascicoli_runtime.py web/services/telematico_runtime.py` | OK | 5/5 passati: documenti reali salvati, catalogo senza file ancora bloccato, nuova pratica non vuota, pratica esistente aggiornata senza cancellare parti locali. |
...
## Audit database / sicurezza pubblica 2.248.85 - 2026-05-29

| Comando / verifica | Esito | Nota |
| --- | --- | --- |
| `python -m py_compile tools\local_signer.py pct\busta.py web\services\quickorganizer_import.py web\services\local_pec_runtime.py web\blueprints\api_v1_react.py web\blueprints\email_client.py web\blueprints\email_ordinaria.py web\bootstrap\admin_database_routes.py web\bootstrap\deposito_routes.py web\bootstrap\deposito_receipt_routes.py web\bootstrap\fascicoli_document_routes.py web\bootstrap\portali_acquisizione_routes.py web\bootstrap\sync_runtime_routes.py web\bootstrap\telematico_local_signer_routes.py web\fatturazione.py web\services\client_document_reader.py web\services\fascicoli_document_rename.py` | OK | Sintassi confermata dopo hardening CodeQL su eccezioni pubbliche, redirect, percorsi runtime, buste e import Studio Telematico. |
| `python -m pytest tests\test_quickorganizer_import.py tests\test_busta.py -q --tb=short` | OK | 10/10 passati: import Studio Telematico e busta telematica continuano a preservare documenti e allegati dopo i controlli sui path runtime. |
| `python -m pytest tests\test_deposito.py tests\test_simulazione_deposito.py tests\test_busta.py tests\test_quickorganizer_import.py -q --tb=short` | OK | 77/77 passati: deposito, simulazione, busta e import confermati dopo messaggi pubblici e SHA-256 sugli ID di prova. |
| `python -m pytest tests\test_local_signer.py -q --tb=short` | OK | Suite completa Local Signer verde: corretto il fallback batch SICID con endpoint fisico `JPW_SICID` e sorgente/dist allineati. |
| `python -m pytest tests\test_email_client.py::test_api_pec_poll_cancelleria_espone_duplicati_e_warning_sync_pubblico tests\test_dashboard_mailbox_sync.py tests\test_app_v2_routing.py -q --tb=short` | OK | 18/18 passati: messaggio IMAP pubblico, report PEC conteggiato, routing interno sicuro e nessun redirect aperto. |
| `python tools\sync_packaging_files.py --check`; `python -m pytest tests\test_utf8_integrity.py -q --tb=short` | OK | Packaging coerente e presidio UTF-8 verde dopo changelog/report/test aggiornati. |
| `python scripts\react-migration\generate_api_contracts.py --check`; `python scripts\validate_openapi.py docs\openapi.yaml`; `python scripts\verify_openapi_provider.py`; `python -m pytest tests\test_openapi_contracts_phase6.py -q --tb=short` | OK | OpenAPI e provider verification riallineati al bump `2.248.85`; test contratti 5/5 verdi. |
| `python scripts\react-migration\generate_backend_security_map.py --check`; `python -m pytest tests\test_backend_security_phase5.py tests\test_ui_api_security_matrix.py -q --tb=short` | OK | Mappa sicurezza backend allineata e 10/10 test di enforcement API/backend security verdi. |
| `python scripts\react-migration\generate_app_v2_test_docs.py --check`; `python -m pytest tests\test_email_client.py tests\test_dashboard_mailbox_sync.py tests\test_app_v2_routing.py -q --tb=short` | OK | Documenti test App V2 allineati e suite email/dashboard/routing completa verde. |
| `python -m pytest tests\test_database.py tests\test_storage_postgres_migration.py -q --tb=short` | OK | 73/73 passati: presidio anti-perdita SQLite/PostgreSQL, audit migrazione e storage full confermati dopo gli hardening collaterali. |
| `python tools\check_repo_governance.py`; `python tools\sync_packaging_files.py --check`; `python -m pytest tests\test_packaging_consistency.py tests\test_release_readiness.py tests\test_build_dist.py -q --tb=short`; `npm --prefix frontend run typecheck` | OK | Governance, packaging/release 15/15 e typecheck frontend confermati prima di commit/push. |

## Import Studio Telematico SQL e presidio CodeQL 2.249.3 - 2026-05-31

| Comando / verifica | Esito | Nota |
| --- | --- | --- |
| `python -m py_compile pct\soggetti.py pct\database.py pct\storage.py pct\storage_postgres.py pct\storage_migration.py pct\storage_migration_full.py web\helpers.py web\services\core_runtime.py web\blueprints\api_v1_react.py scripts\audit_quickorganizer_import.py scripts\audit_sqlite_migration_integrity.py` | OK | Sintassi confermata per import Studio Telematico, repository soggetti/parti SQL, guard storage e audit migrazione. |
| `python -m pytest tests\test_quickorganizer_import.py tests\test_backend_security_phase5.py tests\test_openapi_contracts_phase6.py -q --tb=short` | OK | 28/28 passati: import Studio Telematico, endpoint token presidiati, OpenAPI e contratti backend coperti. |
| `python -m pytest tests\test_storage_postgres_migration.py::test_migrate_full_storage_to_sqlite_copre_repository_estesi tests\test_storage_postgres_migration.py::test_audit_sqlite_migration_integrity_script_blocca_delta tests\test_storage_postgres_migration.py::test_migrate_core_storage_to_postgres_produce_report_consistente -q --tb=short` | OK | 3/3 passati: `soggetti` e `soggetti_parti` migrano in tabelle SQLite/PostgreSQL insieme a clienti e fascicoli. |
| `python scripts\react-migration\generate_api_contracts.py --check`; `python scripts\validate_openapi.py docs\openapi.yaml`; `python tools\sync_packaging_files.py --check`; `python scripts\react-migration\generate_backend_security_map.py --check` | OK | Contratti API/OpenAPI versione `2.249.3`, packaging e mappa sicurezza backend allineati. |
| `python -m pytest tests\test_installation_packs.py tests\test_operational_resilience.py tests\test_openapi_contracts_phase6.py tests\test_build_dist.py::test_build_studio_telematico_packager_pubblica_exe_senza_ps1_primario tests\test_packaging_consistency.py tests\test_release_readiness.py -q --tb=short` | OK | 29/29 passati: fix CodeQL su writer JSON sensibili, packaging e readiness senza regressioni. |
...
## Cassazione PST e PEC penale 2.249.4 - 2026-06-02

| Comando / verifica | Esito | Nota |
| --- | --- | --- |
| `python -m py_compile tools\local_signer.py pct\pec_legal_workflow.py` | OK | Sintassi Local Signer e workflow PEC confermata dopo router Cassazione, SIVG, richieste copie e messaggio SIECIC su `idRuoloJPW`. |
| `python -m pytest -q tests\test_local_signer.py -k "cassazione or siecic or qbuilder or richieste_copie" --tb=short` | OK | 34/34 passati: servizi qbuilder Cassazione, SIECIC, SIGP, famiglie SICID e richieste copie read-only presidiati. |
| `python -m pytest -q tests\test_portali_telematici_matrix.py --tb=short` | OK | 15/15 passati: matrice tabelle ministeriali aggiornata, Cassazione fuori dal servizio generico `RicercaInformazioniFascicoloPerTipo`. |
| Lettura reale Local Signer sorgente `http://127.0.0.1:27273/pst/ricerca`, `JPW_CASSPE`, `12756/2026` | OK | Il PST ha restituito `1` fascicolo valido: `numero=12756`, `anno=2026`, `NRGREALE=202601275600`, ruolo `RICORSO ORDINARIO`, sezione `PRIMA SEZIONE`, data iscrizione `2026-04-14`, udienza `2026-07-09`. |
| Lettura reale annuale Local Signer sorgente `http://127.0.0.1:27273/pst/ricerca`, `JPW_CASSPE`, anno `2026` | OK | Ricerca per anno su `QP_Ricorsi` riuscita: `1` numero ruolo valido trovato, `12756/2026`. |
| Lettura reale Local Signer sorgente `http://127.0.0.1:27273/pst/ricerca`, Tribunale di Palmi `0910011`, `JPW_SICID`, `1025/2024` | OK | Il PST ha restituito `1` fascicolo civile ordinario: ruolo `ORDINARIO CARTABIA`, stato `PROCEDIMENTO DEFINITO`, data iscrizione `2024-09-05`, udienza `2024-12-12`. |
| Snapshot reale Local Signer sorgente `http://127.0.0.1:27273/pst/ricerca-snapshot`, Tribunale di Palmi `0910011`, `JPW_SICID`, `1025/2024` | OK | Snapshot metadati/catalogo riuscito: `16` documenti di catalogo, senza download documenti in questa prova. |
| Lettura reale Local Signer sorgente `http://127.0.0.1:27273/pst/ricerca`, Tribunale di Roma `0760010`, `JPW_SIVG`, `63/2025/VG` | OK | Il PST ha restituito `1` fascicolo valido su namespace `urn:CONS-SIVG-BE`, codice PST `0580910098`, sezione `PRIMA SEZIONE SEPARAZIONI VOLONTARIA FAMIGLIA` e giudice valorizzato. |
| Tentativo Local Signer sorgente `JPW_SIVG`, Tribunale di Palmi `0910011`, `63/2025/VG` | OK diagnostico | La tabella SIVG è stata raggiunta ma ha restituito `available=0`; il caso corretto indicato dall'utente è Roma, poi verificato positivo. |
| Lettura reale richieste copie read-only, proxy `GLRM/JPW_SICID`, `RicercaRichieste` | OK | Risposta SOAP 200 su namespace `urn:RichiestaCopie-consultazioni-distr`, `available=0`: servizio raggiunto, nessuna richiesta copie visibile al certificato in quel momento. |
| Prova diretta `pct.pec_legal_workflow.classifica_pec_legale` su deposito atti penali Cassazione Reggio Calabria | OK | Oggetto `Deposito ricorso per cassazione ... 1365_2016 RG APP ... 1364_2011 RG NR` classificato `cassazione_penale` / `deposito_ricorso_cassazione_penale`, con estrazione di entrambi i registri. |
| `python -m pytest -q tests\test_pec_legal_workflow.py::test_deposito_atti_penali_cassazione_da_giustiziacert_resta_penale tests\test_pec_legal_workflow.py::test_deposito_ricorso_cassazione_penale_estrae_rg_app_e_rg_nr --tb=short` | OK | 2/2 passati: PEC Cassazione penale da `depositoattipenali.ca.reggiocalabria@giustiziacert.it` e caso con prefissi governati. |
| Prova diretta `pct.pec_legal_workflow.classifica_pec_legale` su `generale/2016/001365/Corte di Appello` da `notifichepenali.ca.reggiocalabria@penale.ptel.giustiziacert.it` | OK | Classificato `deposito_penale_pdp` / `comunicazione_penale_pdp`, estratto `1365/2016 RG_APP`, nessuna scadenza automatica senza contenuto fonte. |
| `python -m pytest -q tests\test_pec_legal_workflow.py --tb=short` | OK | 12/12 passati: workflow PEC legale completo confermato dopo i due casi penali reali. |

## Impostazioni Canali SdI, parità SQL e Local Signer 1.6.68 - 2026-06-02

| Comando / verifica | Esito | Nota |
| --- | --- | --- |
| `python -m py_compile pct\config_studio.py pct\impostazioni_config_repository.py pct\database.py pct\storage_postgres.py pct\storage_migration.py pct\storage_migration_full.py web\blueprints\impostazioni.py web\services\react_impostazioni_bridge.py web\services\react_fatturazione_bridge.py web\services\feature_flags.py` | OK | Sintassi confermata per ConfigStudio/SdI, repository `settings_config`, storage SQLite/PostgreSQL e bridge React. |
| `python -m pytest tests\test_impostazioni_sdi_config.py tests\test_database.py::test_migra_verso_sqlite_tabelle tests\test_database.py::test_migra_verso_sqlite_migra_servizi_operativi_aggiuntivi tests\test_database.py::test_schema_settings_config_presente_per_sqlite_e_postgresql tests\test_storage_postgres_migration.py::test_migrate_core_storage_to_postgres_produce_report_consistente tests\test_fonti_ufficiali_registry.py tests\test_react_fatturazione_bridge.py tests\test_fatturazione.py tests\test_react_privacy_registry_sources.py tests\test_feature_flags.py::test_app_v2_route_flags_cover_dynamic_and_root_paths tests\test_app_v2_feature_flags.py::test_app_v2_canonical_flags_have_explicit_rollout_defaults_and_page_scope tests\test_app_v2_feature_flags.py::test_app_v2_route_rules_only_reference_known_flags tests\test_app_v2_feature_flags.py::test_frontend_blocks_flag_off_routes_before_page_fetch -q --tb=short` | OK | 26/26 passati: Canali SdI, fonti ufficiali, parità storage, FatturaPA, feature flag e GDPR coperti. |
| `python -m pytest tests\test_react_shell.py::test_react_route_gate_copre_rotte_profonde_e_preserva_contratti_operativi tests\test_react_shell.py::test_impostazioni_react_api_redige_segreti_e_salva_configurazioni tests\test_react_shell.py::test_impostazioni_react_frontend_copre_local_signer_occhio_e_ai_locale tests\test_utf8_integrity.py -q --tb=short` | OK | 7/7 passati: shell React, redazione segreti Impostazioni, Local Signer UI e integrità UTF-8 confermati. |
| `node scripts\react-migration\check-route-gate.mjs`; `node scripts\react-migration\check-full-react-route-contract.mjs`; `node frontend\scripts\check-react-contracts.mjs` | OK | Route `/impostazioni/sdi` governata come React full e contratti aggiornati. |
| `pnpm --filter @iusentra/studio typecheck`; `pnpm --filter @iusentra/studio build` | OK | TypeScript e build Vite verdi dopo scheda `Canali SdI` e feature flag dedicato. |
| `python tools\build_dist.py`; `python -m pytest tests\test_local_signer.py::test_local_signer_dist_allineato_a_sorgente_e_installer_versionati tests\test_build_dist.py::test_build_windows_ps1_include_versione_e_script_originale tests\test_build_dist.py::test_installer_powershell_non_ha_errori_di_parse -q --tb=short` | OK | Pacchetto Local Signer `1.6.68` rigenerato: sorgente, `tools/dist/local_signer.py`, `SetupLocalSigner-1.6.68.exe`, alias `SetupLocalSigner.exe` e installer PowerShell allineati. |
| `powershell -NoProfile -ExecutionPolicy Bypass -File tools\installa_local_signer_locale.ps1 -Quiet`; `GET http://127.0.0.1:27272/ping`; controllo listener `127.0.0.1:27272` | OK | Installazione locale aggiornata: `/ping` risponde `versione=1.6.68`; il listener è avviato con `pythonw.exe`, non con `python.exe` console. |
| `docker compose up -d --build --force-recreate app`; `/api/pronto`; browser in-app su `http://127.0.0.1:8080/impostazioni/sdi` | OK | Docker locale reale healthy con versione `2.249.6`; la pagina mostra `Canali SdI`, `FatturaPA`, campi canale/intermediario e avvisi professionali. Verifica layout tab: 12 pannelli ARIA presenti, 11 inattivi con `hidden`, altezza `0` e non visibili; un solo pannello visibile (`Canali SdI`), nessun overflow orizzontale. |
| Browser in-app su `http://127.0.0.1:8080/impostazioni`, giro completo su `Dati Studio`, `PEC`, `Firma Digitale`, `Email SMTP`, `WhatsApp`, `Scheduler`, `AI Locale`, `Canali SdI`, `Pagamenti`, `Notifiche`, `Backup`, `Calendari` | OK | 12/12 tab verificate sulla copia Docker reale: un solo pannello visibile per volta, titolo della sezione sopra i campi e sopra l'azione di salvataggio quando presente, nessun overflow orizzontale, console senza errori o warning. Conferma visiva dell'utente: tutte corrette. |
| `python -m pytest tests\test_react_shell.py::test_impostazioni_react_frontend_copre_local_signer_occhio_e_ai_locale tests\test_utf8_integrity.py tests\test_react_asset_retention.py -q --tb=short`; gate React route/contratti; `pnpm --filter @iusentra/studio typecheck`; `pnpm --filter @iusentra/studio build` | OK | 7/7 pytest, contratti React, typecheck e build Vite confermati dopo la correzione dei titoli tab. Gli asset Vite generati sono stati ripuliti secondo `scripts/audit_finale_pst_sqlite_pec.py` e `test_react_asset_retention` resta verde. |
| `python -m py_compile` sui Python modificati/nuovi; `tools\check_repo_governance.py`; `tools\sync_packaging_files.py --check`; `scripts\validate_openapi.py docs\openapi.yaml`; `scripts\verify_openapi_provider.py` | OK | Sintassi Python, governance, packaging, OpenAPI e provider verification confermati prima dello stage. Provider verification: `auth-error=229`, `success=27`, `backend-security=1`. |
| `python -m pytest tests\legal_deposit\test_payment_policies.py tests\legal_deposit\test_penal_deposit_rules.py tests\test_portali_telematici_matrix.py tests\test_pec_legal_workflow.py -q --tb=short` | OK | 44/44 passati: regole pagamenti/CU, deposito penale, matrice portali e workflow PEC confermati. |
| `python -m pytest tests\test_uffici_giudiziari_comuni_db.py tests\test_reginde.py tests\test_strumenti_legali.py tests\test_react_preventivo_wizard_console.py tests\test_deposito_guidato.py -q --tb=short` | OK | Suite uffici giudiziari, RegIndE, strumenti legali, preventivo wizard e deposito guidato confermata: 86/86 passati. |
| `python -m pytest tests\test_local_signer.py::test_local_signer_dist_allineato_a_sorgente_e_installer_versionati tests\test_build_dist.py::test_build_windows_ps1_include_versione_e_script_originale tests\test_build_dist.py::test_installer_powershell_non_ha_errori_di_parse -q --tb=short` | OK | 3/3 passati: dist Local Signer allineato a `1.6.68`, installer PowerShell valido e avvio immediato governato con `pythonw.exe`. |
| `git diff --cached --check`; `docker compose ps`; `GET http://127.0.0.1:8080/api/pronto` | OK | Diff whitespace staged senza errori dopo normalizzazione degli archivi HTML ufficiali; Docker locale reale resta healthy e `/api/pronto` risponde `ok=true`, versione `2.249.6`. |
| `python scripts\react-migration\generate_api_contracts.py`; `python scripts\react-migration\generate_api_contracts.py --check`; `python scripts\validate_openapi.py docs\openapi.yaml`; `python scripts\verify_openapi_provider.py` | OK | Contratti API e OpenAPI riallineati dopo il primo giro CI su GitHub; provider verification confermata con `auth-error=229`, `success=27`, `backend-security=1`. |
| `pnpm --filter @iusentra/studio test` | OK | Test frontend completo verde dopo adeguamento del gate `audit-ui-preset-sequence` alla nuova prop `sequence={false}` nei form e dopo inserimento di `/impostazioni/sdi` nella matrice App V2. |
| `python scripts\react-migration\generate_app_v2_page_registry.py`; `python scripts\react-migration\generate_app_v2_page_registry.py --check`; `pnpm --filter @iusentra/studio test`; `python scripts\react-migration\generate_api_contracts.py --check`; `git diff --check`; `python -m pytest tests\test_utf8_integrity.py -q --tb=short` | OK | Registro App V2, mappa legacy, frontend, contratti API, whitespace e UTF-8 riallineati dopo il gate GitHub `Lint + syntax`; `data/intelligence/tabelle_normative.json` rigenerato dal test è stato ripristinato perché runtime. |
| `python scripts\react-migration\generate_app_v2_test_docs.py`; `python scripts\react-migration\generate_app_v2_test_docs.py --check`; `python scripts\react-migration\generate_app_v2_area_requirements.py`; `python scripts\react-migration\generate_app_v2_area_requirements.py --check`; `python scripts\smoke_app_v2_all.py --subset inventory`; `python -m pytest -q tests\test_app_v2_page_registry.py tests\test_app_v2_test_plan_phase10.py tests\test_ci_cd_gates_phase11.py --tb=short`; `python -m pytest -q tests\test_app_v2_area_requirements_phase8.py tests\test_ui_coverage_phase9.py --tb=short` | OK | Documenti test App V2 e requisiti area rigenerati e verificati dopo il secondo giro `Lint + syntax`; smoke inventory e pytest App V2 verdi. |
...
| `python scripts\react-migration\generate_app_v2_page_registry.py --check`; `python scripts\react-migration\generate_app_v2_test_docs.py --check`; `python scripts\smoke_app_v2_all.py --subset inventory`; `python -m pytest -q tests\test_app_v2_page_registry.py tests\test_app_v2_test_plan_phase10.py tests\test_ci_cd_gates_phase11.py --tb=short` | OK | Documenti App V2 riallineati dopo il bump `2.249.7`; smoke inventory verde e 14/14 test passati, correggendo il blocco GitHub `Lint + syntax` sullo SHA precedente. |
| `python -m pytest -q tests/test_installation_packs.py::test_installation_pack_json_writer_redige_segreti tests/test_operational_resilience.py::test_operational_resilience_writer_redige_segreti --tb=short`; `python -m ruff check tests\test_installation_packs.py tests\test_operational_resilience.py` | OK | 2/2 passati dopo fixture dinamiche: la regressione redazione resta coperta, evitando che CodeQL interpreti i dati di test come segreti statici salvati in chiaro. |
...
| Comando / verifica | Esito | Nota |
| --- | --- | --- |
| `pnpm --filter @iusentra/studio typecheck` | OK | TypeScript verde dopo integrazione ricerca Comune, filtri tipologia ufficio e codici ufficiali nel campo `Autorità giudiziaria` di `/fascicoli/nuovo`. |
| `python -m pytest -q tests\test_react_shell.py -k "fascicolo_nuovo_form_collassabile or filtro_tipologia_per_nuovo_fascicolo" --tb=short` | OK | 2/2 passati: il form React contiene la ricerca “Uffici giudiziari per Comune”, filtri `GDP`/Tribunale/`UNEP`, chiamata agli endpoint reali e endpoint `uffici_competenti` con filtro tipologia. |
| `python -m pytest -q tests\test_uffici_competenti.py tests\test_uffici_giudiziari_comuni_db.py tests\test_territorio_italia.py --tb=short` | OK | 17/17 passati: banca dati Comune -> uffici giudiziari, autocomplete Comuni, filtri tipologia e fonte ministeriale locale confermati. |
| `pnpm --filter @iusentra/studio test`; `pnpm --filter @iusentra/studio build` | OK | Contratti React, governance design system, coverage UI e build Vite confermati dopo la correzione dei codici nascosti del campo autorità. |
| `python scripts\react-migration\generate_api_contracts.py --check`; `python scripts\validate_openapi.py docs\openapi.yaml`; `python scripts\verify_openapi_provider.py`; `python -m pytest -q tests\test_utf8_integrity.py --tb=short`; `python tools\sync_packaging_files.py --check`; `python -m pytest -q tests\test_react_asset_retention.py --tb=short`; `git diff --check` | OK | Contratti API/OpenAPI, provider verification, UTF-8, packaging, asset React referenziati e diff whitespace confermati; `git diff --check` ha solo avvisi CRLF su file storici già tracciati. |
| Docker locale reale `docker compose build --no-cache app`; `docker compose up -d app`; `GET http://127.0.0.1:8080/api/pronto` | OK | Container `iusentra-app` healthy sulla copia utente `127.0.0.1:8080`, versione `2.249.5`, senza server temporanei per la prova finale. |
| Browser in-app su `http://127.0.0.1:8080/fascicoli/nuovo`, Comune `Taurianova` | OK | Verifica autenticata reale: `Taurianova (RC)` restituisce 9 uffici; filtri `GDP`, Tribunale e `UNEP` isolano rispettivamente Giudice di Pace di Palmi `0800570152`, Tribunale di Palmi `0800570094` e UNEP presso Tribunale di Palmi `18080057`; `Usa nel fascicolo` applica `Unep presso il Tribunale di PALMI` e i campi form `codice_ufficio_autorita` / `codice_ministero_autorita` valgono `18080057`. Console senza errori. |
| Browser in-app responsive su `127.0.0.1:8080/fascicoli/nuovo`, viewport tablet `820x900` e mobile `390x844` | OK | Il blocco “Uffici giudiziari per Comune” resta visibile con Comune, filtri e risultati Taurianova; nessun overflow orizzontale (`scrollWidth == clientWidth`) su tablet e mobile. |

## Guida Pratica, termini e Scadenze PDF 2.249.9 - 2026-06-02

| Comando / verifica | Esito | Nota |
| --- | --- | --- |
| `python -m pytest tests\test_pdf_deadline_import.py tests\test_react_scadenziario_additions.py -q` | OK | 5/5 passati: anteprima Scadenze dai PDF, rimozione singola/selezionata/totale delle candidate e skip dei PDF oltre soglia senza lettura pesante. |
| `python -m py_compile web\services\pdf_deadline_import.py` | OK | Sintassi confermata dopo guardia dimensione file, lettura rapida pagine testuali e timeout di reattività. |
| `python -m pytest tests\test_pdf_deadline_import.py tests\test_react_scadenziario_additions.py tests\test_termini_processuali.py tests\test_pec_audit_pipeline.py::test_pec_api_demo_digest_mime_and_quick_action tests\test_document_intelligence_api.py -q` | OK | 25/25 passati: Scadenze PDF, termini processuali Guida Pratica, PEC verso scadenziario/agenda e lettura documenti fascicolo restano coerenti. |
| `python -m pytest tests\test_termini_processuali.py tests\test_import_guida_pratica_termini_processuali.py -q` | OK | 9/9 passati: compreso bootstrap dell'endpoint template termini da ambiente pulito, con template Guida Pratica generati dai moduli KB versionati quando il runtime `/data` non è ancora popolato. |
| `python -m pytest tests\test_termini_processuali.py tests\test_import_guida_pratica_termini_processuali.py tests\test_pdf_deadline_import.py tests\test_react_scadenziario_additions.py tests\test_pec_audit_pipeline.py::test_pec_api_demo_digest_mime_and_quick_action tests\test_document_intelligence_api.py -q` | OK | 27/27 passati dopo il bootstrap runtime dei termini: Guida Pratica, Scadenze PDF, PEC, Agenda e lettura documenti restano coerenti nello stesso giro finale. |
| `python scripts\audit_guida_pratica_user_material_fields.py --fail-on-loss` | OK | Audit voce-per-voce aggiornato: 117 schede, 2.614 righe, 2.061 voci presenti, 0 perdite verso KB completo, servizio/API, UI Guida Pratica e Lex. |
| `python scripts\import_guida_pratica_termini_processuali.py` | OK | Repository termini aggiornato: 3.216 termini Guida Pratica, 1.046 template calcolabili, 2.017 `manual_review`, 50 informativi non calcolabili. |
| `pnpm --filter @iusentra/studio exec tsc --noEmit`; `pnpm --filter @iusentra/studio build` | OK | TypeScript e build Vite verdi; asset Scadenziario aggiornati nella build React pubblicata. |
| `python -m pytest tests\test_utf8_integrity.py -q --tb=short`; `git diff --check` | OK | Integrità UTF-8 confermata e diff whitespace senza errori; `git diff --check` segnala solo warning CRLF storici sui JSON/CSV della Guida Pratica. |
| `python -m pytest tests\test_react_asset_retention.py -q --tb=short` | OK | 2/2 passati dopo pulizia degli asset Vite non referenziati: nessun chunk React pubblicato resta senza asset collegato. |
| Docker reale `docker compose build --no-cache app`; `docker compose up -d app`; `GET http://127.0.0.1:8080/api/pronto` | OK | Copia locale reale dell'utente aggiornata su `127.0.0.1:8080`; container `iusentra-app` healthy e readiness `ok=true`, versione `2.249.9`. |
| Browser in-app reale su `http://127.0.0.1:8080/scadenziario?vista=scadute` e `http://127.0.0.1:8080/scadenziario?id_fascicolo=4B830EB5&guidaPratica=100002`, pannello `Scadenze PDF` | OK | La vista scadute apre lo Scadenziario senza pagina temporaneamente non disponibile; la scansione PDF termina senza bloccare la pagina, mostra `Analisi completata`, `Nessuna nuova scadenza`, i pulsanti `Elimina selezionate` ed `Elimina tutto` sono presenti e disabilitati quando non ci sono candidate. Screenshot reale: `artifacts/guida-pratica/screenshots/scadenziario-pdf-2.249.9.png`. |

## Guida Pratica, calcolatore termini e prova reale 2.249.10 - 2026-06-03

| Comando / verifica | Esito | Nota |
| --- | --- | --- |
| `python scripts\audit_guida_pratica_user_material_fields.py --fail-on-loss` | OK | Audit voce-per-voce confermato: 117 schede, 2.614 righe, 2.061 voci presenti, 0 perdite verso KB completo, servizio/API, UI Guida Pratica e Lex. |
| `python scripts\import_guida_pratica_termini_processuali.py` | OK | Repository termini confermato: 3.216 termini Guida Pratica e 1.046 template calcolabili disponibili per Scadenziario e guida operativa. |
| `python -m pytest tests\test_termini_processuali.py tests\test_import_guida_pratica_termini_processuali.py tests\test_pdf_deadline_import.py tests\test_react_scadenziario_additions.py tests\test_pec_audit_pipeline.py::test_pec_api_demo_digest_mime_and_quick_action tests\test_document_intelligence_api.py -q` | OK | 27/27 passati: Guida Pratica, template termini, Scadenze PDF, PEC, Agenda e lettura documenti fascicolo restano coerenti. |
| `python -m pytest tests\test_react_scadenziario_additions.py tests\test_termini_processuali.py -q` | OK | 11/11 passati dopo la microcopy italiana del calcolatore: presidiate date `gg/mm/aaaa`, accento `è` e assenza di `mostrera`. |
| `python -m pytest tests\test_utf8_integrity.py tests\test_react_asset_retention.py -q --tb=short`; `pnpm --filter @iusentra/studio exec tsc --noEmit`; `pnpm --filter @iusentra/studio build` | OK | UTF-8, retention asset, TypeScript e build Vite confermati dopo la resa italiana di date e testo nel calcolatore termini. |
| `python scripts\react-migration\generate_api_contracts.py --check`; `python scripts\validate_openapi.py docs\openapi.yaml`; `python scripts\verify_openapi_provider.py` | OK | Contratti API/OpenAPI allineati a `2.249.10`; provider verification `auth-error=229`, `success=27`, `backend-security=1`. |
| Docker reale `docker compose build --no-cache app`; `docker compose up -d app`; `GET http://127.0.0.1:8080/api/pronto` | OK | Copia locale reale dell'utente aggiornata su `127.0.0.1:8080`; readiness `ok=true`, versione `2.249.10`. |
| Browser in-app reale su `http://127.0.0.1:8080/scadenziario?id_fascicolo=4B830EB5&guidaPratica=100002#calcolatore-termini-processuali` | OK | Il calcolatore mostra `Scadenza calcolata` in formato italiano, spiegazione con `è`, timeline con date `gg/mm/aaaa`, nessuna data ISO visibile nel riquadro, nessun `e'`, nessuna pagina temporaneamente non disponibile; `Elimina selezionate` ed `Elimina tutto` presenti nel pannello PDF. Screenshot reale: `artifacts/guida-pratica/screenshots/scadenziario-reale-2.249.10.png`. |
## PEC, Lex, Notifiche legali e Scadenziario 2.249.12 - 2026-06-03

| Comando / verifica | Esito | Nota |
| --- | --- | --- |
| `python -m pytest tests/test_lex_operational_knowledge.py::test_all_scadenze_passa_contesto_tenant_al_manager tests/test_lex_operational_knowledge.py::test_scadenziario_non_filtra_studio_id_tecnico_come_tenant tests/test_lex_operational_knowledge.py::test_fascicolo_route_risolve_domanda_lunga_senza_perdere_il_nucleo_pratica tests/test_lex_module.py::test_lex_agenda_tool_filtra_giorni_senza_ripescare_vecchi_eventi tests/test_notifiche_legali.py::test_ui_notifiche_legali_carica_relata_firmata_nel_flusso_operativo tests/test_notifiche_legali.py::test_payload_documenti_pratica_idrata_nome_timestamp_da_contenuto tests/test_notifiche_legali.py::test_payload_react_notifiche_legali_deriva_parti_rg_destinatari_e_nomi_quickorganizer tests/test_pec_audit_pipeline.py::test_pec_api_acquisisce_mime_locale_da_casella_storica tests/test_pec_audit_pipeline.py::test_pec_api_schedula_duplicato_audit_senza_report_da_mime_locale tests/test_pec_audit_pipeline.py::test_email_search_normalizza_plurali_accenti_e_allegati -q` | OK | 10/10 passati: Lex legge repository tenant-aware, scadenziario e agenda non tornano più vuoti, Notifiche legali espone destinatari multipli/RG/parte/documenti leggibili e il presidio PEC usa MIME locali e filtro allegati. |
| `pnpm --filter @iusentra/studio build` | OK | Build Vite completata; manifest e asset React aggiornati per Notifiche legali, Scadenziario e Email PEC. |
| Docker reale `docker compose up -d --build app scheduler-worker ocr-worker`; `GET http://127.0.0.1:8080/api/pronto` | OK | Copia locale reale dell'utente aggiornata su `127.0.0.1:8080`; readiness `ok=true`, versione `2.249.12`; container app, scheduler e OCR healthy. |
| `POST http://127.0.0.1:8080/api/pec/email/acquisisci-locali?limit=1500` autenticato sul tenant `tenant-8bf98719c459` | OK | Presidio PEC reale: 258 MIME locali esaminati, 31 scadenze non scadute già presenti, 227 scadute ignorate, 31 scadenze collegate all'agenda. L'esempio `COMUNICAZIONE 3001/2025/LAV` / `98/2026` non risulta presente nei MIME locali sincronizzati, quindi non è stato inventato. |
| Verifica dati runtime tenant `tenant-8bf98719c459` | OK | Scadenziario: 36 scadenze totali, 31 da presidio PEC non scadute; Agenda: 183 impegni totali, 31 collegati al presidio PEC. |
| Lex reale in `create_app().app_context()` sulla domanda `termine lungo di impugnazione. Riferimento: scadenze rilevanti` | OK | Lex risponde con `Scadenze consultabili: 12` e `Impegni agenda consultabili: 24`, senza gap di sorgente vuota. |
| Lex reale in `create_app().app_context()` sulla domanda `Fammi una sintesi del fascicolo Betti C. MIM: documenti chiave, rischi aperti, cosa manca e prossimi passi` | OK | Lex risolve il fascicolo `D87FC97A`, legge 29 documenti disponibili nel tenant e include estratti leggibili dai PDF invece di fermarsi a nome, data, hash e link editor. |
| Browser reale autenticato su `http://127.0.0.1:8080/notifiche-legali` | OK | Pagina non vuota, nessun overlay, console senza errori, pratica selezionabile, destinatario suggerito multiplo visibile, parte rappresentata/RG/anno valorizzati quando presenti, documenti QuickOrganizer rinominati da contenuto, firma/attestazione/controllo relata operativi. Screenshot: `C:\Users\antmm\AppData\Local\Temp\iusentra-browser-proof-20260603-final\notifiche-legali-final.png`. |
| Browser reale autenticato su `http://127.0.0.1:8080/scadenziario` | OK | Pannello `Scadenze dai PDF` presente con `Elimina selezionate` ed `Elimina tutto`, nessun overlay e console senza errori. Screenshot: `C:\Users\antmm\AppData\Local\Temp\iusentra-browser-proof-20260603-final\scadenziario-final.png`. |
| `python -m py_compile` sui moduli PEC, Lex, agenda, notifiche legali e test toccati | OK | Sintassi Python confermata dopo la pulizia degli artefatti runtime. |
| `python -m pytest tests/test_react_asset_retention.py -q --tb=short`; `python -m pytest tests/test_utf8_integrity.py -q --tb=short`; `git diff --check` | OK | Asset React referenziati e UTF-8 verdi; `git diff --check` senza errori, solo warning CRLF storico su `docs/openapi.yaml`. |
| `python scripts\react-migration\generate_api_contracts.py`; `python scripts\validate_openapi.py docs\openapi.yaml`; `python scripts\verify_openapi_provider.py`; `python scripts\react-migration\generate_api_contracts.py --check`; `python scripts\smoke_app_v2_all.py --subset contracts`; `python -m pytest -q tests\test_openapi_contracts_phase6.py --tb=short` | OK | Contratti API/OpenAPI riallineati dopo le nuove route Notifiche legali e PEC; provider verification `auth-error=230`, `success=27`, `backend-security=1`; smoke contratti PASS=2/SKIP live=1; OpenAPI fase 6 `5/5`. |
| `python scripts\react-migration\generate_app_v2_page_registry.py`; `python scripts\react-migration\generate_app_v2_test_docs.py`; `python scripts\react-migration\generate_app_v2_page_registry.py --check`; `python scripts\react-migration\generate_app_v2_test_docs.py --check`; `python scripts\smoke_app_v2_all.py --subset inventory`; `python -m pytest -q tests\test_app_v2_page_registry.py tests\test_app_v2_test_plan_phase10.py tests\test_ci_cd_gates_phase11.py --tb=short` | OK | Inventario App V2 riallineato dopo il build React: smoke inventory PASS=3, 14/14 test gate passati. |
| `python scripts\react-migration\generate_backend_security_map.py`; `python scripts\react-migration\generate_backend_security_map.py --check`; `python -m pytest -q tests\test_auth.py tests\test_backend_security_phase5.py tests\test_tenant_isolation_runtime.py tests\test_app_v2_feature_flags.py tests\test_app_v2_routing.py tests\test_openapi_contracts_phase6.py --tb=short` | OK | Mappa sicurezza backend riallineata; blocco RBAC/tenant/App V2 del job `Lint + syntax` verde con 77/77 test passati. |

## Editor template atti professionale 2.249.13 - 2026-06-04

| Comando / verifica | Esito | Nota |
| --- | --- | --- |
| `python -m pytest -q tests/test_template_atti_api_strict.py tests/test_template_atti_editor.py tests/test_utf8_integrity.py --tb=short` | OK | 13/13 passati: registry font professionale, layout editor esteso, payload React `/api/v1/ui/template-atti/compila/<codice>`, workflow editor, policy Lex e integrità UTF-8 sono coperti. |
| `pnpm --filter @iusentra/studio typecheck`; `node frontend/scripts/check-react-contracts.mjs`; `pnpm --filter @iusentra/studio test` | OK | TypeScript, contratti React, governance design system, audit sequenza, App V2 frontend, legal skills e coverage UI confermati dopo il workspace editor template atti. |
| `pnpm --filter @iusentra/studio build` | OK | Build Vite completata in 12.52s; asset principali `index-BXavM7j0.js` 499.17 kB / 145.11 kB gzip, `TemplateAttiPage-vD_-xPGj.js` 69.25 kB / 20.19 kB gzip, `TemplateAttiPage-BRows5W5.css` 45.59 kB / 7.98 kB gzip. |
| Docker locale reale `docker compose up -d --build app`; `GET http://127.0.0.1:8080/api/pronto` | OK | Copia locale reale dell'utente aggiornata su `127.0.0.1:8080`, container `iusentra-app` healthy e readiness `ok=true`, versione `2.249.13`; build Docker Vite completata in 12.28s. |
| Browser Playwright su `http://127.0.0.1:8080/template-atti/compila/CIV_COM_001`, desktop/tablet/mobile | OK | Prova completa con Chrome locale: scroll di pagina e pannelli, toolbar, campi, stile/font, Guida Pratica, controlli placeholder/legal_basis/privacy, Lex con 11 azioni, diff accetta/rifiuta/modifica/applica, audit, import TXT, Compilazione Multipla, Nuovo Template, export RTF/DOCX/PDF e copia/salvataggio/rigenerazione. Overflow 0, console applicativa 0, date visibili in italiano. Report: `artifacts/react-migration/template-editor-browser-2.249.13.json`. |
| Export reali dal browser | OK | `Esporta RTF` e tab `RTF` hanno prodotto `comparsa_di_costituzione_e_risposta.rtf` da 174 byte; `DOCX` ha prodotto `comparsa_di_costituzione_e_risposta.doc` da 1509 byte; `PDF` ha aperto `/template-atti/compila/CIV_COM_001/pdf`. |
| Baseline prestazionale browser reale su `/template-atti/compila/CIV_COM_001` | OK | Contenuto visibile: desktop 3738 ms, tablet 3707 ms, mobile 4065 ms; `DOMContentLoaded` rispettivamente 1491 ms, 1371 ms, 1170 ms. Risorse lente principali: documento `CIV_COM_001`, `manifest.json`, `logo-iusentra.png` e script assistenti già lazy/secondari. |
| `python scripts\react-migration\generate_api_contracts.py`; `python scripts\react-migration\generate_api_contracts.py --check`; `python scripts\validate_openapi.py docs\openapi.yaml`; `python scripts\verify_openapi_provider.py` | OK | Contratti API/OpenAPI rigenerati e validi; provider verification `auth-error=230`, `success=27`, `backend-security=1`. |
| `python tools\sync_packaging_files.py --check`; `node scripts/react-migration/check-route-gate.mjs`; `python -m pytest -q tests/test_react_asset_retention.py --tb=short`; `git diff --check` | OK | Packaging sincronizzato, route gate Parte 12A coerente, asset React referenziati 2/2, diff whitespace senza errori; restano solo avvisi CRLF storici su file documentali già tracciati. |

| `python scripts\react-migration\generate_app_v2_page_registry.py --check`; `python scripts\react-migration\generate_app_v2_test_docs.py --check`; `python scripts\smoke_app_v2_all.py --subset inventory`; `python -m pytest -q tests/test_app_v2_page_registry.py tests/test_app_v2_test_plan_phase10.py tests/test_ci_cd_gates_phase11.py --tb=short` | OK | Hotfix post-push del gate GitHub `Lint + syntax`: `docs/test-inventory.md` rigenerato, inventario App V2 PASS=3 e 14/14 test verdi prima del nuovo commit. |

## Correzione editor template atti professionale 2.249.14 - 2026-06-04

| Comando / verifica | Esito | Nota |
| --- | --- | --- |
| `python -m pytest tests\test_template_atti_prefill_strict.py tests\test_template_atti_api_strict.py tests\test_template_atti_editor.py tests\test_template_atti_legal_sources_registry.py tests\test_utf8_integrity.py -q --tb=short` | OK | 20/20 passati: prefill fisico da fascicolo, payload React, bug campi manuali, import/fonti, registry fonti ufficiali e UTF-8 con accenti italiani coperti. |
| `python -m compileall pct\template_atti_prefill.py pct\template_atti_legal_sources.py web\blueprints\template_atti.py web\blueprints\api_v1_react.py scripts\react-migration\template_editor_browser_check.py` | OK | Sintassi confermata sui moduli Python modificati e sullo script di verifica browser. |
| `pnpm --filter @iusentra/studio typecheck`; `pnpm --filter @iusentra/studio test`; `pnpm --filter @iusentra/studio build` | OK | TypeScript, suite frontend e build Vite verdi dopo font, allineamenti, Lex, import, firma e compilazione multipla. |
| `docker compose build --no-cache app`; `docker compose up -d app`; `GET http://127.0.0.1:8080/api/pronto` | OK | Copia locale reale dell'utente aggiornata su `127.0.0.1:8080`; readiness `ok=true`, versione `2.249.14`. |
| `python scripts\react-migration\template_editor_browser_check.py` | OK | Browser Playwright su Docker reale: 73/73 controlli passati nel secondo giro dopo il fix firma/UTF-8. Verificati login/impersonazione, fascicolo `2DE106E6`, campi contestuali, niente troncamento primo carattere, cambio modello, font, allineamenti, timbro spostabile, Guida Pratica, fonti, Lex diff accetta/rifiuta, import RTF con `à è é ì ò ù`, pagina vuota, export RTF/DOCX/PDF, copia, salvataggio, compilazione multipla ZIP e apertura del flusso firma su documento appena salvato. Screenshot desktop/tablet/mobile in `artifacts/react-migration/template-editor-2.249.14-*.png`; report UTF-8 senza marker mojibake in `artifacts/react-migration/template-editor-browser-2.249.14.json`. |
| `python scripts\react-migration\generate_api_contracts.py --check`; `python scripts\validate_openapi.py docs\openapi.yaml`; `python scripts\verify_openapi_provider.py`; `python scripts\smoke_app_v2_all.py --subset contracts`; `python -m pytest -q tests\test_openapi_contracts_phase6.py --tb=short` | OK | Hotfix post-push del gate GitHub `Lint + syntax`: `docs/openapi.yaml` riallineato alla versione `2.249.14` e alle nuove righe delle API template atti; provider verification `auth-error=230`, `success=27`, `backend-security=1`; smoke contratti PASS=2/SKIP live=1; OpenAPI fase 6 `5/5`. |
| `python scripts\react-migration\generate_app_v2_page_registry.py --check`; `python scripts\react-migration\generate_app_v2_test_docs.py --check`; `python scripts\smoke_app_v2_all.py --subset inventory`; `python -m pytest -q tests/test_app_v2_page_registry.py tests/test_app_v2_test_plan_phase10.py tests/test_ci_cd_gates_phase11.py --tb=short` | OK | Hotfix post-push del gate GitHub `Lint + syntax`: `docs/test-inventory.md` e `docs/test-plan-app-v2.md` riallineati dopo l'aggiunta del test fonti ufficiali template atti; smoke inventory PASS=3; test App V2/governance `14/14`. |
| `python -m pytest tests\test_template_atti_prefill_strict.py tests\test_template_atti_editor.py tests\test_template_atti_api_strict.py tests\test_guida_pratica_api.py::test_template_guida_anteprima_render_e_salvataggio_non_bloccati_da_pst_mancante -q --tb=short`; `python -m pytest tests\test_support_remote.py tests\test_uffici_competenti.py tests\test_uffici_giudiziari_comuni_db.py tests\test_territorio_italia.py -q --tb=short`; `python -m pytest tests\test_local_ai.py::test_api_local_ai_status_and_fascicolo_ai tests\test_local_ai.py::test_api_local_ai_bootstrap_aggiorna_cache_runtime_chat tests\test_backend_security_phase5.py -q --tb=short` | OK | Hotfix CodeQL post-push: 18/18 template atti, 44/44 assistenza/uffici, 10/10 AI locale/backend security passati dopo parser RTF lineare, normalizzazione R.G. senza regex, redirect editor interno e messaggi pubblici senza eccezioni. |

## Editor template atti, Guida Pratica e assistenza 2.249.15 - 2026-06-04

| Comando / verifica | Esito | Nota |
| --- | --- | --- |
| `python -m py_compile web\blueprints\api_v1_react.py web\blueprints\template_atti.py pct\template_atti.py pct\legal_layout_profiles.py pct\editor.py scripts\react-migration\template_editor_browser_check.py` | OK | Sintassi confermata sui moduli editor/import/export/timbro e sullo script di audit browser. |
| `python -m pytest tests\test_template_atti_api_strict.py tests\test_support_remote.py tests\test_assistente_redazionale.py tests\test_utf8_integrity.py -q` | OK | 36/36 passati: API template atti, assistenza remota cliente/operatore, Lex redazionale e integrità UTF-8. |
| `python scripts\import_guida_pratica_user_kb_modules.py "C:\Users\antmm\Downloads\files (10).zip" "C:\Users\antmm\Downloads\files (11).zip" "C:\Users\antmm\Downloads\files (12).zip" "C:\Users\antmm\Downloads\files (13).zip" "C:\Users\antmm\Downloads\files (14).zip" "C:\Users\antmm\Downloads\kb_set19.json" "C:\Users\antmm\Downloads\kb_set20.json" --report artifacts\guida-pratica\kb-set15-20-import-report.json` | OK | 73 record ricevuti/integrati e 7 moduli KB nuovi (`kb_98_set17_p1`, `kb_98_set17_p2`, `kb_98_set18_p1`, `kb_98_set18_p2`, `kb_98_set18_p3`, `kb_98_set19`, `kb_98_set20`). |
| `python scripts\merge_legal_kb_modules.py`; `python scripts\import_guida_pratica_termini_processuali.py --report artifacts\guida-pratica\termini-processuali-import-report.json --csv artifacts\guida-pratica\termini-processuali-kb-audit.csv`; `python scripts\validate_guida_pratica.py`; `python scripts\audit_guida_pratica_user_material_fields.py` | OK | KB completa rigenerata con 59 moduli e 1.190 codici; 3.321 termini processuali importati, 1.075 template calcolabili aggiornati, validatore Guida Pratica 1.018/1.018 ufficiali OK e audit materiali utente con 0 perdite verso KB/UI/Lex. |
| `pnpm --dir frontend build` | OK | TypeScript e build Vite verdi dopo il fix CSS finale; chunk editor `TemplateAttiPage-Dc4AYg7-.js` 89.46 kB / 25.84 kB gzip e CSS `TemplateAttiPage-CeHGZIXl.css` 54.31 kB / 9.17 kB gzip. |
...
| `pnpm --filter @iusentra/studio test`; `pnpm --dir frontend build` | OK | Hotfix 2.249.17: contratto React editor template confermato dopo la rimozione dello `style={{ ... }}` dal marker pagina; build Vite rigenerata con asset `TemplateAttiPage-DtAPUvd8.js`, `TemplateAttiPage-BS48ZKAa.css` e `index-BYUseemM.js`. |
| `python scripts\react-migration\generate_api_contracts.py --check`; `python scripts\validate_openapi.py docs\openapi.yaml`; `python scripts\verify_openapi_provider.py`; `python scripts\smoke_app_v2_all.py --subset contracts`; `python -m pytest -q tests\test_openapi_contracts_phase6.py --tb=short` | OK | Hotfix post-push 2.249.17: `docs/openapi.yaml` riallineato alla versione `2.249.17`; provider verification `auth-error=230`, `success=27`, `backend-security=1`; smoke contratti PASS=2/SKIP live=1; OpenAPI fase 6 `5/5`. |
| `python scripts\react-migration\generate_app_v2_page_registry.py --check`; `python scripts\react-migration\generate_app_v2_test_docs.py --check`; `python scripts\smoke_app_v2_all.py --subset inventory`; `python -m pytest -q tests\test_app_v2_page_registry.py tests\test_app_v2_test_plan_phase10.py tests\test_ci_cd_gates_phase11.py --tb=short` | OK | Hotfix post-push 2.249.17: `docs/test-inventory.md` riallineato dopo la nuova classificazione tenant del presidio scadenziario; smoke inventory PASS=3; test App V2/governance `14/14`. |
...
| `python -m pytest -q tests\test_support_remote.py::test_support_remote_pc_click_uses_virtual_desktop_origin tests\test_support_remote.py::test_support_remote_pc_control_scripts_and_agent_are_separate_from_local_signer --tb=short`; `python -m pytest -q tests\test_support_remote.py tests\test_security_headers.py tests\test_utf8_integrity.py --tb=short` | OK | Hotfix 2.249.18: 2/2 e 34/34 passati; il click del controllo PC usa la geometria del desktop virtuale, i comandi remoti sono serializzati con ack e restano presidiati chat/audio/header sicurezza/UTF-8. |
...
| `python scripts\react-migration\generate_app_v2_page_registry.py --check`; `python scripts\react-migration\generate_app_v2_test_docs.py --check`; `python scripts\smoke_app_v2_all.py --subset inventory`; `python -m pytest -q tests/test_app_v2_page_registry.py tests/test_app_v2_test_plan_phase10.py tests/test_ci_cd_gates_phase11.py --tb=short` | OK | Hotfix post-push del gate GitHub `Lint + syntax`: `docs/test-inventory.md` e `docs/test-plan-app-v2.md` riallineati dopo il nuovo test PEC Control Tower; smoke inventory PASS=3 e test App V2/governance 14/14. |

## Manutenzione server, salute sistema e Scorecard Lex 2.249.25 - 2026-06-07

| Comando / verifica | Esito | Nota |
| --- | --- | --- |
| `python -m py_compile web/services/server_maintenance_surface.py web/blueprints/server_maintenance_admin.py pct/operational_resilience.py web/services/system_health_surface.py web/services/observability_runtime.py web/services/lex_eval_scorecard.py` | OK | Sintassi confermata su manutenzione server, backup retention, salute sistema, osservabilità runtime e Scorecard Lex. |
| `python -m pytest tests/test_server_maintenance_surface.py tests/test_operational_resilience.py tests/test_observability_runtime.py tests/test_operational_surfaces.py -q` | OK | 32/32 passati: retention backup a una copia, separazione cartelle legacy/non operative, pulizia Normattiva governata, osservabilità e superfici operative. |
| `python -m pytest tests/test_lex_eval_scorecard_surface.py tests/test_web_bootstrap.py::test_template_principali_usano_copy_italiana_e_date_localizzate tests/test_web_bootstrap.py::test_file_critici_non_contengono_marker_di_mojibake -q` | OK | 4/4 passati: Scorecard Lex non dichiara misure senza risultati reali, calcola pass/fail/fonti/tempi quando il file risultati esiste e mantiene copy/UTF-8 sui file critici. |

## Baseline struttura dati tenant e PEC/scadenziario 2.249.26 - 2026-06-07

| Comando / verifica | Esito | Nota |
| --- | --- | --- |
| `python -m py_compile pct\tenant.py pct\database.py pct\storage_migration.py pct\agenda.py pct\scadenziario.py pct\pec_pipeline.py web\blueprints\pec_pipeline_api.py web\services\react_scadenziario_bridge.py web\services\scadenziario_views.py scripts\audit_tenant_data_structure.py` | OK | Sintassi confermata su bootstrap tenant, mirror JSON/SQLite, agenda, scadenziario, PEC e script audit. |
| `python -m pytest -q tests/test_storage_strategy.py::test_nuovo_studio_crea_configurazione_runtime_agenda_scadenze_notifiche tests/test_storage_strategy.py::test_audit_tenant_data_structure_verifica_json_sqlite_postgres tests/test_storage_strategy.py::test_audit_tenant_data_structure_fallisce_se_manca_mirror_json_sql tests/test_storage_strategy.py::test_audit_tenant_data_structure_fallisce_se_manca_json_tenant` | OK | 4/4 passati: nuovo studio con JSON, `studio.db`, `notifications.db`, mirror SQL e audit rosso su mancanze reali. |
| `python -m pytest -q tests/test_scadenziario.py::test_scadenziario_sqlite_importa_json_tenant_pec_quando_db_vuoto tests/test_scadenziario.py::test_scadenziario_sqlite_salva_anche_mirror_json_tenant tests/test_agenda.py::test_agenda_sqlite_importa_json_tenant_pec_quando_db_vuoto tests/test_agenda.py::test_agenda_sqlite_salva_anche_mirror_json_tenant` | OK | 4/4 passati: scadenziario e agenda riconciliano JSON tenant in SQLite e mantengono mirror JSON. |
| `python -m pytest -q tests/test_pec_audit_pipeline.py::test_pec_api_demo_digest_mime_and_quick_action tests/test_pec_audit_pipeline.py::test_pec_api_schedula_duplicato_audit_senza_report_da_mime_locale` | OK | 2/2 passati: scadenza PEC idempotente e notifica interna tenant-aware create senza duplicati. |
| `python -m pytest -q tests/test_storage_strategy.py tests/test_scadenziario.py tests/test_agenda.py tests/test_pec_audit_pipeline.py` | OK | Blocco completo verde: storage, scadenziario, agenda e PEC audit pipeline. |
| `python scripts\audit_tenant_data_structure.py --repair` | OK | Audit reale locale completato con `nessuna mancanza rilevata`; lo script non crea backup o snapshot. |
| `pnpm --filter @iusentra/studio typecheck` | OK | TypeScript verde dopo estensione del payload React Scadenziario con vista PEC e dettagli operativi. |
| `pnpm --filter @iusentra/studio build` | OK | Build Vite verde dopo la modifica della pagina Scadenziario React; chunk `ScadenziarioPage` rigenerato. |
| `python -m pytest -q tests/test_web_bootstrap.py::test_file_critici_non_contengono_marker_di_mojibake tests/test_web_bootstrap.py::test_template_principali_usano_copy_italiana_e_date_localizzate` | OK | 2/2 passati: controllo UTF-8/copy italiana sui file critici dopo le modifiche dati/UI. |
| `python tools\sync_packaging_files.py --check`; `python scripts\react-migration\generate_api_contracts.py`; `python scripts\react-migration\generate_api_contracts.py --check`; `python scripts\validate_openapi.py docs\openapi.yaml` | OK | Packaging sincronizzato; OpenAPI rigenerato, allineato e valido dopo il bump versione e le nuove API/test documentati. |
| `git diff --check` | OK | Nessun errore whitespace dopo la pulizia della riga `max_backup` in `backup/config.json`; restano solo avvisi CRLF informativi sui report già toccati. |

## PEC operative, udienze audiovisive e notifiche studio 2.249.30 - 2026-06-07

| Comando / verifica | Esito | Nota |
| --- | --- | --- |
| `python -m py_compile pct/pec_pipeline.py web/blueprints/pec_pipeline_api.py web/services/scadenziario_views.py web/services/react_scadenziario_bridge.py web/services/react_impostazioni_notifications.py scripts/presidia_pec_local_archive.py tests/test_pec_audit_pipeline.py tests/test_react_scadenziario_additions.py tests/test_react_shell.py` | OK | Sintassi confermata su pipeline PEC, vista scadenziario, notifiche Impostazioni, CLI presidio e test collegati. |
| `python -m ruff check pct/pec_pipeline.py web/blueprints/pec_pipeline_api.py web/services/scadenziario_views.py web/services/react_scadenziario_bridge.py web/services/react_impostazioni_notifications.py scripts/presidia_pec_local_archive.py tests/test_pec_audit_pipeline.py tests/test_react_scadenziario_additions.py tests/test_react_shell.py` | OK | Lint mirato verde dopo rimozione import inutilizzato e correzione messaggi PEC. |
| `python -m pytest tests/test_pec_audit_pipeline.py tests/test_react_scadenziario_additions.py::test_react_scadenziario_page_collegata_nav_api_e_lex tests/test_react_scadenziario_additions.py::test_react_scadenziario_bridge_espone_link_udienza_remota tests/test_react_scadenziario_additions.py::test_react_scadenziario_vista_pec_mostra_solo_scadenze_operative tests/test_react_shell.py::test_impostazioni_notifiche_mostra_avvisi_operativi_persistenti tests/test_react_shell.py::test_impostazioni_react_frontend_copre_local_signer_occhio_e_ai_locale -q` | OK | 42/42 passati: link Teams launcher lungo non troncato, allegato fonte esposto in UI, vista PEC solo operativa, notifiche verso utenti attivi dello studio. |
...
## PEC link udienze audiovisive spezzati da OCR 2.249.32 - 2026-06-07

| Comando / verifica | Esito | Nota |
| --- | --- | --- |
| `python -m py_compile pct/pec_pipeline.py tests/test_pec_audit_pipeline.py`; `python -m ruff check pct/pec_pipeline.py tests/test_pec_audit_pipeline.py` | OK | Sintassi e lint confermati dopo la ricostruzione dei link Teams/Webex/Meet spezzati da spazi OCR. |
| `python -m pytest tests/test_pec_audit_pipeline.py::test_remote_hearing_extracts_full_teams_launcher_url_without_truncation tests/test_pec_audit_pipeline.py::test_remote_hearing_rebuilds_teams_url_split_by_pdf_ocr_spaces tests/test_pec_audit_pipeline.py::test_refresh_validation_reports_repairs_stale_binary_zip_ocr_for_remote_hearing tests/test_pec_audit_pipeline.py::test_pec_remote_hearing_link_arrives_in_scadenziario_and_agenda -q` | OK | 4/4 passati: URL launcher lungo non troncato, URL Teams `meetup- join` / `thr ead.v2` ricostruito, OCR ZIP riparato e link propagato a scadenziario/agenda. |
| `python -m pytest tests/test_pec_audit_pipeline.py -q` | OK | 40/40 passati sul file PEC completo. |
| `docker compose build app`; `docker compose up -d --no-deps --force-recreate app`; `GET http://127.0.0.1:8080/api/pronto`; script Python runtime nel container app | OK | Docker locale reale su `127.0.0.1:8080` pronto con versione `2.249.32`; prova runtime sul link OCR spezzato passata con URL Teams ricostruito e `integrity=exact`. |

## Portale Cliente link, WhatsApp Web e videocall 2.249.34 - 2026-06-08

| Comando / verifica | Esito | Nota |
| --- | --- | --- |
| `python -m pytest tests/test_client_portal_repository.py tests/test_client_portal_api.py -q --tb=short` | OK | 11/11 passati: token invito salvato solo hash, link cliente assoluto, associazione automatica del fascicolo unico del cliente, upload/chat/privacy e videocall validata lato studio/cliente. |
| `python -m pytest tests/test_feature_flags.py tests/test_app_v2_feature_flags.py -q --tb=short` | OK | 11/11 passati: `routes.appV2.clientPortal.videoCalls` è default-on e resta governato dai flag App V2. |
| `pnpm --filter @iusentra/studio typecheck`; `pnpm --filter @iusentra/studio test`; `pnpm --filter @iusentra/studio build` | OK | TypeScript, gate React e build Vite verdi dopo ricerca cliente, pannello `Link cliente`, azioni `Copia link`/`Invia con WhatsApp Web` e pulsante `Apri videocall`. |
| `python -m pytest tests/test_utf8_integrity.py -q --tb=short`; `git diff --check` | OK | 4/4 UTF-8 passati e nessun errore whitespace. |
| `docker compose build app`; `docker compose up -d --no-deps --force-recreate app`; `GET http://127.0.0.1:8080/api/pronto` | OK | Docker locale reale pronto su `127.0.0.1:8080` con versione `2.249.34` dopo build degli asset React e patch backend. |
| `docker compose build --no-cache app`; `docker compose up -d --no-deps --force-recreate app`; `GET http://127.0.0.1:8080/api/pronto` | OK | Build no-cache finale passata; container app ricreato e poi `healthy`, `/api/pronto` verde con versione `2.249.34`. |
...
## Console pianificazioni canary fonti 2.249.37 - 2026-06-08

| Comando / verifica | Esito | Nota |
| --- | --- | --- |
| `python -m pytest tests/test_scheduler_registry.py tests/test_scheduler_admin.py -q --tb=short` | OK | 15/15 passati: il canary fonte completato senza pubblicazioni nuove non viene più marcato come failure quando ha letto/processato documenti senza timeout né errori. |
| `python -m ruff check pct/scheduler_registry.py web/services/scheduler_admin_surface.py tests/test_scheduler_registry.py tests/test_scheduler_admin.py` | OK | Ruff mirato verde sul registry scheduler, superficie console e test collegati. |
...
## Console pianificazioni manutenzioni pesanti rinviate nel bulk 2.249.41 - 2026-06-08

| Comando / verifica | Esito | Nota |
| --- | --- | --- |
| `python -m pytest tests/test_scheduler_registry.py tests/test_scheduler_admin.py -q --tb=short` | OK | 19/19 passati: qualunque manutenzione pesante viene rinviata dentro un avvio massivo con altre esecuzioni manuali aperte, ma resta avviabile singolarmente. |
| `python -m ruff check pct/scheduler.py pct/scheduler_registry.py web/services/scheduler_admin_surface.py tests/test_scheduler_registry.py tests/test_scheduler_admin.py` | OK | Ruff mirato verde sul registry scheduler e test aggiornati. |
| `python -m pytest tests/test_utf8_integrity.py -q --tb=short` | OK | 4/4 passati dopo le nuove note operative. |
| `python scripts/react-migration/generate_api_contracts.py`; `python scripts/react-migration/generate_api_contracts.py --check`; `python scripts/validate_openapi.py docs/openapi.yaml`; `python tools/sync_packaging_files.py --check`; `git diff --check` | OK | OpenAPI rigenerato e riallineato alla versione `2.249.41`; packaging sincronizzato e diff senza errori whitespace. |
| `docker compose build --no-cache app scheduler-worker ocr-worker`; `docker compose up -d --force-recreate app scheduler-worker ocr-worker`; `GET http://127.0.0.1:8080/api/pronto`; `docker compose ps app scheduler-worker ocr-worker` | OK | Copia Docker locale reale ricostruita e healthy; `/api/pronto` risponde con versione `2.249.41` su `127.0.0.1:8080`. |

## Assistenza remota e Local Signer 1.6.72 - 2026-06-11

| Comando / verifica | Esito | Nota |
| --- | --- | --- |
| `python -m py_compile web\blueprints\react_shell.py web\blueprints\studio_site.py tools\local_signer.py local_signer_mod\support_agent.py tools\dist\local_signer.py tools\dist\local_signer_mod\support_agent.py` | OK | Sintassi confermata su bootstrap React, Sito Studio React, sorgente Local Signer e modulo assistenza incluso nel dist. |
| `node --check web\static\js\local-signer-monitor.js`; `node --check web\static\js\support_customer_room.js` | OK | JavaScript monitor globale e stanza cliente validi dopo auto-update Local Signer e fallback `27272/support`. |
| `python -m pytest -q tests/test_build_dist.py tests/test_local_signer.py::test_download_moduli_local_signer_e_pubblico tests/test_local_signer.py::test_download_requirements_local_signer_e_pubblico tests/test_local_signer.py::test_local_signer_update_endpoint_preferisce_hot_update_sorgenti tests/test_local_signer.py::test_local_signer_update_endpoint_usa_installer_ufficiale_se_hot_update_fallisce --tb=short` | OK | Packaging e endpoint update Local Signer coperti; `support_agent.py` richiesto dal dist. |
| `python -m pytest -q tests/test_support_remote.py::test_support_remote_operator_room_has_full_screen_control_panel tests/test_support_remote.py::test_support_remote_studio_user_can_request_assistance_from_studio tests/test_support_remote.py::test_support_remote_pc_control_scripts_use_browser_screen_and_local_signer_fallback tests/test_support_remote.py::test_support_remote_studio_launcher_opens_customer_room tests/test_web_bootstrap.py::test_local_signer_monitor_globale_verifica_versione_e_installer tests/test_react_shell.py::test_react_blocco_finale_studio_admin_completo tests/test_react_shell.py::test_react_blocco_finale_route_reali_e_vista_classica --tb=short` | OK | 7/7 passati: topbar React, stanza cliente, stanza operatore, monitor Local Signer e contratti React mirati. |
| `pnpm --filter @iusentra/studio build` | OK | Build Vite verde; avviso solo su chunk grande già noto. |
| `python tools\build_dist.py` | OK | Rigenerati `SetupLocalSigner-1.6.72.exe`, alias `SetupLocalSigner.exe`, script macOS/Linux e dist con `local_signer_mod/support_agent.py`. |
| `docker compose up -d --build app scheduler-worker ocr-worker`; `GET http://127.0.0.1:8080/api/pronto`; `docker compose ps` | OK | Copia Docker reale dell'utente ricostruita su `127.0.0.1:8080`; app, scheduler e OCR healthy; `/api/pronto` risponde `2.251.38`. |
| Browser integrato su `http://127.0.0.1:8080/support/operatore/<public_id>` | OK | Stanza operatore senza errori console; controlli `Richiedi controllo PC`, `Microfono`, `Muta microfono`, `Schermo intero` presenti; nota controllo PC aggiornata a `Local Signer aggiornato o agente IUSENTRA Assistenza`. |
| Browser integrato su `http://127.0.0.1:8080/support/join/<token>` | OK | Stanza cliente senza card/monitor scuro, senza `Failed to fetch`, con consensi schermo/microfono/chat e bootstrap `localSignerBase=http://127.0.0.1:27272`, `localSignerLatestVersion=1.6.72`. |
...
## Governance avvio e coverage CI 2.253.5 - 2026-06-13

| Comando / verifica | Esito | Nota |
| --- | --- | --- |
| `python -m py_compile web\bootstrap\runtime_bundle.py web\bootstrap\startup_governance.py web\bootstrap\startup_governance_runner.py`; `python tools\check_repo_governance.py`; `python -m pytest tests\test_web_bootstrap.py tests\test_security_headers.py tests\test_utf8_integrity.py tests\test_studio_voice_assistant.py tests\test_react_asset_retention.py -q --tb=short` | OK | 68/68 passati con temporaneo su `D:`; governance OK e `runtime_bundle.py` riportato a 109 righe. |
| `node frontend\scripts\check-studio-voice-assistant.mjs` | OK | Catalogo assistente confermato dopo il refactor bootstrap: 390 frasi, 350 aggiunte, 59 destinazioni. |
| `python -m pytest tests\test_storage_strategy.py::test_carica_tenant_rinvia_bootstrap_legacy_senza_riconciliazione_pesante tests\test_web_bootstrap.py::test_runtime_bundle_startup_sync_directory_non_rilancia_reconcile_pesante tests\test_web_bootstrap.py::test_impostazioni_pec_espone_controllo_local_signer_e_password_salvata -q --tb=short` | OK | Replica mirata del failure GitHub più test bootstrap collegati: 3/3 passati. |
| `python scripts\run_pytest_phases.py --suite coverage-critical --suite-shard 1 --suite-total-shards 12 --suite-subdivide-items --timeout-minutes 5 -- --cov=lex --cov=pct.auth --cov=pct.storage --cov=pct.storage_postgres --cov=pct.telematico_repository --cov=pct.telematico_workflow --cov-config=config/coverage-critical.ini --cov-report=` | OK | Replica locale dello shard GitHub rosso: 32/32 passati dopo l'aggiornamento del test tenant al comportamento "bootstrap legacy rinviato fuori dalla richiesta". |
| `python -m pytest tests\test_storage_strategy.py::test_login_route_rinvia_bootstrap_legacy_root_data_fuori_dalla_navigazione -q --tb=short`; `python scripts\run_pytest_phases.py --suite coverage-critical --suite-shard 7 --suite-total-shards 12 --suite-subdivide-items --timeout-minutes 5 -- --cov=lex --cov=pct.auth --cov=pct.storage --cov=pct.storage_postgres --cov=pct.telematico_repository --cov=pct.telematico_workflow --cov-config=config/coverage-critical.ini --cov-report=` | OK | Replica locale del secondo rosso GitHub: test mirato 1/1 e shard Coverage 7/12 31/31 passati. La navigazione resta senza migrazione legacy, mentre il bootstrap governato esplicito conserva il recupero clienti. |
## Hardening CodeQL Procedure Completion e messaggi pubblici 2.253.6 - 2026-06-13

| Comando / verifica | Esito | Nota |
| --- | --- | --- |
| `python -m py_compile pct\procedure_completion\extractor.py web\services\react_redazione_guidata_bridge.py web\blueprints\api_v1_react.py web\blueprints\topbar.py web\blueprints\push_notifications.py web\blueprints\installation_pack_admin.py web\blueprints\operational_resilience_admin.py web\bootstrap\fascicoli_create_routes.py web\services\react_client_portal_bridge.py web\services\server_maintenance_surface.py web\services\installation_pack_surface.py` | OK | Sintassi confermata sui file annotati da CodeQL dopo parser normativo deterministico, marcatore dati mancanti lineare e messaggi pubblici senza dettagli interni. |
| `python -m pytest tests/test_procedure_completion_fusion.py tests/test_redazione_guidata.py tests/test_push_notifications.py tests/test_topbar_operational_api.py tests/test_client_portal_api.py tests/test_installation_packs.py tests/test_operational_resilience.py tests/test_utf8_integrity.py -q --tb=short` | OK | 77/77 passati; incluso input patologico su citazioni normative, escaping HTML dei marcatori `[DATO MANCANTE: ...]`, notifiche push, topbar, portale cliente, pack installazione e resilienza operativa. |
| `python tools\sync_packaging_files.py --check`; `git diff --check`; ricerca mirata `str(exc)`/`{exc}` sui file annotati | OK | Packaging sincronizzato alla versione `2.253.6`; diff senza errori whitespace; nessuna esposizione diretta dell'eccezione residua nei percorsi CodeQL segnalati. |

## Hardening CodeQL residui medium 2.253.7 - 2026-06-13

| Comando / verifica | Esito | Nota |
| --- | --- | --- |
| `python -m py_compile web\services\react_fascicoli_bridge.py web\blueprints\installation_pack_admin.py web\blueprints\operational_resilience_admin.py web\blueprints\api_v1_react.py` | OK | Sintassi confermata dopo messaggi pubblici fissi per cambio stato fascicolo, API pack installazione e API resilienza operativa. |
| `python -m pytest tests/test_operational_resilience.py tests/test_installation_packs.py tests/test_redazione_guidata.py tests/test_procedure_completion_fusion.py tests/test_utf8_integrity.py -q --tb=short` | OK | 51/51 passati; warning editor preesistente su truth-testing ElementTree non collegato alla patch. |
| `python scripts\react-migration\generate_api_contracts.py --check`; `python scripts\validate_openapi.py docs\openapi.yaml`; `python tools\sync_packaging_files.py --check` | OK | Contratti API, OpenAPI e packaging riallineati alla versione `2.253.7`. |

## Hardening CodeQL annotazioni residue 2.253.8 - 2026-06-13
...
| GitHub `CodeQL` sullo SHA `e1140d512db36a33e3ac41a0945d5df9152ddbb7` | OK | Check-run `81224064112`: conclusione `success`, titolo `No new alerts in code changed by this pull request`, `annotations_count=0`; endpoint annotazioni vuoto `[]`. |

## Isolamento backup tenant attivi 2.253.11 - 2026-06-14

| Comando / verifica | Esito | Nota |
| --- | --- | --- |
| Eliminazione locale `D:\legale\IUSENTRA\data\tenants\antonella-mammola` | OK | Cartella storica non registrata rimossa dopo verifica percorso; resta solo `tenant-8bf98719c459` nella copia locale reale e lo spazio libero su `D:` è circa `218,24 GB`. |
| `ssh iusentra-hetzner` lettura registry e directory tenant | OK | Produzione Hetzner ha 2 studi attivi registrati e 2 directory tenant corrispondenti: `5adafa47-dadb-4d87-87fc-8dec71f8e3e5` e `studio-legale-giuseppe-montagnese`; nessuna cartella tenant extra/non registrata rilevata. |
| `python -m py_compile web\services\server_maintenance_surface.py tests\test_server_maintenance_surface.py`; `python -m pytest tests\test_server_maintenance_surface.py -q`; `git diff --check` | OK | 16/16 passati; la retention backup scansiona solo i tenant attivi registrati quando esiste `tenants.json`, risolve gli slug espliciti verso la cartella storage canonica, associa hash SHA-256 di identità tenant, rifiuta tenant non registrati e mantiene separati due studi attivi. |
| `docker compose build --no-cache app`; `docker compose up -d --no-deps --force-recreate app`; `GET http://127.0.0.1:8080/api/pronto`; `docker exec iusentra-app ... run_tenant_backup_retention(apply=False, data_root=/data)` | OK | Copia Docker reale dell'utente ricostruita su `127.0.0.1:8080`, versione `2.253.11`, container healthy. Analisi runtime locale: `tenant_backup_roots=1`, `backup_archives_scanned=124`, `archives_to_delete=123`, un solo hash tenant rilevato; nessuna cancellazione applicata ai backup durante il controllo. |

## PEC, Agenda, Scadenziario e deposito React 2.253.13 - 2026-06-14

| Comando / verifica | Esito | Nota |
| --- | --- | --- |
| Analisi sanificata campioni locali `DEPOSITO TELEMATICO` / `COPIA NON CRITTOGRAFATA` RG 1754/2026 | OK | Confermata struttura reale: PEC con `Atto.enc`; copie non crittografate con `DatiAtto.xml.p7m`, atto principale, NIR/procura/allegati, RAC/RdAC, attestazione, decreto fissazione udienza e indice documenti. File con nome `.pdf` ma contenuto MIME gestiti come messaggi, non come PDF. |
| Consultazione fonti PST Ministero Giustizia | OK | Verificate schede e specifiche su busta telematica, `DatiAtto.xml`, atto PDF firmato, formati allegati, procura, invio via PEC, oggetto `DEPOSITO ...`, unico allegato `.enc`, ricevute e controlli automatici. |
| `python -m py_compile pct\practice_engine\deposit_readiness.py web\services\pdf_deadline_import.py web\services\react_agenda_bridge.py web\services\react_scadenziario_bridge.py pct\scadenziario.py web\bootstrap\deposito_routes.py` | OK | Sintassi confermata dopo proposta busta, codice oggetto PST, presidio da documento notificato e testi Agenda/Scadenziario. |
| `node --check scripts\react-migration\pec_agenda_scadenziario_visual_audit.mjs` | OK | Script audit visivo valido sintatticamente dopo estensione controllo dettaglio Agenda e card Scadenziario. |
| `python -m pytest tests\test_practice_engine_validators.py tests\test_regia_api_payloads.py tests\test_regia_ui_react.py -q --tb=short` | OK | 11/11 passati: proposta automatica solo con classificazione certa, documento richiesto non auto-collegato come ricorso, slot manuali e UI deposito React. |
| `python -m pytest tests\test_react_scadenziario_additions.py tests\test_react_shell.py::test_react_agenda_bridge_traduce_pec_udienza_in_linguaggio_professionale -q --tb=short` | OK | 10/10 passati: cliente del fascicolo nello Scadenziario, testi PEC professionali, documento notificato con udienza audiovisiva, link Teams esatto, deduplica e Agenda solo con orario certo. |
| `python -m pytest tests\test_deposito_guidato.py::test_api_validazione_deposito_restituisce_semaforo_e_consente_con_warning tests\test_deposito_guidato.py::test_generazione_busta_usa_codice_oggetto_pst_validato tests\test_deposito_guidato.py::test_orchestratore_blocca_deposito_pct_senza_codice_oggetto_pst -q --tb=short` | OK | 3/3 passati: validazione deposito, blocco senza codice oggetto PST e generazione busta con `Oggetto` allineato al codice PST validato. |
| `pnpm --filter @iusentra/studio typecheck` | OK | TypeScript senza errori dopo la sezione `Proposta busta`, selezione manuale slot e Scadenziario con cliente fascicolo. |
| `pnpm --filter @iusentra/studio build` | OK | Build Vite completata; resta warning governato sui chunk grandi. Asset React rigenerati. |
| `node frontend\scripts\check-react-contracts.mjs`; `node scripts\react-migration\check-route-gate.mjs` | OK | Contratti React e route gate coerenti; la route profonda deposito resta governata in React. |

## Deposito profilo pratica, firma multipla e normativa PST 2.253.14 - 2026-06-14

| Comando / verifica | Esito | Nota |
| --- | --- | --- |
| `python -m py_compile pct\pst_catalog.py pct\practice_engine\profiles.py pct\busta.py pct\practice_engine\evaluator.py pct\deposito.py pct\practice_engine\deposit_readiness.py pct\deposito_guidato.py web\bootstrap\deposito_routes.py web\services\react_fascicoli_bridge.py web\services\react_notifiche_legali_bridge.py` | OK | Sintassi confermata dopo profilo pratica, catalogo PST 11/06/2026, busta AES256, blocco invio diretto e UI deposito. |
| Profilo `PROC_LIC_IMP_001` da console Python | OK | Slot risultanti: `Atto principale` obbligatorio separato, `Procura alle liti` obbligatoria, `Lettera di licenziamento`, `Contratto di lavoro` e `Buste paga` come `DOCUMENTO_PROVA`; `Ricevute ed esiti deposito` non bloccante. |
| `python -m pytest tests\test_busta.py::test_audit_busta_esplicita_simulazione_locale tests\test_regia_api_payloads.py::test_api_regia_payload_completo_e_mock_false tests\test_deposito.py::test_deposito_invia_pec_civile_usa_local_signer_se_server_send_disabilitato tests\test_deposito.py::test_api_pkcs11_firma_documenti_batch_usa_una_sola_sessione tests\test_regia_ui_react.py::test_ui_deposito_prepara_legge_intero_fascicolo_e_distingue_canale tests\test_practice_engine_validators.py::test_profilo_deposito_non_scambia_contratto_per_atto_principale tests\test_deposito_guidato.py::test_pst_xsd_sici_20260611_tracciato_come_anticipazione_non_in_esercizio -q --tb=short` | OK | 7/7 passati: audit busta simulata non valida per trasporto reale, blocco invio diretto senza `Atto.enc` AES256, firma batch backend, UI React deposito, profilo pratica e XSD SICI 11/06/2026 come anticipazione non in esercizio. |
| `pnpm --filter @iusentra/studio typecheck` | OK | TypeScript senza errori dopo pannello firma multipla e profilo pratica visibile nel prepara deposito. |
| `node frontend\scripts\check-react-contracts.mjs`; `node scripts\react-migration\check-route-gate.mjs` | OK | Contratti React e route gate confermano la route deposito profonda governata in React. |
| `pnpm --filter @iusentra/studio build` | OK | Build Vite completata; resta solo warning governato sul chunk principale sopra 500 kB. |
| `python tools\sync_packaging_files.py --check`; `git diff --check`; `python -m pytest tests\test_utf8_integrity.py -q --tb=short` | OK | Packaging sincronizzato a `2.253.14`, diff senza errori whitespace e 4/4 test UTF-8 passati. |
| `docker compose build --no-cache app scheduler-worker ocr-worker`; `docker compose up -d --force-recreate app scheduler-worker ocr-worker`; `GET http://127.0.0.1:8080/api/pronto` | OK | Copia Docker reale ricostruita: app, scheduler e OCR healthy; `/api/pronto` risponde `ok=true`, `versione=2.253.14`. |
| `IUSENTRA_PEC_AGENDA_E2E_VISIBLE=1 CHROME_PATH="C:\Program Files\Google\Chrome\Application\chrome.exe" node scripts\react-migration\pec_agenda_scadenziario_visual_audit.mjs` | OK | Audit Chrome visibile su `127.0.0.1:8080` riuscito. Agenda senza duplicati/tooltip nativi/eventi finti alle 09:00 nel range corrente; Scadenziario scorre top/medio/fondo desktop e mobile, 8 card senza duplicati, card operative uniformi; Calendari mostra sincronizzazione diretta Google/Outlook; la URL `/fascicoli/9B9DF2A1/deposito/prepara` è React/non legacy ma il fascicolo non esiste nella copia locale. |
| `IUSENTRA_PEC_AGENDA_EXTRA_DEPOSIT_PATH=/fascicoli/2DE106E6/deposito/prepara ... pec_agenda_scadenziario_visual_audit.mjs` | OK | Audit Chrome visibile con fascicolo locale reale: pagina deposito React/non legacy, lettura intero fascicolo con 2 documenti, nota `letto integralmente`, topbar presente, nessun errore console e nessun problema di nomi tecnici documento. |
| `GET http://127.0.0.1:27272/ping?light=1` | OK | Local Signer reale raggiungibile: versione `1.6.72`, libreria `C:\Windows\System32\bit4xpki.dll` presente, nessuna sessione PIN attiva. |

## Selezione documenti deposito visibile 2.253.16 - 2026-06-14

| Comando / verifica | Esito | Nota |
| --- | --- | --- |
| `python tools\sync_packaging_files.py --check` | OK | Packaging sincronizzato dopo bump `2.253.16`. |
| `node --check scripts\react-migration\pec_agenda_scadenziario_visual_audit.mjs` | OK | Script audit visivo valido dopo l'estensione del controllo checkbox deposito. |
| `python -m pytest tests\test_regia_ui_react.py::test_ui_deposito_prepara_legge_intero_fascicolo_e_distingue_canale -q --tb=short` | OK | Guardrail React deposito: sezione `Documenti da inviare`, checkbox, scelta manuale non di comunicazione e payload `documenti_selezionati_ids`. |
| `python -m pytest tests\test_regia_api_payloads.py tests\test_regia_ui_react.py -q --tb=short` | OK | 6/6 passati su payload regia e UI deposito. |
| `python -m pytest tests\test_deposito_guidato.py::test_api_validazione_deposito_restituisce_semaforo_e_consente_con_warning tests\test_deposito_guidato.py::test_generazione_busta_usa_codice_oggetto_pst_validato tests\test_deposito_guidato.py::test_orchestratore_blocca_deposito_pct_senza_codice_oggetto_pst -q --tb=short` | OK | 3/3 passati su validazione deposito e generazione busta. |
| `pnpm --filter @iusentra/studio typecheck`; `pnpm --filter @iusentra/studio build` | OK | TypeScript senza errori; build Vite completata con warning storico sui chunk grandi. |
| `node frontend\scripts\check-react-contracts.mjs`; `node scripts\react-migration\check-route-gate.mjs` | OK | Contratti React e route gate coerenti per la route deposito profonda. |
| `docker compose build --no-cache app scheduler-worker ocr-worker`; `docker compose up -d --force-recreate app scheduler-worker ocr-worker`; `GET http://127.0.0.1:8080/api/pronto` | OK | Copia Docker reale ricostruita su `127.0.0.1:8080`; app, scheduler e OCR healthy; `/api/pronto` risponde `versione=2.253.16`. |
| Browser reale visibile su `/fascicoli/2DE106E6/deposito/prepara` | OK | Sezione `Documenti da inviare` verificata: 2 checkbox, pulsanti `Ripristina proposta` e `Seleziona tutti i documenti`, conteggio `2 selezionati`; togliendo la prima spunta diventa `1 selezionato` e ripristinando torna a `2 selezionati`. Scroll fino al fondo senza overflow orizzontale; card `Atto principale` corretta, nome documento non più spezzato in verticale. |
| `python -m pytest tests\test_utf8_integrity.py tests\test_react_asset_retention.py -q --tb=short`; `node frontend\scripts\check-react-contracts.mjs`; `node scripts\react-migration\check-route-gate.mjs` | OK | 6/6 su UTF-8 e asset React referenziati dal manifest; contratti React e route gate ancora verdi dopo pulizia degli asset Vite non referenziati. |
| `python scripts\react-migration\generate_api_contracts.py`; `python scripts\react-migration\generate_api_contracts.py --check`; `python scripts\validate_openapi.py docs\openapi.yaml`; `python scripts\verify_openapi_provider.py`; `python -m pytest tests\test_openapi_contracts_phase6.py -q --tb=short` | OK | Contratti API/OpenAPI riallineati dopo blocco pre-push; provider verification `auth-error=261`, `public-safe=15`, `success=29`, `backend-security=1`; test OpenAPI fase 6 `5/5`. |
| `python scripts\react-migration\generate_app_v2_page_registry.py --check`; `python scripts\react-migration\generate_app_v2_area_requirements.py --check`; `python scripts\react-migration\generate_app_v2_test_docs.py --check`; `python scripts\smoke_app_v2_all.py --subset inventory`; `python -m pytest tests\test_app_v2_page_registry.py tests\test_app_v2_test_plan_phase10.py tests\test_ci_cd_gates_phase11.py -q --tb=short` | OK | Registro App V2, requisiti area e documenti test riallineati dopo blocco pre-push; smoke inventory PASS=3, FAIL=0; test App V2/governance `15/15`. |

## Rientro governance deposito 2.253.17 - 2026-06-14

| Comando / verifica | Esito | Nota |
| --- | --- | --- |
| `python -m py_compile web\bootstrap\deposito_routes.py web\services\deposito_route_helpers.py`; `python tools\check_repo_governance.py`; conteggio righe `deposito_routes.py` | OK | Helper deposito estratti in `web/services/deposito_route_helpers.py`; governance repo verde e `deposito_routes.py` a 987 righe, sotto il limite 1000. |
| `python tools\sync_packaging_files.py --check`; `python scripts\react-migration\generate_api_contracts.py`; `python scripts\react-migration\generate_api_contracts.py --check`; `python scripts\validate_openapi.py docs\openapi.yaml` | OK | Bump `2.253.17`, packaging sincronizzato e OpenAPI valido dopo rigenerazione contratti. |
| `python -m pytest tests\test_busta.py::test_audit_busta_esplicita_simulazione_locale tests\test_regia_api_payloads.py::test_api_regia_payload_completo_e_mock_false tests\test_deposito.py::test_deposito_invia_pec_civile_usa_local_signer_se_server_send_disabilitato tests\test_deposito.py::test_api_pkcs11_firma_documenti_batch_usa_una_sola_sessione tests\test_regia_ui_react.py::test_ui_deposito_prepara_legge_intero_fascicolo_e_distingue_canale tests\test_practice_engine_validators.py::test_profilo_deposito_non_scambia_contratto_per_atto_principale tests\test_deposito_guidato.py::test_pst_xsd_sici_20260611_tracciato_come_anticipazione_non_in_esercizio -q --tb=short` | OK | 7/7 passati: il refactor non cambia busta, firma batch, validazioni profilo pratica e UI React deposito. |
| `python scripts\verify_openapi_provider.py`; `python -m pytest tests\test_openapi_contracts_phase6.py -q --tb=short` | OK | Provider verification `auth-error=261`, `public-safe=15`, `success=29`, `backend-security=1`; test OpenAPI fase 6 `5/5`. |
| `python -m pytest tests\test_packaging_consistency.py tests\test_release_readiness.py tests\test_utf8_integrity.py -q --tb=short`; `docker compose build --no-cache app scheduler-worker ocr-worker`; `docker compose up -d --force-recreate app scheduler-worker ocr-worker`; `GET http://127.0.0.1:8080/api/pronto` | OK | Packaging/readiness/UTF-8 `14/14`; Docker reale locale rebuilt e healthy; `/api/pronto` risponde `versione=2.253.17`. |
| Browser reale visibile su `/fascicoli/2DE106E6/deposito/prepara` dopo rebuild `2.253.17` | OK | Pagina React visibile, sezione `Documenti da inviare`, pulsanti `Ripristina proposta` e `Seleziona tutti i documenti`; 39 checkbox sui dati locali correnti, toggle prima spunta `39 -> 38 -> 39`; nessun overflow orizzontale. Dato locale con documenti duplicati annotato per bonifica/deduplica, ma la selezione e' presente e funzionante. |

## Firma multipla deposito senza blocchi muti 2.253.18 - 2026-06-14

| Comando / verifica | Esito | Nota |
| --- | --- | --- |
| `pnpm --filter @iusentra/studio typecheck` | OK | TypeScript senza errori dopo la correzione del pannello firma multipla React. |
| `python -m pytest tests\test_regia_ui_react.py::test_ui_deposito_prepara_legge_intero_fascicolo_e_distingue_canale tests\test_deposito.py::test_api_pkcs11_firma_documenti_batch_usa_una_sola_sessione -q --tb=short` | OK | 2/2 passati: UI deposito presidia riavvio/PIN della firma multipla e il backend batch PKCS#11 mantiene una sola sessione PIN. |
| `pnpm --filter @iusentra/studio build` | OK | Build Vite completata con asset React rigenerati; resta solo warning storico sui chunk grandi. |
| `python scripts\react-migration\generate_api_contracts.py`; `python scripts\react-migration\generate_api_contracts.py --check`; `python scripts\validate_openapi.py docs\openapi.yaml`; `python scripts\verify_openapi_provider.py` | OK | Contratti API/OpenAPI riallineati a `2.253.18`; provider verification `auth-error=261`, `public-safe=15`, `success=29`, `backend-security=1`. |
| `python tools\sync_packaging_files.py --check`; `python -m pytest tests\test_packaging_consistency.py tests\test_release_readiness.py tests\test_utf8_integrity.py -q --tb=short` | OK | Packaging sincronizzato; release readiness e UTF-8 `14/14` passati. |
| `pnpm --filter @iusentra/studio test`; `python tools\check_repo_governance.py`; `python -m pytest tests\test_openapi_contracts_phase6.py -q --tb=short` | OK | Frontend governance, contratti React, design system, App V2, repo governance e OpenAPI fase 6 verdi. |

## Catalogo ministeriale deposito e documentazione operativa 2.253.19 - 2026-06-14

| Comando / verifica | Esito | Nota |
| --- | --- | --- |
| `python -m pytest tests\test_codici_oggetto_pst_catalog.py -q --tb=short` | OK | 6/6 passati; il nuovo guardrail scorre tutti i 1018 codici oggetto PST ufficiali e verifica normalizzazione codice puro e `codice - descrizione` verso il payload deposito. |
| `python -m pytest tests\test_practice_engine_profiles.py -q --tb=short` | OK | 8/8 passati; profili depositabili con canale risolto, alias canali e matrice non ridotta al solo PCT. |
| `python -m pytest tests\test_busta.py tests\test_deposito_server_dry_run_audit.py tests\test_codici_oggetto_pst_catalog.py tests\test_practice_engine_profiles.py tests\test_react_asset_retention.py tests\test_react_shell.py::test_react_fascicolo_import_quickorganizer_compila_dati_deposito tests\test_deposito_guidato.py::test_api_validazione_deposito_restituisce_semaforo_e_consente_con_warning tests\test_deposito_guidato.py::test_generazione_busta_usa_codice_oggetto_pst_validato tests\test_deposito_guidato.py::test_generazione_busta_blocca_se_selezione_video_non_coincide_con_busta tests\test_deposito_guidato.py::test_orchestratore_blocca_deposito_pct_senza_codice_oggetto_pst tests\test_regia_ui_react.py::test_ui_deposito_prepara_legge_intero_fascicolo_e_distingue_canale tests\test_polisweb.py::test_polisweb_qbuilder_documenti_e_profilo_usano_parametri_pst_live tests\test_polisweb.py::test_polisweb_parse_qbuilder_documenti_xml tests\test_polisweb.py::test_api_importa_documenti_portale_aggancia_file_al_deposito_ufficiale tests\test_polisweb.py::test_route_documenti_polisweb_consente_vista_completa_delle_buste tests\test_deposito.py::test_api_pkcs11_firma_documenti_batch_usa_una_sola_sessione -q --tb=short` | OK | 39/39 passati dopo runner server: busta, audit dry-run, indice, codice PST, selezione video/busta, asset React, portale e firma batch PKCS#11. |
| `python -m pytest tests\test_deposito_server_dry_run_audit.py tests\test_busta.py -q --tb=short`; `python -m py_compile scripts\audit_deposito_server_dry_run.py` | OK | 11/11 passati; audit dry-run distingue copia non crittografata coerente da invio reale non equivalente quando mancano `Atto.enc` AES256 e `DatiAtto.xml.p7m`. |
| `python -m pytest tests\test_deposito_server_dry_run_audit.py tests\test_regia_ui_react.py::test_ui_deposito_prepara_legge_intero_fascicolo_e_distingue_canale -q --tb=short`; `python -m py_compile scripts\server_deposito_dry_run_http.py scripts\audit_deposito_server_dry_run.py` | OK | 4/4 passati dopo runner HTTP server: selezione proposta da payload React, prove notifica incluse quando servono al deposito, comunicazione generica esclusa, script server/audit compilati. |
| `python scripts\audit_deposito_server_dry_run.py --package artifacts\react-migration\dry-run-audit\busta_*.enc --evidence <COPIA NON CRITTOGRAFATA ... Ricorso ...EML> --evidence <DEPOSITO TELEMATICO ... Documento richiesto ...eml>` | OK tecnico, blocco invio reale | Audit preparatorio locale su campioni utente: `ok_control_package=true`, `ok_real_transport=false`, differenze `DATI_ATTO_UNSIGNED` e `ATTO_ENC_AES256_MISSING`; nessuna PEC reale inviata. |
| `python -m pytest tests\test_regia_channels.py tests\test_telematic_registry_fail_closed.py tests\legal_deposit\test_penal_deposit_rules.py tests\legal_deposit\test_payment_policies.py -q --tb=short` | OK | 29/29 passati; canali telematici fail-closed, policy penale/PAT/PTT e pagamenti coerenti. |
| `pnpm --filter @iusentra/studio test`; `pnpm --filter @iusentra/studio build` | OK | Governance frontend e build Vite verdi; il build include `tsc --noEmit`; resta solo warning storico sui chunk grandi. |
| `node scripts\react-migration\check-route-gate.mjs`; `node frontend\scripts\check-react-contracts.mjs`; `python scripts\react-migration\generate_api_contracts.py --check` | OK | Route gate, contratti React e contratti API allineati. |
| `python scripts\react-migration\generate_api_contracts.py`; `python scripts\react-migration\generate_api_contracts.py --check`; `python scripts\validate_openapi.py docs\openapi.yaml`; `python scripts\verify_openapi_provider.py`; `python -m pytest tests\test_openapi_contracts_phase6.py -q --tb=short` | OK | OpenAPI rigenerato su `2.253.19`, provider verification `auth-error=261`, `public-safe=15`, `success=29`, `backend-security=1`; OpenAPI fase 6 `5/5`. |
...
## Deposito, Editor professionale e lettore documenti 2.253.30 - 2026-06-16

| Comando / verifica | Esito | Nota |
| --- | --- | --- |
| `python scripts\react-migration\generate_api_contracts.py`; `python scripts\react-migration\generate_api_contracts.py --check`; `python scripts\validate_openapi.py docs\openapi.yaml`; `python scripts\verify_openapi_provider.py` | OK | OpenAPI rigenerato e validato dopo nuova route `/editor-professionale` e servizi di anteprima allegati; provider verification `auth-error=262`, `public-safe=15`, `success=29`, `backend-security=1`. |
| `python tools\sync_packaging_files.py --check` | OK | Packaging sincronizzato a `2.253.30`. |
| `pnpm --filter @iusentra/studio typecheck` | OK | TypeScript senza errori dopo menu ruolo custom, route Editor professionale e code splitting Vite. |
| `node frontend\scripts\check-react-contracts.mjs`; `node scripts\react-migration\check-route-gate.mjs` | OK | Contratti React e route gate includono `/editor-professionale` come route governata e impediscono regressioni sulla shell. |
| `pnpm --filter @iusentra/studio test` | OK | Suite frontend passata: contratti, preset, governance design, App V2, assistente vocale, legal skills e copertura UI. |
| `python -m pytest tests\test_openapi_contracts_phase6.py -q --tb=short` | OK | 5/5 passati dopo riallineamento OpenAPI. |
| `python -m pytest tests\test_regia_api_payloads.py tests\test_regia_ui_react.py tests\test_react_shell.py::test_react_sidebar_contiene_navigazione_enterprise_completa tests\test_react_shell.py::test_nav_legacy_allineata_react_senza_nascondere_sidebar tests\test_react_shell.py::test_editor_professionale_resta_route_autonoma_distinta_da_redazione_atti tests\test_react_shell.py::test_react_blocco_finale_studio_admin_completo tests\test_react_shell.py::test_react_blocco_finale_route_reali_e_vista_classica tests\test_email_client.py::test_email_pec_visualizza_pdf_interno_da_allegato_pdf_p7m tests\test_email_client.py::test_email_ordinaria_visualizza_pdf_interno_da_allegato_pdf_p7m tests\test_email_client.py::test_email_pec_visualizza_xml_eml_txt_e_xml_p7m_senza_perdere_originale tests\test_email_client.py::test_email_ordinaria_visualizza_xml_eml_txt_e_xml_p7m_senza_perdere_originale tests\test_fascicolo_detail_ux.py::test_documenti_xml_p7m_eml_e_txt_si_visualizzano_e_si_eliminano tests\test_utf8_integrity.py tests\test_openapi_contracts_phase6.py -q --tb=short` | OK | 26/26 passati: deposito/regia, Editor professionale, anteprime PDF/P7M, XML/XML.P7M, EML e TXT per fascicoli, PEC ed email ordinaria, UTF-8 e OpenAPI. |
| `python -m pytest tests\test_react_asset_retention.py -q --tb=short` | OK | 2/2 passati dopo pulizia asset Vite: nessun asset non referenziato rimasto sotto `web/static/react/assets`. |
| `pnpm --filter @iusentra/studio build` | OK | Build Vite completata senza warning del chunk principale sopra 500 kB; chunk principali: `index` circa 202 kB, `vendor` circa 311 kB, `vendor-icons` circa 81 kB. |
| `git diff --check` | OK | Nessun errore whitespace; presenti solo warning CRLF su file già nel perimetro. |
| `python -m pytest tests\test_data_flow_contract.py -q --tb=short`; `python scripts\audit_data_flow_contract.py --registry data\tenants.json --json` | OK | Contratto dati/tenant/React/topbar verde sul tenant locale reale; `Editor professionale` incluso tra route e voci Studio governate, `quick_check=ok`, `moduli_json_records=3821`, nessuna riparazione eseguita. |
| `python tools\check_repo_governance.py`; `python -m py_compile web\bootstrap\fascicoli_document_routes.py web\services\signed_attachment_preview.py` | OK | Governance repo rientrata dopo rimozione dei rami morti `.eml`/`.txt`; sintassi Python confermata. |
...
## Profilo deposito SQL, canali telematici e UI busta 2.253.45 - 2026-06-17

| Comando / verifica | Esito | Nota |
| --- | --- | --- |
| `python -m pytest tests/test_profilo_deposito.py tests/test_canali_telematici_deposito.py tests/test_busta.py tests/test_simulazione_deposito.py tests/test_deposito_server_dry_run_audit.py tests/test_scheduler_registry.py -q --tb=short` | OK | Blocco mirato deposito/canali/busta/scheduler passato: profilo deposito SQL, PCT/PDP/PAT/PTT, cifratura PST, audit dry-run e scheduler restano coerenti. |
| `python -m py_compile pct/profilo_deposito.py pct/pst_cifratura.py scripts/precarica_certificati_cifratura_pst.py web/bootstrap/deposito_routes.py web/bootstrap/fascicoli_create_routes.py web/services/deposito_route_helpers.py web/services/local_pec_runtime.py web/services/react_fascicoli_bridge.py` | OK | Sintassi confermata per nuovi moduli profilo/certificati PST, route deposito/fascicoli e bridge React. |
| `python -m pytest tests/test_regia_ui_react.py::test_ui_deposito_prepara_legge_intero_fascicolo_e_distingue_canale -q --tb=short` | OK | Guardrail React deposito passato dopo la nuova fase busta con ufficio/PEC, prova senza invio e anteprima indice. |
| `pnpm --filter @iusentra/studio build` | OK | TypeScript e build Vite passati; bundle React rigenerato in `web/static/react`. |
| `python -m pytest tests/test_react_asset_retention.py -q --tb=short`; `python tools/sync_packaging_files.py --check`; `git diff --check` | OK | Retention asset React 2/2, packaging sincronizzato e whitespace senza errori; resta solo warning Git CRLF non bloccante su `FascicoliPage.tsx`. |
...
## Governance firma 2.253.60 - 2026-06-18

| Comando / verifica | Esito | Nota |
| --- | --- | --- |
| `python tools\check_repo_governance.py` | OK | `fascicoli_signature_routes.py` riportato a 633 righe, sotto il limite 650, spostando helper firma nel service senza cambio funzionale. |
| `python -m py_compile web\bootstrap\fascicoli_signature_routes.py web\services\fascicoli_signature_options.py`; `python -m pytest -q tests\test_fascicoli_signature_options.py tests\test_deposito.py::test_api_pkcs11_firma_documento_blocca_pades tests\test_deposito.py::test_firma_documento_ajax_rifiuta_p7m_non_valido tests\test_deposito.py::test_firma_documento_ajax_rifiuta_pdf_con_testo_firmato_ma_senza_pades --tb=short` | OK | 5/5 passati: messaggi CAdES/PAdES preservati dopo l'estrazione helper. |
| `python -m py_compile web\bootstrap\fascicoli_signature_routes.py web\services\fascicoli_signature_options.py pct\pst_cifratura.py web\bootstrap\deposito_routes.py web\bootstrap\deposito_legacy_send_routes.py web\bootstrap\admin_database_routes.py web\blueprints\api_v1_react.py`; `python -m pytest -q tests\test_fascicoli_signature_options.py tests\test_deposito.py::test_api_pkcs11_firma_documento_blocca_pades tests\test_deposito.py::test_firma_documento_ajax_rifiuta_p7m_non_valido tests\test_deposito.py::test_firma_documento_ajax_rifiuta_pdf_con_testo_firmato_ma_senza_pades tests\test_canali_telematici_deposito.py::test_cache_certificati_pst_normalizza_codici_senza_path_traversal --tb=short` | OK | Sintassi e 6/6 test mirati confermati sul perimetro CodeQL: firma, deposito, cache PST e messaggi pubblici. |
