...
| API contracts | API contract | tests/test_document_capture_contracts.py | file | provider verification copre campione; estendere schema response P0/P1 puntuali | censito |
...
| Agenda | API contract | tests/test_agenda.py | 403/RBAC, tenant, audit | provider verification copre campione; estendere schema response P0/P1 puntuali | censito |
...
| Backend domain | Backend | tests/test_collaudo_pagine_cancello.py | contratto | estendere solo se emerge una route/area non coperta dalla matrice | censito |
...
| Backend domain | Backend | tests/test_lessico_hunspell.py | feature flag | estendere solo se emerge una route/area non coperta dalla matrice | censito |
...
| Backend domain | Backend | tests/test_mediazione_resource_catalog.py | happy/edge path dominio | estendere solo se emerge una route/area non coperta dalla matrice | censito |
...
| Backend domain | Frontend/UI | tests/test_catalogo_profilo_parziale.py | happy/edge path dominio | nessun runner component/VRT dedicato; copertura via gate statici e browser smoke | censito |
...
| Backend domain | Frontend/UI | tests/test_dati_periodici_ufficiali.py | happy/edge path dominio | nessun runner component/VRT dedicato; copertura via gate statici e browser smoke | censito |
...
| Backend domain | Frontend/UI | tests/test_lettura_pdf.py | happy/edge path dominio | nessun runner component/VRT dedicato; copertura via gate statici e browser smoke | censito |
...
| Backend domain | Frontend/UI | tests/test_mobile_responsive_css.py | happy/edge path dominio | nessun runner component/VRT dedicato; copertura via gate statici e browser smoke | censito |
| Backend domain | Frontend/UI | tests/test_moduli_compilabili.py | happy/edge path dominio | nessun runner component/VRT dedicato; copertura via gate statici e browser smoke | censito |
| Backend domain | Frontend/UI | tests/test_moduli_compilazione_contratto.py | happy/edge path dominio | nessun runner component/VRT dedicato; copertura via gate statici e browser smoke | censito |
...
| Backend domain | Frontend/UI | tests/test_pdf_reader_text.py | happy/edge path dominio | nessun runner component/VRT dedicato; copertura via gate statici e browser smoke | censito |
...
| Backend domain | Frontend/UI | tests/test_pianificazioni_registro_e_codice.py | happy/edge path dominio | nessun runner component/VRT dedicato; copertura via gate statici e browser smoke | censito |
...
| Backend domain | Frontend/UI | tests/test_presidio_economico_letture_ripetute.py | happy/edge path dominio | nessun runner component/VRT dedicato; copertura via gate statici e browser smoke | censito |
...
| Backend domain | Frontend/UI | tests/test_sveglia_lettura_sentenze.py | happy/edge path dominio | nessun runner component/VRT dedicato; copertura via gate statici e browser smoke | censito |
...
| Backend domain | Frontend/UI | tests/test_tabelle_doppia_rappresentazione.py | happy/edge path dominio | nessun runner component/VRT dedicato; copertura via gate statici e browser smoke | censito |
...
| Backend domain | RBAC | tests/test_ptt_sigit_api.py | happy/edge path dominio | estendere solo se emerge una route/area non coperta dalla matrice | censito |
...
| Backend domain | RBAC | tests/test_revisione_2410_sicurezza.py | 401 anonimo, 403/RBAC, audit | estendere solo se emerge una route/area non coperta dalla matrice | censito |
...
| Backend domain | RBAC | tests/test_stato_fascicolo.py | happy/edge path dominio | estendere solo se emerge una route/area non coperta dalla matrice | censito |
...
| Backend domain | RBAC | tests/test_ufficio_amministrativo_e_profilo_pec.py | tenant | estendere solo se emerge una route/area non coperta dalla matrice | censito |
...
| Backend domain | RBAC | tests/test_verifica_fedelta.py | happy/edge path dominio | estendere solo se emerge una route/area non coperta dalla matrice | censito |
...
| Backend domain | Tenant isolation | tests/test_daily_plan_eventi_unici.py | tenant, audit | estendere solo se emerge una route/area non coperta dalla matrice | censito |
...
| Backend domain | Tenant isolation | tests/test_mediazione_offices_directory.py | 401 anonimo, tenant | estendere solo se emerge una route/area non coperta dalla matrice | censito |
| Backend domain | Tenant isolation | tests/test_mediazione_source_refresh.py | tenant, contratto, audit | estendere solo se emerge una route/area non coperta dalla matrice | censito |
...
| Backend domain | Tenant isolation | tests/test_provenienza_ai.py | tenant, contratto | estendere solo se emerge una route/area non coperta dalla matrice | censito |
...
| File/document security | Frontend/UI | tests/test_tabelle_documento_fedele.py | happy/edge path dominio | nessun runner component/VRT dedicato; copertura via gate statici e browser smoke | censito |
...
| Frontend React | API contract | tests/test_template_editor_acquisition_contract.py | file | provider verification copre campione; estendere schema response P0/P1 puntuali | censito |
...
| Frontend React | Tenant isolation | tests/test_react_shell.py | 401 anonimo, 403/RBAC, tenant, feature flag, contratto, file, audit | estendere solo se emerge una route/area non coperta dalla matrice | censito |
...
| Lex/Ricerca | Backend | tests/test_revisione_2410_lex.py | happy/edge path dominio | estendere solo se emerge una route/area non coperta dalla matrice | censito |
...
| Telematico | RBAC | tests/test_deposito_workflow.py | happy/edge path dominio | estendere solo se emerge una route/area non coperta dalla matrice | censito |
