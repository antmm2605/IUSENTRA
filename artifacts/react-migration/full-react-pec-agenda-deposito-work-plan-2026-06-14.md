...
| Docker/browser reale deposito `2.253.17` | completata | Dopo refactor governance e rebuild no-cache, `/api/pronto` risponde `versione=2.253.17`; browser visibile su `/fascicoli/2DE106E6/deposito/prepara` conferma React, sezione `Documenti da inviare`, pulsanti `Ripristina proposta` e `Seleziona tutti i documenti`, 39 checkbox nei dati locali correnti, toggle `39 -> 38 -> 39`, zero overflow orizzontale. Il numero alto deriva da duplicati gia' presenti nel fascicolo locale e resta da presidiare come bonifica dati/regole deduplica, non come assenza della selezione. |
...
| Report, bump versione, commit, push, deploy Hetzner | in corso | Bump `2.253.17` dopo refactor governance `deposito_routes.py` sotto 1000 righe; gate tecnici e verifica visiva selezione documenti passati. Restano firma multipla con PIN utente, nuovo push branch gemelli, check GitHub/CodeQL e deploy Hetzner sul commit finale. |
