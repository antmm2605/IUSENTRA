...
## Vista economica senza colonna Totale 2.253.133 - 2026-06-28

- Segnalazione utente: la colonna `Totale` non aggiungeva funzione operativa e sottraeva spazio alla matrice, mentre il pulsante `Modifica controllo economico` era ancora troppo dentro la colonna economica e rendeva difficile leggere ogni fascicolo come blocco unico.
- Correzione React: la tabella economica desktop ora ha solo sei colonne: selezione, `Rif.`, `Cliente`, `Prossima scad.`, `Stato`, `Controllo economico`. Il totale non è più una colonna autonoma: resta come riepilogo inline dentro `Controllo economico`, legato alla stessa riga e senza spingere i campi nel cliente successivo.
- Il pulsante `Modifica controllo economico` è stato spostato sotto i dati del cliente, nella stessa cella `Cliente`, così l'azione principale è vicina a cliente e scadenza come richiesto. L'editor dei quattro campi economici resta in riga dedicata sotto il fascicolo, quindi non può sovrapporsi alla riga successiva.
- Le larghezze della tabella sono state riallineate per lasciare spazio reale a `Prossima scad.` e alla matrice economica; la colonna `Controllo economico` contiene `Contributo`, `Spese/esborsi`, `Liquidazione giudice`, `Parcella` e il totale inline.
- Prova reale locale già eseguita su Docker `127.0.0.1:8080`, versione `2.253.133`: header senza `Totale`, `buttonsInsideClient=7`, `buttonsInsideMatrix=0`, `totalInlineCount=7`, `summaryCount=28`, nessun overflow desktop/tablet/mobile e apertura editor con `overlapsNextRow=false`.
- La patch non modifica parser sentenze, pipeline PEC/OCR, job incrementali, repository economici, importi, proforme o dati dei fascicoli: è una correzione mirata di layout e componentizzazione React della vista economica.

## Vista economica produzione 2.253.133 - 2026-06-28

- Commit distribuito su Hetzner: `46f4551a9d025575a3c224a1b5360c54be2ff71e`; `/api/pronto` produzione `ok=true`, `versione=2.253.133`, timestamp `2026-06-28T11:37:43+02:00`, `timezone=Europe/Rome`.
- GitHub Actions sullo SHA: `184` check-run completati, zero failure o pending; `CodeQL`, `CI`, `CI Required Gates`, `Frontend React CI`, `Security Supply Chain`, `CI Quality Overlay` e `Deploy Hetzner CPX42` verdi. Nel deploy il passo `Backup preventivo` risulta `skipped` grazie a `[no-backup]`.
- Server Hetzner: repository su commit `46f4551a9d025575a3c224a1b5360c54be2ff71e`; container `iusentra-app-1`, `iusentra-scheduler-worker-1`, `iusentra-ocr-worker-1`, `iusentra-redis-1`, `iusentra-audit-postgres-1` e `iusentra-audit-worm-1` healthy; `docker builder prune --all --force` eseguito e `/opt/iusentra/tmp-backup-snapshot` assente.
- Prova visiva produzione su `https://app.iusentra.it/fascicoli?vista=economica`, login `admin`, viewport desktop `1440x900`: header a sei colonne `Rif. / Cliente / Prossima scad. / Stato / Controllo economico`, senza colonna `Totale`; `buttonsInsideClient=25`, `buttonsInsideMatrix=0`, `totalInlineCount=25`, `summaryCount=100`, `wrapScrollWidth=wrapClientWidth=1091`, `bodyScrollWidth=bodyClientWidth=1425`, `overlapsAny=false`.
- Click reale su `Modifica controllo economico`: il pulsante è sotto la cella cliente, passa a `Chiudi modifica`, l'editor dedicato si apre sotto la riga con `editorFitsWrap=true`, `overlapsNextRow=false`, `formCount=4`, `selectCount=4`, `inputCount=16`, console senza errori.
- Responsive produzione: tablet `768x1024` e mobile `390x844` senza overflow orizzontale (`753/753` e `375/375`); la tabella desktop è nascosta, le card mobile sono `25` e la prima card mostra `Totale registrato`, `Contributo unificato`, `Spese/esborsi`, `Liquidazione giudice` e `Parcella` dentro lo stesso fascicolo.

## Vista economica senza totale inline e editor leggibile 2.253.134 - 2026-06-28

- Segnalazione utente: dopo la rimozione della colonna `Totale`, il riepilogo `Totale registrato` rimasto dentro `Controllo economico` non aveva utilità operativa e occupava spazio; in più l'editor economico aperto al centro della pagina richiedeva o suggeriva scroll orizzontale per vedere tutti i campi.
- Correzione React: eliminato il componente `EconomicTotalSummary` e rimosso `Totale registrato` anche dalle card mobile. La matrice economica mostra solo `Contributo unificato`, `Spese/esborsi`, `Liquidazione giudice` e `Parcella`, senza una quinta voce di riepilogo.
- Correzione layout: la cella `iu-fas-economic-matrix` torna `display: table-cell`, così il contenuto economico partecipa al calcolo altezza della riga e non invade il cliente successivo. L'editor economico usa due colonne e `width: min(100%, calc(100vw - 400px))`, lasciando margine reale a destra anche nel breakpoint desktop stretto.
- Prova produzione server-first su `https://app.iusentra.it/fascicoli?vista=economica`, versione `2.253.134`: container `app`, `scheduler-worker` e `ocr-worker` healthy; `/api/pronto` `ok=true`, timezone `Europe/Rome`.
- Prova visiva produzione con browser integrato: desktop `1110x800` con editor aperto, `editor.width=710`, `marginRight=103`, colonne `337px 337px`, `descendantOverflow=0`, righe con `contentInside=true`, `hasTotalRegistrato=false`, console senza errori. Desktop `1280x900`: `editor.width=880`, colonne `422px 422px`, `marginRight=103`, nessun overflow interno.
- Responsive produzione: mobile `390x800`, tabella nascosta, `25` card visibili, `bodyHorizontalOverflow=false`, `hasTotalRegistrato=false`, nessun overflow nelle prime card economiche.
- Gate applicativi: `pnpm --filter @iusentra/studio build` verde; `python -m pytest tests/test_react_shell.py::test_react_fascicoli_suite_completa_route_componenti_e_lex -q` verde. `python tools/codex_harness/run_codex_quality_gate.py --mode ui-support` è stato eseguito e ha fallito per scope perché il task modifica file prodotto React/versione/statici, non per un errore della vista.
- Prova locale reale su Docker `127.0.0.1:8080`, versione `2.253.134`: `app`, `scheduler-worker` e `ocr-worker` healthy; `/api/pronto` `ok=true`, timezone `Europe/Rome`. Desktop locale `1110x800` con editor aperto: `width=710`, `marginRight=103`, colonne `337px 337px`, `descendantOverflow=0`, `hasTotalRegistrato=false`, righe con `contentInside=true`, console senza errori. Mobile locale `390x800`: `7` card, tabella nascosta, `bodyHorizontalOverflow=false`, `hasTotalRegistrato=false`, console senza errori.
- Deploy ufficiale GitHub/Hetzner completato sul commit `206d139c0db31c8ffde0fd512c3f9c08265f5133`: `184` check-run `success`, server sullo stesso commit, container healthy, `/api/pronto` produzione `versione=2.253.134`. Prova visiva post-deploy ripetuta su produzione `1110x800`: asset `FascicoliPage-DKKc8Jdi.css`, editor `width=710`, `marginRight=103`, colonne `337px 337px`, `descendantOverflow=0`, `hasTotalRegistrato=false`, console senza errori; `docker builder prune --all --force` eseguito e `/opt/iusentra/tmp-backup-snapshot` assente.
