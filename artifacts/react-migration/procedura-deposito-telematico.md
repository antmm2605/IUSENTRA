...
Correzione post-prova produzione 05/08/2026, versione `2.276.7`: il fascicolo locale `31541BDA` non era presente nel tenant produzione, quindi il click su `Aggiungi controparte selezionata` ha ricevuto una pagina HTML 404. La route POST `/fascicoli/<id>/parti/aggiungi` ora risponde JSON `Fascicolo non trovato.` per richieste React/XHR e l'API `/api/v1/ui/fascicoli/<id>/modifica` non apre più un form vuoto quando l'id non esiste nel repository tenant. Sono stati aggiunti guardrail mirati per evitare che HTML 404 rientri nella card del form.

Modello dati parti fascicolo: se la controparte è già censita, il form React usa `id_soggetto_controparte` e la route POST `/fascicoli/<id>/parti/aggiungi` collega il soggetto esistente al fascicolo con ruolo `CONTROPARTE`, senza creare duplicati anagrafici. Il collegamento viene letto dal repository soggetti tramite `parti_fascicolo(id_fascicolo)` e torna nel payload React come `linkedSubjects`, quindi la sezione `Parti già collegate al fascicolo` mostra il dato salvato e il pulsante resta `Già collegata` quando la relazione esiste già. Se la controparte non è ancora censita, il checkbox `Crea anche la scheda soggetto della controparte` o il percorso `/soggetti/nuovo?...id_fascicolo=...` creano la scheda soggetto nello stesso archivio tenant e la collegano al fascicolo, rendendola riutilizzabile negli altri fascicoli dello studio.

Correzione post-prova produzione 05/08/2026, versione `2.276.8`: su `https://app.iusentra.it/fascicoli/31541BDA/modifica` la route API rispondeva correttamente JSON 404, ma il form React normalizzava il 404 in dati di fallback e mostrava campi editabili vuoti. Il normalizzatore del form ora conserva `notFound/requestError` anche per i 401/403/404 e `FascicoloFormPage` mostra l'empty state `Fascicolo non trovato` prima di renderizzare `JsonPostForm`. Guardrail statico aggiunto per impedire il ritorno del form vuoto sui fascicoli mancanti.
...
La finestra PagoPA del fascicolo apre direttamente la pagina PST di nuova richiesta (`pagopa_nuovarich.wp`) tramite la proxy interna già presidiata. La modale può essere portata a tutto schermo e conserva la memoria del calcolo del contributo con totale, RG, cliente e oggetto del ricorso. Quando il portale espone campi compilabili nella stessa finestra, IUSENTRA prova a inserire soltanto campi vuoti compatibili con importo, oggetto/causale, riferimento RG/fascicolo e cliente; non invia pagamenti, non conferma richieste e non sovrascrive campi già compilati dall'utente.

Stato anti-regressione: la compilazione PagoPA resta assistita e deve essere controllata dall'avvocato nel portale. La prova di pagamento continua a essere solo ricevuta PagoPA/F23/F24, documento di esenzione o evidenza equivalente collegata al fascicolo; la memoria del calcolo è un supporto operativo e non registra pagamenti né sostituisce il controllo economico o il presidio documenti.

Aggiornamento correttivo 2026-07-27: il presidio economico del fascicolo non legge più soltanto `fascicolo.documenti`. Per evitare falsi `Non trovato`, la scansione del contributo unificato fonde i documenti locali con i record Document AI già indicizzati sul server per lo stesso tenant e fascicolo, deduplicati per id, hash e nome. Le note storiche che parlavano solo di `documenti correnti` vengono rigenerate quando l'analisi rileva ancora il contributo da registrare, così il pannello mostra il perimetro reale `documenti del fascicolo + documenti indicizzati in archivio centrale`. Il pannello `Modifica controllo economico` dentro la finestra contestuale monta ora l'editor economico reale, con stato, importo, data, dettagli, proforma e salvataggi tramite API esistenti; quando il presidio richiede conferma degli importi espone `Proforma da preparare` e il messaggio `Importo o fonte economica letta dal fascicolo: verifica se emettere la proforma.`. La board operativa del dettaglio non ripete più la stessa prossima azione in più punti e usa il presidio economico come contesto per ricevuta, esenzione o contributo da verificare.

Guardrail locali eseguiti: `python -m pytest tests\test_react_shell.py -k "fascicoli_economia" -q`, `python -m pytest tests\test_react_shell.py::test_react_fascicoli_suite_completa_route_componenti_e_lex tests\test_regia_ui_react.py::test_ui_fascicolo_menu_contestuale_azioni_reali tests\test_fascicolo_operational_presidio.py -q`, `npm --prefix frontend run typecheck`, `npm --prefix frontend run build`, `python scripts\validate_openapi.py docs\openapi.yaml`, `python -m py_compile web\services\react_fascicoli_bridge.py pct\presidio_documentale_state.py`.
...
- Richieste UNEP, esecuzioni, 492-bis e pagamenti collegati.
- Il software prepara richiesta, allegati, pagamenti se dovuti e ricevute portale/UNEP.

### Canali esterni al PCT

Le procedure esterne al PCT sono documentate nei dossier dedicati e non governano la busta di deposito.
...
- sorgenti decompilati in `%TEMP%/quickorganizer_decompiled_full`, in particolare `FormMain.cs`, `WizardImportaPraticheDaPolisWeb.cs`, `PCT.cs`, `BrowserForm.cs`, `Common.cs` e `UfficioRegistroRuolo.cs`;
...
### Aggiornamento 27/07/2026 - Fascicoli: caricamento lista e scadenze operative

Difetto segnalato su produzione `https://app.iusentra.it/fascicoli`: la pagina restava ferma su `Sincronizzazione fascicoli...` e il presidio operativo non riportava più le scadenze aperte già scadute, incluse le voci del `15/05/2026` `Presidio ricevute PEC da completare` e `Valuta risposta a diffida`.
...
- la prima correzione ha riportato in evidenza le scadenze aperte già scadute, ma le ha esposte con etichetta `Scadenze 7g` / `Scadenze entro 7 giorni`, semanticamente errata al `27/07/2026` per date come `15/05/2026`.
...
- la lista operativa mantiene le scadenze aperte scadute e quelle dei prossimi 7 giorni come `Scadenze urgenti`, senza perdere presidi non collegati.

Prove eseguite:

- test mirati prima correzione: `python -m pytest tests/test_fascicoli_pagination.py::test_fascicoli_scadenze7_conta_aperte_scadute_non_collegate tests/test_fascicoli_pagination.py::test_fascicoli_vista_operativa_non_legge_document_ai_server_in_lista tests/test_fascicoli_pagination.py::test_fascicoli_vista_economica_legge_dato_consolidato_senza_presidio_massivo tests/test_fascicoli_pagination.py::test_importo_parcella_lista_usa_aliquota_cassa_in_cache -q`;
...
- controllo visuale nel browser integrato su `https://app.iusentra.it/fascicoli`: pagina renderizzata con il presidio operativo delle scadenze aperte, inclusi `Presidio ricevute PEC da completare` e `Valuta risposta a diffida` del `15/05/2026`;
...
### Aggiornamento 27/07/2026 - Fascicoli: distinzione scadute / entro 7 giorni

Correzione applicata dopo il controllo semantico richiesto dall'utente: al `27/07/2026` non è corretto classificare date del `15/05/2026` o `24/05/2026` come `entro 7 giorni`. Il payload e la UI separano ora:

- `summary.deadlines7`: solo scadenze aperte con data da oggi ai successivi 7 giorni;
- `summary.overdueDeadlines`: scadenze aperte già scadute;
- `summary.urgentDeadlines`: totale operativo scadute + prossimi 7 giorni;
- `deadlines`: lista urgente mostrata nella pagina, ancora comprensiva delle scadenze scadute perché richiedono presidio.

La pagina React sostituisce la card `Scadenze 7g` con `Scadenze urgenti` e mostra la nota con il dettaglio `scadute` / `entro 7 giorni`. Il titolo del riquadro diventa `Scadenze scadute`, `Scadenze entro 7 giorni` oppure `Scadenze scadute e prossimi 7 giorni` in base ai dati reali.

Guardrail eseguiti:

- `python -m pytest tests/test_fascicoli_pagination.py::test_fascicoli_scadenze_urgenti_distingue_scadute_da_entro_7_giorni -q`;
...
- Correzione CI 05/08/2026 versione 2.276.11: il guardrail `test_api_react_fascicoli_usa_loader_runtime_condiviso` non usa più un controllo a sottostringa che bloccava anche `get_fascicoli_loader`; riprodotto localmente lo shard `Pytest core fase 8/10 OCR parte 1/3` con esito verde.

## Aggiornamento 11/08/2026 - Deposito B494AAB9: confronto esito positivo e correzione busta

Perimetro: solo deposito telematico del fascicolo `B494AAB9`; notifiche legali e relata L. 53 restano fuori da questa procedura.

Riferimento positivo controllato:

- EML locale `C:\Users\antmm\Downloads\pec_00119fb0a3713fdb69faaf7d.eml`;
- data `30/06/2026 08:44:07`;
- oggetto `ACCETTAZIONE DEPOSITO TELEMATICO - RICORSO - Tribunale di Vicenza`;
- `IDBUSTA: 152649431`;
- `CodiceEsito: 2`;
- `NumeroRuolo: 1084/2026`;
- esito `Accettazione manuale avvenuta con successo`.
...
- EML esito `C:\Users\antmm\Downloads\ESITO CONTROLLI AUTOMATICI DEPOSITO TELEMATICO - RICORSO - Tribunale di Vicenza.eml`;
- `IDBUSTA: 154386629`;
- `CodiceEsito: -1`;
...
- la PEC di consegna odierna contiene `Atto.enc` da `8.191.901` byte con struttura CMS/PKCS#7 `EnvelopedData`;
- l'EML positivo è una ricevuta/esito, non contiene la busta originaria accettata da confrontare byte per byte;
- non è stato trovato nel filesystem locale il `DatiAtto.xml.p7m` sciolto del deposito odierno.

Correzioni applicate:

- il risolutore del contributo unificato usa i documenti selezionati nella busta: per Carta docente/lavoro pubblico contro MIM riconosce `autocertificazione reddituale.pdf` come esenzione, non scambia le richieste di pagamento Carta docente per ricevute del contributo e riconosce invece una vera ricevuta PagoPA/contributo quando presente;
- `DatiAtto.xml` firmato tramite PKCS#11/Local Signer include ora gli attributi CAdES-BES `content-type`, `message-digest`, `signing-time` e `signing-certificate-v2`;
- il Local Signer è stato portato a `1.6.105` e i pacchetti versionati Windows/macOS/Linux sono stati rigenerati.
...
## Aggiornamento 13/08/2026 - Controllo tra fascicolo aperto e destinazione reale

- Il flusso non assume più che la voce scelta nel menu sia sufficiente: prima della busta confronta i dati del fascicolo con le tabelle reali estratte da Studio Telematico.
