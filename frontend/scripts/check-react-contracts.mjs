...
const reactEntry = read('src/reactEntry.tsx')
...
const dailyPlanPage = read('src/pages/daily-plan/OggiPage.tsx')
const dailyPlanApi = read('src/pages/daily-plan/api.ts')
const dailyPlanDetail = read('src/pages/daily-plan/ItemDetailPanel.tsx')
const dailyPlanDateControls = read('src/pages/daily-plan/DailyPlanDateControls.tsx')
const dailyPlanPageParts = read('src/pages/daily-plan/DailyPlanPageParts.tsx')
const dailyPlanItemCard = read('src/pages/daily-plan/ItemCard.tsx')
const dailyPlanSourceWorkspace = read('src/pages/daily-plan/SourceWorkspace.tsx')
const dailyPlanSourceViewer = read('src/pages/daily-plan/SourceViewer.tsx')
...
const dashboardHook = read('src/hooks/useDashboardData.ts')
...
const fascicoliCss = read('src/components/FascicoliPage.css')
...
const fascicoloDocumentPresidio = read('../pct/fascicolo_document_presidio.py')
const fascicoloOperationalPresidio = read('../pct/fascicolo_operational_presidio.py')
const praticheCollegateCatalog = read('../pct/data/pratiche_collegate_catalog.json')
const codiciOggettoPstCatalog = read('../pct/data/cataloghi/codici_oggetto_pst.json')
const praticheCollegateTs = read('src/data/praticheCollegateCatalog.ts')
...
const apiBridgeNormalized = apiBridge.replace(/\r\n/g, '\n')
...
const iusentraPreset = read('src/components/iusentra/IusentraPreset.tsx')
...
assertContains(runSafeReactMigration, 'pnpm --filter @iusentra/studio test', 'runner esegue test frontend')
assertContains(runSafeReactMigration, 'pnpm --filter @iusentra/studio typecheck', 'runner esegue typecheck frontend')
assertContains(runSafeReactMigration, 'pnpm --filter @iusentra/studio build', 'runner esegue build frontend')
...
const allowedGovernedUnlocks = new Set(['/', '/admin/database', '/agenda', '/agenda/importa', '/agenda/nuovo', '/amministrazione', '/app/portale-clienti', '/audit', '/backup', '/cartelle-condivise', '/clienti', '/clienti/:id/cartella', '/clienti/:id/collaboratori', '/clienti/nuovo', '/compensi-forensi', '/deposito/checklist', '/documenti', '/email', '/email-ordinaria', '/notifiche-legali', '/fascicoli', '/fascicoli/archivio', '/fascicoli/nuovo', '/fascicoli/:id/deposito/prepara', '/fatturazione', '/fatturazione/nuova', '/giurisprudenza', '/giurisprudenza/nuova', '/global-search', '/importa-pratiche', '/importa-pratiche-studio-telematico', '/impostazioni', '/impostazioni-studio', '/impostazioni/calendario', '/impostazioni/pagamenti', '/impostazioni/sdi', '/incassi-pagamenti', '/legal-intelligence', '/legal-intelligence/mediazione', '/legal-intelligence/news', '/legal-skills', '/messaggi', '/messaggi/nuovo', '/notifiche', '/notifiche-whatsapp', '/oggi', '/portale-cliente', '/preventivi', '/preventivi/conferimento/:id', '/preventivi/conferimento/nuovo', '/preventivi/nuovo', '/preventivi/wizard', '/privacy/registro', '/privacy/registro/nuovo', '/procedure-completion', '/profilo', '/profili', '/redazione-atti', '/regia-operativa', '/registro-attivita', '/registro-gdpr', '/ricerca-legale', '/ricerca-studio', '/scadenziario', '/scadenziario/nuova', '/scadenziario/:id', '/scadenziario/:id/modifica', '/sincronizzazione-calendari', '/sito-studio', '/sito-studio/articoli/:id/modifica', '/sito-studio/builder', '/sito-studio/contatti', '/sito-studio/redazione-ai', '/soggetti', '/soggetti/nuovo', '/statistiche', '/strumenti-legali', '/strumenti-operativi', '/studio', '/tariffario', '/template-atti', '/template-atti/catalogo', '/timesheet', '/utenti', '/utenti/nuovo', '/wizard-pro', '/workflow-agents', '/workspace-intelligente'])
...
const governedExpectedStatuses = new Map([
  ...
  ['/clienti/:id/cartella', 'react_operational_full'],
  ['/clienti/:id/collaboratori', 'react_operational_full'],
  ...
  ['/fascicoli/:id/deposito/prepara', 'react_operational_full'],
  ...
  ['/importa-pratiche', 'react_operational_full'],
  ['/importa-pratiche-studio-telematico', 'react_operational_full'],
  ...
  ['/impostazioni/sdi', 'react_operational_full'],
  ['/impostazioni/sdi', 'react_operational_full'],
  ...
  ['/procedure-completion', 'react_operational_full'],
...
for (const route of governedTelematicoGraphicalRoutes) governedExpectedStatuses.set(route, 'react_operational_full')
assertContains(dailyPlanPage, 'role="status"', 'Oggi espone il feedback aggiornamento')
assertContains(dailyPlanPage, 'window.setTimeout(poll, 5000)', 'Oggi riallinea il piano richiesto')
assertContains(dailyPlanPage, 'Piano aggiornato con le attività disponibili.', 'Oggi conferma il piano aggiornato')
assertContains(dailyPlanPage, 'fetchStatoAggiornamento', 'Oggi legge lo stato reale del job di aggiornamento')
assertContains(dailyPlanPage, "stato === 'done'", 'Oggi aggiorna i dati solo dopo il completamento del job')
assertContains(dailyPlanPage, "stato === 'running'", 'Oggi comunica l’elaborazione in corso')
assertContains(dailyPlanPage, "stato === 'failed'", 'Oggi comunica il fallimento reale del job')
assertContains(dailyPlanPage, 'Non ho ancora ricevuto l’esito dell’elaborazione.', 'Oggi distingue il timeout da un esito positivo')
assertContains(dailyPlanPage, '05:30, ora italiana', 'Oggi spiega la generazione automatica del piano')
assertContains(dailyPlanPage, 'formatTimeIt', 'Oggi formatta gli orari in Europe/Rome')
assertNotContains(dailyPlanPage, 'evento.data_ora.slice', 'Oggi non espone orari ISO tagliati')
assertContains(dailyPlanApi, 'avvio_immediato_richiesto?: boolean', 'API Oggi tipizza il dispatch immediato')
assertContains(dailyPlanApi, '/daily-plan/jobs/', 'API Oggi richiede lo stato del job')
assertContains(dailyPlanApi, 'run_id?: string', 'API Oggi conserva l’identificativo del dispatch')
assertContains(dailyPlanApi, 'crypto.randomUUID()', 'Oggi usa una chiave distinta per ogni refresh esplicito')
assertContains(dailyPlanApi, 'date: targetDate', 'Oggi invia al job la data selezionata')
assertContains(dailyPlanApi, 'pianoCache = new Map', 'Oggi separa la cache ETag per data e utente')
assertContains(dailyPlanDateControls, "label: 'Domani'", 'Piano del giorno espone Domani')
assertContains(dailyPlanDateControls, "label: 'Dopodomani'", 'Piano del giorno espone Dopodomani')
assertContains(dailyPlanDateControls, 'type="date"', 'Piano del giorno permette una data libera')
assertContains(dailyPlanDateControls, 'new FormData(event.currentTarget)', 'Piano del giorno conferma la data dal form nativo')
assertContains(dailyPlanDateControls, 'aria-label="Apri data selezionata"', 'Piano del giorno espone il comando data accessibile')
assertContains(dailyPlanPageParts, 'sequence={false}', 'Piano del giorno mantiene i titoli sopra i contenuti locali')
assertContains(dailyPlanDetail, 'role="alert"', 'dettaglio Oggi espone gli errori delle evidenze')
assertContains(dailyPlanDetail, '> Riprova', 'dettaglio Oggi permette di ricaricare le evidenze')
assertContains(dailyPlanItemCard, 'onClick={() => onOpenSource(item)}', 'Oggi: Apri apre la fonte sopra la pagina, non il fascicolo')
assertContains(dailyPlanPage, '<SourceWorkspace', 'Oggi monta il lettore della fonte sopra il piano')
assertContains(dailyPlanApi, '/fonti`', 'API Oggi risolve le fonti puntuali dell’attività')
assertContains(dailyPlanSourceViewer, '<SourceDocumentReader', 'Oggi usa il lettore interno unico delle fonti')
assertContains(dailyPlanSourceWorkspace, "event.key === 'ArrowRight'", 'Oggi permette di scorrere le attività dal lettore')
...
for (const [route, status] of [
  ...
  ['/polisWeb', 'react_operational_full'],
  ['/pdp', 'react_operational_full'],
  ['/pat', 'react_operational_full'],
  ['/sigit', 'react_operational_full'],
  ['/tribunali', 'react_operational_full'],
  ['/guida/firma-digitale', 'react_operational_full'],
  ['/portali/pst/acquisizione', 'react_operational_full'],
...
assertContains(iusentraPreset, 'IusentraPageShell', 'preset globale espone PageShell')
assertContains(iusentraPreset, 'IusentraMainArea', 'preset globale espone MainArea')
assertContains(iusentraPreset, 'IusentraMainSurface', 'preset globale espone MainSurface')
assertContains(iusentraPreset, 'IusentraSupportRail', 'preset globale espone SupportRail')
assertContains(iusentraPreset, 'IusentraDataSurface', 'preset globale espone DataSurface')
assertContains(iusentraPreset, 'IusentraFiltersBar', 'preset globale espone FiltersBar')
assertContains(iusentraPreset, 'IusentraContextFilters', 'preset globale espone ContextFilters')
...
assertContains(iusentraPreset, "classifySequenceSlot(child) ?? 'main-content'", 'preset globale manda ogni blocco non riconosciuto nel contenuto principale')
assertContains(iusentraPreset, '/(?:tabs|tablist|switcher)$/', 'preset globale classifica tab e switcher come filtri')
assertContains(iusentraPreset, '/(?:note|summary)$/', 'preset globale classifica note e riepiloghi come contesto')
...
for (const routeLayoutSelector of [
  ...
  assertContains(iusentraPreset, routeLayoutSelector, `preset globale registra ${routeLayoutSelector}`)
}
...
assertNotContains(mainEntrypoint, "import React from 'react'", 'entrypoint React leggero senza import React statico')
...
assertNotContains(reactEntry, "await import('./components/SupportOperatorRoom')", 'reactEntry non usa import dinamico supporto che reimporta index')
...
assertContains(mainEntrypoint, 'window.__IUSENTRA_INLINE_REACT_ENTRY__ === true', 'entrypoint React parte quando la shell lo carica inline')
assertContains(mainEntrypoint, 'if (shouldRunBootstrap)', 'entrypoint React non riesegue il bootstrap quando importato come helper Vite')
assertContains(mainEntrypoint, 'document.documentElement.dataset.iusentraEntryRuntime', 'entrypoint React registra gli errori runtime del bootstrap')
assertContains(reactShellBlueprint, 'def _global_manifest_css', 'shell React include CSS globale prodotto da Vite')
assertContains(reactShellBlueprint, 'manifest.get("style.css")', 'shell React aggancia style.css quando cssCodeSplit=false')
...
assertNotContains(notificheLegali, 'Deposito prova notifica', 'NotificheLegaliPage senza prova deposito')
assertContains(notificheLegali, 'Comunica al cliente', 'NotificheLegaliPage separa comunicazione cliente')
assertContains(notificheLegali, 'Anteprima modello relata', 'NotificheLegaliPage mostra anteprima modelli relata')
assertContains(notificheLegali, 'Anteprima compilata', 'NotificheLegaliPage mostra relata compilata')
assertContains(notificheLegali, 'Modifica bozza relata', 'NotificheLegaliPage consente bozza relata modificabile')
assertContains(notificheLegali, 'Salva bozza per questa notifica', 'NotificheLegaliPage salva bozza distinta dal modello')
assertContains(notificheLegali, 'Modelli comunicazione cliente', 'NotificheLegaliPage separa modelli comunicazione cliente')
assertContains(notificheLegali, 'Nuovo modello su misura', 'NotificheLegaliPage consente nuovi modelli relata')
assertContains(notificheLegali, 'Campi automatici disponibili', 'NotificheLegaliPage espone campi IUSENTRA inseribili')
assertNotContains(notificheLegali, 'Compilazione da pratica IUSENTRA', 'NotificheLegaliPage senza blocco deposito precompilato')
...
assertNotContains(notificheLegali, 'Ricevuta di consegna completa', 'NotificheLegaliPage senza ricevuta successiva all invio')
...
assertContains(notificheLegaliData, 'saveLegalRelataDraft', 'NotificheLegaliData salva bozze relata')
assertContains(notificheLegaliData, 'modelliComunicazioneCliente', 'NotificheLegaliData normalizza modelli cliente separati')
assertContains(notificheLegaliData, 'campiDisponibili', 'NotificheLegaliData normalizza campi automatici')
assertContains(notificheLegaliData, '/api/v1/ui/notifiche-legali/notifica', 'NotificheLegaliData endpoint notifica')
assertContains(notificheLegaliData, '/api/v1/ui/notifiche-legali/anteprima-relata', 'NotificheLegaliData endpoint anteprima relata')
assertContains(notificheLegaliData, '/api/v1/ui/notifiche-legali/bozze-relata', 'NotificheLegaliData endpoint bozze relata')
...
assertNotContains(notificheLegaliData, '/api/v1/ui/notifiche-legali/prova-deposito', 'NotificheLegaliData senza endpoint prova deposito')
assertContains(notificheLegaliData, '/api/v1/ui/notifiche-legali/modelli-relata', 'NotificheLegaliData endpoint modelli relata')
assertContains(notificheLegaliBridge, 'LEGAL_NOTIFICATION_SUBJECT', 'bridge notifiche legali oggetto obbligatorio')
assertNotContains(notificheLegaliBridge, 'depositProofWithOriginalReceipts', 'bridge notifiche legali senza prova deposito')
...
assertContains(apiBridge, '@api_v1_react.post("/notifiche-legali/anteprima-relata")', 'API notifiche legali anteprima relata')
assertContains(apiBridge, '@api_v1_react.post("/notifiche-legali/bozze-relata")', 'API notifiche legali bozze relata')
...
assertNotContains(apiBridge, '@api_v1_react.post("/notifiche-legali/prova-deposito")', 'API notifiche legali senza prova deposito')
assertContains(apiBridge, '@api_v1_react.post("/notifiche-legali/modelli-relata")', 'API notifiche legali salva modelli relata')
...
assertContains(preventivoWizardPage, 'CodiceOggettoPstSelect', 'PreventivoWizardPage seleziona codice oggetto PST governato')
assertContains(preventivoWizardPage, 'CodiceOggettoPstSearch', 'PreventivoWizardPage usa ricerca rapida CodiceOggetto PST')
assertContains(preventivoWizardPage, 'codiceOggettoPstSource', 'PreventivoWizardPage usa fonte ufficiale del CodiceOggetto')
assertContains(preventivoWizardPage, 'codice_oggetto_pst: codiceOggettoPst', 'PreventivoWizardPage invia codice oggetto PST')
assertContains(preventivoWizardData, 'codiceOggettoPst', 'preventivoWizardData normalizza codice oggetto da fascicolo')
...
assertContains(apiBridge, 'codice_oggetto_pst_non_valido', 'PreventivoWizardPage rifiuta codice oggetto non ufficiale')
assertContains(preventivoWizardBridge, 'codice_oggetto_pst_payload', 'bridge wizard preventivo propaga codice oggetto ufficiale')
...
assertContains(fatturazionePage, 'Nuova proforma', 'FatturazionePage espone il flusso operativo di creazione proforma')
assertNotContains(fatturazionePage, 'Percorso di recupero', 'FatturazionePage non espone fallback tecnico nella UI')
...
assertContains(documentiAiPage, 'Presidio documenti', 'stato operativo Documenti AI')
...
assertContains(dashboardData, 'getDashboard(options: { refresh?: boolean; allowStale?: boolean } = {})', 'dashboard refresh esplicito')
assertContains(dashboardData, "if (options.allowStale === false) query.set('stale', '0')", 'dashboard riallinea il quadro mostrato in attesa')
assertContains(dashboardData, 'DASHBOARD_TRANSIENT_STATUSES.has(res.status)', 'dashboard ritenta 502/503/504 prima di dichiararsi non raggiungibile')
assertContains(dashboardHook, 'if (payload.stale && !options.revalidate)', 'panoramica mostra subito l ultimo quadro valido e lo riallinea')
assertContains(app, 'aggiornamento in corso...', 'panoramica dichiara l ora del quadro in riallineamento')
...
assertContains(app, 'useDashboardData(!effectiveStandalonePage)', 'panoramica usa lo stato dashboard governato')
assertContains(dashboardHook, 'syncDashboardMailboxes()', 'panoramica avvia sync mailbox dopo primo payload')
assertContains(dashboardHook, "getDashboard({refresh:true})", 'panoramica ricarica cache dopo sync')
assertContains(dashboardHook, "document.visibilityState !== 'visible'", 'auto refresh solo a scheda visibile')
assertContains(dashboardData, "status: 'errore'", 'panoramica dichiara il quadro non raggiungibile')
...
assertContains(fascicoli, 'id="presidio-fascicolo"', 'presidio fascicolo unico')
assertContains(fascicoli, 'id="cabina-regia"', 'ancora storica cabina di regia preservata')
assertContains(fascicoli, 'Presidio del fascicolo', 'titolo presidio fascicolo unico')
assertContains(fascicoli, 'ProceduralProfileConfirmation', 'presidio propone il profilo procedurale senza applicarlo in lettura')
assertContains(fascicoli, "page_state === 'profilo_da_confermare'", 'presidio gestisce la conferma esplicita del profilo')
assertContains(fascicoli, 'Profilo procedurale da confermare', 'proposta procedurale spiegabile')
assertContains(fascicoli, 'Conferma profilo', 'applicazione profilo esplicita e auditabile')
assertContains(fascicoli, 'id="lettura-fascicolo"', 'sezione Lettura del fascicolo presente nel dettaglio')
assertContains(fascicoli, 'LetturaFascicoloPanel', 'lettura del fascicolo servita dal pannello dedicato')
...
assertNotContains(fascicoli, 'className="iu-fas-ai-board"', 'rimozione duplicazione quadro AI fascicolo')
assertNotContains(fascicoli, 'className="iu-fas-smart-board"', 'rimozione duplicazione quadro operativo fascicolo')
assertContains(fascicoli, 'Catalogazione confermata manualmente', 'catalogo espone correzione governata dell’avvocato')
assertContains(fascicoli, 'Acquisisci dal PST', 'documenti portale espongono acquisizione reale')
assertContains(fascicoli, 'Catalogato dal contenuto', 'catalogazione da contenuto distinta dai metadati')
assertContains(fascicoliCss, '.iu-fas-doc-action', 'azioni documentali con icona ed etichetta visibile')
assertContains(fascicoliCss, '.iu-fas-doc-actions.iu-fas-actions>a.iu-fas-doc-action', 'azioni documentali testuali non compresse dal layout icon-only')
assertContains(fascicoliCss, '.iu-fas-catalog__correction', 'form di correzione catalogo accessibile')
assertContains(fascicoli, 'portalDocumentsToAcquire', 'quadro fascicolo separa documenti acquisiti e documenti portale')
assertContains(fascicoli, 'Caricamento quadro fascicolo', 'quadro fascicolo espone stato di caricamento senza dati fittizi')
assertContains(fascicoli, 'Riesegui lettura documentale', 'presidio documenti espone la riesecuzione reale dell’indice')
assertContains(fascicoli, 'Apri fonte', 'presidio documenti apre la fonte collegata nel lettore interno')
assertContains(fascicoli, 'Prepara scadenza', 'presidio documenti precompila la scadenza senza registrarla automaticamente')
assertContains(fascicoli, "if (section === 'scadenze') return ['scadenze', 'documenti']", 'apertura diretta delle udienze carica anche i metadati della fonte')
assertContains(fascicoli, 'Registra attività processuale', 'avanzamento vuoto porta alla registrazione reale di attività')
assertContains(fascicoli, 'Il bundle si abilita dopo il primo evento registrato.', 'audit non propone bundle vuoti')
assertContains(fascicoli, 'iu-fas-quadro-mini__action', 'ogni mini-card del quadro espone un’azione contestuale')
assertContains(fascicoli, 'actionLabel="Apri controlli economici"', 'asse economico del quadro ha un’azione contestuale')
assertContains(fascicoliCss, '.iu-fascicolo-quadro-page .iu-fas-quadro-flow{display:grid!important', 'mini-carte del quadro non vengono compresse da regole generiche')
assertContains(fascicoliCss, '.iu-fascicolo-quadro-page .iu-fas-quadro-quality>span{display:grid!important', 'stati qualità del quadro mantengono badge e testo allineati')
assertContains(fascicoliCss, '.iu-fascicolo-quadro-page .iu-fas-back-top{display:flex!important', 'controllo torna su del quadro mantiene icona centrata')
assertContains(fascicoliCss, '.iu-fascicolo-quadro-page .iu-fas-quadro-axis__action', 'azione asse quadro con focus e target dedicato')
assertContains(fascicoloDocumentPresidio, 'verifica_collegamento_documento', 'documento collegato senza R.G. genera verifica esplicita')
assertContains(fascicoloDocumentPresidio, 'Nessun termine è escluso finché', 'presidio non esclude silenziosamente termini')
assertNotContains(fascicoloDocumentPresidio, 'documento ignorato perché non contiene riferimenti sufficienti', 'presidio non scarta documenti già collegati')
assertContains(fascicoliBridge, 'Lettura documentale non completata', 'bridge espone mancanza di testo come stato aperto')
assertContains(fascicoloOperationalPresidio, 'Verifica PEC necessaria', 'presidio PEC richiede una verifica esplicita quando mancano prove')
assertContains(fascicoloOperationalPresidio, 'Controllo doppioni completato', 'presidio doppioni attesta il controllo solo dopo la comparazione')
assertContains(fascicoloOperationalPresidio, 'Controllo doppioni non eseguibile', 'presidio doppioni espone il blocco quando manca il perimetro')
assertNotContains(fascicoloOperationalPresidio, 'Nessun doppione rilevato', 'presidio doppioni non usa una certezza generica')
assertContains(fascicoliBridge, 'nessuna_notifica', 'bridge fascicoli distingue l’assenza di notifica dal monitoraggio attivo')
...
assertContains(fascicoli, 'PraticheCollegateField', 'campo catalogo pratiche collegate nel form fascicolo')
assertContains(fascicoli, 'CodiceOggettoPstSearch', 'form fascicolo usa ricerca rapida CodiceOggetto PST')
assertContains(fascicoli, 'codice_oggetto_pst', 'codice oggetto PST salvato dal form fascicolo')
assertContains(fascicoli, 'fonte_codice_oggetto', 'fonte codice oggetto salvata dal form fascicolo')
assertContains(fascicoli, 'IusentraPageShell', 'Fascicoli usa PageShell del preset globale')
assertContains(fascicoli, 'IusentraMainArea', 'Fascicoli usa MainArea del preset globale')
assertContains(fascicoli, 'IusentraMainSurface', 'Fascicoli usa MainSurface del preset globale')
assertContains(fascicoli, 'IusentraSupportRail', 'Fascicoli usa SupportRail del preset globale')
assertContains(fascicoli, 'IusentraDataSurface', 'Fascicoli usa DataSurface del preset globale')
assertContains(fascicoli, 'IusentraFiltersBar', 'Fascicoli usa FiltersBar del preset globale')
assertContains(fascicoli, 'IusentraContextFilters', 'Fascicoli usa ContextFilters del preset globale')
assertContains(fascicoli, 'iu-fas-context-summary', 'Fascicoli espone sempre il riepilogo del contesto filtri')
assertContains(fascicoli, 'fill', 'Fascicoli fa crescere la DataSurface')
assertContains(fascicoli, 'footer={paginationControls}', 'Fascicoli ancora la paginazione nella DataSurface')
assertContains(fascicoli, 'className="iu-fas-page-size"', 'Fascicoli mantiene Per pagina in alto a destra')
assertContains(fascicoli, 'Precedente', 'Fascicoli espone paginazione precedente')
assertContains(fascicoli, 'Successiva', 'Fascicoli espone paginazione successiva')
assertContains(fascicoli, 'fascicoliListCacheKey', 'Fascicoli conserva cache per cambio pagina rapido')
assertContains(fascicoli, 'pageCacheRef', 'Fascicoli conserva payload pagina gia letti')
assertContains(fascicoli, 'pageRequestsRef', 'Fascicoli evita richieste duplicate durante la paginazione')
assertContains(fascicoli, 'onPagePrefetch', 'Fascicoli prelegge le pagine al passaggio/focus')
assertContains(fascicoli, 'Caricamento pagina {pendingPage}...', 'Fascicoli mostra stato visibile quando una pagina non e pronta')
assertContains(fascicoli, 'warmEconomicFirstPages', 'Fascicoli scalda le prime pagine della vista economica')
assertContains(fascicoli, 'window.setTimeout(() => warmEconomicFirstPages(params), 250)', 'Fascicoli scalda pagina 2 e 3 dopo il primo payload visibile')
assertContains(fascicoli, '[2, 3].forEach', 'Fascicoli pre-carica pagina 2 e 3 prima del click operativo')
assertContains(fascicoli, 'const presidioDue = Number(data.summary.economicAnalysisDue || 0)', 'Fascicoli avvia il presidio automatico solo se ci sono documenti da analizzare')
assertNotContains(fascicoli, 'data.summary.invoicesToIssue || 0) + Number(data.summary.economicAnalysisDue', 'Fascicoli non rilancia il presidio solo per parcelle da emettere')
assertNotContains(fascicoli, 'data.generatedAt, data.summary.economicAnalysisDue', 'Fascicoli non rilancia il presidio a ogni payload generato')
assertContains(fascicoliCss, '.iu-fas-layout.iusentra-main-area', 'CSS Fascicoli applica preset MainArea')
assertContains(fascicoliCss, '.iu-fas-insights.iusentra-support-rail', 'CSS Fascicoli applica preset SupportRail')
assertContains(fascicoliCss, '.iu-fas-table-card.iusentra-data-surface', 'CSS Fascicoli applica preset DataSurface')
assertContains(fascicoliCss, '.iu-fas-page-loading', 'CSS Fascicoli mantiene indicatore pagina in caricamento')
assertContains(fascicoliCss, '.iu-fas-advanced.is-compact', 'CSS Fascicoli mantiene ContextFilters visibile anche chiuso')
assertContains(fascicoliCss, 'grid-template-columns:minmax(0,1fr) minmax(340px,360px)', 'CSS Fascicoli usa rail desktop coerente')
assertContains(fascicoliCss, 'position:sticky', 'CSS Fascicoli mantiene rail desktop allineata')
...
assertContains(codiciOggettoPstCatalog, '"versione": "2026-09-11.pst-xsd-official"', 'catalogo tecnico ufficiale CodiceOggetto allineato agli XSD SICI in esercizio dal 14/05/2026')
assertContains(codiciOggettoPstCatalog, '"zipFonte": "XSD_SICI_20260508.zip"', 'catalogo CodiceOggetto SICID derivato dal pacchetto XSD SICI in esercizio')
assertNotContains(codiciOggettoPstCatalog, 'XSD_SICI_20260116', 'catalogo CodiceOggetto senza riferimenti al pacchetto SICI superato')
...
assertContains(fascicoli, "const FascicoloDepositoPage = lazy(() => import('./FascicoloDepositoPage')", 'deposito telematico separato dal chunk iniziale fascicoli')
assertContains(fascicoli, "const OfficeDocumentsPanel = lazy(() => import('./OfficeDocumentsPanel')", 'documenti d’ufficio separati dal chunk iniziale fascicoli')
...
assertContains(fascicoliBridge, '_source_practice_prefill', 'preventivi e conferimenti precompilano apertura fascicolo')
...
assertContains(editorProfessionalePage, 'Archivio documenti', 'Editor professionale espone archivio documenti')
assertContains(editorProfessionalePage, 'data.facets.formats.map', 'Archivio documenti espone i formati reali disponibili')
...
assertContains(cartellaCliente, 'Faldone cliente', 'azione faldone cliente senza fallback classico')
assertNotContains(cartellaCliente, '?_legacy=1', 'cartella cliente non deve esporre fallback classico')
...
assertContains(app, 'const openMobileLex = () => {', 'azione Lex AI nella nav mobile')
assertContains(app, 'className="iu-mobile__lex"', 'pulsante Lex AI nella nav mobile')
assertContains(css, '.iu-mobile__lex', 'stile pulsante Lex AI mobile')
assertContains(css, 'bottom:calc(84px + env(safe-area-inset-bottom,0px))!important', 'lex react sopra nav mobile')
assertNotContains(css, '.iu-lex-float{display:none!important}', 'lex react non nascosto su tablet e mobile')
...
assertContains(reactShell, 'window.__IUSENTRA_REACT_BOOTSTRAP_STATE__', 'stato bootstrap react contro pagina bianca')
assertContains(reactShell, 'data-iusentra-react-entry', 'entry react primaria inline con riferimento diagnostico all asset hashato')
assertContains(reactShell, 'inline_entry_code | safe', 'entry react inline per evitare il fallimento del modulo esterno principale')
assertContains(reactShell, 'window.__IUSENTRA_INLINE_REACT_ENTRY__ = true', 'shell segnala al bootstrap React il caricamento inline')
assertContains(reactShell, "iusentraEntryScript='loaded'", 'telemetria non visibile per load entry react')
assertContains(reactShell, "iusentraEntryScript='error'", 'telemetria non visibile per errore entry react')
assertContains(reactShell, 'entrySource()', 'watchdog react usa anche l entry inline come sorgente diagnostica')
assertContains(reactShell, 'retryEntryUrl()', 'recupero avvio entry react con URL anti cache fallita se il root resta vuoto')
assertContains(reactShell, 'iu_boot_retry', 'recupero avvio entry react evita il modulo fallito in cache')
assertContains(reactShell, 'Interfaccia non avviata', 'messaggio utente se React non parte')
...
assertContains(reactShell, '<script type="module" src="{{ js_file }}?v={{ app_version }}"', 'script react principale hashato con versione anti-cache')
...
assertContains(viteConfig, 'manualChunks(', 'build React separa i vendor in chunk governati')
assertContains(viteConfig, "return 'vendor-react'", 'build React isola il runtime React')
assertContains(viteConfig, "return 'vendor-icons'", 'build React isola le icone condivise')
assertContains(viteConfig, 'enforceBundleBudget()', 'build React blocca asset superiori al budget')
assertContains(mainEntrypoint, 'function renderLoadingShell', 'entry React mostra subito stato di caricamento governato')
...
assertContains(legalIntelligencePage, 'Contesto in IUSENTRA', 'LegalIntelligencePage mostra il contesto fonte dentro IUSENTRA')
assertContains(legalIntelligencePage, 'Scheda interna IUSENTRA', 'LegalIntelligencePage chiarisce che il contenuto vive in IUSENTRA')
assertContains(legalIntelligencePage, 'Leggi scheda', 'LegalIntelligencePage evita schede solo collegamento')
assertContains(legalIntelligencePage, 'sourceContext.join', 'LegalIntelligencePage filtra anche sul contesto fonte')
assertContains(legalIntelligencePage, 'record.officialContext', 'LegalIntelligencePage rende il testo letto in IUSENTRA')
assertContains(legalIntelligencePage, 'record.contextSummary', 'LegalIntelligencePage rende il riepilogo di contesto')
assertContains(legalIntelligencePage, 'record.keyPoints', 'LegalIntelligencePage rende i punti scheda')
assertContains(legalIntelligencePage, 'record.operationalChecks', 'LegalIntelligencePage rende i controlli studio')
assertContains(legalIntelligencePage, 'record.contextCompleted', 'LegalIntelligencePage rende lo stato di completezza del contesto')
assertContains(legalIntelligencePage, 'Punti della scheda', 'LegalIntelligencePage mostra punti interni della scheda')
...
assertContains(legalIntelligencePage, 'onArchiveSearch', 'LegalIntelligencePage collega fonti e archivi alla ricerca')
assertContains(legalIntelligencePage, '/ricerca-legale?q=', 'LegalIntelligencePage usa /ricerca-legale come ricerca primaria')
assertContains(legalIntelligenceCss, '.iu-li-filter-grid', 'LegalIntelligencePage espone filtri strutturati')
assertContains(legalIntelligenceCss, 'repeat(3, minmax(0, 1fr))', 'LegalIntelligencePage mostra risultati compatti in colonne')
assertContains(legalIntelligenceCss, '.iu-li-card-points', 'LegalIntelligencePage mostra punti scheda compatti nelle card')
assertContains(legalIntelligenceCss, '.iu-li-detail__status', 'LegalIntelligencePage evidenzia la scheda interna IUSENTRA nel dettaglio')
assertContains(giurisprudenzaPage, 'giurisprudenzaMetricHref', 'GiurisprudenzaPage rende operative le metriche')
assertContains(giurisprudenzaPage, 'practicalUse', 'GiurisprudenzaPage mostra uso professionale della scheda')
assertContains(giurisprudenzaCss, '.iu-legal-record-grid', 'GiurisprudenzaPage mostra provvedimenti compatti in colonne')
...
assertContains(legalIntelligenceData, 'sourceContext: string[]', 'legalIntelligenceData normalizza contesto fonte')
assertContains(legalIntelligenceData, 'officialContext: string', 'legalIntelligenceData normalizza testo ufficiale letto')
assertContains(legalIntelligenceData, 'contextSummary: string', 'legalIntelligenceData tipizza riepilogo contesto')
assertContains(legalIntelligenceData, 'keyPoints: string[]', 'legalIntelligenceData normalizza punti scheda')
assertContains(legalIntelligenceData, 'operationalChecks: string[]', 'legalIntelligenceData normalizza controlli operativi')
assertContains(legalIntelligenceData, 'contextCompleted: boolean', 'legalIntelligenceData tipizza completezza contesto')
assertContains(legalIntelligenceData, 'practicalUse: string', 'legalIntelligenceData normalizza uso pratico')
assertContains(legalIntelligenceData, 'officialContext: display(item.officialContext)', 'legalIntelligenceData legge officialContext dal payload')
assertContains(legalIntelligenceData, 'contextSummary: display(item.contextSummary)', 'legalIntelligenceData legge contextSummary dal payload')
assertContains(legalIntelligenceData, 'keyPoints: textList(item.keyPoints)', 'legalIntelligenceData legge keyPoints dal payload')
assertContains(legalIntelligenceData, 'operationalChecks: textList(item.operationalChecks)', 'legalIntelligenceData legge operationalChecks dal payload')
assertContains(legalIntelligenceData, 'contextCompleted: item.contextCompleted === true', 'legalIntelligenceData legge contextCompleted dal payload')
assertContains(legalIntelligenceBridge, '_enrich_record_context', 'bridge legal intelligence arricchisce il contesto fonte')
assertContains(legalIntelligenceBridge, '"sourceContext"', 'bridge legal intelligence espone contesto fonte')
assertContains(legalIntelligenceBridge, '_extract_news_excerpt', 'bridge legal intelligence ripulisce news cumulative')
assertContains(legalIntelligenceBridge, '_news_area_branch', 'bridge legal intelligence corregge area da contesto fonte')
assertContains(legalIntelligenceBridge, '_complete_record_context_from_official_source', 'bridge legal intelligence completa contesto da fonte ufficiale')
assertContains(legalIntelligenceBridge, '"officialContext"', 'bridge legal intelligence espone testo letto in pagina')
assertContains(legalIntelligenceBridge, '"keyPoints"', 'bridge legal intelligence espone punti scheda')
assertContains(legalIntelligenceBridge, '"operationalChecks"', 'bridge legal intelligence espone controlli studio')
...
assertContains(apiBridgeNormalized, '"ricerca-legale",\n        "artifacts/react-migration/legacy-contracts/ricerca-legale.json"', 'API /ricerca-legale usa ricerca operativa')
