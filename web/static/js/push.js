/**
 * IUSENTRA — Browser Notification API
 *
 * Funzionalità:
...
const PCT_NOTIFICHE = {
  ...
  async richiediPermesso() {
    ...
    if (perm === 'granted') {
      if (btn) btn.remove();
      this._mostra('IUSENTRA', 'Notifiche abilitate — riceverai avvisi per le scadenze urgenti.', '/scadenziario');
      this.controllaPendenti();
    } else {
  ...
  _suSyncEvent(ev) {
    ...
    const titolo = `IUSENTRA — ${moduli[ev.modulo] || 'Record'} ${etichette[ev.tipo] || ''}`;
