-- Intake CRM ed Entity Graph - SQLite
-- Fonte operativa: studio.db del tenant. Il JSON storico resta solo mirror.

-- Registro delle migrazioni runtime: impedisce che un archivio storico venga
-- ricopiato a ogni avvio dopo il passaggio al database canonico del tenant.
...
CREATE TABLE IF NOT EXISTS crm_leads (
    ...
    conflitto_verificato INTEGER NOT NULL DEFAULT 0,
    conflitto_esito_json TEXT NOT NULL DEFAULT '{}',
...
-- Adeguata verifica antiriciclaggio. I dati strutturati vivono nel database
-- canonico del tenant; ``antiriciclaggio/verifiche.json`` è solo mirror
-- rigenerabile per import/export storico.
CREATE TABLE IF NOT EXISTS aml_verifications (
    ...
    cliente_pep INTEGER NOT NULL DEFAULT 0,
    paese_alto_rischio INTEGER NOT NULL DEFAULT 0,
    titolare_effettivo_json TEXT NOT NULL DEFAULT '{}',
    indici_json TEXT NOT NULL DEFAULT '[]',
