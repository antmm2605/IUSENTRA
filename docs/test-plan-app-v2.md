...
- UI: `pnpm test`, `pnpm typecheck`, `pnpm build`, `pnpm build:storybook` e `scripts/validate_ui_coverage.py`; Chromatic/VRT resta predisposto e non bloccante senza token.
...
| Frontend statico | pnpm test | contratti React, App V2 e UI coverage via workspace |
| Frontend build | pnpm typecheck && pnpm build && pnpm build:storybook | typecheck, build Vite e Storybook |
...
| P0 | pnpm test && pnpm typecheck && pnpm build && pnpm build:storybook | frontend App V2 e Storybook |
