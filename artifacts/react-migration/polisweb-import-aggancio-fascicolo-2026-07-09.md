...
Aggiornamento prestazionale 09/07/2026: aprendo direttamente `#documenti`, la UI non deve più chiamare due volte i dati Lex. L'endpoint documenti include già il riepilogo `Indicizzazione Lex`, quindi la sezione documenti aggiorna anche `lexIndexing` e marca `lex` come caricata. Inoltre il primo payload del dettaglio usa `include=documenti` quando l'URL arriva già su `#documenti`, evitando la sequenza dettaglio vuoto + caricamento documenti. Il test `test_react_fascicolo_documenti_non_duplica_chiamata_lex_lazy` presidia il comportamento.
