...
Per il piano di migrazione React vedi [docs/REACT_MIGRATION_MASTER_PLAN.md](docs/REACT_MIGRATION_MASTER_PLAN.md). La tranche `2.198.120` aggiunge audit Full React, inventario Jinja, shell/app/API/features governate, primitive UI legali e runner anti-mascheramento senza promuovere route non ancora verificate. La tranche `2.198.121` introduce il design system IUSENTRA basato su shadcn/ui, Lucide Icons e Tailwind, documentato in [docs/UI_DESIGN_SYSTEM.md](docs/UI_DESIGN_SYSTEM.md), mantenendo invariati route, API e fallback governati. Le tranche `2.198.122`-`2.198.125` aggiungono il runner pytest a fasi documentato in [docs/PYTEST_PHASES.md](docs/PYTEST_PHASES.md), dividono `Pytest core`, coverage critica, Local Signer, overlay qualita', E2E e frontend React in shard CI con aggregatori e mantengono i guardrail governance sui target storici, per evitare timeout monolitici senza ridurre la copertura finale richiesta.
