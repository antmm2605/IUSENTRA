...
- Su Windows, mentre `curl` attende il certificato client del PST, il Local Signer deve provare in modo best-effort a portare in primo piano la finestra PIN/Sicurezza Windows/smart card/CNS/CIE/Bit4id/Aruba/token, includendo anche dialog di credenziali/PIN senza titolo esplicito ma riconoscibili da classe o testo figlio, senza cambiare il numero di PIN richiesti e senza salvare PIN o chiavi. Il processo `curl` deve essere avviato senza finestra console o presenza autonoma sulla barra delle applicazioni: l'unica finestra visibile deve essere quella del PIN quando il sistema operativo la richiede.
- Il comportamento certificato aggiornato resta: un solo PIN per la sessione fascicolo quando la ricerca/anteprima ha già autenticato il PST; lo scarico batch deve provare prima i cookie della stessa sessione. Un secondo PIN è ammesso solo come fallback reale se il portale rifiuta la sessione già aperta o il token/certificato richiede una nuova autorizzazione.
