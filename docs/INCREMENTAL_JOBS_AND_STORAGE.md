...
| Lettura automatica dei fascicoli (`archivio_letture_automatico`, ogni 10 min) e collaudo del lettore (`collaudo_lettore`, 04:10) | tabella `letture_fatti` nello stesso database; `intelligence/collaudo_lettore.json` | i due motori leggono solo gli oggetti che il registro dice non letti (tetto `IUSENTRA_ARCHIVIO_LETTURE_LIMITE` per giro); a fascicolo invariato il giro non apre alcun file; caricamento documento, collegamento PEC e fine OCR alimentano l'archivio subito, in sfondo |
