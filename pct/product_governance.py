...
STORAGE_PARITY_REGISTRY: tuple[StorageParityEntry, ...] = (
    StorageParityEntry(
        ...
        postgres_read=True,
        postgres_write=True,
        parity_note="Parita' R/W chiusa anche su PostgreSQL: il tenant puo' attivarlo come backend effettivo con migrazione e report di consistenza.",
        migration_wave="Wave 1 - identita' e audit",
        fallback_mode="Nessun fallback invisibile quando PostgreSQL e' attivo; SQLite resta percorso locale o di pre-cutover.",
        consistency_checks=(
    ...
    StorageParityEntry(
        ...
        postgres_read=True,
        postgres_write=True,
        parity_note="Parita' R/W chiusa su JSON, SQLite e PostgreSQL per il repository clienti tenant-aware.",
        migration_wave="Wave 2 - core operativo",
        fallback_mode="SQLite o JSON solo come percorso dichiarato di bootstrap; nessun fallback invisibile se PostgreSQL e' attivo.",
        consistency_checks=(
    ...
    StorageParityEntry(
        ...
        postgres_read=True,
        postgres_write=True,
        parity_note="I metadati fascicolo hanno parita' R/W anche su PostgreSQL; l'archivio documentale resta filesystem-first per tenant.",
        migration_wave="Wave 2 - core operativo",
        fallback_mode="Documenti sempre sul filesystem tenant; nessun rientro invisibile a JSON quando PostgreSQL e' backend attivo dei metadati core.",
        consistency_checks=(
    ...
    StorageParityEntry(
        ...
        postgres_read=True,
        postgres_write=True,
        parity_note="Agenda e sincronizzazione calendario hanno parita' R/W anche su PostgreSQL per i tenant attivati.",
        migration_wave="Wave 2 - core operativo",
        fallback_mode="SQLite resta fallback dichiarato solo prima del cutover; PostgreSQL attivo non degrada in modo invisibile.",
        consistency_checks=(
    ...
    StorageParityEntry(
        ...
        postgres_read=True,
        postgres_write=True,
        parity_note="Scadenziario portato in parita' R/W anche su PostgreSQL come backend core tenant-aware.",
        migration_wave="Wave 2 - core operativo",
        fallback_mode="JSON solo in bootstrap o fallback controllato locale; PostgreSQL attivo resta esplicito e presidiato.",
        consistency_checks=(
    ...
    StorageParityEntry(
        module_id="termini_processuali",
        domain="Programmazione",
        label="Calcolatore termini processuali e audit",
        source_of_truth="DeadlinePracticeRepository + motore spiegabile termini processuali",
        json_read=True,
        json_write=True,
        sqlite_read=True,
        sqlite_write=True,
        postgres_read=True,
        postgres_write=True,
        parity_note="Template, calendario ufficiale, audit hashato e log promemoria hanno schema JSON/SQLite/PostgreSQL governato.",
        migration_wave="Wave 2 - core operativo",
        fallback_mode="JSON tenant-aware come bootstrap controllato; SQLite/PostgreSQL disponibili con schemi dedicati e nessun fallback invisibile su backend attivo.",
        consistency_checks=(
            "Template scadenze versionati e coerenti per codice.",
            "Audit calcoli con sha256 immutabile e versioni engine/ruleset/calendario.",
            "Promemoria PEC idempotenti collegati a calcolo e scadenza.",
        ),
        test_refs=("tests/test_termini_processuali.py", "tests/test_storage_governance.py"),
    ),
    StorageParityEntry(
        ...
        postgres_read=True,
        postgres_write=True,
        parity_note="Template custom, repository editor e preferenze layout hanno parita' R/W anche su PostgreSQL tenant-aware.",
        migration_wave="Wave 3 - workspace professionali",
        fallback_mode="JSON resta formato di export e bootstrap, senza fallback invisibili se PostgreSQL e' attivo.",
        consistency_checks=(
    ...
    StorageParityEntry(
        ...
        postgres_read=True,
        postgres_write=True,
        parity_note="Preventivi, conferimenti e workflow commerciale hanno percorso ufficiale JSON -> SQLite -> PostgreSQL con parity chiusa.",
        migration_wave="Wave 4 - economico",
        fallback_mode="JSON solo come ponte di migrazione o bootstrap controllato.",
        consistency_checks=(
    ...
    StorageParityEntry(
        ...
        postgres_read=True,
        postgres_write=True,
        parity_note="Parcelle, fatture, incassi e saldo cliente hanno parita' R/W chiusa anche su PostgreSQL.",
        migration_wave="Wave 4 - economico",
        fallback_mode="Cutover ufficiale con report di consistenza e nessun fallback invisibile.",
        consistency_checks=(
    ...
    StorageParityEntry(
        ...
        postgres_read=True,
        postgres_write=True,
        parity_note="Monitoraggio fonti, alert, audit trace e stato runtime sono persistiti anche su repository SQL/PostgreSQL.",
        migration_wave="Wave 5 - intelligence",
        fallback_mode="JSON tenant-aware come export e recovery locale, senza fallback invisibili su PostgreSQL attivo.",
        consistency_checks=(
    ...
    StorageParityEntry(
        ...
        postgres_read=True,
        postgres_write=True,
        parity_note="Corpus runtime, registry fonti e tassonomia giurisprudenziale hanno parita' R/W anche su PostgreSQL.",
        migration_wave="Wave 5 - intelligence",
        fallback_mode="JSON tenant-aware come export e import storico controllato.",
        consistency_checks=(
    ...
    StorageParityEntry(
        ...
        postgres_read=True,
        postgres_write=True,
        parity_note="Repository capability, catalogo fonti e monitoraggio telematico hanno parita' R/W anche su PostgreSQL; file e buste restano filesystem-first.",
        migration_wave="Wave 3 - workspace professionali",
        fallback_mode="Metadati e repository su SQL/PostgreSQL; file e buste sempre su filesystem tenant.",
        consistency_checks=(
    ...
    StorageParityEntry(
        ...
        postgres_read=True,
        postgres_write=True,
        parity_note="Lo snapshot operativo della cabina intelligente e' persistito anche su repository SQL/PostgreSQL.",
        migration_wave="Wave 5 - intelligence",
        fallback_mode="JSON come export derivato; il repository SQL e' la persistenza operativa quando attivo.",
        consistency_checks=(
...
E2E_FLOWS: tuple[E2EFlow, ...] = (
    ...
    E2EFlow(
        flow_id="studio_reale_flow",
        label="Studio reale: cliente -> preventivo -> conferimento -> fascicolo -> parcella -> incasso",
        coverage="nuovo presidio",
        systems=("clienti", "workflow commerciale", "fascicoli", "timesheet", "fatturazione", "pagamenti"),
        test_refs=("tests/e2e/test_studio_reale_flow.py",),
    ),
    E2EFlow(
        flow_id="storage_migration",
        label="Migrazione tenant completa con snapshot, diff e rollback",
        coverage="nuovo presidio",
        systems=("assistente migrazione", "pct.database", "tenant storage", "runtime storage"),
        test_refs=("tests/e2e/test_tenant_migration_full.py", "tests/test_storage_governance.py"),
    ),
    E2EFlow(
        flow_id="coverage_ai_pipeline",
        label="Coverage AI completa: audit -> gaps -> draft -> review -> publish SQL",
        coverage="nuovo presidio",
        systems=("coverage repository", "review audit", "publish SQL", "training implicito"),
        test_refs=("tests/e2e/test_ai_pipeline_full.py", "tests/test_legal_coverage_pipeline.py"),
    ),
    E2EFlow(
