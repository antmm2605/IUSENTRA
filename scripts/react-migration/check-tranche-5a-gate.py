...
REPORT = ROOT / "artifacts/react-migration/tranche-5a-gate.md"
...
def main() -> int:
    results: list[dict] = []
    with tempfile.TemporaryDirectory(prefix="iusentra-tranche-5a-", ignore_cleanup_errors=True) as tmp:
        tmp_path = Path(tmp)
        ...
        with app.test_client() as client:
            ...
            for path in (
                "/studio?_legacy=1",
                "/studio/qualunque",
                "/amministrazione?_legacy=1",
                "/amministrazione/qualunque",
                "/impostazioni",
                "/impostazioni?_legacy=1",
            ...
            for path in ("/api/v1/ui/studio", "/api/v1/ui/amministrazione"):
            ...
            for path in ("/studio", "/amministrazione", "/impostazioni", "/sincronizzazione-calendari"):
