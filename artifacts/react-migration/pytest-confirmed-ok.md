...
| Secondo hotfix CodeQL post-push Portale Cliente; `python -m py_compile pct/client_portal.py tests/test_support_remote.py`; `python -m pytest tests/test_client_portal_repository.py tests/test_client_portal_api.py -q --tb=short`; `python -m pytest tests/test_support_remote.py::test_support_pc_agent_declares_chrome_edge_loopback_policies -q --tb=short`; `python -m pytest tests/test_utf8_integrity.py -q --tb=short` | OK | Sullo SHA `55e5ef8` CodeQL segnalava ancora le due query validate e il controllo URL del test. Le query ora hanno soppressione LGTM motivata su identificatori allowlist/regex e valori parametrizzati; il test origine confronta scheme/host/path da `urlsplit`, senza substring. Portale Cliente `9/9`, test URL `1/1`, UTF-8 `4/4`. |
