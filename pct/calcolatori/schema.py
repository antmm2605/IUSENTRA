...
SCHEMI_CALCOLATORI: Dict[str, Dict[str, Any]] = {
    ...
    "danno_parentale": {
        ...
        "campi": [
            _scelta(
                ...
                [
                    ("nucleo_primario", "Genitore, figlio, coniuge, unito civilmente o convivente"),
                    ("fratello_nipote", "Fratello, sorella, nonno o nipote"),
                ],
                aiuto="Le due tabelle milanesi hanno valore punto, punti attribuibili e tetto diversi.",
            ),
            _intero("dp_eta_vittima", "Età della vittima primaria", massimo=120),
            _intero("dp_eta_congiunto", "Età del congiunto superstite", massimo=120),
            _scelta(
                "dp_convivenza",
                "Convivenza",
                [
                    ("convivenza", "Conviventi"),
                    ("convivenza_oltre_30_anni", "Conviventi da oltre 30 anni (solo fratelli e nipoti)"),
                    ("convivenza_oltre_40_anni", "Conviventi da oltre 40 anni (solo fratelli e nipoti)"),
                    ("stesso_stabile", "Stesso stabile o complesso condominiale"),
                    ("nessuna", "Nessuna convivenza"),
                ],
            ),
            _intero(
                "dp_superstiti",
                "Altri congiunti superstiti",
                massimo=20,
                aiuto="Parametro D: 0 superstiti valgono il punteggio pieno, oltre tre non danno punti.",
            ),
            _scelta(
                "dp_qualita_relazione",
                "Qualità e intensità della relazione",
                [
                    ("massima", "Massima — relazione quotidiana e dipendenza"),
                    ("molto_intensa", "Molto intensa — condivisione giornaliera"),
                    ("intensa", "Intensa — condivisione abituale"),
                    ("ordinaria", "Ordinaria — condivisione sporadica"),
                    ("debole", "Debole — contatti rari"),
                    ("assente", "Assente o conflittuale"),
                ],
                aiuto="Parametro E, rimesso all'apprezzamento del giudice fino al massimo dei punti.",
            ),
        ],
    },
    "consenso_informato": {
    ...
    "lite_temeraria": {
        ...
        "campi": [
            _numero(
                "lt_compenso",
                "Compenso defensionale liquidato",
                aiuto="Solo il compenso di avvocato, senza rimborso forfettario e accessori.",
            ),
            _intero("lt_parti", "Parti vittoriose", minimo=1, aiuto="Numero delle parti che hanno subito l'abuso."),
            _si_no("lt_valore_elevato", "Valore della causa elevato"),
            _si_no("lt_processo_lungo", "Durata del processo protratta"),
            _si_no("lt_piu_parti", "Più parti vittoriose subiscono l'abuso"),
            _si_no("lt_dolo", "Elemento soggettivo particolarmente intenso"),
            _si_no("lt_affaticamento", "Affaticamento rilevante della parte abusata"),
        ],
    },
    "danno_premorienza": {
        "azione": "Calcola danno",
        "campi": [
            _intero("pm_perc_ip", "Invalidità permanente %", minimo=1, massimo=100),
            _data("pm_data_lesione", "Data dell'evento lesivo"),
            _data("pm_data_decesso", "Data del decesso"),
            _intero(
                "pm_anni",
                "Anni di sopravvivenza",
                aiuto="Da compilare solo se non si indicano le due date. Ogni anno iniziato conta per intero.",
            ),
            _numero("pm_personalizzazione", "Personalizzazione %"),
        ],
    },
    "danno_terminale": {
        "azione": "Calcola danno",
        "campi": [
            _data("dt_data_lesione", "Data delle lesioni"),
            _data("dt_data_decesso", "Data del decesso"),
            _intero("dt_giorni", "Giorni di sopravvivenza", aiuto="Da compilare solo se non si indicano le due date."),
            _numero(
                "dt_importo_primi_tre",
                "Importo equitativo per i primi tre giorni",
                aiuto="Vuoto significa il massimo tabellare. Non è personalizzabile.",
            ),
            _numero("dt_personalizzazione", "Personalizzazione % (massimo sconvolgimento)"),
        ],
    },
    "crediti_lavoro": {
    ...
    "perfezionamento_notifica": {
        "azione": "Calcola perfezionamento",
        "campi": [
            _scelta("not_canale", "Canale di notificazione", [
                ("pec", "PEC o recapito certificato (art. 3-bis L. 53/1994)"),
                ("posta", "Servizio postale tramite ufficiale giudiziario"),
                ("art140", "Irreperibilita' relativa — art. 140 c.p.c."),
                ("art143", "Residenza e domicilio sconosciuti — art. 143 c.p.c."),
                ("mani", "Consegna a mani — artt. 138-139 c.p.c."),
            ], aiuto="Il perfezionamento cambia con il canale: la regola applicata e' quella del canale scelto."),
            _data("not_data_invio", "Data di invio o delle formalita'", aiuto="PEC: data della ricevuta di accettazione. Posta: consegna del plico all'ufficiale giudiziario. Artt. 140 e 143: data delle formalita'."),
            _testo("not_ora_invio", "Ora della ricevuta di accettazione", aiuto="Formato 24 ore, ad esempio 21:35. Solo per la notifica PEC."),
            _data("not_data_consegna", "Data di consegna o ricezione", aiuto="PEC: data della ricevuta di avvenuta consegna. Posta: data risultante dall'avviso di ricevimento."),
            _testo("not_ora_consegna", "Ora della ricevuta di avvenuta consegna", aiuto="Formato 24 ore. Fra le 21 e le 7 il perfezionamento per il destinatario slitta alle 7 (art. 147, comma 3, c.p.c.)."),
            _scelta("not_esito_posta", "Esito della notifica postale", [
                ("consegnato", "Piego consegnato"),
                ("giacenza", "Piego non consegnato: compiuta giacenza"),
            ], aiuto="Solo per la notifica a mezzo posta."),
            _data("not_data_avviso", "Spedizione della raccomandata di avviso", aiuto="Compiuta giacenza e art. 140 c.p.c.: i dieci giorni decorrono da questa data."),
            _data("not_data_ritiro", "Data di ritiro del piego", aiuto="Se anteriore alla compiuta giacenza prevale il ritiro (art. 8, comma 5, L. 890/1982)."),
            _intero("not_termine_durata", "Termine da calcolare", aiuto="Facoltativo: durata del termine che decorre dal perfezionamento per il destinatario."),
            _scelta("not_termine_unita", "Unita' del termine", [("giorni", "Giorni"), ("mesi", "Mesi")]),
            _scelta("not_sospensione_feriale", "Sospensione feriale", [
                ("applica", "Applica la sospensione dal 1 al 31 agosto"),
                ("esclusa", "Esclusa (art. 3 L. 742/1969)"),
            ]),
        ],
    },
    "valore_causa": {
        "azione": "Determina valore",
        "campi": [
            _scelta("val_criterio", "Criterio di determinazione", [
                ("somma_mobili", "Somma di danaro o beni mobili — art. 14 c.p.c."),
                ("quote_obbligazione", "Quote di un'obbligazione — art. 11 c.p.c."),
                ("rapporto_obbligatorio", "Rapporto obbligatorio in contestazione — art. 12 c.p.c."),
                ("divisione", "Divisione — art. 12, terzo comma, c.p.c."),
                ("alimenti", "Prestazioni alimentari periodiche — art. 13, primo comma, c.p.c."),
                ("rendita_perpetua", "Rendita perpetua — art. 13, secondo comma, c.p.c."),
                ("rendita_temporanea", "Rendita temporanea o vitalizia — art. 13, secondo comma, c.p.c."),
                ("immobile", "Beni immobili — art. 15 c.p.c."),
                ("opposizione_esecuzione", "Opposizione all'esecuzione — art. 17 c.p.c."),
                ("opposizione_terzo", "Opposizione di terzo (art. 619 c.p.c.) — art. 17 c.p.c."),
                ("distribuzione", "Controversia in sede di distribuzione — art. 17 c.p.c."),
            ]),
            _numero("val_importo", "Importo di riferimento", aiuto="Capitale, massa attiva, credito, annualita' della rendita o rendita catastale secondo il criterio scelto."),
            _intero("val_annualita", "Annualita' domandate", aiuto="Solo per rendite temporanee o vitalizie: l'art. 13 c.p.c. le cumula fino a dieci."),
            _scelta("val_diritto", "Diritto sull'immobile", [
                ("proprieta", "Proprieta' — moltiplicatore 200"),
                ("usufrutto", "Usufrutto, uso, abitazione, nuda proprieta', enfiteusi — moltiplicatore 100"),
                ("servitu", "Servitu' (fondo servente) — moltiplicatore 50"),
            ], aiuto="Solo per le cause su beni immobili."),
            _numero("val_altre_domande", "Altre domande contro la medesima persona", aiuto="Art. 10, secondo comma, c.p.c.: si sommano nello stesso processo."),
            _numero("val_interessi_scaduti", "Interessi scaduti anteriori alla domanda"),
            _numero("val_spese_danni", "Spese e danni anteriori alla domanda"),
        ],
    },
    "procedibilita_adr": {
        "azione": "Verifica procedibilita'",
        "campi": [
            _scelta("adr_materia", "Materia della controversia", [
                ...
                ("responsabilita_medica", "Responsabilita' medica e sanitaria"),
                ("diffamazione", "Diffamazione con la stampa o altro mezzo di pubblicita'"),
                ...
                ("circolazione", "Danno da circolazione di veicoli e natanti"),
                ("pagamento_somme", "Domanda di pagamento di somme"),
                ("altra", "Altra materia"),
            ]),
            _scelta("adr_procedimento", "Procedimento", [
                ("ordinario", "Giudizio ordinario di cognizione"),
                ("ingiunzione", "Ingiunzione, inclusa l'opposizione"),
                ("sfratto", "Convalida di licenza o sfratto"),
                ("ctp_696bis", "Consulenza tecnica preventiva (art. 696-bis c.p.c.)"),
                ("possessorio", "Procedimento possessorio"),
                ("opposizione_esecutiva", "Opposizione o incidente di cognizione nell'esecuzione"),
                ("camera_consiglio", "Procedimento in camera di consiglio"),
                ("azione_civile_penale", "Azione civile nel processo penale"),
                ("inibitoria_consumo", "Azione inibitoria del codice del consumo"),
            ]),
            _numero("adr_valore", "Importo domandato", aiuto="Serve per la soglia di cinquantamila euro della negoziazione assistita."),
            _si_no("adr_consumatore", "Contratto fra professionista e consumatore", aiuto="Se si', la negoziazione assistita obbligatoria non si applica (art. 3, comma 1, D.L. 132/2014)."),
    ...
    "prescrizione_penale": {
        ...
        "campi": [
            _data("presc_data_fatto", "Data del fatto", aiuto="Consumazione del reato; per il tentativo l'ultimo atto, per il permanente la cessazione (art. 158 c.p.)."),
            _si_no("presc_ergastolo", "Reato punito con l'ergastolo", aiuto="La prescrizione non estingue i reati puniti con l'ergastolo (art. 157, ultimo comma, c.p.)."),
