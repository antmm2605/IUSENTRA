...
def _serve_attachment_pdf_reader(
    *,
    ...
    if page_value:
        try:
            ...
            current_app.logger.warning(
                "Anteprima pagina allegato email ordinaria non disponibile email=%s allegato=%s page=%s: %s",
                id_email,
                indice_allegato,
                page_value,
                exc,
    ...
    try:
        ...
        current_app.logger.warning(
            "Lettore PDF allegato email ordinaria non disponibile email=%s allegato=%s: %s",
            id_email,
            indice_allegato,
            exc,
        )
...
def allegato(id_email: str, indice_allegato: int):
    ...
    contenuto_archivio = None if percorso else gestore.leggi_allegato(email_obj, indice_allegato)
    if not percorso:
