...
| `python tools\performance_smoke.py --strict --output "$env:TEMP\iusentra-performance-smoke-codeql-final.json"` | OK | Smoke strict verde senza `SECRET_KEY` sintetica nel config benchmark: `startup_ms=2358.75` su soglia cold-start CI `3200`, `login_ms=13.9`, `health_ms=57.06`, `runtime_metrics_ms=103.64`, `lex_context_build_ms=0.01`, `lex_retrieval_ms=0.01`; il benchmark non interroga sorgenti Lex lente. |
