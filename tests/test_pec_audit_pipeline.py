...
def test_pct_controls_follow_certified_delivery_transport_message_id(tmp_path):
    from pct.fascicoli import GestioneFascicoli, TipoFascicolo
    ...
    original_message_id = "179018056167.20200.6167084769917992854@xn--gi-3ja"
    transport_message_id = "jpec13219.20260923182325.82661.4006.1.1@pec.aruba.it"
    deposito = fascicoli.aggiungi_esito_deposito(
        fascicolo.id,
        tipo_atto="RICORSO",
        pec_destinatario="tribunale.vicenza@civile.ptel.giustiziacert.it",
    ...
    fascicoli.modifica_esito_deposito(
        fascicolo.id,
        deposito.id,
        tipo_atto=deposito.tipo_atto,
        pec_destinatario=deposito.pec_destinatario,
        stato="CONSEGNATO",
        messaggio=deposito.messaggio,
        ricevuta_consegna=(
            "Ricevuta di avvenuta consegna. "
            f"Message-ID deposito: {transport_message_id}"
        ),
        modificato_da="codex-test",
    )
    repo = PecAuditRepository(
        tmp_path / "pec_audit.sqlite",
        tenant_id="default",
    ...
    <EsitoAtto>
      <IdMsgMitt>DEPOSITO TELEMATICO - RICORSO - Tribunale di Vicenza</IdMsgMitt>
      <IdMsg>&lt;{transport_message_id}&gt;</IdMsg>
      <DatiEsito><MsgEsito>
        <CodiceEsito>1</CodiceEsito>
        <DescrizioneEsito>IDBUSTA: 155701307 Controlli terminati con successo</DescrizioneEsito>
      </MsgEsito><Tempo><Data>2026-09-23</Data><Ora zoneDesignator="+0200">18:24:28</Ora></Tempo></DatiEsito>
    </EsitoAtto>"""
    msg = EmailMessage(policy=policy.SMTP)
    msg["Subject"] = "POSTA CERTIFICATA: ESITO CONTROLLI AUTOMATICI DEPOSITO TELEMATICO - RICORSO - Tribunale di Vicenza"
    msg["From"] = '"Per conto di: tribunale.vicenza@civile.ptel.giustiziacert.it" <posta-certificata@legalmail.it>'
    msg["To"] = "giuseppe.montagnese94@pec.it"
    msg["Date"] = "Wed, 23 Sep 2026 18:24:29 +0200"
    msg["Message-ID"] = "<144180E1.01344800.CF153837.C4805BEE.posta-certificata@legalmail.it>"
    msg["X-Riferimento-Message-ID"] = "<2132109252.7637.1790180668712.JavaMail.jboss@veglpecw01.giustizia.it>"
    msg.set_content("IDBUSTA: 155701307 Controlli terminati con successo. Busta in attesa di accettazione")
    msg.add_attachment(controls_xml.encode("utf-8"), maintype="application", subtype="octet-stream", filename="EsitoAtto.xml")
    msg.add_attachment(
        b"<postacert><tipo>accettazione</tipo><destinatari><destinatario tipo='certificato'>giuseppe.montagnese94@pec.it</destinatario></destinatari><msgid>&lt;2132109252.7637.1790180668712.JavaMail.jboss@veglpecw01.giustizia.it&gt;</msgid></postacert>",
        maintype="application",
        subtype="xml",
        filename="daticert.xml",
    )
    controls = repo.ingest_mime(msg.as_bytes(policy=policy.SMTP))
    worker = repo.run_pending_jobs(limit=30, actor="codex-test")
    assert worker["failed"] == 0
    ...
    reference = metadata["archive_receipt_reference"]
    assert row["linked_fascicolo_id"] == fascicolo.id
    assert reference["deposito_id"] == deposito.id
    assert reference["strategy"] == "certified_receipt_chain"
    assert reference["source_message_id"] == transport_message_id
    assert reference["matched_by"] == ["certified_receipt_chain"]
...
def test_presidio_documentale_registra_udienza_remota_passata_senza_scadenza_futura(tmp_path):
    ...
    fascicoli.aggiungi_documento(
        fascicolo.id,
        "Appunti strategici causa.txt",
        TipoDocumento.ALTRO,
        b"Documento interno senza fissazione udienza.",
    )
    doc = fascicoli.aggiungi_documento(
        fascicolo.id,
        "Decreto fissazione udienza.txt",
    ...
    assert report["new_or_changed_documents"] == 1
    assert report["pending_new_or_changed_documents"] == 1
    assert report["candidate_dates"] == 1
    assert report["scheduled"] == 0
    assert report["past_remote_hearings_recorded"] == 1
    assert report["errors"] == []
    assert report["items"][0]["status"] == "past_remote_hearing_recorded"
    assert report["items"][0]["document"] == "Decreto fissazione udienza.txt"
    assert GestioneScadenziario(str(scadenziario_db), studio_db=studio_db).tutte(solo_aperte=False) == []
    assert Agenda(str(agenda_db), studio_db=studio_db).tutti() == []

    saved = GestioneFascicoli(str(fascicoli_db), documents_dir=str(fascicoli_docs), studio_db=studio_db).get(fascicolo.id)
    assert saved is not None
    assert saved.data_prossima_udienza == ""
    activities = [att for att in saved.attivita if att.tipo == TipoAttivita.UDIENZA]
    assert len(activities) == 1
    assert activities[0].data == hearing_day.isoformat()
    assert activities[0].id_documento == doc.id
    assert f"Link udienza audiovisiva: {link}" in activities[0].descrizione
    assert "Fonte: documento fascicolo indicizzato da Lex AI." in activities[0].note

    second = repo.recover_missing_hearings_from_fascicolo_documents(limit=1, actor="codex-test")
    assert second["new_or_changed_documents"] == 1
    assert second["past_remote_hearings_recorded"] == 0
    saved_second = GestioneFascicoli(str(fascicoli_db), documents_dir=str(fascicoli_docs), studio_db=studio_db).get(fascicolo.id)
    assert saved_second is not None
    assert len([att for att in saved_second.attivita if att.tipo == TipoAttivita.UDIENZA]) == 1
...
def test_presidio_documentale_file_non_indicizzabile_non_fallisce_job_e_viene_marcato(tmp_path, monkeypatch):
    ...
    fascicoli = GestioneFascicoli(str(fascicoli_db), documents_dir=str(fascicoli_docs), studio_db=studio_db)
    fascicolo = fascicoli.nuovo(
        "Ricorso lavoro con deposito grande",
        TipoFascicolo.LAVORO,
        nome_cliente="Cliente Documento Grande",
        tribunale="Tribunale di Vicenza",
        numero_rg="888",
        anno_rg=2026,
        controparte="MIM",
    ...
    class OversizedDocumentService:
        calls = 0
        ...
        def process_lex_indexing_sources(self, tenant_id, fascicolo_id, sources, user_context, *, retry_errors):
            ...
            return SimpleNamespace(
                indexed=0,
                skipped=0,
                errors=[f"{sources[0].filename}: File superiore al limite configurato per i documenti AI."],
            )

        def list_fascicolo_documents(self, tenant_id, fascicolo_id, user_context):
            return []

    service = OversizedDocumentService()
    monkeypatch.setattr(repo, "_document_ai_service_for_fascicoli", lambda manager: service)
    ...
    assert '"reason":"file_superiore_limite_documenti_ai"' in checked[0]
