...
**Il ciclo non si spezza.** Un giro che fallisce non fa sparire il fascicolo: lo lascia dichiarato in errore con il motivo, e il giro successivo riprende. Un evento che non arriva non lo lascia indietro per sempre: un ricontrollo periodico, ogni ventiquattro ore, riesamina anche i fascicoli fermi. Il pannello del registro non sparisce più per colpa di un passo accessorio, e l'endpoint riporta il motivo tecnico reale invece di «non disponibile».

**L'archivio consegna ai presìdi e tiene il conto.** Nuova tabella `letture_consegne` (SQLite e PostgreSQL gemelli): per ogni fatto e per ogni presidio, che cosa l'archivio ha dato e che cosa il presidio ne ha fatto — **consegnato** con il riferimento della riga creata, **non pertinente**, **rifiutato** con il motivo. Un fatto consegnato non viene più riproposto: niente doppioni. Il registro dei presìdi (`pct/archivio_letture/distribuzione.py`) dichiara chi consuma che cosa: scadenziario i termini, agenda le udienze, presidio notifiche le prove, presidio economico gli importi, dati del fascicolo il numero di ruolo. Si consegnano solo i fatti verificati o corretti dall'avvocato: un fatto soltanto plausibile si chiede, non si scrive.

**Gli importi economici diventano fatti letti una volta sola.** La categoria «importo» era dichiarata e nessun motore la produceva: contributo unificato, compenso liquidato dal giudice, spese ed esborsi, fondo spese venivano estratti aprendo i PDF dal disco a ogni richiesta dell'avvocato. Ora li legge il motore documenti (`pct/archivio_letture/estrazione_importi.py`), **riusando le regole già dichiarate** in `pct/fascicolo_sentenza_economica.py` invece di riscriverle, e il collaudo li verifica: un importo che coincide con un pagamento registrato o con una parcella è verificato, uno letto otticamente senza riscontro resta da confermare. Il presidio economico consulta l'archivio prima di aprire qualsiasi file. Base normativa: D.P.R. 115/2002 art. 13; D.M. 55/2014; art. 91 c.p.c.

**Niente OCR dentro la richiesta dell'avvocato.** Le due funzioni che indicizzavano in modo sincrono nel tempo di risposta ora, dentro una richiesta HTTP, usano solo ciò che è già indicizzato e chiedono ai motori di leggere il resto in sfondo. Fuori da una richiesta — scheduler, worker, thread — lavorano come prima.

**Tre dati che i motori leggevano e nessuno usava.** La prossima scadenza dedotta dai documenti, che gira nella lista dei fascicoli, chiamava l'analisi **senza** i fatti dell'archivio: ora l'archivio viene prima. Le prove di notifica riconosciute dentro un allegato PEC e le comunicazioni di cancelleria riconosciute nel contenuto venivano scartate: ora valgono. Il numero di ruolo letto in un provvedimento viene proposto al fascicolo che diceva «RG da acquisire», dichiarando che è una lettura da confermare. Le udienze e i termini dell'archivio entrano nella cronologia della Lettura del fascicolo. Gli **eventi processuali** che le PEC comunicano — rinvio d'ufficio, fissazione d'udienza, deposito del provvedimento — erano un numero nel riassunto e nient'altro: ora il motore PEC allega a ciascuno il giorno di ricezione certificato (D.P.R. 68/2005 art. 6) ed entrano in cronologia con la loro data, l'origine e il verdetto del collaudo. Nessuna categoria prodotta dai motori resta senza un presidio che la usa, ed è un test a garantirlo: un motore che legge un dato che nessuno mostra lavora per niente.

**Autocontrollo sui dati veri.** `scripts/verifica_catena_letture.py` si esegue sul server e riporta lo stato reale della catena sui fascicoli dello studio: se il registro si apre, dove sta il ciclo di ogni fascicolo e perché, che cosa hanno scritto i motori per categoria e verdetto, quanto ogni presidio ha preso e quanto gli resta, quali categorie nessun presidio usa. Non è una prova con dati inventati: apre l'archivio dello studio e dice che cosa c'è dentro.
...
**Il dizionario italiano viene usato davvero.** Il correttore leggeva il solo `.dic` e si fermava alle radici: `udienza/Q` non contiene «udienze», che nasce dalla regola `SFX Q a e [^gc]a` del file `.aff`. Ora `legal_ocr/lessico/hunspell.py` usa entrambi i file come li usa Hunspell — codifica dichiarata, forma delle sigle, regole di suffisso e prefisso — e verifica una parola applicando le regole al contrario, senza espandere il dizionario in memoria. Sul pacchetto Debian reale: 95.209 radici, 2.358 suffissi, 408 prefissi in meno di un secondo; una pagina d'atto corretta in cinque millesimi. L'immagine installa anche l'eseguibile `hunspell`, utile a verificare il dizionario dal container.

Test: ciclo delle letture 12, dizionario Hunspell 18, correttore del lessico 33, catena nell'app 3 (il ciclo si ferma e non riapre nulla, un documento nuovo lo risveglia e lo richiude, un giro fallito resta in errore e riprende).

## 2.318.1 - 16/09/2026

**Il registro delle letture non sparisce più per colpa di un passo accessorio.** In produzione il pannello «Letture e verifiche» diceva soltanto «Registro delle letture non disponibile», senza motivo: una singola eccezione in uno dei passi accessori (PEC collegate, allineamento del presidio PEC, verifica delle date lette dalle PEC, novità dall'ultima apertura, anomalie, archivio) faceva cadere l'intera risposta. Ora ogni passo è isolato: quello che non riesce si dichiara nel campo `degradato` con il proprio motivo, il resto del registro resta visibile, e l'endpoint riporta il motivo tecnico reale invece della frase generica. Un guasto si legge, non si indovina.

**Il dizionario italiano viene usato davvero.** Il correttore di lettura leggeva il solo `it_IT.dic` e si fermava alle radici: un dizionario Hunspell però non è un elenco di parole, è un elenco di radici con le sigle delle regole che le flettono (`udienza/Q` non contiene «udienze», che nasce dalla regola `SFX Q a e [^gc]a` dichiarata nel `.aff`). Il nuovo `legal_ocr/lessico/hunspell.py` usa **entrambi** i file come li usa Hunspell: dal `.aff` la codifica dichiarata (`SET`), la forma delle sigle (`FLAG`) e le regole di suffisso e prefisso; dal `.dic` le radici con le loro sigle. La verifica non espande il dizionario in memoria: applica le regole al contrario, come fa Hunspell. Misurato sul pacchetto Debian `hunspell-it` reale: 95.209 radici, 2.358 suffissi e 408 prefissi caricati in meno di un secondo, «udienze», «ricorrenti», «notificati», «impugnazioni» riconosciute, una pagina d'atto corretta in 5 millesimi di secondo. L'immagine installa ora anche l'eseguibile `hunspell`, che serve a verificare il dizionario dal container, non al codice. Senza dizionario installato nulla si blocca: restano le parole dichiarate nel repository.

**Presìdi: tre dati letti dai motori che nessuno usava.** La prossima scadenza dedotta dai documenti — che gira nella lista dei fascicoli, cioè nel percorso caldo dell'avvocato — chiamava l'analisi documentale **senza** i fatti dell'archivio: ogni documento ricadeva nel parsing immediato e le date già lette e collaudate dai due motori venivano ignorate e ricalcolate. Ora l'archivio viene prima. Il presidio notifiche scartava le prove riconosciute dentro un allegato PEC (relate e ricevute che arrivano dentro un messaggio invece che come documento) e le comunicazioni di cancelleria riconosciute nel contenuto: ora valgono come le altre.

**Difetto di scrittura.** La lacuna delle conferme ripeteva la data quando l'etichetta del fatto la portava già: «termine del 22/11/2026 del 22/11/2026».

Test: dizionario Hunspell 18 (con il dizionario reale dove installato). Ricognizione completa dei presìdi contro l'archivio: risultati e lacune registrati come attività aperte.

## 2.318.0 - 16/09/2026

**Presidio del fascicolo: la busta si valida quando si deposita, come nel deposito reale.** Il presidio applicava a ogni fascicolo i controlli della busta telematica — PDF/A ex art. 12 D.M. 44/2011, firma CAdES, limiti dimensionali delle specifiche DGSIA, codice oggetto PST — anche quando nessuno stava depositando: un fascicolo importato dal portale, che contiene copie di atti già depositati da altri, risultava «Bloccato da requisiti mancanti» con «Converti il documento e ripeti la verifica» su documenti che non devono essere convertiti. Ora la **fase di deposito** è dichiarata (`pct/practice_engine/fase_deposito.py`): *preparazione* quando l'avvocato ha scelto i documenti della busta o il tipo di deposito telematico, *in corso* quando la busta è stata inviata e si attendono le ricevute, *non richiesta* negli altri casi (procedura non depositabile, fascicolo definito o archiviato, deposito già eseguito, fascicolo importato dal portale, nessuna busta preparata). Fuori dalla fase di deposito i controlli della busta non si eseguono e non bloccano: restano dichiarati come rinviati, con il motivo in italiano, e i documenti richiesti risultano «collegato» invece di «da correggere». In fase di deposito valgono esattamente come nel percorso di deposito, sugli stessi documenti e con gli stessi messaggi.

**La percentuale di completamento dice la verità.** Restava al 33% anche a fascicolo in ordine, per due difetti: le voci obbligatorie della checklist non erano collegate ad alcun controllo e quindi non diventavano mai «completate» (`("COMPLETATO" if not item.required else "DA_COMPLETARE")`), e i blocchi venivano cercati per una chiave che non corrispondeva mai a quella dei controlli. Ora ogni voce dichiara i propri controlli (`pct/practice_engine/voci_checklist.py`: «Codice oggetto PST ufficiale» → `codice_oggetto_pst_valido`; «Firma digitale e busta ministeriale» → firma, busta generabile, limiti, DatiAtto.xml; «Verifica dati cliente e parti» → cliente, codice fiscale, ufficio, registro) ed è completata quando i suoi controlli sono verdi, bloccata con il messaggio del controllo che blocca, rinviata quando riguarda la busta e non si sta depositando. La percentuale (`pct/practice_engine/completamento.py`) conta solo le verifiche che il software può misurare ora e si spiega: «2 verifiche superate su 2», con il dettaglio di quante voci sono misurate, quante rinviate e quante restano all'avvocato.

**Il deposito inviato e accettato non risulta più «non inviato».** Lo stato del deposito veniva dedotto dalla sola sessione della regia: senza sessione diceva «Non inviato» anche a fronte di un deposito registrato nel fascicolo e accettato dalla cancelleria. Ora il presidio legge i depositi reali (`depositi_pct`) e ne riporta lo stato con l'etichetta del flusso ufficiale PCT: accettato dal gestore PEC, consegnato al sistema ministeriale, controlli automatici (superati, con avvisi, con errori), accettato o rifiutato dalla cancelleria. Lo stato operativo del fascicolo segue il deposito reale, i motivi di blocco non si ripetono e le etichette tecniche hanno il loro nome italiano nell'interfaccia.

**Un riferimento normativo non è mai una data.** «l. 69/2023» finiva nello scadenziario come 1° giugno 2023, perché la «l» di «legge» veniva letta come un 1. Le abbreviazioni forensi sono ora dichiarate in un punto solo (`legal_ocr/formulario/riferimenti_normativi.py`: leggi e decreti, articoli e commi, numerazioni, provvedimenti, numeri di ruolo, fonti europee, protocolli e versioni): dentro un riferimento normativo il formulario non corregge nulla e il lettore non trova date. Una data annunciata da una preposizione — «del 10/11/2026», «in data 05/09/2026», «lì 20/09/2026» — resta una data. L'ancoraggio non attraversa più il punto fermo: un'ancora negativa della frase precedente non nega più la data della frase successiva.

**Correttore di lettura governato: lessico forense e vocabolario italiano.** «cornparsa», «istan2a», «u1teriore», «perentorìo» non esistono in italiano; «comparsa», «istanza», «ulteriore», «perentorio» sì. Il nuovo correttore (`legal_ocr/lessico/`) cambia una parola solo quando la parola letta non è nel lessico e **una sola** parola del lessico si ottiene applicando le confusioni già dichiarate (cifre lette al posto di lettere, coppie di segni fuse — rn/m, cl/d, ii/n, vv/w —, accento in mezzo alla parola, che l'italiano non usa): se le parole candidate sono due non si corregge, perché scegliere fra due parole esistenti sarebbe interpretare il testo. Il lessico è dichiarato: forense (`forense.py`, dal linguaggio dei codici e delle leggi che il gestionale già cita) e italiano (`italiano.py`, nucleo dichiarato più il dizionario `hunspell-it` installato nell'immagine; senza dizionario di sistema il correttore funziona su meno parole e nulla si blocca). Non si tocca nulla dentro un riferimento normativo, un numero, una data o un codice. La regola `lessico.parola.v1` è l'ultima del formulario, che passa alla v3: i documenti già letti tornano in coda ai motori, come previsto dal registro delle letture.

**Riconvalida: ciò che è registrato torna alle regole di oggi.** Le anomalie aperte e i fatti già registrati con regole più larghe restavano a chiedere conferme che il software oggi non chiederebbe. A ogni giro di lettura le anomalie aperte si ripassano con le regole correnti: quelle che oggi non nascerebbero — «l. 69/2023» segnalato come data — si chiudono da sole, con il motivo che dichiara la regola superata e l'autore «riconvalida automatica»; la riga resta nel registro, la chiusura è tracciata e non cancellata. I fatti dell'archivio che oggi si riconoscono come riferimenti normativi vengono respinti, con la prova che lo dice. Le decisioni dell'avvocato (confermata, corretta, ignorata) non si toccano mai.

**Conferme mirate e nessun oggetto in attesa per sempre.** Il riquadro chiedeva conferma per dodici date indistinte; ora chiede solo quelle che, confermate, cambiano l'agenda o lo scadenziario: udienze, termini e costituzioni **future**, una volta sola per data. Una data già passata non si chiede: l'udienza si è tenuta, il termine è scaduto. Le lacune della Lettura del fascicolo dicono *quali* date attendono conferma e *quale* oggetto la lettura automatica deve ancora leggere, con il motivo. Un oggetto dell'inventario che non corrisponde più a un documento o a una PEC collegata veniva saltato a ogni giro e restava «da leggere» per sempre: ora la sua lettura si chiude come non leggibile, con il motivo, e la lettura automatica può risultare completa.

**Difetto latente corretto.** Il motore di lettura teneva in cache la lingua verificata usando l'identificativo dell'oggetto adattatore: raccolto quello dal garbage collector, l'identificativo veniva riassegnato a un adattatore nuovo, che saltava il controllo del dizionario italiano. Ora la cache conserva anche l'oggetto, e il controllo non si salta.

Test: presidio del fascicolo (22 nuovi: fase di deposito, stato reale del deposito, voci e controlli, percentuale spiegata, blocchi solo in deposito), riconvalida e conferme mirate (10), correttore del lessico (33), riferimenti normativi e ancoraggio (+22 casi), API della regia (+1).

## 2.317.1 - 16/09/2026

**Catalogo: la carta d'identità del cliente si riconosce come tale.** «Carta d'identità.PDF» restava «da verificare» per due ragioni: il catalogo riconosceva un documento d'identità solo se una riga d'intestazione combaciava con «carta d'identità», cosa che l'OCR di una scansione (etichette sparse, «IDENTITA'» con apostrofo, doppia lingua, zona a lettura ottica) quasi mai produce; e senza testo leggibile il nome del file veniva ignorato per principio. Ora la scansione di un documento di riconoscimento — carta d'identità cartacea o elettronica, passaporto, patente, permesso di soggiorno — si riconosce dai suoi campi (`pct/document_intelligence/catalog_identita_personale.py`: tipo del documento più almeno due fra cognome, nome, nascita, cittadinanza, residenza, scadenza, statura, autorità emittente, codice fiscale, riga a lettura ottica), e quando cognome e nome del cliente del fascicolo compaiono nel testo il catalogo lo intesta: «Carta d'identità di Anna Bianchi», confidenza 99, sezione allegati. Un atto che allega o cita la carta d'identità (procura, ricorso, dichiarazione) resta un atto. Unica eccezione dichiarata alla regola «il nome del file non cataloga»: un nome inequivoco di documento di riconoscimento («Carta d'identità.PDF», «CIE», «Passaporto», «Patente») viene proposto come tale anche prima che il contenuto sia leggibile, con confidenza 78 e l'evidenza che lo dice; il contenuto letto poi conferma o smentisce. Il cliente del fascicolo entra nel contesto del catalogo. Base normativa: art. 35 D.P.R. 445/2000; D.M. 23 dicembre 2015 (CIE). Catalogo v26: i documenti già catalogati vengono ricatalogati dal registro delle letture al prossimo giro. Test: 5 nuovi.

## 2.317.0 - 16/09/2026

**Archivio di alimentazione: due motori leggono, i presìdi consultano.** I dati letti dai documenti e dalle PEC del fascicolo — date con il loro campo (udienza, termine, costituzione, notifica, deposito, accettazione, consegna, comunicazione, provvedimento, data dell'atto), numeri di ruolo, prove di notifica (relata, ricevuta di accettazione, ricevuta di avvenuta consegna, attestazione di conformità, atto notificato, comunicazione di cancelleria), eventi — ora entrano in un archivio unico (`letture_fatti`, nello stesso database del registro delle letture, schema SQLite/PostgreSQL gemello) attraverso due soli motori (`pct/archivio_letture/`): il **motore documenti** legge il testo di ogni documento (nativo dal PDF, OCR, indice documentale) e il **motore PEC** traduce ciò che il presidio PEC ha già letto (udienze, termini, eventi, ricevute riconosciute dall'oggetto certificato dal gestore) e legge gli allegati. I presìdi non rileggono più: il **presidio documentale** prende udienze e termini dall'archivio (anche quando l'indice documentale non ha ancora il testo), il **presidio notifiche** riconosce relata, ricevute e atto notificato dal contenuto del documento e non solo dal nome del file, la **Lettura del fascicolo** ha la sezione «archivio» (udienze, termini, prove, ruoli, dati da confermare, stato della lettura automatica, collaudo) e le lacune la citano.

**Il software prova ogni dato, non l'avvocato.** Ogni fatto passa dal **collaudo automatico** con prove registrate: calendario, forma del token (quanti segni ha corretto il formulario), ancoraggio al contesto, orizzonte del fascicolo, **doppia lettura** (la stessa data in una seconda estrazione indipendente dello stesso documento: testo nativo e OCR, o indice) e **concordanza** con agenda, scadenziario, PEC del presidio e portale. Il verdetto è `verificata` (riscontro indipendente, o origine non ottica senza correzioni), `plausibile` (ben formata e ancorata ma senza riscontro) o `respinta` (smentita: resta nell'archivio con le prove, non si propone a nessuno). Solo le udienze e i termini plausibili vengono chiesti in conferma nel riquadro «Letture e verifiche» («È giusta», «Correggi», «Ignora», `POST …/letture/fatti/<id>`); le decisioni dell'avvocato sopravvivono alle riletture dello stesso contenuto e la decisione presa sul pannello delle anomalie si propaga ai fatti. Un fatto plausibile viene ricollaudato a ogni giro con le date note di oggi: quando l'agenda o una PEC lo confermano, diventa verificato da solo.

**Le date sono fatti solo se sono date.** Il formulario corregge O/l/S/B/Z dentro un token a forma di data solo se ne esce una data vera del calendario con anno a quattro cifre, almeno metà dei segni già cifre, al massimo una correzione per giorno e per mese e due nell'anno: «1O/O3/2O26» e «1S/O8/2O2S» diventano date, «SOS/OS/OO», «IS/OB/ZOZS», «l/S/BZ», «prot. 12/3/26», «vers. 1.2.34» e «30/02/2026» no, mai. Una data è un fatto solo se il testo dice che cos'è (**ancoraggio**: «udienza del», «entro il», «notificato il», «ricevuta di avvenuta consegna … ore»); un'ancora vale per la prima data che la segue e non per una tabella di date; date di nascita, documenti d'identità, protocolli, versioni, periodi «dal…al» e date di leggi o decreti citati sono ancore negative. Il presidio documentale usa la stessa estrazione: non propone nulla che l'archivio non proporrebbe, e le date in tabella o senza contesto non compaiono più come termini. `date_nel_testo` restituisce solo date vere: nessuna anomalia inutile.

**Lettura automatica e mantenimento.** Nuovo job «Lettura automatica dei fascicoli» (ogni 10 minuti, prima i fascicoli aperti, tetto di oggetti per giro): chiede al registro delle letture che cosa i motori non hanno ancora letto e legge solo quello; a fascicolo invariato non apre alcun file. Il caricamento o la sostituzione di un documento e il collegamento di una PEC fanno leggere l'oggetto nuovo in un thread di sfondo, mai nella richiesta; il worker OCR alimenta l'archivio da solo a testo pronto; le verifiche automatiche all'apertura della Lettura e «Leggi i nuovi» chiamano i motori. Aprire un fascicolo non aspetta più nessuna lettura.

**Collaudo del lettore.** Un corpus di pagine di prova (`legal_ocr/collaudo/`: decreto di fissazione dell'udienza con termine e notifica; relata con dati anagrafici, documento d'identità e ricevute; comunicazione di cancelleria con tabella di date, protocollo, versione e periodo) viene renderizzato e letto con l'OCR reale (Tesseract e formulario) a ogni esecuzione dei test e ogni notte (job «Collaudo del lettore», 04:10): i motori devono trovare tutti i dati attesi e nessuna trappola. L'esito compare nel riquadro «Letture e verifiche»; se il collaudo non passa, il pannello lo dice. Il collaudo ha già corretto tre difetti dell'ancoraggio prima del rilascio (la data di una legge presa per notifica, l'ancora presa dalla riga successiva, la variante «lì'»).

Test: motori e collaudo (8), catena completa nell'app (4: documento → archivio → presìdi con seconda passata che non rilegge nulla; PEC → archivio con concordanza fra i due motori; endpoint; caricamento che fa leggere solo il documento nuovo), collaudo del lettore (3, con OCR reale dove c'è Tesseract), formulario (+12 casi negativi e posizioni), JS (+2). Documentazione: `docs/REGISTRO_LETTURE.md`, matrice storage, job incrementali, regole AGENTS/CLAUDE.

## 2.316.0 - 15/09/2026

**Registro delle letture: il gestionale sa che cosa ha già letto e non lo rilegge.** Fino a oggi ogni lettore decideva da solo, e alcuni rileggevano sempre: l'indice documentale decifrava e ricalcolava l'impronta di tutti i documenti a ogni apertura del fascicolo, la Lettura del fascicolo rilanciava l'indicizzazione completa a ogni richiesta, la proforma automatica rileggeva i PDF dal disco ogni quindici minuti, la coda OCR accettava di nuovo documenti già letti. Ora esiste un registro unico per studio (`pct/registro_letture/`, `intelligence/registro_letture.db`, schema SQLite e PostgreSQL gemelli): l'**inventario** del fascicolo — documenti, PEC collegate, allegati PEC — con l'impronta SHA-256 del contenuto e le chiavi di associazione (nome e cognome del cliente, numero e anno di ruolo); le **letture** per oggetto e per lettore (testo e ricerca, indice documentale, catalogo, assistente locale, presidio PEC, presidio economico, proforma automatica, lettura del fascicolo) con versione del lettore e stato; l'**impronta del fascicolo** per lettore, che ferma il lettore prima ancora di aprire un file se nulla è cambiato; le **viste** per utente. Un documento invariato non si rilegge; cambia l'hash, si rilegge solo quello; cambia il motore di un lettore, torna da leggere tutto per quel lettore soltanto. Documenti caricati, sostituiti dall'editor, rinominati, ripristinati, cestinati o eliminati e PEC collegate aggiornano l'inventario e invalidano la lettura in cache. L'indice documentale non decifra più i documenti di cui il registro conosce l'impronta in chiaro (la impara alla prima lettura) e non elabora un fascicolo invariato; la coda OCR rifiuta i documenti già letti; il presidio PEC riusa il testo di un allegato identico già letto nello studio invece di rifare l'OCR; l'analisi delle sentenze deduplica per impronta e carica i testi solo se c'è qualcosa da analizzare; la proforma automatica non rilegge un fascicolo invariato; l'assistente locale e il presidio economico registrano i propri esiti nello stesso registro.

**Verifica delle letture: le date lette si controllano, non si prendono per buone.** Ogni data estratta da documenti e PEC passa da controlli deterministici con codice e motivo in italiano: data inesistente nel calendario (31/02), giorno e mese invertiti, anno oltre tre anni nel futuro o troppo remoto rispetto all'anno di ruolo, data di un termine o di un'udienza prima dell'apertura del fascicolo o prima della PEC che la comunica, incoerenza con la data del portale, lettere lette al posto delle cifre. Le anomalie finiscono nel registro e nel nuovo riquadro **«Letture e verifiche»** della Lettura del fascicolo, con «È giusto», «Correggi» (anche con il valore proposto dal controllo) e «Ignora»; le correzioni dell'avvocato prevalgono sul valore letto nel presidio documentale. Lo stesso riquadro mostra per ogni lettore quanti oggetti sono letti e quanti da leggere, quali documenti aspettano quale lettore, le **novità dall'ultima apertura** di chi guarda («nuovi 1 documento e 1 PEC · cambiato 1 documento») e il pulsante **«Leggi i nuovi»**, che accoda solo ciò che manca. Endpoint `GET/POST /api/v1/ui/fascicoli/<id>/letture[...]`.

**Confusioni tipiche dell'OCR dichiarate una volta sola.** `legal_ocr/formulario/confusioni.py` è la tabella unica delle confusioni del lettore ottico (O/0, l/1, I/1, S/5, B/8, Z/2 e le altre) usata da tutte le regole; la correzione avviene solo dove la struttura dice che cosa deve esserci: date numeriche e ISO, **date per esteso** («1O rnarzo 2O26» → 10 marzo 2026, mesi abbreviati o letti male, «primo»/«1º»), **orari** («ore 9.30» → ore 9:30), importi e numeri di ruolo, **codice fiscale** (posizioni lettera/cifra e carattere di controllo verificato), **partita IVA**, **CAP**, **IBAN**, **parole** con cifre dentro («R0MA», «MILAN0», «Mi1ano»; «TRIBUNA1E» sciolto con il lessico degli atti). Formulario 2026.09.15 v2.

**Fonti.** Il riferimento al D.P.C.M. 40/2016 (PAT), rimasto senza testo il 14/09, ora cita letteralmente il documento ufficiale della Giustizia amministrativa sulle nuove regole tecnico-operative (d.P.C.S. 9 maggio 2025, deposito con Formweb e moduli PEC) conservato nel repository con impronta. Normattiva carica il testo degli articoli solo via JavaScript con sessione, quindi in questa release non sono state aggiunte altre norme: nessuna fonte viene citata senza il testo consultato.

Test: 12 sul registro, 6 sul runtime e sugli endpoint, 8 nuovi sul formulario, 4 in JavaScript; contratto di parità SQLite/PostgreSQL; documentazione in `docs/REGISTRO_LETTURE.md`.

## 2.315.0 - 14/09/2026

**Un solo motore di lettura del testo (OCR), legal-grade, per tutto il gestionale.** Sei implementazioni diverse — fascicolo, editor professionale, indice di ricerca, pipeline probatoria, presidio PEC, lettore delle notifiche — leggevano ognuna a modo suo. Ora esiste un motore solo, `legal_ocr/motore`, e tutte ci passano: `runtime` (dove sta Tesseract, dizionari, un thread per processo), `immagine` (raddrizzamento, luce uniforme, ritaglio del bordo, densità a 300 dpi), `lettura` (la strategia: prima passata con l'impostazione giusta per un atto; se è sicura ci si ferma, altrimenti le altre impostazioni in parallelo e, per le copie sbiadite, la pagina binarizzata), `consenso` (il **secondo lettore**: PDF Inspector, rete PP-OCR su ONNX già nel container di produzione, letto in parallelo e usato per confermare o correggere le sole parole di cui Tesseract è incerto), `pagina` (impaginazione, formato, correzioni, riferimenti) e `testo` (solo testo, per indice, Lex, editor e lettori automatici). Misurato su una pagina A4 a 300 dpi: Tesseract legge in circa un secondo per passata con un thread per processo (prima le passate in parallelo saturavano le CPU e una pagina costava decine di secondi), con lettura al 100% su testo pulito e al 99,95% su pagina ruotata e sfocata; il secondo lettore al 99,7-100% in circa un secondo. Se il dizionario italiano manca, il motore se lo procura da solo dalla fonte ufficiale (`tessdata_fast` 4.1.0, la stessa del pacchetto Debian di produzione) e lo usa solo se l'impronta SHA-256 corrisponde; su Windows lo script `scripts/installa_tesseract_windows.ps1` installa Tesseract con winget e il dizionario verificato.

**Formulario legale di post-lettura (`legal_ocr/formulario`).** Regole deterministiche, ognuna con identificativo, etichetta e motivo, dichiarate all'avvocato con il numero di occorrenze: **caratteri** (legature ﬁ/ﬂ, apostrofi letti come accenti, virgolette, grado, caratteri invisibili, composizione Unicode), **abbreviazioni** (art./artt. con spazio e punto, c.p.c., c.p.p., c.c., c.p., c.p.a., disp. att., D.Lgs., D.P.R., D.P.C.M., D.M., D.L., R.D., L., R.G., R.G.N.R., R.G.A.C., N.R.G., P.Q.M., C.T.U., C.T.P., G.I.P., G.U.P., U.N.E.P., G.d.P., PEC, P.IVA, C.F., S.p.A., S.r.l., S.r.l.s., S.a.s., S.n.c., Sez. Un., Corte cost., punto delle abbreviazioni avv., dott., sig., lett., cfr., pag., all., doc., ecc.), **punteggiatura** (elisioni «l' atto» → «l'atto», spazi prima e dopo i segni, parentesi, indirizzi PEC spezzati), **cifre** (O e l al posto di 0 e 1 in date, importi, numeri di ruolo, anni e dopo art./n./co.), **numeri romani** (Sez. Vl → VI, comma lll → III, Titolo lV → IV, con validazione della forma), **euro** (€ al posto di E/C/EUR/Euro davanti all'importo, spazi e decimali all'italiana: «€ 1.250,00»), **accenti** secondo l'ortografia (perché/poiché/né/sé con l'acuto, è/cioè/caffè con il grave; a, i, o, u finali sempre gravi; «E'» → «È»; i troncamenti po', va', da', sta' non si toccano; accento reintegrato su parole che senza accento non esistono: società, responsabilità, città, così, più, già, può, lunedì…), **elenchi** (marcatori puntati, numerati, per lettere, romani e a livelli, con la continuità dell'elenco: «c.» è una voce solo dopo «b.», «i)» è la lettera i dopo «h)» e il romano I in apertura). La pipeline probatoria usa le stesse regole con testo prima/dopo per ogni intervento.

**Formattazione migliore.** L'impaginazione riconosce le voci di elenco con il loro marcatore e tiene insieme le righe di continuazione rientrate; il numero di pagina in testa o in coda esce dal testo; il capoverso a filo di entrambi i margini è **giustificato**; nel documento le voci consecutive dello stesso elenco diventano un solo elenco con il tipo (numeri, lettere, romani) e il numero di partenza dell'atto, sia in Word (segno scritto con rientro sporgente quando Word non sa numerarlo) sia in PDF. La revisione mostra tipo di elenco, numero di pagina (escluso dal documento, ripristinabile) e il pulsante «Giustifica»; le correzioni portano l'etichetta dichiarata dal server; il riepilogo dice se il secondo lettore ha confermato parole.

**Utility «Da scansione a Word o PDF» in Strumenti forensi (categoria Utility).** Un file (PDF, immagine, o più immagini della stessa scansione, unite in un solo documento), la destinazione — sul computer o **nel fascicolo di un cliente**, cercato per nome e cognome con i fascicoli mostrati con numero, oggetto e **stato** — la lettura con avanzamento per pagina (due pagine per volta), la revisione accanto alla pagina, la scelta del formato (**Word** modificabile o **PDF** del testo corretto) e la **conferma esplicita** prima del salvataggio; nel fascicolo il documento entra come atto nuovo con collegamento all'editor. L'endpoint del documento riconosciuto accetta `formato=docx|pdf`.

Test: 40 sul formulario, 13 sul motore (strategia, binarizzazione, dizionario mancante, consenso, elenchi, rientri, numero di pagina, giustificato, provisioning con impronta), 4 sulla utility, 3 in JavaScript sui blocchi; tutte le suite OCR esistenti (fascicolo, editor, indice, pipeline probatoria, PDF inspector, PEC) passano sul motore unico.
...
**Conoscenza procedurale versionata (`pct/procedura_fasi`).** Sessanta fonti in un registro, ognuna con norma, titolo, URL ufficiale, data di consultazione ed estratto letterale del testo vigente (Normattiva, Portale dei Servizi Telematici, Specifiche tecniche DGSIA 7/8/2024). Su quel registro poggiano le schede: **depositi** per ognuno dei quattro canali del software — PCT civile (art. 196-quater e 196-sexies disp. att. c.p.c., D.M. 44/2011 art. 13, Specifiche art. 17: ricevuta di avvenuta consegna come momento del deposito, WARN/ERROR/FATAL, PEC di rifiuto o di avvenuto deposito), PDP penale (art. 111-bis c.p.p., Specifiche art. 19: ricevuta anno/numero, INVIATO, IN TRANSITO, ACCETTATO, IN VERIFICA, RIFIUTATO, ERRORE TECNICO), PAT amministrativo (c.p.a. artt. 29, 45, 46 e 92), PTT tributario; **notifiche** per ogni canale — PEC in proprio (L. 53/1994 artt. 3-bis e 9, art. 147 c.p.c.: senza limiti orari, perfezionata per il notificante con la RAC e per il destinatario con la RdAC, alle ore 7 se fra le 21 e le 7; pubblici elenchi art. 16-ter D.L. 179/2012; deposito della prova nella busta, Specifiche art. 26), UNEP, posta (L. 890/1982 art. 8: deposito entro due giorni lavorativi, compiuta giacenza a dieci giorni), comunicazioni di cancelleria (art. 136 c.p.c., D.M. 44/2011 art. 16, Specifiche art. 21); **riti** — ordinario Cartabia (163-bis, 165, 166, 171-bis, 171-ter, 183, 189, 325, 327), semplificato (281-undecies), lavoro (414-416, 435), decreto ingiuntivo (641, 644, 647), esecuzione (480, 481, 497, 518, 543, 557, 615, 617), cautelare (669-terdecies), mediazione (D.Lgs. 28/2010 artt. 5, 6, 8), amministrativo, tributario, penale (415-bis, 429, 552, 585). Ogni stato dei depositi e dei presìdi notifica è ricondotto alla sua fase, senza toccare le macchine a stati esistenti; ogni termine con un template del motore dei termini lo cita, e la lettura rimanda al calcolo. Verificando le fonti è emerso che **gli artt. 18-23 del D.Lgs. 546/1992 sono abrogati dal D.Lgs. 175/2024, Testo unico della giustizia tributaria** (mod. D.L. 200/2025): il processo tributario ora cita gli artt. 61, 64, 67, 68 e 69 del testo unico, e le fonti tributarie del catalogo documentale sono state aggiornate di conseguenza. L'art. 16-bis del D.L. 179/2012, anch'esso abrogato dalla riforma Cartabia, non è più citato: il momento del deposito è nell'art. 196-sexies disp. att. c.p.c. Il D.P.C.M. 40/2016 (PAT) è citato come riferimento senza testo, perché il sito della giustizia amministrativa non era raggiungibile: la scheda lo dice.

**Lettura del fascicolo, completa dei presìdi.** Oltre a documenti, depositi, notifiche e scadenze, la lettura ora legge il **presidio PEC** — i messaggi collegati al fascicolo, quelli che citano il numero di ruolo e quelli che citano nome e cognome dell'assistito, con la corrispondenza dichiarata, gli eventi, i termini e le udienze che il presidio ne ha estratto e non sono ancora nello scadenziario o in agenda — e il **presidio economico con gli importi**: preventivato, conferito, fatturato, incassato, saldo aperto, parcelle scadute con numero e importo. Il **presidio del fascicolo** entra con il profilo da confermare e i blocchi della regia. I **prossimi passaggi** nascono da tutti i presìdi, in ordine di urgenza, ognuno con la ragione, la **norma** (dal registro verificato), il punto in cui agire nella pagina e, quando il termine è calcolabile, il rimando al calcolo. Un fascicolo **definito, pronto per l'archivio o archiviato non ha passaggi**: la lettura lo dichiara invece di proporne; un fascicolo sospeso li mostra con l'avvertenza. La lettura carica in poche decine di millisecondi (la conformità completa, che costava mezzo secondo, è sostituita dai dati che la regia calcola già; il quadro economico non fa più la ricerca semantica dei preventivi) e l'endpoint tiene una cache di venti secondi che il pulsante «Aggiorna» salta.

**Le verifiche le fanno i presìdi, l'avvocato decide.** Nessun passo chiede più all'avvocato di «verificare le ricevute», «controllare la PEC» o «confermare la catalogazione»: all'apertura del fascicolo (e poi ogni quindici minuti, o subito con «Aggiorna») un servizio di sfondo esegue le verifiche automatiche del fascicolo — il **presidio PEC** collega al fascicolo i messaggi che citano il numero di ruolo quando il mittente è un ufficio giudiziario (`giustiziacert.it`, `giustizia.it`), con audit e riavvio del presidio sul messaggio, così termini e udienze vengono estratti e registrati; il linker automatico non sovrascrive più un collegamento certificato; le PEC che citano solo l'assistito restano da confermare (omonimie) — il **presidio notifiche** riallinea i presidi aperti alle prove già nel fascicolo — il **presidio depositi** controlla le ricevute con il polling PEC/PDP dello studio — il **presidio documentale** indicizza e cataloga i documenti senza testo come attore di sistema. L'esito è registrato per fascicolo (`intelligence/lettura_verifiche.json`) e mostrato nella lettura, presidio per presidio. All'avvocato restano solo le decisioni: un deposito senza esito da giorni nonostante il controllo automatico (contattare la cancelleria o ripetere), una casella PEC non configurata (senza la quale le ricevute non si leggono), l'autenticità di una PEC con firma non valida, un termine o un evento che il presidio chiede di rivedere, una copia illeggibile da rifornire, la classificazione di documenti che il contenuto non identifica, la procura da acquisire quando tutti i documenti sono stati letti e non c'è, la conferma con un clic delle proposte pronte. Gli atti acquisiti dal fascicolo d'ufficio (PolisWeb/PST) non sono più letti come depositi in attesa di ricevute; le acquisizioni tecniche dei file non compaiono più nella cronologia dei fatti.

**Presidio economico completo, sulla stessa fonte di Fatturazione.** La lettura porta il controllo pagamenti del fascicolo: la **liquidazione disposta dal giudice**, letta dalla sentenza dal controllo economico già esistente (con data, fonte e stato nel presidio), e se quella somma è **davvero arrivata sul conto dello studio** — i pagamenti registrati in Fatturazione con metodo bonifico e data — così la stessa cifra si legge identica nel fascicolo e nella pagina Fatturazione; le anticipazioni da recuperare (contributo unificato, spese), fatturato, incassato, saldo aperto e parcelle scadute con importi. I passi che ne nascono sono decisioni: confermare la liquidazione letta dalla sentenza, riscuotere la somma liquidata non ancora bonificata (o completarne la riscossione), emettere la parcella per un bonifico ricevuto senza fattura, recuperare le anticipazioni.

**Lacune di conoscenza dichiarate.** Se il fascicolo cita un rito senza scheda, un canale di deposito sconosciuto o una norma assente dal registro delle fonti verificate, la lettura lo dichiara con la chiave esatta da acquisire («Conoscenza da completare») invece di dedurre termini: il software non inventa, e dice che cosa manca perché il presidio normativo lo colmi dalle fonti ufficiali.

**Pannello «Lettura del fascicolo».** In cima alla pagina del fascicolo, aperto per primo: fase con le sue prove e la prossima udienza; i prossimi passaggi numerati con urgenza, azione cliccabile, ragione, presidio d'origine, norma (che si apre sulle fonti: titolo, estratto, testo ufficiale) e «Calcola il termine»; sei card dei presìdi — documentazione, depositi, notifiche, PEC, scadenziario, economico — con numeri, colore e collegamento alla sezione; di che cosa tratta, con la domanda citata dall'atto; che cosa è stato fatto; lacune; e, a richiesta, «Fasi, termini e norme del procedimento»: le schede del rito, del canale di deposito e delle notifiche di questa pratica con tutte le fonti. Voce «Lettura» nella navigazione delle sezioni.

**«Conferma tutte le proposte».** Nel pannello Catalogazione documentale un pulsante conferma in blocco le proposte: mostra quante sono, chiede l'attestazione di lettura delle prove, conferma solo le proposte che una prova ce l'hanno (le altre restano proposte e vengono elencate), registra ogni conferma nella revisione del fascicolo come la conferma singola.

Test: 10 sulla conoscenza (fonti con URL ufficiale e data, URN Normattiva, schede che citano solo fonti registrate e template esistenti, termini dichiarati coincidenti con i template del motore, canali del software tutti coperti, fasi PCT/PDP/PEC/posta, testo unico tributario), 6 nuovi sulla lettura (PEC con termini e udienze, fascicolo definito e da archiviare, economico con importi, fasi procedurali, norme delle scadenze), 3 sul presidio PEC per fascicolo (registro SQLite reale), 5 sulle verifiche automatiche (registro, collegamento PEC per ruolo con mittente ufficiale su SQLite reale, orchestrazione con esiti ed errori, avvio dall'endpoint), 3 sulla conferma massiva, 1 sulla cache dell'endpoint, 7 in JavaScript sulle trasformazioni del pannello; i contratti React verificano sezione, pannello, conferma massiva e attestazione.

## 2.312.0 - 14/09/2026

La catalogazione documentale arriva al 100% dei documenti leggibili, e Lex sa leggere il fascicolo: che cosa tratta, che cosa è stato fatto, a che punto siamo, che cosa fare adesso.

**Catalogazione dal titolo, con la norma che definisce ogni atto.** Sul corpus di prova (46 documenti di un fascicolo reale) 17 documenti restavano «da verificare» e almeno 6 erano catalogati con sicurezza ma nella voce sbagliata: la classificazione cercava parole nel corpo del testo, dove «decreto ingiuntivo» compare anche in una comparsa di risposta e «sentenza» in qualunque atto che ne citi una. Ora la prima cosa che si legge è il **titolo dell'atto**: una riga a sé, nelle prime righe del documento (o due-tre righe spezzate dall'OCR, ricomposte), e il titolo vale solo se il corpo lo conferma con la formula che quel tipo di atto contiene per legge. Ogni regola dichiara la norma che definisce quell'atto e la fonte è una pagina di Normattiva consultata oggi: 141 regole in 14 aree — esecuzioni (8), penale (21), tributario (8), amministrativo (11), lavoro (12), famiglia (16), locazioni (9), crisi d'impresa (5), immigrazione (3), civile (21), sinistri (6), atti dello studio (7), comuni (8), generiche (6) — con 46 fonti normative nuove (c.p.c., c.p.p., c.p.a., D.Lgs. 546/1992, L. 241/1990, D.P.R. 602/1973, D.P.R. 445/2000, D.Lgs. 231/2007, codice civile per condominio, successioni, locazioni, L. 392/1978, D.Lgs. 28/2010, D.Lgs. 14/2019, D.Lgs. 286/1998 e altre). Le ricevute del deposito telematico si riconoscono dall'oggetto della PEC prima del contenuto; i messaggi PEC dalla loro intestazione; la lettera di licenziamento dal suo «Oggetto:». Le identità strutturali generiche («Provvedimento dell'ufficio giudiziario») non prevalgono più su un titolo preciso: un decreto che dispone il giudizio o un'intimazione di sfratto ora hanno il loro nome. Risultato sul corpus: 1 documento su 46 resta da verificare, ed è un'immagine senza testo; 141 campioni riconosciuti su 141, nessun falso positivo su un corpus di atti che citano quei titoli senza esserlo. La versione del risolutore è aggiornata: «Aggiorna catalogazione» rilegge i fascicoli esistenti con le regole nuove.

**Lex legge il fascicolo.** Prima, alla domanda «riassumi il fascicolo», Lex rispondeva con un testo generico e il modello non vedeva né i depositi né le notifiche. Ora la lettura è costruita dai dati, non dal modello, e ha una prova per ogni frase: **quadro** (parti, ufficio, ruolo, valore), **di che cosa tratta** (l'oggetto dichiarato e la domanda citata testualmente dall'atto principale, dalla formula «chiede/conclude/voglia»), **che cosa è stato fatto** (cronologia di attività, udienze, depositi, notifiche, provvedimenti), **depositi e notifiche** con la fase esatta — un deposito è perfezionato solo con l'accettazione della cancelleria (D.M. 44/2011) e una notifica è provata solo con la ricevuta di avvenuta consegna (L. 53/1994, art. 3-bis) —, **documentazione** dal catalogo, **conformità ed economico**, **a che punto siamo** (la fase la dicono le prove: sentenza catalogata, udienza tenuta, CTU, precetto, non lo stato dichiarato, e le incoerenze si segnalano), **prossimi passaggi** in ordine di urgenza con la ragione di ciascuno e la norma quando c'è (art. 165 c.p.c. per la costituzione, art. 171-ter per le memorie, artt. 325 e 327 per l'impugnazione, art. 13 L. 247/2012 per preventivo e conferimento) e **lacune** dichiarate: ruolo mancante, ricevute mancanti, documenti senza testo. La lettura risponde alla domanda: «a che punto siamo con le notifiche» mostra notifiche e passi, «di cosa tratta» oggetto e documenti, «riassumi» tutto. È la risposta del provider deterministico, entra nel contesto del modello locale come unica fonte sui fatti, è una fonte del recupero evidenze e ha un endpoint proprio (`/api/v1/ui/fascicoli/<id>/lettura`).

Test: 162 sulla catalogazione dal titolo (ogni regola, falsi positivi, titolo spezzato dall'OCR, fonti registrate con URL ufficiale e data), 15 sulla lettura (caso completo, fascicolo vuoto, sentenza → fase decisa con termine d'impugnazione, deposito rifiutato → passo urgente, notifica fallita e senza consegna, scadenza scaduta, fascicolo chiuso, date miste con e senza fuso), 7 sul percorso Lex e sull'endpoint.
...
- **Rilevamento del foglio su telefono.** Il riquadro automatico sbagliava quasi sempre con la luce di una scrivania: ora l'immagine viene prima appiattita nell'illuminazione (si divide per lo sfondo sfocato, così l'ombra dell'angolo non conta più come bordo) e ripulita con un'apertura morfologica che toglie i granelli. Il rilevamento prova tre maschere diverse e sceglie la migliore con un punteggio che pesa il sostegno dei bordi, quanto il quadrilatero è davvero rettangolare e quanta pagina occupa. Le tolleranze geometriche sono state allargate a fogli ripresi di sbieco (angoli fra 32° e 148°, lati fino a 1:6, area dal 10% dell'inquadratura) e la risoluzione di analisi è salita da 320 a 384 px.
- **OCR professionale su tutta la pagina.** Prima del riconoscimento la pagina viene raddrizzata (inclinazione stimata dal profilo di proiezione), uniformata nell'illuminazione, portata ad almeno 300 dpi e messa a fuoco con una maschera di contrasto. Il testo viene poi letto con quattro configurazioni Tesseract diverse (blocco unico, colonne, automatica, testo sparso) e si tiene la lettura con il punteggio migliore, calcolato su confidenza e quantità di testo utile: su un documento a due colonne o con un'intestazione fuori squadro cambia il risultato, non solo la confidenza.
- **La formattazione del documento viene ricostruita, non appiattita.** Il nuovo modulo `legal_ocr/page_layout.py` raggruppa le parole riconosciute in righe e blocchi e riconosce titoli, capoversi, voci di elenco e **tabelle** — le colonne si individuano dalle interruzioni ricorrenti fra le parole di righe consecutive, non da linee disegnate, così regge anche le tabelle senza bordi. Le parole spezzate a fine riga vengono ricomposte. Le zone con inchiostro ma senza testo (firme, timbri, **grafici e immagini**) vengono segnalate come figure con la loro posizione, perché l'avvocato sappia che lì c'è qualcosa che il testo non riporta.
- **Revisione modificabile prima dell'inserimento.** Dopo l'OCR il testo non entra più direttamente nell'atto: si apre una revisione blocco per blocco dove si corregge il testo, si cambia la natura di un blocco che il motore ha classificato male, si modificano le singole celle di una tabella e si eliminano i blocchi inutili. Solo alla conferma il contenuto entra nell'editor, e ci entra come **HTML strutturato** — titoli in grassetto, elenchi puntati, tabelle come tabelle — invece che come un blocco di testo piatto.
- **Ricerca del fascicolo per nome e cognome del cliente.** Al salvataggio del documento acquisito non c'è più un elenco a tendina con tutto l'archivio: si scrive il nome della persona assistita. La ricerca regge l'ordine invertito («Rossi Mario» e «Mario Rossi»), ignora accenti e punteggiatura, riconosce le ragioni sociali, funziona mentre si digita e cerca in subordine anche nel numero e nell'oggetto della pratica. I fascicoli archiviati restano fuori. La regola vive in `pct/ricerca_cliente_fascicolo.py`, fuori dal web, così vale anche per il presidio PEC e per ogni altro punto in cui un documento va ricondotto al fascicolo giusto.
- Nuovo endpoint `GET /api/v1/ui/document-tools/fascicoli`; `POST /ocr-page` restituisce ora anche blocchi, tabelle, figure, confidenza media e passaggi eseguiti.
- 33 nuovi test (`tests/test_ocr_impaginazione.py`, `tests/test_ricerca_cliente_fascicolo.py`).
...
- Aggiornato il frontend alle versioni correnti: Vite 8, TypeScript 7, Tailwind CSS 4 con plugin Vite ufficiale, Lucide 1, React 19.2.8, Radix, shadcn, Storybook e dipendenze collegate. La migrazione conserva la configurazione IUSENTRA esplicita e viene verificata con typecheck, build e prova reale.
- Storybook ora monta tutte le **73 pagine React** nei domini operativi reali e le integra con il design system: la suite conta 90 storie in Chromium. Il gate verifica la parità delle sorgenti, provider, bootstrap e fixture senza trasformare le pagine in demo decorative.
- L'accessibilità automatica è diventata bloccante: axe intercetta nelle storie contrasto, heading, landmark, label e nomi accessibili. Corretti i contrasti del design system, i filtri e moduli, le intestazioni, i landmark e la riga PEC con selezione/apertura davvero navigabile da tastiera.
- La CI Frontend React installa Chromium, esegue `test:storybook` e costruisce Storybook statico; il job aggregato fallisce se uno dei nuovi controlli non riesce. La prova materiale autenticata su `127.0.0.1:8080` resta obbligatoria e separata.
- Sicurezza dipendenze: aggiornato l'override workspace di `js-yaml` a `4.3.1`, rimuovendo l'advisory ad alta severità GHSA-5p4m-2wfm-xmqj dalla catena di Changesets e shadcn; audit produzione senza vulnerabilità note.

## 2.278.60 - 2026-08-17

- **Blocco fiscale IRPEF: nove nuovi calcolatori (Lotto 2B-2).** IRPEF lorda a scaglioni **versionati per anno d'imposta** — 2024-2025 al 23/35/43% e 2026 col taglio della seconda aliquota al 33% (art. 1, c. 3, L. 199/2025, verificato su fonti ufficiali), anni non coperti rifiutati; acconto IRPEF/cedolare col metodo storico (confine 257,52 trattato come da prassi AE: a quel valore esatto due rate); rateazione ex art. 20 D.Lgs. 241/1997 aggiornata al D.Lgs. 1/2024 (tutte le rate il 16 del mese, ultima il 16 dicembre) con semplificazione a mesi interi dichiarata come stima per eccesso; detrazioni per familiari a carico (art. 12 TUIR post assegno unico: figli 21-30, soglia di degressione sul totale dei figli a carico come da Circ. 4/E/2022, **quozienti troncati alle quattro cifre decimali** come impone il c. 4); detrazioni per tipo di reddito (art. 13 TUIR con maggiorazioni fisse per fascia — inclusa quella da 50 euro per gli altri redditi 11-17k che mancava — e misure cuneo L. 207/2024 con base sul solo reddito di lavoro dipendente); detrazione canoni di locazione (art. 16 TUIR); regime forfettario (fuoriuscita immediata oltre 100.000 con imposta sostitutiva dichiarata non applicabile); fattura agente con Enasarco (aliquota ritenuta bloccata per legge al primo scaglione); ricevuta di prestazione occasionale (rimborsi nella base della ritenuta salvo anticipazioni documentate ex R.M. 49/E/2013, avviso gestione separata ex art. 44 D.L. 269/2003).
- **Doppia verifica adversariale prima del rilascio.** Sei agenti (lente normativa con riscontro web + lente matematica per modulo): recepiti 2 errori normativi, 8 imprecisioni e una serie di riferimenti puntuali (scadenze post D.Lgs. 1/2024, sterilizzazione 440 euro oltre 200k dal 2026, rinvio del c. 5-bis al solo c. 3 per l'assegno dal coniuge).
- **Cinque utility reali nel pannello /applicazioni**: variazione media del fatturato, calcolo orario inizio/fine con pausa, calcolatore di frazioni esatte, conversioni di unità con fattori esatti, e giorni lavorativi ora al netto delle **festività nazionali** (Pasqua e lunedì dell'Angelo col metodo di Gauss); età anagrafica in anni e mesi.
- **Catalogo funzioni: da 117 a 137 voci operative** (20 riclassifiche validate fail-closed). Curatore fallimentare, delegato alle vendite e diritti di copia restano esclusi finché i D.M. tariffari non saranno versionati in docs/specs. Suite `tests/test_strumenti_lotto2b2.py` (60 test).

## 2.278.59 - 2026-08-15

- **Tredici nuovi calcolatori giuridici (Lotto 2B-1 dell'inventario catalogo).** Finanza: TAEG di verifica (art. 121 T.U.B.: include commissioni e imposte note al finanziatore, spese annue attualizzate a scadenza propria come da allegato I dir. 2008/48/CE), surroga del mutuo (art. 120-quater T.U.B., con ambito persone fisiche/micro-imprese dichiarato), rendimento BOT (imposta sostitutiva 12,5% anticipata alla sottoscrizione, D.Lgs. 239/1996) e pronti contro termine (ritenuta 26%). Famiglia e successioni: grado di parentela (artt. 74-78 c.c., con basi successorie differenziate: artt. 570-571 per i fratelli, 572 per gli altri collaterali, affini esclusi ex art. 565), pensione di reversibilità (aliquote art. 22 L. 903/1965 e art. 1 c. 41 L. 335/1995, **clausola di salvaguardia di fascia in cascata** e cap Corte Cost. 162/2022), imposte di successione (aliquote e franchigie D.L. 262/2006, unioni civili equiparate ex L. 76/2016). Immobili: valore catastale (moltiplicatori distinti registro/successioni per il gruppo B: 168 vs 140), IMU (L. 160/2019, detrazione abitazioni di lusso ripartita tra i contitolari residenti come da c. 749), imposte di compravendita (con termine di rivendita prima casa aggiornato a due anni ex L. 207/2024), riparto spese col metodo del resto maggiore, categorie catastali. Rivalutazione su media annua ISTAT con principio nominalistico (art. 1277 c.c.) e stime provvisorie marcate.
- **Verifica adversariale multi-agente prima del rilascio.** Dieci giuristi indipendenti hanno contestato costanti e formule dei moduli: 2 errori normativi corretti (il TAEG escludeva le imposte applicando per errore la regola del TEG antiusura; la salvaguardia sul cumulo della reversibilità operava solo sulla prima fascia), un bug di parsing silenzioso («1.500» letto come 1,5 nelle quote di riparto) e oltre venti imprecisioni su fonti e casi limite recepite (basi normative puntuali, warning su perimetri non coperti). Ogni tool dichiara le proprie fonti ufficiali nel risultato.
- **Catalogo funzioni: da 103 a 117 voci operative.** Le voci TAEG, surroga, rivalutazione media, BOT, PcT, parentela, reversibilità, successioni, catastale, IMU, compravendita, spese condominiali, utenze e categorie catastali aprono ora i calcolatori reali. Suite `tests/test_strumenti_lotto2b.py` (63 test) a guardia di formule, riclassifiche e fonti.
...
- **Interessi convenzionali nel calcolatore interessi.** Terza modalità accanto a legali e mora commerciale: tasso pattuito su base annua pro-rata die, con avvertenze obbligatorie su forma scritta (art. 1284 c.3 c.c.) e verifica antiusura (L. 108/1996) rimessa al professionista.
- **Catalogo funzioni: 25 voci in più realmente operative (da 78 a 103).** Le 18 voci «già operative di fatto» scartate nel Lotto 1 per endpoint non dichiarato ora hanno deep-link verificato: conta giorni/tempo trascorso, scorporo IVA, percentuali e proporzioni, codice fiscale (calcolo e decodifica) puntano ai nuovi tool; verifica P.IVA/IBAN, minuti in centesimali, giorni lavorativi ed età anagrafica puntano alla scheda con pannello di calcolo reale in /applicazioni; parametri D.M. 55/2014 (civile, penale, stragiudiziale e tabelle) → Compensi Forensi; parcelle → Parcelle e Fatture; scadenze impugnazioni → calcolatore termini art. 325/327 c.p.c.; deposito telematico → regia telematica; controllo cause a ruolo → PolisWeb/PST. Le tre voci tabellari ISTAT e le tabelle interessi puntano alle nuove viste. Guardia permanente estesa (`tests/test_strumenti_lotto2a.py`, 27 test): ogni deep-link del lotto è validato contro il registro reale dei tool e dei pannelli.
...
- **Pannello «Variazioni dal registro» nel dettaglio fascicolo (residuo Fase 4).** Sotto «Aggiorna dal registro»: storico delle variazioni con orario di rilevazione, colori per tipo (spostata/nuova/rimossa), espandibile. Route `/api/v1/ui/fascicoli/<id>/registro-cancelleria`. Il messaggio di sincronizzazione riporta il numero di variazioni rilevate. Suite `tests/test_polisweb_diff.py` (9 test).
- **Comunicazioni di cancelleria e notifiche da ritirare (residuo Fase 1).** Nuovo `consulta_comunicazioni` sul client PST: interroga le classi ministeriali `Comunicazioni` (attestazione, descrizione evento) e `NotificaDaRitiro` (catalogo SICID v1.52) e alimenta il pannello variazioni come **presidio informativo** — la fonte primaria delle comunicazioni resta la PEC ex D.M. 44/2011 artt. 16 ss., nessuna scadenza operativa da qui. Attivato `scadTermini='1'` nel `ProfiloFascicolo` (il profilo include ora scadenze e termini). Cassazione e registri non SICID esclusi senza errori.
- **Contatore «proposte da confermare» in topbar (residuo Fase 3) + fix conteggi.** Il pannello Scadenze della topbar mostra il numero di proposte in BOZZA con link diretto allo scadenziario; corretto in pari tempo un difetto pre-esistente scoperto in ricognizione: **le bozze venivano conteggiate tra le scadenze operative** (oggi/domani/urgenti/scadute) contro la convenzione dello scadenziario — ora hanno il contatore dedicato e restano fuori dai conteggi urgenti e dal badge.
