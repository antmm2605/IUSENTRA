 ...
 *   - usa_nome=true  -> <input name="..."> riceve u.nome
 *   - usa_nome=false -> <input name="..."> riceve u.codice
 */
...
(function (global) {
  ...
  function initUfficioPicker(uid) {
    ...
    function render(data) {
      ...
      if (!data.length) {
        dropdown.innerHTML =
          '<div class="uff-empty"><i class="bi bi-search me-1"></i>Nessun ufficio trovato<div class="mt-1 small">Prova con un nome di citta diverso</div></div>';
      ...
      const gruppi = {};
      data.forEach((u) => {
        const tipo = u.tipo || 'TRIBUNALE';
        if (!gruppi[tipo]) gruppi[tipo] = [];
        gruppi[tipo].push(u);
      });

      const tipiPresenti = GRUPPI_ORDER.filter((tipo) => gruppi[tipo]);
      Object.keys(gruppi).forEach((tipo) => {
        if (!tipiPresenti.includes(tipo)) tipiPresenti.push(tipo);
      });
      ...
      dropdown.querySelectorAll('.uff-item').forEach((el) => {
        el.addEventListener('mousedown', (e) => {
          e.preventDefault();
          seleziona(items[+el.dataset.idx]);
        });
      });
    }
    ...
    function muoviFocus(dir) {
      ...
      if (els[focusIdx]) els[focusIdx].classList.remove('focused');
    ...
    input.addEventListener('keydown', function (e) {
      ...
      if (e.key === 'Enter') {
        const els = dropdown.querySelectorAll('.uff-item');
        if (focusIdx >= 0 && els[focusIdx]) {
          e.preventDefault();
          seleziona(items[focusIdx]);
        }
        return;
      }
    ...
    window.addEventListener('scroll', () => {
      if (dropdown.classList.contains('show')) posiziona();
    }, { passive: true });
