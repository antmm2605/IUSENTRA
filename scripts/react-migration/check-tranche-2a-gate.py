...
REPORT = ROOT / "artifacts/react-migration/tranche-2a-gate.md"
...
def main() -> int:
    results: list[dict] = []
    with tempfile.TemporaryDirectory(prefix="iusentra-tranche-2a-", ignore_cleanup_errors=True) as tmp:
        tmp_path = Path(tmp)
        ...
        with app.test_client() as client:
            ...
            for path in ("/statistiche", "/audit", "/registro-attivita"):
            ...
            for path in ("/statistiche?_legacy=1", "/audit?_legacy=1", "/registro-attivita?_legacy=1"):
            ...
            for path in ("/utenti", "/profili", "/backup"):
                response = client.get(path, headers={"Accept": "text/html"})
                _record(
                    results,
                    f"Ancora legacy {path}",
                    response.status_code == 200 and not _is_react_shell(response),
                    f"status={response.status_code}",
                )

            for path in ("/api/v1/ui/statistiche", "/api/v1/ui/audit", "/api/v1/ui/registro-attivita"):
