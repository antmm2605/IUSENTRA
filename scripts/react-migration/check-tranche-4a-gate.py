...
REPORT = ROOT / "artifacts/react-migration/tranche-4a-gate.md"
...
def main() -> int:
    results: list[dict] = []
    with tempfile.TemporaryDirectory(prefix="iusentra-tranche-4a-", ignore_cleanup_errors=True) as tmp:
        tmp_path = Path(tmp)
        ...
        with app.test_client() as client:
            ...
            for path in ("/backup", "/sito-studio", "/sito-studio/contatti"):
            ...
            for path in ("/api/v1/ui/backup", "/api/v1/ui/sito-studio", "/api/v1/ui/sito-studio/contatti"):
            ...
            for path in ("/backup", "/sito-studio", "/sito-studio/builder"):
