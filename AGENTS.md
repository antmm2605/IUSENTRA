...
- **Tranche Cassazione/corpus 2026-05-18:** prima del corpus il backfill Cassazione deve produrre report qualità con pagina, PDF/allegato, hash, OCR/testo, norme, R.G., discrepanze, link cliccabile, stato pronto/non pronto e `question_matrix` con le domande da avvocato. Il download allegati deve riusare prima i PDF già presenti sul server (`/data/intelligence/downloads` o directory configurate) e il generatore corpus deve filtrare solo schede documentali Cassazione, escludendo pagine generiche del sito.
