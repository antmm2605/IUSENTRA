...
def test_apertura_fascicolo_da_pratica_commerciale_genera_regia(tmp_path):
    ...
    pe_repo = ctx.repo.from_fascicoli_db(str(ctx.fascicoli.db_path))
    assert pe_repo.get_profile_snapshot(fascicolo.id)
    assert pe_repo.list_checklist(fascicolo.id)
    assert pe_repo.list_slots(fascicolo.id)
    assert pe_repo.latest_state(fascicolo.id) == "FASCICOLO_APERTO"
