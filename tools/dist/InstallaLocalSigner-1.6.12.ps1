# IUSENTRA Local Signer Setup v1.6.12
# Pacchetto generato il 2026-04-25 00:05:07
...
$embeddedPythonUrlFallback = "$defaultAllowedOrigins/polisWeb/local-signer/download/python-embedded"
...
function Copy-LocalSignerModule {
    ...
    foreach ($moduleFile in @("__init__.py", "ai_cache.py", "ai_handlers.py", "security.py", "server_bootstrap.py")) {
        $source = Join-Path $moduleSourceDir $moduleFile
        if (-not (Test-Path $source)) {
            $source = Join-Path $toolsDir ("local_signer_mod__" + $moduleFile)
        }
        if (-not (Test-Path $source)) {
            $downloadUrl = "$defaultAllowedOrigins/polisWeb/local-signer/download/local-signer-mod/$moduleFile"
