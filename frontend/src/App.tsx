...
const CalcolaTerminiPage = lazyPage(() => import('./components/ScadenziarioPage').then((module) => ({ default: module.CalcolaTerminiPage })))
...
const PdpPenalePage = lazyPage(() => import('./components/penalePdp/PdpPenalePage'))
const StudioModulePage = lazyPage(() => import('./components/StudioModulePage').then((module) => ({ default: module.StudioModulePage })))
const DocumentToolsPage = lazyPage(() => import('./components/DocumentToolsPage').then((module) => ({ default: module.DocumentToolsPage })))
...
const StrumentiLegaliPage = lazyPage(() => import('./components/StrumentiLegaliPage'))
...
const LexLearningPage = lazyPage(() => import('./components/LexLearningPage').then((module) => ({ default: module.LexLearningPage })))
...
const PromptLibraryPage = lazyPage(() => import('./features/legal-skills/pages/PromptLibraryPage').then((module) => ({ default: module.PromptLibraryPage })))
const PromptPathwaysPage = lazyPage(() => import('./features/legal-skills/pages/PromptPathwaysPage').then((module) => ({ default: module.PromptPathwaysPage })))
const OggiPage = lazyPage(() => import('./pages/daily-plan/OggiPage').then((module) => ({ default: module.OggiPage })))
...
const navSections: NavSection[] = [
  ...
  },
  {
    ...
    icon: UsersRound,
    items: [
      { label: 'Acquisizione incarichi', icon: UserPlus, href: '/crm' },
  ...
  },
  {
    ...
    items: [
      ...
      { label: 'Apprendimento Lex', icon: BrainCircuit, href: '/lex-apprendimento', featureFlag: 'lex.autonomousLearning', requiresAnyPermission: ['ai.usa'] },
...
function requiredPermissionsForHref(href: string): string[] {
  ...
  if (route === '/oggi' || route.startsWith('/oggi/')) return ['agenda.leggi', 'scadenziario.leggi']
  ...
  if (route.startsWith('/sito-studio') || route.startsWith('/studio') || route.startsWith('/statistiche') || route.startsWith('/strumenti-legali') || route.startsWith('/strumenti-operativi') || route.startsWith('/strumenti-documentali') || route.startsWith('/applicazioni')) return ['admin.leggi', 'fascicoli.leggi']
...
/** Il PDP non ha API per i gestionali: /pdp è la pagina dei depositi penali preparati in IUSENTRA. */
function isPdpPenaleRoute(path: string): boolean {
  const route = normaliseRoutePath(path).toLowerCase()
  return route === '/pdp' || route.startsWith('/pdp/') || route.startsWith('/portali/pdp')
}
...
function dashboardSyncLabel({ data, loading, mailSyncing, revalidating = false }:{data:DashboardData; loading:boolean; mailSyncing:boolean; revalidating?:boolean}): string {
  ...
  if (revalidating && data.stale) return hour?`Dati delle ${hour} · aggiornamento in corso...`:'Aggiornamento in corso...'
...
function DashboardPage({
  ...
  const syncStateClass = loading || mailSyncing || (revalidating && data.stale) ? '' : data.status === 'errore' ? 'danger' : data.status === 'parziale' ? 'warn' : 'ok'
  return (
    <main className="iu-content">
      <div className="iu-page-heading">
        <div>
        ...
        <div className="iu-page-heading__actions">
          <span className={`iu-sync ${syncStateClass}`} role="status" aria-live="polite">{dashboardSyncLabel({data,loading,mailSyncing,revalidating})}</span>
...
function AppStudio() {
  ...
  const isClientCollaboratorsPage = /^\/clienti\/[^/]+\/collaboratori$/.test(routeKey)
  ...
  const isCalculatorPage = routeKey === '/scadenziario/calcola-termini'
  const isScadenziarioPage = !isCalculatorPage && !isNewDeadlinePage && !isDeadlineEditPage && (routeKey === '/scadenziario' || routeKey.startsWith('/scadenziario/'))
  ...
  const isLexLearningPage = routeKey === '/lex-apprendimento'
  ...
  const isDocumentToolsPage = routeKey === '/strumenti-documentali'
  ...
  const isStrumentiLegaliPage = routeKey === '/strumenti-legali'
  ...
  const isPromptLibraryPage = routeKey === '/legal-skills/prompt' || routeKey === '/app/legal-skills/prompt'
  const isPromptPathwaysPage = routeKey === '/legal-skills/percorsi' || routeKey === '/app/legal-skills/percorsi'
  const isLegalSkillsPage = isLegalSkillsCatalogPage || isLegalSkillsProfilePage || isColdStartInterviewPage || isLegalSkillsRunPage || isLegalSkillsReviewPage || isPromptLibraryPage || isPromptPathwaysPage
  const isOggiPage = routeKey === '/oggi'
  ...
  const isStudioModulePage = !isOggiPage && !isClientPortalStudioPage && !isStudioPage && !isEditorProfessionalePage && !isAmministrazionePage && !isFatturazionePage && !isIncassiPagamentiPage && !isPreventivoWizardPage && !isPreventiviPage && !isCompensiForensiPage && !isStrumentiLegaliPage && !isTariffarioPage && !isTemplateAttiPage && !isRedazioneAttiPage && !isGiurisprudenzaPage && !isLegalIntelligencePage && !isLexLearningPage && !isLegalSkillsPage && !isWorkflowAgentsPage && !isAdminDatabasePage && !isQuickOrganizerImportPage && !isStatistichePage && !isImpostazioniPage && !isAuditPage && !isRegistroAttivitaPage && !isUtentiPage && !isProfiliPage && !isBackupPage && !isSitoStudioBuilderPage && !isSitoStudioRedazioneAiPage && !isSitoStudioPage && !isTimesheetPage && !isCartelleCondivisePage && !isWizardProPage && isStudioModuleRoute(routeKey)
  const isPresetExcludedPage = isSitoStudioBuilderPage || isFascicoloDetailViewPage || isStudioModulePage
  const isDashboardPage = !isOggiPage && !isClientPortalStudioPage && !isSearchPage && !isAgendaImportPage && !isAgendaPage && !isNewAppointmentPage && !isAppointmentEditPage && !isRegiaPage && !isDocumentEditorPage && !isFascicoliPage && !isClientiPage && !isClientFolderPage && !isClientCollaboratorsPage && !isClientEditPage && !isNewClientPage && !isSoggettiPage && !isNewSubjectPage && !isSubjectEditPage && !isEmailComposePage && !isEmailOrdinariaComposePage && !isNotificheLegaliPage && !isEmailPage && !isEmailOrdinariaPage && !isMessagesPage && !isNewMessagePage && !isScadenziarioPage && !isCalculatorPage && !isNewDeadlinePage && !isDeadlineEditPage && !isTimesheetPage && !isCartelleCondivisePage && !isWizardProPage && !isTelematicoPage && !isTelematicoSurfacePage && !isPrivacyRegistroPage && !isAdminDatabasePage && !isQuickOrganizerImportPage && !isDocumentToolsPage && !isStatistichePage && !isImpostazioniPage && !isAuditPage && !isRegistroAttivitaPage && !isUtentiPage && !isProfiliPage && !isProfiloPage && !isBackupPage && !isSitoStudioBuilderPage && !isSitoStudioRedazioneAiPage && !isSitoStudioPage && !isStudioPage && !isEditorProfessionalePage && !isAmministrazionePage && !isFatturazionePage && !isIncassiPagamentiPage && !isPreventivoWizardPage && !isPreventiviPage && !isCompensiForensiPage && !isStrumentiLegaliPage && !isTariffarioPage && !isTemplateAttiPage && !isRedazioneAttiPage && !isGiurisprudenzaPage && !isLegalIntelligencePage && !isLexLearningPage && !isLegalSkillsPage && !isWorkflowAgentsPage && !isStudioModulePage
  const isStandalonePage = isOggiPage || isClientPortalStudioPage || isSearchPage || isAgendaImportPage || isAgendaPage || isNewAppointmentPage || isAppointmentEditPage || isDocumentEditorPage || isFascicoliPage || isClientiPage || isClientFolderPage || isClientCollaboratorsPage || isClientEditPage || isNewClientPage || isSoggettiPage || isNewSubjectPage || isSubjectEditPage || isEmailComposePage || isEmailOrdinariaComposePage || isNotificheLegaliPage || isEmailPage || isEmailOrdinariaPage || isMessagesPage || isNewMessagePage || isScadenziarioPage || isCalculatorPage || isNewDeadlinePage || isDeadlineEditPage || isTimesheetPage || isCartelleCondivisePage || isWizardProPage || isTelematicoPage || isTelematicoSurfacePage || isPrivacyRegistroPage || isAdminDatabasePage || isQuickOrganizerImportPage || isDocumentToolsPage || isStatistichePage || isImpostazioniPage || isAuditPage || isRegistroAttivitaPage || isUtentiPage || isProfiliPage || isProfiloPage || isBackupPage || isSitoStudioBuilderPage || isSitoStudioRedazioneAiPage || isSitoStudioPage || isStudioPage || isEditorProfessionalePage || isAmministrazionePage || isFatturazionePage || isIncassiPagamentiPage || isPreventivoWizardPage || isPreventiviPage || isCompensiForensiPage || isStrumentiLegaliPage || isTariffarioPage || isTemplateAttiPage || isRedazioneAttiPage || isGiurisprudenzaPage || isLegalIntelligencePage || isLexLearningPage || isLegalSkillsPage || isWorkflowAgentsPage || isStudioModulePage
  const effectiveStandalonePage = isStandalonePage || isCrmPage || appV2FlagDenied || appV2UnknownRoute
  const embedMode = new URLSearchParams(window.location.search).get('embed')
  const embeddedViewer = embedMode === 'agenda' || embedMode === 'source'
  ...
  const {data,loading,revalidating:dashboardRevalidating,mailSyncing,refresh:refreshDashboard,syncMailboxes:syncMailboxesNow}=useDashboardData(!effectiveStandalonePage)
      ...
      <div className={`iu-shell ${sidebarCollapsed?'iu-shell--collapsed':''} ${mobileMenuOpen&&!embeddedViewer?'iu-shell--mobile-menu-open':''} ${guidePanelExpanded && isFascicoliPage?'iu-shell--guide-open':''} ${isPresetExcludedPage?'iusentra-preset-excluded':'iusentra-preset-active'} ${embeddedViewer?'iu-shell--embedded-viewer':''}`} data-iusentra-preset-root={isPresetExcludedPage?'excluded':'active'}>
        ...
        {!embeddedViewer&&!mobileMenuOpen?<nav className={`iu-mobile ${mobileNavCollapsed?'is-collapsed':''}`} aria-label="Navigazione mobile">
          <div id="iu-mobile-links" className="iu-mobile__rail" hidden={mobileNavCollapsed}>
            ...
            <a className={isClientiPage||isClientFolderPage||isClientCollaboratorsPage||isClientEditPage||isNewClientPage||isCartelleCondivisePage||isSoggettiPage||isNewSubjectPage||isSubjectEditPage?'active':''} href="/clienti"><UsersRound size={18}/><span>Clienti</span></a>
