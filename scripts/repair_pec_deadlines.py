...
def run_repair(*, registry: Path, tenant: str = "", limit: int = 0, actor: str = "pec-maintenance") -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        ...
        "studios": {},
        "total": {"checked": 0, "updated": 0, "deleted": 0, "skipped": 0},
    }
    for studio in studios:
        ...
        if not pec_db.exists():
            result = {"ok": True, "checked": 0, "updated": 0, "deleted": 0, "skipped": 0, "message": "Archivio PEC audit non presente."}
        else:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Bonifica scadenze PEC automatiche: rimuove falsi termini e riallinea udienze reali.")
    ...
    result = run_repair(registry=_registry_path(args.registry), tenant=args.tenant, limit=args.limit, actor=args.actor)
