...
- `2.249.38`: la prova `Avvia tutti` ha evidenziato che alcune richieste manuali venivano marcate come `running` già al solo accodamento APScheduler. La richiesta resta ora `requested` finché il wrapper non parte davvero; al riavvio del worker, le richieste manuali rimaste aperte dal processo precedente vengono chiuse come interrotte invece di restare appese.
- `2.249.39`: la prova produzione ha mostrato che `Avvia tutti` può mettere in parallelo UTF-8, crash test operativi e backup blindato, generando archivi temporanei e lock sul database. Le manutenzioni manuali pesanti sono ora esclusive: quando una è richiesta o in corso, le altre vengono registrate come non avviate per proteggere spazio, database e reattività.
- `2.249.40`: la prova su `2.249.39` ha mostrato che il backup blindato può essere il primo job pesante nell'ordine reale della console. Ora il backup viene rinviato quando arriva dentro un avvio massivo con altri job manuali già in coda o in corso; resta avviabile singolarmente quando la console è libera.
- `2.249.41`: la prova su `2.249.40` ha mostrato che anche il primo crash test operativo pesante può restare accodato. Ora tutte le manutenzioni pesanti manuali vengono rinviate quando arrivano dentro un avvio massivo con altre esecuzioni manuali già aperte; restano avviabili singolarmente.
