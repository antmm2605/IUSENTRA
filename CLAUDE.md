...
- **OCR**: **un solo motore**, `legal_ocr/motore/` (runtime Tesseract, immagine, lettura, consenso con PDF Inspector, pagina, testo, provisioning) + `legal_ocr/formulario/` (correzioni legali dichiarate). `pct/ocr.py`, `pct/editor.py`, `pct/document_intelligence/extraction.py`, `legal_ocr/engines.py`, `pct/pec_ocr_pipeline.py`, `lex/tools/_doc_extractor.py` e `web/services/document_ocr*.py` sono adattatori: **mai chiamare `pytesseract` direttamente altrove** (vedi `docs/OCR_LEGAL.md`)
