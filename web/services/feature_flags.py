...
APP_V2_DEFAULT_ON_FLAGS = frozenset(
    {
        ...
        # Preventivo, conferimento incarico, documento d'identita' e firma
        # elettronica del Portale Cliente: il percorso e' completo sui due lati
        # (pagina dello studio e mini app del cliente) e non ha alcun
        # interruttore nelle impostazioni dello studio. Lasciandolo default-off
        # l'avvocato vedeva solo «Il percorso di firma non e' attivo per questo
        # studio» senza alcun modo di attivarlo dall'applicazione.
...
FEATURE_FLAG_DEFINITIONS: tuple[FeatureFlagDefinition, ...] = (
    ...
    _flag("routes.appV2.settings.sdi", "Canali SdI e FatturaPA nelle impostazioni App V2."),
    ...
    _flag("lex.autonomousLearning", "Superficie read-only dell'apprendimento autonomo Lex (memoria, letture, proposte in revisione umana)."),
    ...
    _flag("lex.dailyPlan.enabled", "Piano del giorno (Lex Oggi): lettura snapshot e azioni sugli item."),
    _flag("lex.dailyPlan.scheduledRuns", "Generazione automatica del piano del giorno dallo scheduler."),
    _flag("lex.dailyPlan.writeProposals", "Proposte applicative approvabili dalla pagina Oggi."),
    ...
    _flag("routes.appV2.legalSkills.promptLibrary", "Libreria prompt LegalSkills Italia nella shell App V2."),
    _flag("routes.appV2.legalSkills.pathways", "Percorsi guidati per procedimento nella shell App V2."),
    ...
    _flag("routes.appV2.dailyPlan.home", "Pagina Oggi (piano del giorno) nella shell App V2."),
    _flag("routes.appV2.dailyPlan.reviewQueue", "Coda revisione del piano del giorno nella shell App V2."),
    ...
    _flag("routes.appV2.clientPortal.signingWorkflow", "Workflow professionale preventivo, conferimento incarico, documento identità e firma elettronica con evidenze nel Portale Cliente."),
...
APP_V2_ROUTE_FLAGS: tuple[tuple[str, str], ...] = (
    ...
    ("/impostazioni/sdi", "routes.appV2.settings.sdi"),
    ("/impostazioni/canali-sdi", "routes.appV2.settings.sdi"),
