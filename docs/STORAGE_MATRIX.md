...
| Operativita' fascicolo | Registro delle letture (documenti, PEC, allegati; letture per lettore; viste; anomalie) | - | R/W | R/W | schema governato | Wave 5 - intelligence | `REGISTRO_LETTURE_DB` tenant-aware (`intelligence/registro_letture.db`); SQLite/PostgreSQL gemelli in `pct/sql/20260915_registro_letture*.sql`; nessun JSON, nessun fallback invisibile; vedi `docs/REGISTRO_LETTURE.md` |
| Operativita' fascicolo | Archivio di alimentazione (fatti letti dai motori documenti e PEC, collaudati: date, ruoli, prove di notifica, eventi) | - | R/W | R/W | schema governato | Wave 5 - intelligence | tabella `letture_fatti` nello stesso database del registro; scritta solo dai due motori (`web/services/archivio_letture_runtime.py`, worker OCR), letta dai presìdi; decisioni dell'avvocato conservate; esito del collaudo del lettore in `intelligence/collaudo_lettore.json` |
