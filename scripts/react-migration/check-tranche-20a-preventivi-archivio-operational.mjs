...
const route = manifest.routes.find((item) => item.route === '/preventivi')
if (route?.status !== 'react_operational_full') violations.push('manifest /preventivi non react_operational_full.')
if (/\bLegacyPostForm\b/.test(page)) violations.push('PreventiviPage contiene LegacyPostForm.')
if (!/\bgetPreventiviPage\b/.test(page)) violations.push('PreventiviPage non usa getPreventiviPage.')
if (!/\bgetPreventivoDetail\b/.test(page)) violations.push('PreventiviPage non usa dettaglio JSON.')
...
if (!api.includes('@api_v1_react.get("/preventivi")')) violations.push('manca GET JSON /preventivi.')
if (!api.includes('@api_v1_react.get("/preventivi/<id_preventivo>")')) violations.push('manca GET JSON /preventivi/<id>.')
...
if (violations.length) {
  console.error(`Tranche 20A non conforme:\n- ${violations.join('\n- ')}`)
  process.exit(1)
}
console.log('Tranche 20A preventivi archivio operational OK.')
