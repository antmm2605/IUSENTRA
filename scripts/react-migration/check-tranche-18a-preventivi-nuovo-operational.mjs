...
const route = manifest.routes.find((item) => item.route === '/preventivi/nuovo')
if (route?.status !== 'react_operational_full') violations.push('manifest /preventivi/nuovo non react_operational_full.')
if (/\bLegacyPostForm\b/.test(page)) violations.push('PreventiviPage contiene LegacyPostForm.')
if (!/\bcreatePreventivo\b/.test(page)) violations.push('PreventiviPage non usa createPreventivo.')
...
if (!api.includes('@api_v1_react.post("/preventivi/nuovo")')) violations.push('manca POST JSON /preventivi/nuovo.')
...
if (violations.length) {
  console.error(`Tranche 18A non conforme:\n- ${violations.join('\n- ')}`)
  process.exit(1)
}
console.log('Tranche 18A preventivi/nuovo operational OK.')
