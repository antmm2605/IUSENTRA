...
def test_react_shell_mobile_sblocca_scroll_e_compatta_card():
    ...
    assert "document.querySelector('#iu-mobile-links a.active')?.scrollIntoView({ behavior: 'smooth', block: 'nearest', inline: 'center' })" in app_source
    assert "},[embeddedViewer,mobileNavCollapsed,routeKey])" in app_source
...
def test_react_fascicoli_economia_legge_rt_xml_importato_come_atto_giudiziario(monkeypatch, tmp_path: Path):
    import web.services.react_fascicoli_bridge as bridge
    ...
    fascicoli.aggiungi_documento(
        fascicolo.id,
        "17_03_2026 ore_10_05 ACCETTAZIONE_Richiesta pagamento annualita_CARTA DEL DOCENTE_.eml",
        TipoDocumento.COMUNICAZIONE,
        b"Richiesta pagamento annualita carta docente: messaggio PEC non riferito al contributo unificato.",
        data_documento="2026-03-17",
        note="Import QuickOrganizer.",
...
<pay_j:RT xmlns:pay_j=\"http://www.digitpa.gov.it/schemas/2011/Pagamenti/\">
  ...
  <pay_j:datiPagamento>
    ...
    documento = fascicoli.aggiungi_documento(
        ...
        xml,
        data_documento="2026-05-12",
        nome_originale="20260512122523100.xml",
        nome_portale="rt_33E000GLVE6L4BIFLARMYPA0VKIRL7DIRYT.xml",
    ...
    assert response.status_code == 200
    assert documento.tipo == TipoDocumento.ATTO_GIUDIZIARIO
    assert contributo["status"] == "pagato"
    assert contributo["importo"] == 49.0
    assert contributo["importoLabel"] == "€ 49,00"
    assert contributo["dataPagamento"] == "12/05/2026"
