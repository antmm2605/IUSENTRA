...
Aggiornamento `2.253.75`: dopo lo SHA `3e42314` CodeQL è risultato verde, ma la matrice CI ha segnalato il gate `Pytest core fase 7/10 observability parte 3/3`. La causa era nel controllo SQLite anti-bootstrap (`immutable=1` non vedeva WAL appena committati), non nel bridge PagoPA o nella sessione. Il runtime ora legge il database in `mode=ro` senza `immutable`, preservando il blocco anti-perdita JSON. Shard locale ripetuto: `32/32` OK; test storage mirati: `2/2` OK. Da riconfermare su nuovo push, deploy Hetzner e prova server `https://app.iusentra.it/fascicoli/9B9DF2A1`.

Aggiornamento `2.253.76`: durante la prova visiva locale del fascicolo/PagoPA, Chrome segnalava errore MIME sul foglio opzionale PST `print.css`, restituito dal Ministero come HTML. Il bridge ora serve quel solo foglio come CSS vuoto valido quando il PST non restituisce CSS, lasciando invariati HTML, DWR, form, PDF ricevuta e sessione `iusentra_session`. Prova reale locale su Docker `127.0.0.1:8080`, fascicolo `DC5BF1DB`: click `PagoPA`, `+ Nuovo pagamento`, tipo `Contributo unificato e/o Diritti di cancelleria`, distretto `TORINO`, `Tribunale Ordinario - Torino` (`0012720095`), 66 uffici caricati, `Paga subito` visibile, 0 errori console e nessun pagamento inviato.

Prova server reale `2.253.76` eseguita su `https://app.iusentra.it/fascicoli/9B9DF2A1`, commit `d80b9ce`: fascicolo `RG 3950/2026`, rif. interno `2026/308`, `Spagnolo Sara c. MIM`, 53 documenti visualizzati in scheda. Click `PagoPA`, iframe PST incorporato, `+ Nuovo pagamento`, tipo contributo unificato, distretto `TORINO`, `Tribunale Ordinario - Torino` (`0012720095`), 66 uffici caricati, nominativo/CF fittizi, `Paga subito` visibile e non premuto. `print.css` verificato HTTP 200 `text/css; charset=utf-8`, DWR uffici HTTP 200 `text/plain`, nessun errore console applicativo. Verificati anche `Cliente` e `Soggetti` nello stesso modello di modale iframe: `/clienti/2A1216AA/modifica` e `/soggetti?fascicolo=9B9DF2A1`.
